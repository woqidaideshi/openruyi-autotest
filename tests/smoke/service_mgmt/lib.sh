...
smokeServiceMgmtCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        ...
        rlLogInfo "smoke-service_mgmt: 清理完成（最后一个测试）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$SMOKE_SERVICE_MGMT_FLAG"
        rlLogInfo "smoke-service_mgmt: 保留（还有 $ref 个测试未完成）"
    fi
}
