...
smokeNetworkCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        ...
        rlLogInfo "smoke-network: 清理完成（最后一个测试）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$SMOKE_NETWORK_FLAG"
        rlLogInfo "smoke-network: 保留（还有 $ref 个测试未完成）"
    fi
}
