...
smokeShellBasicsCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        ...
        rlLogInfo "smoke-shell_basics: 清理完成（最后一个测试）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$SMOKE_SHELL_BASICS_FLAG"
        rlLogInfo "smoke-shell_basics: 保留（还有 $ref 个测试未完成）"
    fi
}
