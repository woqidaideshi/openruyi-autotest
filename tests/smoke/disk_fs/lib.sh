...
smokeDiskFsCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        ...
        rlLogInfo "smoke-disk_fs: 清理完成（最后一个测试）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$SMOKE_DISK_FS_FLAG"
        rlLogInfo "smoke-disk_fs: 保留（还有 $ref 个测试未完成）"
    fi
}
