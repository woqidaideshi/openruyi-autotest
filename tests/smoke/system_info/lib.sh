...
smokeSystemInfoCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        ...
        rlLogInfo "smoke-system_info: 清理完成（最后一个测试）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$SMOKE_SYSTEM_INFO_FLAG"
        rlLogInfo "smoke-system_info: 保留（还有 $ref 个测试未完成）"
    fi
}
