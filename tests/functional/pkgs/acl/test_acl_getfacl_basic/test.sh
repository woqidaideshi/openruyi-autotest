...
rlJournalStart
    ...
    rlPhaseStartTest "getfacl 基本功能"
        # 测试 1.1: 查看文件默认 ACL
        rlRun "getfacl testfile 2>&1 | grep -qE \"user::|group::|other::\"" 0 "查看文件默认 ACL 含权限条目"

        # 测试 1.2: 查看目录默认 ACL
        rlRun "getfacl testdir 2>&1 | grep -qE \"user::|group::|other::\"" 0 "查看目录默认 ACL 含权限条目"

        # 测试 1.3: 使用 -a 参数只显示 access ACL
        rlRun "getfacl -a testfile 2>&1 | grep -qE \"user::|group::\"" 0 "使用 -a 参数查看 access ACL"
        rlAssertGrep "user::" "$(getfacl -a testfile 2>&1)" "-a 输出包含 access ACL 条目"

        # 测试 1.4: 使用 -d 参数只显示 default ACL
        rlRun "getfacl -d testfile 2>&1 | grep -qE \"user::|default\"" 0 "使用 -d 参数查看 default ACL 含 default 条目"
        ...
        rlRun "getfacl -n testfile 2>&1 | grep -qE \"[0-9]+\"" 0 "使用 -n 参数显示数字 ID"

        # 测试 1.7: 使用 -t 参数使用表格输出格式
        rlRun "getfacl -t testfile 2>&1 | grep -qE \"[r-][w-][x-]\"" 0 "使用 -t 参数表格输出含权限位"
    rlPhaseEnd
