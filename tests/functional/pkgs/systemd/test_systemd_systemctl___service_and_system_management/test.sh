...
rlJournalStart
    ...
    rlPhaseStartTest "systemctl---Service-and-system-management"
        ...
        rlRun "systemctl is-active systemd-journald.service 2>&1 | grep -qiE \"error|Error|not found|No such|无法\" || echo expected-error" 1 "systemctl is-active: check service status"
        rlRun "systemctl is-enabled systemd-journald.service 2>&1 | grep -qiE \"error|Error|not found|No such|无法\" || echo expected-error" 1 "systemctl is-enabled: check enabled"
        rlRun "systemctl is-failed 2>&1 | grep -qiE \"error|Error|not found|No such|无法\" || echo expected-error" 1 "systemctl is-failed: list failed units"
        ...
        rlRun "systemctl list-machines 2>&1 | grep -qiE \"error|Error|not found|No such|无法\" || echo expected-error" 1 "systemctl list-machines"
    rlPhaseEnd
