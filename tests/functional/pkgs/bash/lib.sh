...
bashSetup() {
 ...
 rlLogInfo "already bash soft ()"
 else
 echo "installed=0" > "$PKG_FLAG"
 rlLogInfo "bash softalready exists"
 fi
 echo "ref=1" >> "$PKG_FLAG"
 else
 local ref
 ...
 rlLogInfo "bash alreadybyothertest, reference count: $ref"
 fi
...
bashCleanup() {
 ...
 rlLogInfo "already bash soft (posttest)"
 fi
 rm -f "$PKG_FLAG"
 else
 sed -i "s/^ref=.*/ref=$ref/" "$PKG_FLAG"
 rlLogInfo "bash Retain (still have $ref test(s) not completed)"
 fi
}
