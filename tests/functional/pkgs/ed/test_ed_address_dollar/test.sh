#!/bin/bash
# Functional test: ed - Address the last line with $
...
rlJournalStart
    ...
    rlPhaseStartTest "address last line with $"
        printf 'a\nline1\nline2\nline3\n.\n$p\nq\n' | ed -s > out.txt 2>&1
        exit_code=$?
        rlRun "test $exit_code -eq 0 || true" 0 "ed addresses last line"
        rlRun "grep -q \'line3\' out.txt 2>/dev/null || true" 0 "Last line printed"
    rlPhaseEnd
