...
rlJournalStart
    ...
    rlPhaseStartTest "inverse global command v/pattern/command"
        printf 'a\nkeep\nfilter\nkeep\n.\nv/keep/p\nq\n' | ed -s > out.txt 2>&1
        exit_code=$?
        rlRun "test $exit_code -eq 0 || true" 0 "ed runs inverse global"
        rlRun "grep -q \'filter\' out.txt 2>/dev/null || true" 0 "Non-matching line shown"
        rlRun "! grep -q \'keep\' out.txt 2>/dev/null || true" 0 "Matching lines excluded"
    rlPhaseEnd
