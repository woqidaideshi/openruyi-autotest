...
cmockaSetup() {
    if [ ! -f "$PKG_FLAG" ]; then
        if ! rpm -q cmocka 2>/dev/null; then
            echo openruyi | sudo -S dnf install -y cmocka 2>/dev/null
            echo "installed=1" > "$PKG_FLAG"
            rlLogInfo "已安装 cmocka 软件包（首次）"
        else
            echo "installed=0" > "$PKG_FLAG"
            rlLogInfo "cmocka 软件包已存在"
        fi
        echo "ref=1" >> "$PKG_FLAG"
    else
        local ref
        ...
        rlLogInfo "cmocka 已由其他测试安装，引用计数: $ref"
    fi
...
cmockaCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        if grep -q "^installed=1" "$PKG_FLAG"; then
            ...
            rlLogInfo "已卸载 cmocka 软件包（最后一个测试）"
        fi
        rm -f "$PKG_FLAG"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$PKG_FLAG"
        rlLogInfo "cmocka 保留（还有 $ref 个测试未完成）"
    fi
}
