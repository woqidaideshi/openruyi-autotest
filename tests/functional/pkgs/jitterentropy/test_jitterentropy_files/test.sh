#!/bin/bash
# Functional test: jitterentropy -ļ֤
...
rlJournalStart
    ...
    rlRun "pkg-config --libs jitterentropy 2>&1 | grep -qiE \"error|Error|not found|No such|Unable to\" || echo expected-error" 1 "pkg-config Ϣ"
    rlPhaseEnd
    ...
    # jitterentropy Package managed by lib.sh 's reference counting auto-uninstall
    rlPhaseEnd
