#!/bin/bash
# Functional test: coreutils - link/unlink operations
...
rlJournalStart
    ...
    rlPhaseStartTest "link/unlink operations"
    rlRun "link file1.txt link_test.txt" 0 "link create hard link"
    rlRun "test -f link_test.txt" 0 "link: verify exists"
    rlRun "unlink link_test.txt" 0 "unlink remove hard link"
    rlRun "test ! -f link_test.txt" 0 "unlink: verify removed"
    rlPhaseEnd
