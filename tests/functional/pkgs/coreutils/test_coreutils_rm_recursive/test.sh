#!/bin/bash
# Functional test: coreutils - rm -rf recursive remove
...
rlJournalStart
    ...
    rlPhaseStartTest "rm -rf recursive remove"
    rlRun "cp -r ls_testdir rm_testdir" 0 "Create dir to remove"
    rlRun "rm -rf rm_testdir" 0 "rm -rf recursive force"
    rlRun "test ! -d rm_testdir" 0 "rm -rf: directory removed"
    rlPhaseEnd
