#!/bin/bash
# Functional test: coreutils - test [ ] expression evaluation
...
rlJournalStart
    ...
    rlPhaseStartTest "test [ ] expression evaluation"
    rlRun "[ -f file1.txt ]" 0 "test -f file exists"
    rlRun "[ ! -f nonexistent ]" 0 "test ! -f file not exists"
    rlRun "[ 1 -eq 1 ]" 0 "test numeric equality"
    rlPhaseEnd
