#!/bin/bash
# Functional test: coreutils - mv rename/move file
...
rlJournalStart
    ...
    rlPhaseStartTest "mv rename/move file"
    rlRun "cp file1.txt mv_test.txt" 0 "Create file to move"
    rlRun "mv mv_test.txt mv_renamed.txt" 0 "mv rename file"
    rlRun "test ! -f mv_test.txt" 0 "mv: old name gone"
    rlRun "test -f mv_renamed.txt" 0 "mv: new name exists"
    rlPhaseEnd
