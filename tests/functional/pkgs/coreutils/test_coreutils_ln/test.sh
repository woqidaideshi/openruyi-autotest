#!/bin/bash
# Functional test: coreutils - ln create links
...
rlJournalStart
    ...
    rlPhaseStartTest "ln create links"
    rlRun "ln -s file1.txt symlink.txt" 0 "ln -s create symbolic link"
    rlRun "test -L symlink.txt" 0 "ln -s: verify symlink exists"
    rlRun "ln file1.txt hardlink.txt" 0 "ln create hard link"
    rlRun "test -f hardlink.txt" 0 "ln: verify hard link exists"
    rlPhaseEnd
