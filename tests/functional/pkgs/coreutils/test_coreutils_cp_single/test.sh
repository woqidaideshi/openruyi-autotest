#!/bin/bash
# Functional test: coreutils - cp single file
...
rlJournalStart
    ...
    rlPhaseStartTest "cp single file"
    rlRun "cp file1.txt cp_single_out.txt" 0 "cp copy file"
    rlRun "test -f cp_single_out.txt" 0 "cp: verify copy exists"
    rlRun "diff file1.txt cp_single_out.txt" 0 "cp: files identical"
    rlPhaseEnd
