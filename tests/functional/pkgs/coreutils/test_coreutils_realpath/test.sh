#!/bin/bash
# Functional test: coreutils - realpath absolute path
...
rlJournalStart
    ...
    rlPhaseStartTest "realpath absolute path"
    rlRun "realpath file1.txt" 0 "realpath resolve absolute path"
    rlRun "realpath -e /usr/bin/grep" 0 "realpath -e existing path"
    rlPhaseEnd
