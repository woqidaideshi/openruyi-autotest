...
rlJournalStart
    ...
    rlRun "pkg-config --libs libpsl 2>&1 | grep -qiE \"error|Error|not found|No such|Unable to\" || echo expected-error" 1 "pkg-config Ϣ"
    rlPhaseEnd
    ...
    # libpsl Package managed by lib.sh 's reference counting auto-uninstall
    rlPhaseEnd
