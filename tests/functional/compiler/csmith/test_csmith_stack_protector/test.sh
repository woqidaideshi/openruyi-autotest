#!/bin/bash
# csmith - 栈保护: -fstack-protector/-fstack-protector-strong/-fstack-protector-all
...
CEOF
    ...
    rlPhaseStartTest "GCC stack-protector"
        # 正常程序 + stack protector
        for mode in "stack-protector" "stack-protector-strong" "stack-protector-all"; do
            rlRun "gcc -f${mode} -o stack_gcc_${mode} buf.c && ./stack_gcc_${mode} | grep STACK_OK" 0 "GCC -f${mode} 正常程序"
        done

        # 验证 stack protector 生效: 编译产物应包含 __stack_chk 符号
        rlRun "nm stack_gcc_stack-protector | grep -q '__stack_chk'" 0 "GCC stack-protector: __stack_chk_fail 符号存在"

        # 溢出程序 + stack protector（应检测到并终止）
        ...
        if [ -x ./overflow_gcc ]; then
            ...
            local rc=$?
            rlLogInfo "overflow 程序退出码: $rc"
            if grep -qi "stack smashing\|stack overflow\|abort\|core dump" /tmp/overflow_gcc.txt; then
                rlPass "GCC -fstack-protector-all 成功检测栈溢出"
            elif [ "$rc" -ne 0 ]; then
                rlPass "GCC stack protector 异常终止溢出程序 (exit=$rc)"
            else
                rlLogWarning "stack protector 未阻止溢出（可能是小缓冲区优化）"
            fi
        fi
    rlPhaseEnd
    ...
    rlPhaseStartTest "Clang stack-protector"
        ...
        rlRun "nm stack_clang | grep -q '__stack_chk'" 0 "Clang stack protector 符号存在"
    rlPhaseEnd
