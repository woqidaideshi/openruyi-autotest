#!/bin/bash
# Functional test: compiler - jotai - 优化级别
...
CEOF
    ...
    rlPhaseStartTest "GCC 优化"
        ...
        diff -q /tmp/gcc_O0.txt /tmp/gcc_O2.txt >/dev/null 2>&1 && rlPass "GCC -O0 vs -O2 一致" || echo "差异"
    rlPhaseEnd

    rlPhaseStartTest "Clang 优化"
        for o in O0 O1 O2 O3 Os Ofast; do
        ...
        diff -q /tmp/clang_O0.txt /tmp/clang_O2.txt >/dev/null 2>&1 && rlPass "Clang -O0 vs -O2 一致" || echo "差异"
    rlPhaseEnd

    rlPhaseStartTest "GCC vs Clang -O2"
        diff -q /tmp/gcc_O2.txt /tmp/clang_O2.txt >/dev/null 2>&1 && rlPass "GCC/Clang -O2 一致" || rlLogInfo "GCC/Clang -O2 有差异"
    rlPhaseEnd
