...
yarpgenSetup() {
    if [ ! -f "$YARPGEN_FLAG" ]; then
        ...
        if [ -f "$YARPGEN_DIR/build/yarpgen" ]; then
            ...
            rlLogInfo "YARPGen 编译成功: $YARPGEN_DIR/build/yarpgen"
        else
            rlLogWarning "YARPGen 编译失败"
...
yarpgenCleanup() {
    ...
    if [ "$ref" -le 0 ]; then
        rm -f "$YARPGEN_FLAG"
        rlLogInfo "yarpgen 测试套清理完成（保留构建产物以供复用）"
    else
        sed -i "s/^ref=.*/ref=$ref/" "$YARPGEN_FLAG"
        rlLogInfo "yarpgen 保留（还有 $ref 个测试）"
    fi
}
