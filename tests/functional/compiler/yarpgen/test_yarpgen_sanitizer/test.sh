...
CEOF
    ...
    rlPhaseStartTest "GCC ASAN"
        ...
        if [ -x ./asan_gcc ]; then
            ...
            grep -qi "ERROR\|AddressSanitizer" /tmp/asan_gcc.txt && rlFail "GCC ASAN 误报" || rlPass "GCC ASAN 正常程序无误报"
        fi
    rlPhaseEnd

    rlPhaseStartTest "GCC UBSAN"
        rlRun "gcc -fsanitize=undefined -g -o ubsan_gcc clean.c && ./ubsan_gcc" 0 "GCC UBSAN 正常"
        rlPass "GCC UBSAN 无未定义行为"
    rlPhaseEnd
    ...
    rlPhaseStartTest "Clang ASAN"
        ...
        grep -qi "ERROR\|AddressSanitizer" /tmp/asan_clang.txt && rlFail "Clang ASAN 误报" || rlPass "Clang ASAN 正常"
    rlPhaseEnd

    rlPhaseStartTest "Clang UBSAN"
        rlRun "clang -fsanitize=undefined -g -o ubsan_clang clean.c && ./ubsan_clang" 0 "Clang UBSAN"
        rlPass "Clang UBSAN 无未定义行为"
    rlPhaseEnd
