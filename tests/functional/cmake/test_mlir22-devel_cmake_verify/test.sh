#!/bin/bash
# Functional test: mlir22-devel - cmake export integrity verification
...
find_package(MLIR REQUIRED CONFIG
  PATHS /usr/lib64/llvm22/lib/cmake/mlir
  NO_DEFAULT_PATH)

message(STATUS "find_package(MLIR) succeeded")
EOF

            rlLogInfo "Verifying find_package(MLIR CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package MLIR CONFIG)"; then
                rlPass "find_package(MLIR CONFIG) verification passed"
            else
                rlFail "find_package(MLIR CONFIG) verification failed"
            fi
