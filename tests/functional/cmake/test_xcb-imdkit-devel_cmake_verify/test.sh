#!/bin/bash
# Functional test: xcb-imdkit-devel - cmake export integrity verification
...
find_package(XCBImdkit REQUIRED CONFIG)

message(STATUS "find_package(XCBImdkit) succeeded")
EOF

            rlLogInfo "Verifying find_package(XCBImdkit CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package XCBImdkit CONFIG)"; then
                rlPass "find_package(XCBImdkit CONFIG) verification passed"
            else
                rlFail "find_package(XCBImdkit CONFIG) verification failed"
            fi
