#!/bin/bash
# Functional test: kpipewire-devel - cmake export integrity verification
...
find_package(KPipeWire REQUIRED CONFIG)

message(STATUS "find_package(KPipeWire) succeeded")
EOF

            rlLogInfo "Verifying find_package(KPipeWire CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KPipeWire CONFIG)"; then
                rlPass "find_package(KPipeWire CONFIG) verification passed"
            else
                rlFail "find_package(KPipeWire CONFIG) verification failed"
            fi
