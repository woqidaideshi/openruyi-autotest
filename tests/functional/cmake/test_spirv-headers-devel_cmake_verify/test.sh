#!/bin/bash
# Functional test: spirv-headers-devel - cmake export integrity verification
...
message(STATUS "find_package(SPIRV-Headers) succeeded")
EOF

            rlLogInfo "Verifying find_package(SPIRV-Headers CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package SPIRV-Headers CONFIG)"; then
                rlPass "find_package(SPIRV-Headers CONFIG) verification passed"
            else
                rlFail "find_package(SPIRV-Headers CONFIG) verification failed"
            fi
