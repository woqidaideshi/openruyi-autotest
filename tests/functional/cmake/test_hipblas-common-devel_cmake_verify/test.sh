#!/bin/bash
# Functional test: hipblas-common-devel - cmake export integrity verification
...
find_package(hipblas-common REQUIRED CONFIG)

message(STATUS "find_package(hipblas-common) succeeded")
EOF

            rlLogInfo "Verifying find_package(hipblas-common CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package hipblas-common CONFIG)"; then
                rlPass "find_package(hipblas-common CONFIG) verification passed"
            else
                rlFail "find_package(hipblas-common CONFIG) verification failed"
            fi
