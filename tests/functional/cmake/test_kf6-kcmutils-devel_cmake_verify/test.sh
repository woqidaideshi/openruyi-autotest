#!/bin/bash
# Functional test: kf6-kcmutils-devel - cmake export integrity verification
...
find_package(KF6KCMUtils REQUIRED CONFIG)

message(STATUS "find_package(KF6KCMUtils) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6KCMUtils CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6KCMUtils CONFIG)"; then
                rlPass "find_package(KF6KCMUtils CONFIG) verification passed"
            else
                rlFail "find_package(KF6KCMUtils CONFIG) verification failed"
            fi
