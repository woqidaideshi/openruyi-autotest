#!/bin/bash
# Functional test: kf6-qqc2-desktop-style-devel - cmake export integrity verification
...
PKG="kf6-qqc2-desktop-style-devel"
...
find_package(KF6QQC2DesktopStyle REQUIRED CONFIG)

message(STATUS "find_package(KF6QQC2DesktopStyle) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6QQC2DesktopStyle CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6QQC2DesktopStyle CONFIG)"; then
                rlPass "find_package(KF6QQC2DesktopStyle CONFIG) verification passed"
            else
                rlFail "find_package(KF6QQC2DesktopStyle CONFIG) verification failed"
            fi
