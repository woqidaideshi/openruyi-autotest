#!/bin/bash
# Functional test: librdkafka-devel - cmake export integrity verification
...
EOF
            ...
            rlLogInfo "Verifying find_package(RdKafka CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package RdKafka CONFIG)"; then
                rlPass "find_package(RdKafka CONFIG) verification passed"
            else
                rlFail "find_package(RdKafka CONFIG) verification failed"
            fi
