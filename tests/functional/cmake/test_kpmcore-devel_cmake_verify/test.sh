#!/bin/bash
# Functional test: kpmcore-devel - cmake export integrity verification
...
find_package(KPMcore REQUIRED CONFIG)

message(STATUS "find_package(KPMcore) succeeded")
EOF

            rlLogInfo "Verifying find_package(KPMcore CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KPMcore CONFIG)"; then
                rlPass "find_package(KPMcore CONFIG) verification passed"
            else
                rlFail "find_package(KPMcore CONFIG) verification failed"
            fi
