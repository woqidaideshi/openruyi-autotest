#!/bin/bash
# Functional test: grpc-devel - cmake export integrity verification
...
message(STATUS "find_package(gRPC) succeeded")
EOF

            rlLogInfo "Verifying find_package(gRPC CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package gRPC CONFIG)"; then
                rlPass "find_package(gRPC CONFIG) verification passed"
            else
                rlFail "find_package(gRPC CONFIG) verification failed"
            fi
