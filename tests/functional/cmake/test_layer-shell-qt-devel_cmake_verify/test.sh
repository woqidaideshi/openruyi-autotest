#!/bin/bash
# Functional test: layer-shell-qt-devel - cmake export integrity verification
...
message(STATUS "find_package(LayerShellQt) succeeded")
EOF

            rlLogInfo "Verifying find_package(LayerShellQt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LayerShellQt CONFIG)"; then
                rlPass "find_package(LayerShellQt CONFIG) verification passed"
            else
                rlFail "find_package(LayerShellQt CONFIG) verification failed"
            fi
