#!/bin/bash
# Functional test: kf6-kservice-devel - cmake export integrity verification
...
find_package(KF6Service REQUIRED CONFIG)

message(STATUS "find_package(KF6Service) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Service CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Service CONFIG)"; then
                rlPass "find_package(KF6Service CONFIG) verification passed"
            else
                rlFail "find_package(KF6Service CONFIG) verification failed"
            fi
