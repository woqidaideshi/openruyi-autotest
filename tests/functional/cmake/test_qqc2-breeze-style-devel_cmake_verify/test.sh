#!/bin/bash
# Functional test: qqc2-breeze-style-devel - cmake export integrity verification
...
find_package(QQC2BreezeStyle REQUIRED CONFIG)

message(STATUS "find_package(QQC2BreezeStyle) succeeded")
EOF

            rlLogInfo "Verifying find_package(QQC2BreezeStyle CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package QQC2BreezeStyle CONFIG)"; then
                rlPass "find_package(QQC2BreezeStyle CONFIG) verification passed"
            else
                rlFail "find_package(QQC2BreezeStyle CONFIG) verification failed"
            fi
