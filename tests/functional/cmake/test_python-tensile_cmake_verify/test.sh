#!/bin/bash
# Functional test: python-tensile - cmake export integrity verification
...
find_package(Tensile REQUIRED CONFIG
  PATHS /usr/lib/python3.13/site-packages/Tensile/cmake
  NO_DEFAULT_PATH)

message(STATUS "find_package(Tensile) succeeded")
EOF

            rlLogInfo "Verifying find_package(Tensile CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Tensile CONFIG)"; then
                rlPass "find_package(Tensile CONFIG) verification passed"
            else
                rlFail "find_package(Tensile CONFIG) verification failed"
            fi
