#!/bin/bash
# Functional test: libogg-devel - cmake export integrity verification
...
message(STATUS "find_package(Ogg) succeeded")
EOF

            rlLogInfo "Verifying find_package(Ogg CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Ogg CONFIG)"; then
                rlPass "find_package(Ogg CONFIG) verification passed"
            else
                rlFail "find_package(Ogg CONFIG) verification failed"
            fi
