#!/bin/bash
# Functional test: harfbuzz-devel - cmake export integrity verification
...
message(STATUS "find_package(harfbuzz) succeeded")
EOF

            rlLogInfo "Verifying find_package(harfbuzz CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package harfbuzz CONFIG)"; then
                rlPass "find_package(harfbuzz CONFIG) verification passed"
            else
                rlFail "find_package(harfbuzz CONFIG) verification failed"
            fi
