#!/bin/bash
# Functional test: utf8cpp - cmake export integrity verification
...
message(STATUS "find_package(utf8cpp) succeeded")
EOF

            rlLogInfo "Verifying find_package(utf8cpp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package utf8cpp CONFIG)"; then
                rlPass "find_package(utf8cpp CONFIG) verification passed"
            else
                rlFail "find_package(utf8cpp CONFIG) verification failed"
            fi
