#!/bin/bash
# Functional test: zxing-cpp-devel - cmake export integrity verification
...
message(STATUS "find_package(ZXing) succeeded")
EOF

            rlLogInfo "Verifying find_package(ZXing CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package ZXing CONFIG)"; then
                rlPass "find_package(ZXing CONFIG) verification passed"
            else
                rlFail "find_package(ZXing CONFIG) verification failed"
            fi
