#!/bin/bash
# Functional test: vulkan-headers - cmake export integrity verification
...
message(STATUS "find_package(VulkanHeaders) succeeded")
EOF

            rlLogInfo "Verifying find_package(VulkanHeaders CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package VulkanHeaders CONFIG)"; then
                rlPass "find_package(VulkanHeaders CONFIG) verification passed"
            else
                rlFail "find_package(VulkanHeaders CONFIG) verification failed"
            fi
