#!/bin/bash
# Functional test: shiboken6 - cmake export integrity verification
...
find_package(Shiboken6Tools REQUIRED CONFIG)

message(STATUS "find_package(Shiboken6Tools) succeeded")
EOF

            rlLogInfo "Verifying find_package(Shiboken6Tools CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Shiboken6Tools CONFIG)"; then
                rlPass "find_package(Shiboken6Tools CONFIG) verification passed"
            else
                rlFail "find_package(Shiboken6Tools CONFIG) verification failed"
            fi
