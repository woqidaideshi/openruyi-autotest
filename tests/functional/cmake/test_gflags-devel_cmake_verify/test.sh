#!/bin/bash
# Functional test: gflags-devel - cmake export integrity verification
...
message(STATUS "find_package(gflags) succeeded")
EOF

            rlLogInfo "Verifying find_package(gflags CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package gflags CONFIG)"; then
                rlPass "find_package(gflags CONFIG) verification passed"
            else
                rlFail "find_package(gflags CONFIG) verification failed"
            fi
