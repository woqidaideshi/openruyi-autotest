#!/bin/bash
# Functional test: kf6-extra-cmake-modules - cmake export integrity verification
...
message(STATUS "find_package(ECM) succeeded")
EOF

            rlLogInfo "Verifying find_package(ECM CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package ECM CONFIG)"; then
                rlPass "find_package(ECM CONFIG) verification passed"
            else
                rlFail "find_package(ECM CONFIG) verification failed"
            fi
