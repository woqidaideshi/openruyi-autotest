#!/bin/bash
# Functional test: glog-devel - cmake export integrity verification
...
message(STATUS "find_package(glog) succeeded")
EOF

            rlLogInfo "Verifying find_package(glog CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package glog CONFIG)"; then
                rlPass "find_package(glog CONFIG) verification passed"
            else
                rlFail "find_package(glog CONFIG) verification failed"
            fi
