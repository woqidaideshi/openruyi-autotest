#!/bin/bash
# Functional test: vulkan-loader-devel - cmake export integrity verification
...
message(STATUS "find_package(VulkanLoader) succeeded")
EOF

            rlLogInfo "Verifying find_package(VulkanLoader CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package VulkanLoader CONFIG)"; then
                rlPass "find_package(VulkanLoader CONFIG) verification passed"
            else
                rlFail "find_package(VulkanLoader CONFIG) verification failed"
            fi
