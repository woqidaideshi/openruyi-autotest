#!/bin/bash
# Functional test: kf6-kxmlgui-devel - cmake export integrity verification
...
find_package(KF6XmlGui REQUIRED CONFIG)

message(STATUS "find_package(KF6XmlGui) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6XmlGui CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6XmlGui CONFIG)"; then
                rlPass "find_package(KF6XmlGui CONFIG) verification passed"
            else
                rlFail "find_package(KF6XmlGui CONFIG) verification failed"
            fi
