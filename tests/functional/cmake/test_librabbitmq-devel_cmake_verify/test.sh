#!/bin/bash
# Functional test: librabbitmq-devel - cmake export integrity verification
...
message(STATUS "find_package(rabbitmq-c) succeeded")
EOF

            rlLogInfo "Verifying find_package(rabbitmq-c CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rabbitmq-c CONFIG)"; then
                rlPass "find_package(rabbitmq-c CONFIG) verification passed"
            else
                rlFail "find_package(rabbitmq-c CONFIG) verification failed"
            fi
