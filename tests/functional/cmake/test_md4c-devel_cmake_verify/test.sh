#!/bin/bash
# Functional test: md4c-devel - cmake export integrity verification
...
find_package(md4c REQUIRED CONFIG)

message(STATUS "find_package(md4c) succeeded")
EOF

            rlLogInfo "Verifying find_package(md4c CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package md4c CONFIG)"; then
                rlPass "find_package(md4c CONFIG) verification passed"
            else
                rlFail "find_package(md4c CONFIG) verification failed"
            fi
