#!/bin/bash
# Functional test: gtest-devel - cmake export integrity verification
...
message(STATUS "find_package(GTest) succeeded")
EOF

            rlLogInfo "Verifying find_package(GTest CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package GTest CONFIG)"; then
                rlPass "find_package(GTest CONFIG) verification passed"
            else
                rlFail "find_package(GTest CONFIG) verification failed"
            fi
