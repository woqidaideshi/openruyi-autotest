#!/bin/bash
# Functional test: openblas-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenBLAS) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenBLAS CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenBLAS CONFIG)"; then
                rlPass "find_package(OpenBLAS CONFIG) verification passed"
            else
                rlFail "find_package(OpenBLAS CONFIG) verification failed"
            fi
