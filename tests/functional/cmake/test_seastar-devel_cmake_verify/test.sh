#!/bin/bash
# Functional test: seastar-devel - cmake export integrity verification
...
message(STATUS "find_package(Seastar) succeeded")
EOF

            rlLogInfo "Verifying find_package(Seastar CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Seastar CONFIG)"; then
                rlPass "find_package(Seastar CONFIG) verification passed"
            else
                rlFail "find_package(Seastar CONFIG) verification failed"
            fi
