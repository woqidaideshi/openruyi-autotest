#!/bin/bash
# Functional test: rust-libnghttp2-sys-0.1 - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "cargo check - Verify crate build integrity"
        ...
        # --- cargo check libnghttp2-sys ---
        rlLogInfo "Verifying crate: libnghttp2-sys v$CRATE_VERSION"
        test_proj="$TmpDir2/test_libnghttp2_sys"
        rm -rf "$test_proj"
        mkdir -p "$test_proj/src"

        cat > "$test_proj/Cargo.toml" << CARGOEOF
[package]
name = "cmake_verify_libnghttp2_sys"
version = "0.1.0"
edition = "2021"

[dependencies]
libnghttp2-sys = "${CRATE_VERSION}"
CARGOEOF
        echo 'fn main() { println!("cmake verify ok"); }' > "$test_proj/src/main.rs"

        cd "$test_proj"
        if rlRun "cargo check 2>&1" 0 \
            "cargo check libnghttp2-sys v$CRATE_VERSION"; then
            rlPass "cargo check libnghttp2-sys passed"
        else
            rlLogWarning "cargo check libnghttp2-sys failed (may need network to download dependencies)"
        fi
