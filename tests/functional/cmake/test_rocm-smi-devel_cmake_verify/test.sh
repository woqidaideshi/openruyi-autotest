#!/bin/bash
# Functional test: rocm-smi-devel - cmake export integrity verification
...
find_package(rocm_smi REQUIRED CONFIG)

message(STATUS "find_package(rocm_smi) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocm_smi CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocm_smi CONFIG)"; then
                rlPass "find_package(rocm_smi CONFIG) verification passed"
            else
                rlFail "find_package(rocm_smi CONFIG) verification failed"
            fi
