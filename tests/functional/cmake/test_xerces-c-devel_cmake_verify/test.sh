#!/bin/bash
# Functional test: xerces-c-devel - cmake export integrity verification
...
message(STATUS "find_package(XercesC) succeeded")
EOF

            rlLogInfo "Verifying find_package(XercesC CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package XercesC CONFIG)"; then
                rlPass "find_package(XercesC CONFIG) verification passed"
            else
                rlFail "find_package(XercesC CONFIG) verification failed"
            fi
