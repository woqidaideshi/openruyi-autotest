#!/bin/bash
# Functional test: libwebp-devel - cmake export integrity verification
...
message(STATUS "find_package(WebP) succeeded")
EOF

            rlLogInfo "Verifying find_package(WebP CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package WebP CONFIG)"; then
                rlPass "find_package(WebP CONFIG) verification passed"
            else
                rlFail "find_package(WebP CONFIG) verification failed"
            fi
