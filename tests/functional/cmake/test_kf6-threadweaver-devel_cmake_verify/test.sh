#!/bin/bash
# Functional test: kf6-threadweaver-devel - cmake export integrity verification
...
find_package(KF6ThreadWeaver REQUIRED CONFIG)

message(STATUS "find_package(KF6ThreadWeaver) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ThreadWeaver CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ThreadWeaver CONFIG)"; then
                rlPass "find_package(KF6ThreadWeaver CONFIG) verification passed"
            else
                rlFail "find_package(KF6ThreadWeaver CONFIG) verification failed"
            fi
