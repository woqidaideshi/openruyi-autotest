#!/bin/bash
# Functional test: python-pyside6-devel - cmake export integrity verification
...
message(STATUS "find_package(PySide6) succeeded")
EOF

            rlLogInfo "Verifying find_package(PySide6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PySide6 CONFIG)"; then
                rlPass "find_package(PySide6 CONFIG) verification passed"
            else
                rlFail "find_package(PySide6 CONFIG) verification failed"
            fi
