#!/bin/bash
# Functional test: kirigami-addons-devel - cmake export integrity verification
...
message(STATUS "find_package(KF6KirigamiAddons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6KirigamiAddons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6KirigamiAddons CONFIG)"; then
                rlPass "find_package(KF6KirigamiAddons CONFIG) verification passed"
            else
                rlFail "find_package(KF6KirigamiAddons CONFIG) verification failed"
            fi
