#!/bin/bash
# Functional test: kwayland6-devel - cmake export integrity verification
...
find_package(KWayland REQUIRED CONFIG)

message(STATUS "find_package(KWayland) succeeded")
EOF

            rlLogInfo "Verifying find_package(KWayland CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KWayland CONFIG)"; then
                rlPass "find_package(KWayland CONFIG) verification passed"
            else
                rlFail "find_package(KWayland CONFIG) verification failed"
            fi
