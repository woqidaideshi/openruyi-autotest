#!/bin/bash
# Functional test: portaudio-devel - cmake export integrity verification
...
message(STATUS "find_package(portaudio) succeeded")
EOF

            rlLogInfo "Verifying find_package(portaudio CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package portaudio CONFIG)"; then
                rlPass "find_package(portaudio CONFIG) verification passed"
            else
                rlFail "find_package(portaudio CONFIG) verification failed"
            fi
