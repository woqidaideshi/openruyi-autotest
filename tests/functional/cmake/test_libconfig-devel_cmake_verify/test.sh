#!/bin/bash
# Functional test: libconfig-devel - cmake export integrity verification
...
message(STATUS "find_package(libconfig) succeeded")
EOF

            rlLogInfo "Verifying find_package(libconfig CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libconfig CONFIG)"; then
                rlPass "find_package(libconfig CONFIG) verification passed"
            else
                rlFail "find_package(libconfig CONFIG) verification failed"
            fi
