#!/bin/bash
# Functional test: msgpack-devel - cmake export integrity verification
...
message(STATUS "find_package(msgpack) succeeded")
EOF

            rlLogInfo "Verifying find_package(msgpack CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package msgpack CONFIG)"; then
                rlPass "find_package(msgpack CONFIG) verification passed"
            else
                rlFail "find_package(msgpack CONFIG) verification failed"
            fi
