#!/bin/bash
# Functional test: gstreamer-devel - cmake export integrity verification
...
find_package(GStreamer MODULE QUIET)
EOF

        rlLogInfo "Verifying find_package(GStreamer MODULE) ..."
        rm -rf "$TmpDir2/build_0"
        if rlRun "cmake -S $TmpDir2 -B $TmpDir2/build_0" 0 \
            "cmake configuration (find_package GStreamer MODULE)"; then
            rlPass "find_package(GStreamer MODULE) verification passed"
        else
            rlLogWarning "find_package(GStreamer MODULE) verification failed(the library being searched may not be installed)"
        fi
