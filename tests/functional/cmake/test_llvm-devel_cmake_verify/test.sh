#!/bin/bash
# Functional test: llvm-devel - cmake export integrity verification
...
message(STATUS "find_package(LLVM) succeeded")
EOF

            rlLogInfo "Verifying find_package(LLVM CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LLVM CONFIG)"; then
                rlPass "find_package(LLVM CONFIG) verification passed"
            else
                rlFail "find_package(LLVM CONFIG) verification failed"
            fi
