#!/bin/bash
# Functional test: libavif-devel - cmake export integrity verification
...
message(STATUS "find_package(libavif) succeeded")
EOF

            rlLogInfo "Verifying find_package(libavif CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libavif CONFIG)"; then
                rlPass "find_package(libavif CONFIG) verification passed"
            else
                rlFail "find_package(libavif CONFIG) verification failed"
            fi
