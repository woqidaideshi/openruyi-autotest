#!/bin/bash
# Functional test: highway-devel - cmake export integrity verification
...
message(STATUS "find_package(hwy) succeeded")
EOF

            rlLogInfo "Verifying find_package(hwy CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package hwy CONFIG)"; then
                rlPass "find_package(hwy CONFIG) verification passed"
            else
                rlFail "find_package(hwy CONFIG) verification failed"
            fi
