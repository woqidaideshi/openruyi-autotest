#!/bin/bash
# Functional test: kf6-attica-devel - cmake export integrity verification
...
find_package(KF6Attica REQUIRED CONFIG)

message(STATUS "find_package(KF6Attica) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Attica CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Attica CONFIG)"; then
                rlPass "find_package(KF6Attica CONFIG) verification passed"
            else
                rlFail "find_package(KF6Attica CONFIG) verification failed"
            fi
