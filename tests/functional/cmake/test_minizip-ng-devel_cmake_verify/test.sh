#!/bin/bash
# Functional test: minizip-ng-devel - cmake export integrity verification
...
message(STATUS "find_package(minizip) succeeded")
EOF

            rlLogInfo "Verifying find_package(minizip CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package minizip CONFIG)"; then
                rlPass "find_package(minizip CONFIG) verification passed"
            else
                rlFail "find_package(minizip CONFIG) verification failed"
            fi
