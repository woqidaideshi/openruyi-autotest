#!/bin/bash
# Functional test: python-shiboken6-devel - cmake export integrity verification
...
message(STATUS "find_package(Shiboken6) succeeded")
EOF

            rlLogInfo "Verifying find_package(Shiboken6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Shiboken6 CONFIG)"; then
                rlPass "find_package(Shiboken6 CONFIG) verification passed"
            else
                rlFail "find_package(Shiboken6 CONFIG) verification failed"
            fi
