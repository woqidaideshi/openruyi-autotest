#!/bin/bash
# Functional test: libvncserver-devel - cmake export integrity verification
...
find_package(LibVNCServer REQUIRED CONFIG)

message(STATUS "find_package(LibVNCServer) succeeded")
EOF

            rlLogInfo "Verifying find_package(LibVNCServer CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LibVNCServer CONFIG)"; then
                rlPass "find_package(LibVNCServer CONFIG) verification passed"
            else
                rlFail "find_package(LibVNCServer CONFIG) verification failed"
            fi
