#!/bin/bash
# Functional test: kf6-kpty-devel - cmake export integrity verification
...
find_package(KF6Pty REQUIRED CONFIG)

message(STATUS "find_package(KF6Pty) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Pty CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Pty CONFIG)"; then
                rlPass "find_package(KF6Pty CONFIG) verification passed"
            else
                rlFail "find_package(KF6Pty CONFIG) verification failed"
            fi
