#!/bin/bash
# Functional test: libqaccessibilityclient-devel - cmake export integrity verification
...
PKG="libqaccessibilityclient-devel"
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(QAccessibilityClient6) ---
...
find_package(QAccessibilityClient6 REQUIRED CONFIG)

message(STATUS "find_package(QAccessibilityClient6) succeeded")
EOF

            rlLogInfo "Verifying find_package(QAccessibilityClient6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package QAccessibilityClient6 CONFIG)"; then
                rlPass "find_package(QAccessibilityClient6 CONFIG) verification passed"
            else
                rlFail "find_package(QAccessibilityClient6 CONFIG) verification failed"
            fi
