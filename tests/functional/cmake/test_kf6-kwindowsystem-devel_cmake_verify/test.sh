#!/bin/bash
# Functional test: kf6-kwindowsystem-devel - cmake export integrity verification
...
find_package(KF6WindowSystem REQUIRED CONFIG)

message(STATUS "find_package(KF6WindowSystem) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6WindowSystem CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6WindowSystem CONFIG)"; then
                rlPass "find_package(KF6WindowSystem CONFIG) verification passed"
            else
                rlFail "find_package(KF6WindowSystem CONFIG) verification failed"
            fi
