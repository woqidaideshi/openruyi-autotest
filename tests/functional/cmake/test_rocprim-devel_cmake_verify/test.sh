#!/bin/bash
# Functional test: rocprim-devel - cmake export integrity verification
...
message(STATUS "find_package(rocprim) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocprim CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocprim CONFIG)"; then
                rlPass "find_package(rocprim CONFIG) verification passed"
            else
                rlFail "find_package(rocprim CONFIG) verification failed"
            fi
