#!/bin/bash
# Functional test: qhull-devel-8.1~alpha6 - cmake export integrity verification
...
message(STATUS "find_package(Qhull) succeeded")
EOF

            rlLogInfo "Verifying find_package(Qhull CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Qhull CONFIG)"; then
                rlPass "find_package(Qhull CONFIG) verification passed"
            else
                rlFail "find_package(Qhull CONFIG) verification failed"
            fi
