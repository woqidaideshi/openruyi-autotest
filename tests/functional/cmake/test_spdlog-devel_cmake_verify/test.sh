#!/bin/bash
# Functional test: spdlog-devel - cmake export integrity verification
...
message(STATUS "find_package(spdlog) succeeded")
EOF

            rlLogInfo "Verifying find_package(spdlog CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package spdlog CONFIG)"; then
                rlPass "find_package(spdlog CONFIG) verification passed"
            else
                rlFail "find_package(spdlog CONFIG) verification failed"
            fi
