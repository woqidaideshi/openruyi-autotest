#!/bin/bash
# Functional test: re2-devel - cmake export integrity verification
...
message(STATUS "find_package(re2) succeeded")
EOF

            rlLogInfo "Verifying find_package(re2 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package re2 CONFIG)"; then
                rlPass "find_package(re2 CONFIG) verification passed"
            else
                rlFail "find_package(re2 CONFIG) verification failed"
            fi
