#!/bin/bash
# Functional test: kf6-kdoctools-devel - cmake export integrity verification
...
find_package(KF6DocTools REQUIRED CONFIG)

message(STATUS "find_package(KF6DocTools) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6DocTools CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6DocTools CONFIG)"; then
                rlPass "find_package(KF6DocTools CONFIG) verification passed"
            else
                rlFail "find_package(KF6DocTools CONFIG) verification failed"
            fi
