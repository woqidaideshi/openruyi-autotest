#!/bin/bash
# Functional test: libpamtest-devel - cmake export integrity verification
...
find_package(pamtest REQUIRED CONFIG)

message(STATUS "find_package(pamtest) succeeded")
EOF

            rlLogInfo "Verifying find_package(pamtest CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package pamtest CONFIG)"; then
                rlPass "find_package(pamtest CONFIG) verification passed"
            else
                rlFail "find_package(pamtest CONFIG) verification failed"
            fi
