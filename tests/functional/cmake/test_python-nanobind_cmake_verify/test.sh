#!/bin/bash
# Functional test: python-nanobind - cmake export integrity verification
...
find_package(nanobind REQUIRED CONFIG
  PATHS /usr/lib/python3.13/site-packages/nanobind/cmake
  NO_DEFAULT_PATH)

message(STATUS "find_package(nanobind) succeeded")
EOF

            rlLogInfo "Verifying find_package(nanobind CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package nanobind CONFIG)"; then
                rlPass "find_package(nanobind CONFIG) verification passed"
            else
                rlFail "find_package(nanobind CONFIG) verification failed"
            fi
