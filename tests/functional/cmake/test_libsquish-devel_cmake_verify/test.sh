#!/bin/bash
# Functional test: libsquish-devel - cmake export integrity verification
...
message(STATUS "find_package(libsquish) succeeded")
EOF

            rlLogInfo "Verifying find_package(libsquish CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libsquish CONFIG)"; then
                rlPass "find_package(libsquish CONFIG) verification passed"
            else
                rlFail "find_package(libsquish CONFIG) verification failed"
            fi
