#!/bin/bash
# Functional test: PackageKit-Qt-devel - cmake export integrity verification
...
find_package(packagekitqt6 REQUIRED CONFIG)

message(STATUS "find_package(packagekitqt6) succeeded")
EOF

            rlLogInfo "Verifying find_package(packagekitqt6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package packagekitqt6 CONFIG)"; then
                rlPass "find_package(packagekitqt6 CONFIG) verification passed"
            else
                rlFail "find_package(packagekitqt6 CONFIG) verification failed"
            fi
