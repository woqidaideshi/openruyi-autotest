#!/bin/bash
# Functional test: rocsparse-devel - cmake export integrity verification
...
message(STATUS "find_package(rocsparse) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocsparse CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocsparse CONFIG)"; then
                rlPass "find_package(rocsparse CONFIG) verification passed"
            else
                rlFail "find_package(rocsparse CONFIG) verification failed"
            fi
