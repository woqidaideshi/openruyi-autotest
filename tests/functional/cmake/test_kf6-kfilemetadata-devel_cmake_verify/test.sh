#!/bin/bash
# Functional test: kf6-kfilemetadata-devel - cmake export integrity verification
...
find_package(KF6FileMetaData REQUIRED CONFIG)

message(STATUS "find_package(KF6FileMetaData) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6FileMetaData CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6FileMetaData CONFIG)"; then
                rlPass "find_package(KF6FileMetaData CONFIG) verification passed"
            else
                rlFail "find_package(KF6FileMetaData CONFIG) verification failed"
            fi
