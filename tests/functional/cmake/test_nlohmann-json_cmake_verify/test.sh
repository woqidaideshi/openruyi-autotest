#!/bin/bash
# Functional test: nlohmann-json - cmake export integrity verification
...
message(STATUS "find_package(nlohmann_json) succeeded")
EOF

            rlLogInfo "Verifying find_package(nlohmann_json CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package nlohmann_json CONFIG)"; then
                rlPass "find_package(nlohmann_json CONFIG) verification passed"
            else
                rlFail "find_package(nlohmann_json CONFIG) verification failed"
            fi
