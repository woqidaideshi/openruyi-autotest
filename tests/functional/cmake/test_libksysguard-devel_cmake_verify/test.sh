#!/bin/bash
# Functional test: libksysguard-devel - cmake export integrity verification
...
find_package(KSysGuard REQUIRED CONFIG)

message(STATUS "find_package(KSysGuard) succeeded")
EOF

            rlLogInfo "Verifying find_package(KSysGuard CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KSysGuard CONFIG)"; then
                rlPass "find_package(KSysGuard CONFIG) verification passed"
            else
                rlFail "find_package(KSysGuard CONFIG) verification failed"
            fi
