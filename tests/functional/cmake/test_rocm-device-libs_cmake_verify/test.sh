#!/bin/bash
# Functional test: rocm-device-libs - cmake export integrity verification
...
message(STATUS "find_package(AMDDeviceLibs) succeeded")
EOF

            rlLogInfo "Verifying find_package(AMDDeviceLibs CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package AMDDeviceLibs CONFIG)"; then
                rlPass "find_package(AMDDeviceLibs CONFIG) verification passed"
            else
                rlFail "find_package(AMDDeviceLibs CONFIG) verification failed"
            fi
