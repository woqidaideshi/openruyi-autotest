#!/bin/bash
# Functional test: libxml2-devel - cmake export integrity verification
...
message(STATUS "find_package(libxml2) succeeded")
EOF

            rlLogInfo "Verifying find_package(libxml2 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libxml2 CONFIG)"; then
                rlPass "find_package(libxml2 CONFIG) verification passed"
            else
                rlFail "find_package(libxml2 CONFIG) verification failed"
            fi
