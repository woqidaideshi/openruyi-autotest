#!/bin/bash
# Functional test: llhttp-devel - cmake export integrity verification
...
message(STATUS "find_package(llhttp) succeeded")
EOF

            rlLogInfo "Verifying find_package(llhttp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package llhttp CONFIG)"; then
                rlPass "find_package(llhttp CONFIG) verification passed"
            else
                rlFail "find_package(llhttp CONFIG) verification failed"
            fi
