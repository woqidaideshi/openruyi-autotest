#!/bin/bash
# Functional test: rust-curl-sys-0.4 - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "cargo check - Verify crate build integrity"
        ...
        # --- cargo check curl-sys ---
        rlLogInfo "Verifying crate: curl-sys v$CRATE_VERSION"
        test_proj="$TmpDir2/test_curl_sys"
        rm -rf "$test_proj"
        mkdir -p "$test_proj/src"

        cat > "$test_proj/Cargo.toml" << CARGOEOF
[package]
name = "cmake_verify_curl_sys"
version = "0.1.0"
edition = "2021"

[dependencies]
curl-sys = "${CRATE_VERSION}"
CARGOEOF
        echo 'fn main() { println!("cmake verify ok"); }' > "$test_proj/src/main.rs"

        cd "$test_proj"
        if rlRun "cargo check 2>&1" 0 \
            "cargo check curl-sys v$CRATE_VERSION"; then
            rlPass "cargo check curl-sys passed"
        else
            rlLogWarning "cargo check curl-sys failed (may need network to download dependencies)"
        fi
