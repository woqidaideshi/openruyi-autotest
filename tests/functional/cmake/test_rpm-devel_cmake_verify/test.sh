#!/bin/bash
# Functional test: rpm-devel - cmake export integrity verification
...
find_package(rpm REQUIRED CONFIG)

message(STATUS "find_package(rpm) succeeded")
EOF

            rlLogInfo "Verifying find_package(rpm CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rpm CONFIG)"; then
                rlPass "find_package(rpm CONFIG) verification passed"
            else
                rlFail "find_package(rpm CONFIG) verification failed"
            fi
