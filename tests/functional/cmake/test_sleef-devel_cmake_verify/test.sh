#!/bin/bash
# Functional test: sleef-devel - cmake export integrity verification
...
message(STATUS "find_package(sleef) succeeded")
EOF

            rlLogInfo "Verifying find_package(sleef CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package sleef CONFIG)"; then
                rlPass "find_package(sleef CONFIG) verification passed"
            else
                rlFail "find_package(sleef CONFIG) verification failed"
            fi
