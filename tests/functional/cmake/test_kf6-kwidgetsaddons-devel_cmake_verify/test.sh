#!/bin/bash
# Functional test: kf6-kwidgetsaddons-devel - cmake export integrity verification
...
find_package(KF6WidgetsAddons REQUIRED CONFIG)

message(STATUS "find_package(KF6WidgetsAddons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6WidgetsAddons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6WidgetsAddons CONFIG)"; then
                rlPass "find_package(KF6WidgetsAddons CONFIG) verification passed"
            else
                rlFail "find_package(KF6WidgetsAddons CONFIG) verification failed"
            fi
