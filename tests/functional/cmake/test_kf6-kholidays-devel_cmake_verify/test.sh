#!/bin/bash
# Functional test: kf6-kholidays-devel - cmake export integrity verification
...
find_package(KF6Holidays REQUIRED CONFIG)

message(STATUS "find_package(KF6Holidays) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Holidays CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Holidays CONFIG)"; then
                rlPass "find_package(KF6Holidays CONFIG) verification passed"
            else
                rlFail "find_package(KF6Holidays CONFIG) verification failed"
            fi
