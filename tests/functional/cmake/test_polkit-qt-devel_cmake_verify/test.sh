#!/bin/bash
# Functional test: polkit-qt-devel - cmake export integrity verification
...
find_package(PolkitQt6-1 REQUIRED CONFIG)

message(STATUS "find_package(PolkitQt6-1) succeeded")
EOF

            rlLogInfo "Verifying find_package(PolkitQt6-1 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PolkitQt6-1 CONFIG)"; then
                rlPass "find_package(PolkitQt6-1 CONFIG) verification passed"
            else
                rlFail "find_package(PolkitQt6-1 CONFIG) verification failed"
            fi
