#!/bin/bash
# Functional test: range-v3-devel - cmake export integrity verification
...
message(STATUS "find_package(range-v3) succeeded")
EOF

            rlLogInfo "Verifying find_package(range-v3 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package range-v3 CONFIG)"; then
                rlPass "find_package(range-v3 CONFIG) verification passed"
            else
                rlFail "find_package(range-v3 CONFIG) verification failed"
            fi
