#!/bin/bash
# Functional test: polly22-devel - cmake export integrity verification
...
message(STATUS "find_package(Polly) succeeded")
EOF

            rlLogInfo "Verifying find_package(Polly CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Polly CONFIG)"; then
                rlPass "find_package(Polly CONFIG) verification passed"
            else
                rlFail "find_package(Polly CONFIG) verification failed"
            fi
