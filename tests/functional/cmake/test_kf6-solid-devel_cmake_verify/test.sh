#!/bin/bash
# Functional test: kf6-solid-devel - cmake export integrity verification
...
find_package(KF6Solid REQUIRED CONFIG)

message(STATUS "find_package(KF6Solid) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Solid CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Solid CONFIG)"; then
                rlPass "find_package(KF6Solid CONFIG) verification passed"
            else
                rlFail "find_package(KF6Solid CONFIG) verification failed"
            fi
