#!/bin/bash
# Functional test: libjwt-devel - cmake export integrity verification
...
message(STATUS "find_package(LibJWT) succeeded")
EOF

            rlLogInfo "Verifying find_package(LibJWT CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LibJWT CONFIG)"; then
                rlPass "find_package(LibJWT CONFIG) verification passed"
            else
                rlFail "find_package(LibJWT CONFIG) verification failed"
            fi
