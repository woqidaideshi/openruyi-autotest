#!/bin/bash
# Functional test: kf6-kauth-devel - cmake export integrity verification
...
find_package(KF6Auth REQUIRED CONFIG)

message(STATUS "find_package(KF6Auth) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Auth CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Auth CONFIG)"; then
                rlPass "find_package(KF6Auth CONFIG) verification passed"
            else
                rlFail "find_package(KF6Auth CONFIG) verification failed"
            fi
