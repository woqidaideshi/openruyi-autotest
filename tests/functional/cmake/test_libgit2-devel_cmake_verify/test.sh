#!/bin/bash
# Functional test: libgit2-devel - cmake export integrity verification
...
message(STATUS "find_package(libgit2) succeeded")
EOF

            rlLogInfo "Verifying find_package(libgit2 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libgit2 CONFIG)"; then
                rlPass "find_package(libgit2 CONFIG) verification passed"
            else
                rlFail "find_package(libgit2 CONFIG) verification failed"
            fi
