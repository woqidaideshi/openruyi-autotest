#!/bin/bash
# Functional test: kf6-kcoreaddons-devel - cmake export integrity verification
...
find_package(KF6CoreAddons REQUIRED CONFIG)

message(STATUS "find_package(KF6CoreAddons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6CoreAddons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6CoreAddons CONFIG)"; then
                rlPass "find_package(KF6CoreAddons CONFIG) verification passed"
            else
                rlFail "find_package(KF6CoreAddons CONFIG) verification failed"
            fi
