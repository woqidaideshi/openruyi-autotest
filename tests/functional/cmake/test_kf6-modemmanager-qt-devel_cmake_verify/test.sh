#!/bin/bash
# Functional test: kf6-modemmanager-qt-devel - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(KF6ModemManagerQt) ---
...
find_package(KF6ModemManagerQt REQUIRED CONFIG)

message(STATUS "find_package(KF6ModemManagerQt) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ModemManagerQt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ModemManagerQt CONFIG)"; then
                rlPass "find_package(KF6ModemManagerQt CONFIG) verification passed"
            else
                rlFail "find_package(KF6ModemManagerQt CONFIG) verification failed"
            fi
