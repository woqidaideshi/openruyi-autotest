#!/bin/bash
# Functional test: libical-devel - cmake export integrity verification
...
find_package(LibIcal REQUIRED CONFIG)

message(STATUS "find_package(LibIcal) succeeded")
EOF

            rlLogInfo "Verifying find_package(LibIcal CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LibIcal CONFIG)"; then
                rlPass "find_package(LibIcal CONFIG) verification passed"
            else
                rlFail "find_package(LibIcal CONFIG) verification failed"
            fi
