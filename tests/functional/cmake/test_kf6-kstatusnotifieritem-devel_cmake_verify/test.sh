#!/bin/bash
# Functional test: kf6-kstatusnotifieritem-devel - cmake export integrity verification
...
PKG="kf6-kstatusnotifieritem-devel"
...
find_package(KF6StatusNotifierItem REQUIRED CONFIG)

message(STATUS "find_package(KF6StatusNotifierItem) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6StatusNotifierItem CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6StatusNotifierItem CONFIG)"; then
                rlPass "find_package(KF6StatusNotifierItem CONFIG) verification passed"
            else
                rlFail "find_package(KF6StatusNotifierItem CONFIG) verification failed"
            fi
