#!/bin/bash
# Functional test: onnx-devel - cmake export integrity verification
...
message(STATUS "find_package(ONNX) succeeded")
EOF

            rlLogInfo "Verifying find_package(ONNX CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package ONNX CONFIG)"; then
                rlPass "find_package(ONNX CONFIG) verification passed"
            else
                rlFail "find_package(ONNX CONFIG) verification failed"
            fi
