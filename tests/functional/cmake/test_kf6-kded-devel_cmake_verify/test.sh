#!/bin/bash
# Functional test: kf6-kded-devel - cmake export integrity verification
...
find_package(KF6KDED REQUIRED CONFIG)

message(STATUS "find_package(KF6KDED) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6KDED CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6KDED CONFIG)"; then
                rlPass "find_package(KF6KDED CONFIG) verification passed"
            else
                rlFail "find_package(KF6KDED CONFIG) verification failed"
            fi
