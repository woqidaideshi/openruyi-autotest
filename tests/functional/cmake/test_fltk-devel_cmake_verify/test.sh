#!/bin/bash
# Functional test: fltk-devel - cmake export integrity verification
...
message(STATUS "find_package(FLTK) succeeded")
EOF

            rlLogInfo "Verifying find_package(FLTK CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package FLTK CONFIG)"; then
                rlPass "find_package(FLTK CONFIG) verification passed"
            else
                rlFail "find_package(FLTK CONFIG) verification failed"
            fi
