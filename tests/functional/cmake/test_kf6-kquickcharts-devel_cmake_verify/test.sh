#!/bin/bash
# Functional test: kf6-kquickcharts-devel - cmake export integrity verification
...
find_package(KF6QuickCharts REQUIRED CONFIG)

message(STATUS "find_package(KF6QuickCharts) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6QuickCharts CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6QuickCharts CONFIG)"; then
                rlPass "find_package(KF6QuickCharts CONFIG) verification passed"
            else
                rlFail "find_package(KF6QuickCharts CONFIG) verification failed"
            fi
