#!/bin/bash
# Functional test: kf6-ktexteditor-devel - cmake export integrity verification
...
find_package(KF6TextEditor REQUIRED CONFIG)

message(STATUS "find_package(KF6TextEditor) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6TextEditor CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6TextEditor CONFIG)"; then
                rlPass "find_package(KF6TextEditor CONFIG) verification passed"
            else
                rlFail "find_package(KF6TextEditor CONFIG) verification failed"
            fi
