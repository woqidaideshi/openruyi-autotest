#!/bin/bash
# Functional test: jsoncpp-devel - cmake export integrity verification
...
message(STATUS "find_package(jsoncpp) succeeded")
EOF

            rlLogInfo "Verifying find_package(jsoncpp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package jsoncpp CONFIG)"; then
                rlPass "find_package(jsoncpp CONFIG) verification passed"
            else
                rlFail "find_package(jsoncpp CONFIG) verification failed"
            fi
