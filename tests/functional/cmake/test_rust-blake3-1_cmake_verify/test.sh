#!/bin/bash
# Functional test: rust-blake3-1 - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "cargo check - Verify crate build integrity"
        ...
        # --- cargo check blake3 ---
        rlLogInfo "Verifying crate: blake3 v$CRATE_VERSION"
        test_proj="$TmpDir2/test_blake3"
        rm -rf "$test_proj"
        mkdir -p "$test_proj/src"

        cat > "$test_proj/Cargo.toml" << CARGOEOF
[package]
name = "cmake_verify_blake3"
version = "0.1.0"
edition = "2021"

[dependencies]
blake3 = "${CRATE_VERSION}"
CARGOEOF
        echo 'fn main() { println!("cmake verify ok"); }' > "$test_proj/src/main.rs"

        cd "$test_proj"
        if rlRun "cargo check 2>&1" 0 \
            "cargo check blake3 v$CRATE_VERSION"; then
            rlPass "cargo check blake3 passed"
        else
            rlLogWarning "cargo check blake3 failed (may need network to download dependencies)"
        fi
