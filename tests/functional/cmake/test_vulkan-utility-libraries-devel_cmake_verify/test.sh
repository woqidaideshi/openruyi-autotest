#!/bin/bash
# Functional test: vulkan-utility-libraries-devel - cmake export integrity verification
...
PKG="vulkan-utility-libraries-devel"
...
message(STATUS "find_package(VulkanUtilityLibraries) succeeded")
EOF

            rlLogInfo "Verifying find_package(VulkanUtilityLibraries CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package VulkanUtilityLibraries CONFIG)"; then
                rlPass "find_package(VulkanUtilityLibraries CONFIG) verification passed"
            else
                rlFail "find_package(VulkanUtilityLibraries CONFIG) verification failed"
            fi
