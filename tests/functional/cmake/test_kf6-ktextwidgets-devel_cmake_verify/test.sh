#!/bin/bash
# Functional test: kf6-ktextwidgets-devel - cmake export integrity verification
...
find_package(KF6TextWidgets REQUIRED CONFIG)

message(STATUS "find_package(KF6TextWidgets) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6TextWidgets CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6TextWidgets CONFIG)"; then
                rlPass "find_package(KF6TextWidgets CONFIG) verification passed"
            else
                rlFail "find_package(KF6TextWidgets CONFIG) verification failed"
            fi
