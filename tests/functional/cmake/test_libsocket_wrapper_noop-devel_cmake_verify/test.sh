#!/bin/bash
# Functional test: libsocket_wrapper_noop-devel - cmake export integrity verification
...
PKG="libsocket_wrapper_noop-devel"
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(socket_wrapper_noop) ---
...
find_package(socket_wrapper_noop REQUIRED CONFIG
  PATHS /usr/lib64/cmake/socket_wrapper
  NO_DEFAULT_PATH)

message(STATUS "find_package(socket_wrapper_noop) succeeded")
EOF

            rlLogInfo "Verifying find_package(socket_wrapper_noop CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package socket_wrapper_noop CONFIG)"; then
                rlPass "find_package(socket_wrapper_noop CONFIG) verification passed"
            else
                rlFail "find_package(socket_wrapper_noop CONFIG) verification failed"
            fi
