#!/bin/bash
# Functional test: tbb-devel - cmake export integrity verification
...
message(STATUS "find_package(TBB) succeeded")
EOF

            rlLogInfo "Verifying find_package(TBB CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package TBB CONFIG)"; then
                rlPass "find_package(TBB CONFIG) verification passed"
            else
                rlFail "find_package(TBB CONFIG) verification failed"
            fi
