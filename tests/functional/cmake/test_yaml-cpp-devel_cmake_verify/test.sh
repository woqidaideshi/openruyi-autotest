#!/bin/bash
# Functional test: yaml-cpp-devel - cmake export integrity verification
...
message(STATUS "find_package(yaml-cpp) succeeded")
EOF

            rlLogInfo "Verifying find_package(yaml-cpp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package yaml-cpp CONFIG)"; then
                rlPass "find_package(yaml-cpp CONFIG) verification passed"
            else
                rlFail "find_package(yaml-cpp CONFIG) verification failed"
            fi
