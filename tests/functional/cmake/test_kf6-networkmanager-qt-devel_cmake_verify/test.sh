#!/bin/bash
# Functional test: kf6-networkmanager-qt-devel - cmake export integrity verification
...
PKG="kf6-networkmanager-qt-devel"
...
find_package(KF6NetworkManagerQt REQUIRED CONFIG)

message(STATUS "find_package(KF6NetworkManagerQt) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6NetworkManagerQt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6NetworkManagerQt CONFIG)"; then
                rlPass "find_package(KF6NetworkManagerQt CONFIG) verification passed"
            else
                rlFail "find_package(KF6NetworkManagerQt CONFIG) verification failed"
            fi
