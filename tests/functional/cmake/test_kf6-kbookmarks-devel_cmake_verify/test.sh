#!/bin/bash
# Functional test: kf6-kbookmarks-devel - cmake export integrity verification
...
find_package(KF6Bookmarks REQUIRED CONFIG)

message(STATUS "find_package(KF6Bookmarks) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Bookmarks CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Bookmarks CONFIG)"; then
                rlPass "find_package(KF6Bookmarks CONFIG) verification passed"
            else
                rlFail "find_package(KF6Bookmarks CONFIG) verification failed"
            fi
