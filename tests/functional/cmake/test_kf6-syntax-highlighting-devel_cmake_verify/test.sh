#!/bin/bash
# Functional test: kf6-syntax-highlighting-devel - cmake export integrity verification
...
PKG="kf6-syntax-highlighting-devel"
...
find_package(KF6SyntaxHighlighting REQUIRED CONFIG)

message(STATUS "find_package(KF6SyntaxHighlighting) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6SyntaxHighlighting CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6SyntaxHighlighting CONFIG)"; then
                rlPass "find_package(KF6SyntaxHighlighting CONFIG) verification passed"
            else
                rlFail "find_package(KF6SyntaxHighlighting CONFIG) verification failed"
            fi
