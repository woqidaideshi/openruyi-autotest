#!/bin/bash
# Functional test: pulseaudio-qt-qt6-devel - cmake export integrity verification
...
find_package(KF6PulseAudioQt REQUIRED CONFIG)

message(STATUS "find_package(KF6PulseAudioQt) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6PulseAudioQt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6PulseAudioQt CONFIG)"; then
                rlPass "find_package(KF6PulseAudioQt CONFIG) verification passed"
            else
                rlFail "find_package(KF6PulseAudioQt CONFIG) verification failed"
            fi
