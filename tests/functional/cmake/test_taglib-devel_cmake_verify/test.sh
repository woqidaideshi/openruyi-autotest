#!/bin/bash
# Functional test: taglib-devel - cmake export integrity verification
...
message(STATUS "find_package(taglib) succeeded")
EOF

            rlLogInfo "Verifying find_package(taglib CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package taglib CONFIG)"; then
                rlPass "find_package(taglib CONFIG) verification passed"
            else
                rlFail "find_package(taglib CONFIG) verification failed"
            fi
