#!/bin/bash
# Functional test: kf6-kjobwidgets-devel - cmake export integrity verification
...
message(STATUS "find_package(KF6JobWidgets) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6JobWidgets CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6JobWidgets CONFIG)"; then
                rlPass "find_package(KF6JobWidgets CONFIG) verification passed"
            else
                rlFail "find_package(KF6JobWidgets CONFIG) verification failed"
            fi
