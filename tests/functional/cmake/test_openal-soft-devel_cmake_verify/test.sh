#!/bin/bash
# Functional test: openal-soft-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenAL) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenAL CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenAL CONFIG)"; then
                rlPass "find_package(OpenAL CONFIG) verification passed"
            else
                rlFail "find_package(OpenAL CONFIG) verification failed"
            fi
