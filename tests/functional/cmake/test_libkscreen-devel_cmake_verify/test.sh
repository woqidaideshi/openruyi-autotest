#!/bin/bash
# Functional test: libkscreen-devel - cmake export integrity verification
...
find_package(KF6Screen REQUIRED CONFIG)

message(STATUS "find_package(KF6Screen) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Screen CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Screen CONFIG)"; then
                rlPass "find_package(KF6Screen CONFIG) verification passed"
            else
                rlFail "find_package(KF6Screen CONFIG) verification failed"
            fi
