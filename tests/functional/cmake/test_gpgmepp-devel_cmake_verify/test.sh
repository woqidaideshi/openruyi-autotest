#!/bin/bash
# Functional test: gpgmepp-devel - cmake export integrity verification
...
find_package(Gpgmepp REQUIRED CONFIG)

message(STATUS "find_package(Gpgmepp) succeeded")
EOF

            rlLogInfo "Verifying find_package(Gpgmepp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Gpgmepp CONFIG)"; then
                rlPass "find_package(Gpgmepp CONFIG) verification passed"
            else
                rlFail "find_package(Gpgmepp CONFIG) verification failed"
            fi
