#!/bin/bash
# Functional test: kf6-kcrash-devel - cmake export integrity verification
...
find_package(KF6Crash REQUIRED CONFIG)

message(STATUS "find_package(KF6Crash) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Crash CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Crash CONFIG)"; then
                rlPass "find_package(KF6Crash CONFIG) verification passed"
            else
                rlFail "find_package(KF6Crash CONFIG) verification failed"
            fi
