#!/bin/bash
# Functional test: glfw-devel - cmake export integrity verification
...
message(STATUS "find_package(glfw3) succeeded")
EOF

            rlLogInfo "Verifying find_package(glfw3 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package glfw3 CONFIG)"; then
                rlPass "find_package(glfw3 CONFIG) verification passed"
            else
                rlFail "find_package(glfw3 CONFIG) verification failed"
            fi
