#!/bin/bash
# Functional test: marisa-devel - cmake export integrity verification
...
message(STATUS "find_package(Marisa) succeeded")
EOF

            rlLogInfo "Verifying find_package(Marisa CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Marisa CONFIG)"; then
                rlPass "find_package(Marisa CONFIG) verification passed"
            else
                rlFail "find_package(Marisa CONFIG) verification failed"
            fi
