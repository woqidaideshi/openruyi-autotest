#!/bin/bash
# Functional test: lttng-ust-devel - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        ...
        # --- find_package(LTTngUST MODULE) ---
        cat > "$TmpDir2/CMakeLists.txt" << 'EOF'
cmake_minimum_required(VERSION 3.13.4)
project(cmake_module_verify
  VERSION "0.1"
  LANGUAGES C)
list(APPEND CMAKE_MODULE_PATH "/usr/share/doc/lttng-ust/examples/cmake-multiple-shared-libraries")
find_package(LTTngUST MODULE QUIET)
EOF

        rlLogInfo "Verifying find_package(LTTngUST MODULE) ..."
        rm -rf "$TmpDir2/build_0"
        if rlRun "cmake -S $TmpDir2 -B $TmpDir2/build_0" 0 \
            "cmake configuration (find_package LTTngUST MODULE)"; then
            rlPass "find_package(LTTngUST MODULE) verification passed"
        else
            rlLogWarning "find_package(LTTngUST MODULE) verification failed(the library being searched may not be installed)"
        fi
