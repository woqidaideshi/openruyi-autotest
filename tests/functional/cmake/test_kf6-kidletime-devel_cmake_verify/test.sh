#!/bin/bash
# Functional test: kf6-kidletime-devel - cmake export integrity verification
...
find_package(KF6IdleTime REQUIRED CONFIG)

message(STATUS "find_package(KF6IdleTime) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6IdleTime CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6IdleTime CONFIG)"; then
                rlPass "find_package(KF6IdleTime CONFIG) verification passed"
            else
                rlFail "find_package(KF6IdleTime CONFIG) verification failed"
            fi
