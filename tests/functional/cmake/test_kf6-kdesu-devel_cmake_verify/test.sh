#!/bin/bash
# Functional test: kf6-kdesu-devel - cmake export integrity verification
...
find_package(KF6Su REQUIRED CONFIG)

message(STATUS "find_package(KF6Su) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Su CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Su CONFIG)"; then
                rlPass "find_package(KF6Su CONFIG) verification passed"
            else
                rlFail "find_package(KF6Su CONFIG) verification failed"
            fi
