#!/bin/bash
# Functional test: libpng-devel - cmake export integrity verification
...
message(STATUS "find_package(PNG) succeeded")
EOF

            rlLogInfo "Verifying find_package(PNG CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PNG CONFIG)"; then
                rlPass "find_package(PNG CONFIG) verification passed"
            else
                rlFail "find_package(PNG CONFIG) verification failed"
            fi
