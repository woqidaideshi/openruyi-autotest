#!/bin/bash
# Functional test: glslang-devel - cmake export integrity verification
...
message(STATUS "find_package(glslang) succeeded")
EOF

            rlLogInfo "Verifying find_package(glslang CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package glslang CONFIG)"; then
                rlPass "find_package(glslang CONFIG) verification passed"
            else
                rlFail "find_package(glslang CONFIG) verification failed"
            fi
