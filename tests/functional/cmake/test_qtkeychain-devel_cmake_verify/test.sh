#!/bin/bash
# Functional test: qtkeychain-devel - cmake export integrity verification
...
find_package(Qt6Keychain REQUIRED CONFIG)

message(STATUS "find_package(Qt6Keychain) succeeded")
EOF

            rlLogInfo "Verifying find_package(Qt6Keychain CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Qt6Keychain CONFIG)"; then
                rlPass "find_package(Qt6Keychain CONFIG) verification passed"
            else
                rlFail "find_package(Qt6Keychain CONFIG) verification failed"
            fi
