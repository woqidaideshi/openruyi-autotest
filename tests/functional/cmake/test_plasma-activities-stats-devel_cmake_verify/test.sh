#!/bin/bash
# Functional test: plasma-activities-stats-devel - cmake export integrity verification
...
PKG="plasma-activities-stats-devel"
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(PlasmaActivitiesStats) ---
...
find_package(PlasmaActivitiesStats REQUIRED CONFIG)

message(STATUS "find_package(PlasmaActivitiesStats) succeeded")
EOF

            rlLogInfo "Verifying find_package(PlasmaActivitiesStats CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PlasmaActivitiesStats CONFIG)"; then
                rlPass "find_package(PlasmaActivitiesStats CONFIG) verification passed"
            else
                rlFail "find_package(PlasmaActivitiesStats CONFIG) verification failed"
            fi
