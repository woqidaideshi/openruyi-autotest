#!/bin/bash
# Functional test: kf6-kdbusaddons-devel - cmake export integrity verification
...
find_package(KF6DBusAddons REQUIRED CONFIG)

message(STATUS "find_package(KF6DBusAddons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6DBusAddons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6DBusAddons CONFIG)"; then
                rlPass "find_package(KF6DBusAddons CONFIG) verification passed"
            else
                rlFail "find_package(KF6DBusAddons CONFIG) verification failed"
            fi
