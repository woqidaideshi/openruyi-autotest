#!/bin/bash
# Functional test: kf6-kitemviews-devel - cmake export integrity verification
...
find_package(KF6ItemViews REQUIRED CONFIG)

message(STATUS "find_package(KF6ItemViews) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ItemViews CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ItemViews CONFIG)"; then
                rlPass "find_package(KF6ItemViews CONFIG) verification passed"
            else
                rlFail "find_package(KF6ItemViews CONFIG) verification failed"
            fi
