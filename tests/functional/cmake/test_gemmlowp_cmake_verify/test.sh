#!/bin/bash
# Functional test: gemmlowp - cmake export integrity verification
...
message(STATUS "find_package(gemmlowp) succeeded")
EOF

            rlLogInfo "Verifying find_package(gemmlowp CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package gemmlowp CONFIG)"; then
                rlPass "find_package(gemmlowp CONFIG) verification passed"
            else
                rlFail "find_package(gemmlowp CONFIG) verification failed"
            fi
