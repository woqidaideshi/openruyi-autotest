#!/bin/bash
# Functional test: kf6-karchive-devel - cmake export integrity verification
...
find_package(KF6Archive REQUIRED CONFIG)

message(STATUS "find_package(KF6Archive) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Archive CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Archive CONFIG)"; then
                rlPass "find_package(KF6Archive CONFIG) verification passed"
            else
                rlFail "find_package(KF6Archive CONFIG) verification failed"
            fi
