#!/bin/bash
# Functional test: magic_enum - cmake export integrity verification
...
message(STATUS "find_package(magic_enum) succeeded")
EOF

            rlLogInfo "Verifying find_package(magic_enum CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package magic_enum CONFIG)"; then
                rlPass "find_package(magic_enum CONFIG) verification passed"
            else
                rlFail "find_package(magic_enum CONFIG) verification failed"
            fi
