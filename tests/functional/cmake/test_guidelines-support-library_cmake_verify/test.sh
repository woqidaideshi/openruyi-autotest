#!/bin/bash
# Functional test: guidelines-support-library - cmake export integrity verification
...
PKG="guidelines-support-library"
...
message(STATUS "find_package(Microsoft.GSL) succeeded")
EOF

            rlLogInfo "Verifying find_package(Microsoft.GSL CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Microsoft.GSL CONFIG)"; then
                rlPass "find_package(Microsoft.GSL CONFIG) verification passed"
            else
                rlFail "find_package(Microsoft.GSL CONFIG) verification failed"
            fi
