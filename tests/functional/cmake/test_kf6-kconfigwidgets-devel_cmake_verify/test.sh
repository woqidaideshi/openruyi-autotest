#!/bin/bash
# Functional test: kf6-kconfigwidgets-devel - cmake export integrity verification
...
find_package(KF6ConfigWidgets REQUIRED CONFIG)

message(STATUS "find_package(KF6ConfigWidgets) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ConfigWidgets CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ConfigWidgets CONFIG)"; then
                rlPass "find_package(KF6ConfigWidgets CONFIG) verification passed"
            else
                rlFail "find_package(KF6ConfigWidgets CONFIG) verification failed"
            fi
