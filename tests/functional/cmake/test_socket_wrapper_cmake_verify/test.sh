#!/bin/bash
# Functional test: socket_wrapper - cmake export integrity verification
...
find_package(socket_wrapper REQUIRED CONFIG)

message(STATUS "find_package(socket_wrapper) succeeded")
EOF

            rlLogInfo "Verifying find_package(socket_wrapper CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package socket_wrapper CONFIG)"; then
                rlPass "find_package(socket_wrapper CONFIG) verification passed"
            else
                rlFail "find_package(socket_wrapper CONFIG) verification failed"
            fi
