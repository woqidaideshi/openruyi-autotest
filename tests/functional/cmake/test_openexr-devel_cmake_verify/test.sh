#!/bin/bash
# Functional test: openexr-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenEXR) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenEXR CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenEXR CONFIG)"; then
                rlPass "find_package(OpenEXR CONFIG) verification passed"
            else
                rlFail "find_package(OpenEXR CONFIG) verification failed"
            fi
