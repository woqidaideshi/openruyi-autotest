#!/bin/bash
# Functional test: rust-libz-sys-1 - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "cargo check - Verify crate build integrity"
        ...
        # --- cargo check libz-sys ---
        rlLogInfo "Verifying crate: libz-sys v$CRATE_VERSION"
        test_proj="$TmpDir2/test_libz_sys"
        rm -rf "$test_proj"
        mkdir -p "$test_proj/src"

        cat > "$test_proj/Cargo.toml" << CARGOEOF
[package]
name = "cmake_verify_libz_sys"
version = "0.1.0"
edition = "2021"

[dependencies]
libz-sys = "${CRATE_VERSION}"
CARGOEOF
        echo 'fn main() { println!("cmake verify ok"); }' > "$test_proj/src/main.rs"

        cd "$test_proj"
        if rlRun "cargo check 2>&1" 0 \
            "cargo check libz-sys v$CRATE_VERSION"; then
            rlPass "cargo check libz-sys passed"
        else
            rlLogWarning "cargo check libz-sys failed (may need network to download dependencies)"
        fi
