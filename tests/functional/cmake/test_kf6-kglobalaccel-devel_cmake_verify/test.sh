#!/bin/bash
# Functional test: kf6-kglobalaccel-devel - cmake export integrity verification
...
find_package(KF6GlobalAccel REQUIRED CONFIG)

message(STATUS "find_package(KF6GlobalAccel) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6GlobalAccel CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6GlobalAccel CONFIG)"; then
                rlPass "find_package(KF6GlobalAccel CONFIG) verification passed"
            else
                rlFail "find_package(KF6GlobalAccel CONFIG) verification failed"
            fi
