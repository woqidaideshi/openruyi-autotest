#!/bin/bash
# Functional test: kf6-purpose-devel - cmake export integrity verification
...
find_package(KF6Purpose REQUIRED CONFIG)

message(STATUS "find_package(KF6Purpose) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Purpose CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Purpose CONFIG)"; then
                rlPass "find_package(KF6Purpose CONFIG) verification passed"
            else
                rlFail "find_package(KF6Purpose CONFIG) verification failed"
            fi
