#!/bin/bash
# Functional test: libssh-devel - cmake export integrity verification
...
message(STATUS "find_package(libssh) succeeded")
EOF

            rlLogInfo "Verifying find_package(libssh CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libssh CONFIG)"; then
                rlPass "find_package(libssh CONFIG) verification passed"
            else
                rlFail "find_package(libssh CONFIG) verification failed"
            fi
