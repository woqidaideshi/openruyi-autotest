#!/bin/bash
# Functional test: openjpeg-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenJPEG) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenJPEG CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenJPEG CONFIG)"; then
                rlPass "find_package(OpenJPEG CONFIG) verification passed"
            else
                rlFail "find_package(OpenJPEG CONFIG) verification failed"
            fi
