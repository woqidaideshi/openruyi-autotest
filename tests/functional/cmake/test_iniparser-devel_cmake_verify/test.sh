#!/bin/bash
# Functional test: iniparser-devel - cmake export integrity verification
...
message(STATUS "find_package(iniparser) succeeded")
EOF

            rlLogInfo "Verifying find_package(iniparser CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package iniparser CONFIG)"; then
                rlPass "find_package(iniparser CONFIG) verification passed"
            else
                rlFail "find_package(iniparser CONFIG) verification failed"
            fi
