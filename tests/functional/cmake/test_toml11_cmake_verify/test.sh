#!/bin/bash
# Functional test: toml11 - cmake export integrity verification
...
message(STATUS "find_package(toml11) succeeded")
EOF

            rlLogInfo "Verifying find_package(toml11 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package toml11 CONFIG)"; then
                rlPass "find_package(toml11 CONFIG) verification passed"
            else
                rlFail "find_package(toml11 CONFIG) verification failed"
            fi
