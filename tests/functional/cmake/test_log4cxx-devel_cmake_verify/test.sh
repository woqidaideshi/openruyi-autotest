#!/bin/bash
# Functional test: log4cxx-devel - cmake export integrity verification
...
message(STATUS "find_package(log4cxx) succeeded")
EOF

            rlLogInfo "Verifying find_package(log4cxx CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package log4cxx CONFIG)"; then
                rlPass "find_package(log4cxx CONFIG) verification passed"
            else
                rlFail "find_package(log4cxx CONFIG) verification failed"
            fi
