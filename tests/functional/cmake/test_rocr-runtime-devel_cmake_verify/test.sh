#!/bin/bash
# Functional test: rocr-runtime-devel - cmake export integrity verification
...
message(STATUS "find_package(hsa-runtime64) succeeded")
EOF

            rlLogInfo "Verifying find_package(hsa-runtime64 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package hsa-runtime64 CONFIG)"; then
                rlPass "find_package(hsa-runtime64 CONFIG) verification passed"
            else
                rlFail "find_package(hsa-runtime64 CONFIG) verification failed"
            fi
