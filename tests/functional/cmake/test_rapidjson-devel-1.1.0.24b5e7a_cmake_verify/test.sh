#!/bin/bash
# Functional test: rapidjson-devel-1.1.0.24b5e7a - cmake export integrity verification
...
PKG="rapidjson-devel-1.1.0.24b5e7a"
...
message(STATUS "find_package(RapidJSON) succeeded")
EOF

            rlLogInfo "Verifying find_package(RapidJSON CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package RapidJSON CONFIG)"; then
                rlPass "find_package(RapidJSON CONFIG) verification passed"
            else
                rlFail "find_package(RapidJSON CONFIG) verification failed"
            fi
