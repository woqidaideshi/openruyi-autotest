#!/bin/bash
# Functional test: rocsolver-devel - cmake export integrity verification
...
message(STATUS "find_package(rocsolver) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocsolver CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocsolver CONFIG)"; then
                rlPass "find_package(rocsolver CONFIG) verification passed"
            else
                rlFail "find_package(rocsolver CONFIG) verification failed"
            fi
