#!/bin/bash
# Functional test: tensorpipe-devel - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) + link - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            rlLogWarning "$PKG provides no .cmake files, skipping cmake verification"
        else
            # --- find_package(Tensorpipe) + link verification ---
            # Create a minimal C++ source file for link verification
...
EOF
            ...
            rlLogInfo "Verifying find_package(Tensorpipe CONFIG) + link ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package + link)"; then
                rlPass "find_package(Tensorpipe CONFIG) + link verification passed"
            else
                rlFail "find_package(Tensorpipe CONFIG) + link verification failed"
            fi
