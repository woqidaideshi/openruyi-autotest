#!/bin/bash
# Functional test: kf6-kcolorscheme-devel - cmake export integrity verification
...
find_package(KF6ColorScheme REQUIRED CONFIG)

message(STATUS "find_package(KF6ColorScheme) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ColorScheme CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ColorScheme CONFIG)"; then
                rlPass "find_package(KF6ColorScheme CONFIG) verification passed"
            else
                rlFail "find_package(KF6ColorScheme CONFIG) verification failed"
            fi
