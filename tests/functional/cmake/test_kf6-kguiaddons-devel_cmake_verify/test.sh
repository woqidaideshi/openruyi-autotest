#!/bin/bash
# Functional test: kf6-kguiaddons-devel - cmake export integrity verification
...
find_package(KF6GuiAddons REQUIRED CONFIG)

message(STATUS "find_package(KF6GuiAddons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6GuiAddons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6GuiAddons CONFIG)"; then
                rlPass "find_package(KF6GuiAddons CONFIG) verification passed"
            else
                rlFail "find_package(KF6GuiAddons CONFIG) verification failed"
            fi
