#!/bin/bash
# Functional test: gtk-doc - cmake export integrity verification
...
find_package(GtkDoc REQUIRED CONFIG)

message(STATUS "find_package(GtkDoc) succeeded")
EOF

            rlLogInfo "Verifying find_package(GtkDoc CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package GtkDoc CONFIG)"; then
                rlPass "find_package(GtkDoc CONFIG) verification passed"
            else
                rlFail "find_package(GtkDoc CONFIG) verification failed"
            fi
