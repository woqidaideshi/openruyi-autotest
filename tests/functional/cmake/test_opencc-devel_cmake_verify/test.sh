#!/bin/bash
# Functional test: opencc-devel - cmake export integrity verification
...
find_package(OpenCC REQUIRED CONFIG)

message(STATUS "find_package(OpenCC) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenCC CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenCC CONFIG)"; then
                rlPass "find_package(OpenCC CONFIG) verification passed"
            else
                rlFail "find_package(OpenCC CONFIG) verification failed"
            fi
