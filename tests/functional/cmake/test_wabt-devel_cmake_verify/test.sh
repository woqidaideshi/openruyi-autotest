#!/bin/bash
# Functional test: wabt-devel - cmake export integrity verification
...
find_package(wabt REQUIRED CONFIG)

message(STATUS "find_package(wabt) succeeded")
EOF

            rlLogInfo "Verifying find_package(wabt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package wabt CONFIG)"; then
                rlPass "find_package(wabt CONFIG) verification passed"
            else
                rlFail "find_package(wabt CONFIG) verification failed"
            fi
