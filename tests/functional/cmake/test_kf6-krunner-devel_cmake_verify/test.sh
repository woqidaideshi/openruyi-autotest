#!/bin/bash
# Functional test: kf6-krunner-devel - cmake export integrity verification
...
find_package(KF6Runner REQUIRED CONFIG)

message(STATUS "find_package(KF6Runner) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Runner CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Runner CONFIG)"; then
                rlPass "find_package(KF6Runner CONFIG) verification passed"
            else
                rlFail "find_package(KF6Runner CONFIG) verification failed"
            fi
