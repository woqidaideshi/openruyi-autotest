#!/bin/bash
# Functional test: openssl-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenSSL) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenSSL CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenSSL CONFIG)"; then
                rlPass "find_package(OpenSSL CONFIG) verification passed"
            else
                rlFail "find_package(OpenSSL CONFIG) verification failed"
            fi
