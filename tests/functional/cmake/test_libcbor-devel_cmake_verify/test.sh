#!/bin/bash
# Functional test: libcbor-devel - cmake export integrity verification
...
message(STATUS "find_package(libcbor) succeeded")
EOF

            rlLogInfo "Verifying find_package(libcbor CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libcbor CONFIG)"; then
                rlPass "find_package(libcbor CONFIG) verification passed"
            else
                rlFail "find_package(libcbor CONFIG) verification failed"
            fi
