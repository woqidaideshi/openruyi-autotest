#!/bin/bash
# Functional test: snappy-devel - cmake export integrity verification
...
message(STATUS "find_package(Snappy) succeeded")
EOF

            rlLogInfo "Verifying find_package(Snappy CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Snappy CONFIG)"; then
                rlPass "find_package(Snappy CONFIG) verification passed"
            else
                rlFail "find_package(Snappy CONFIG) verification failed"
            fi
