#!/bin/bash
# Functional test: kdecoration-devel - cmake export integrity verification
...
find_package(KDecoration3 REQUIRED CONFIG)

message(STATUS "find_package(KDecoration3) succeeded")
EOF

            rlLogInfo "Verifying find_package(KDecoration3 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KDecoration3 CONFIG)"; then
                rlPass "find_package(KDecoration3 CONFIG) verification passed"
            else
                rlFail "find_package(KDecoration3 CONFIG) verification failed"
            fi
