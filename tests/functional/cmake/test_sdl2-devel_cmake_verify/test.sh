#!/bin/bash
# Functional test: SDL2-devel - cmake export integrity verification
...
message(STATUS "find_package(SDL2) succeeded")
EOF

            rlLogInfo "Verifying find_package(SDL2 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package SDL2 CONFIG)"; then
                rlPass "find_package(SDL2 CONFIG) verification passed"
            else
                rlFail "find_package(SDL2 CONFIG) verification failed"
            fi
