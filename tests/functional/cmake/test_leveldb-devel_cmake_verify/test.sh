#!/bin/bash
# Functional test: leveldb-devel - cmake export integrity verification
...
message(STATUS "find_package(leveldb) succeeded")
EOF

            rlLogInfo "Verifying find_package(leveldb CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package leveldb CONFIG)"; then
                rlPass "find_package(leveldb CONFIG) verification passed"
            else
                rlFail "find_package(leveldb CONFIG) verification failed"
            fi
