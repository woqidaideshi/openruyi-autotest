#!/bin/bash
# Functional test: protobuf-devel - cmake export integrity verification
...
message(STATUS "find_package(protobuf) succeeded")
EOF

            rlLogInfo "Verifying find_package(protobuf CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package protobuf CONFIG)"; then
                rlPass "find_package(protobuf CONFIG) verification passed"
            else
                rlFail "find_package(protobuf CONFIG) verification failed"
            fi
...
message(STATUS "find_package(utf8_range) succeeded")
EOF

            rlLogInfo "Verifying find_package(utf8_range CONFIG) ..."
            rm -rf "$TmpDir/build_1"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_1" 0 \
                "cmake configuration (find_package utf8_range CONFIG)"; then
                rlPass "find_package(utf8_range CONFIG) verification passed"
            else
                rlFail "find_package(utf8_range CONFIG) verification failed"
            fi
