#!/bin/bash
# Functional test: kf6-kio-devel - cmake export integrity verification
...
find_package(KF6KIO REQUIRED CONFIG)

message(STATUS "find_package(KF6KIO) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6KIO CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6KIO CONFIG)"; then
                rlPass "find_package(KF6KIO CONFIG) verification passed"
            else
                rlFail "find_package(KF6KIO CONFIG) verification failed"
            fi
