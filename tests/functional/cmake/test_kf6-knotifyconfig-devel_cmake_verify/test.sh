#!/bin/bash
# Functional test: kf6-knotifyconfig-devel - cmake export integrity verification
...
find_package(KF6NotifyConfig REQUIRED CONFIG)

message(STATUS "find_package(KF6NotifyConfig) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6NotifyConfig CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6NotifyConfig CONFIG)"; then
                rlPass "find_package(KF6NotifyConfig CONFIG) verification passed"
            else
                rlFail "find_package(KF6NotifyConfig CONFIG) verification failed"
            fi
