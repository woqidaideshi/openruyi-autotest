#!/bin/bash
# Functional test: plasma-wayland-protocols - cmake export integrity verification
...
find_package(PlasmaWaylandProtocols REQUIRED CONFIG)

message(STATUS "find_package(PlasmaWaylandProtocols) succeeded")
EOF

            rlLogInfo "Verifying find_package(PlasmaWaylandProtocols CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PlasmaWaylandProtocols CONFIG)"; then
                rlPass "find_package(PlasmaWaylandProtocols CONFIG) verification passed"
            else
                rlFail "find_package(PlasmaWaylandProtocols CONFIG) verification failed"
            fi
