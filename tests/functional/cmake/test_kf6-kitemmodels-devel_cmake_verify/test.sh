#!/bin/bash
# Functional test: kf6-kitemmodels-devel - cmake export integrity verification
...
find_package(KF6ItemModels REQUIRED CONFIG)

message(STATUS "find_package(KF6ItemModels) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ItemModels CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ItemModels CONFIG)"; then
                rlPass "find_package(KF6ItemModels CONFIG) verification passed"
            else
                rlFail "find_package(KF6ItemModels CONFIG) verification failed"
            fi
