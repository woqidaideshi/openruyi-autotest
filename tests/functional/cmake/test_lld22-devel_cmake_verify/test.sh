#!/bin/bash
# Functional test: lld22-devel - cmake export integrity verification
...
message(STATUS "find_package(LLD) succeeded")
EOF

            rlLogInfo "Verifying find_package(LLD CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package LLD CONFIG)"; then
                rlPass "find_package(LLD CONFIG) verification passed"
            else
                rlFail "find_package(LLD CONFIG) verification failed"
            fi
