#!/bin/bash
# Functional test: libkexiv2-qt6-devel - cmake export integrity verification
...
find_package(KExiv2Qt6 REQUIRED CONFIG)

message(STATUS "find_package(KExiv2Qt6) succeeded")
EOF

            rlLogInfo "Verifying find_package(KExiv2Qt6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KExiv2Qt6 CONFIG)"; then
                rlPass "find_package(KExiv2Qt6 CONFIG) verification passed"
            else
                rlFail "find_package(KExiv2Qt6 CONFIG) verification failed"
            fi
