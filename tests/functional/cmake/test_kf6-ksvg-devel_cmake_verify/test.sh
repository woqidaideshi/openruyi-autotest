#!/bin/bash
# Functional test: kf6-ksvg-devel - cmake export integrity verification
...
find_package(KF6Svg REQUIRED CONFIG)

message(STATUS "find_package(KF6Svg) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Svg CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Svg CONFIG)"; then
                rlPass "find_package(KF6Svg CONFIG) verification passed"
            else
                rlFail "find_package(KF6Svg CONFIG) verification failed"
            fi
