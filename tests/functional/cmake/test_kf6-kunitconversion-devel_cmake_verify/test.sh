#!/bin/bash
# Functional test: kf6-kunitconversion-devel - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(KF6UnitConversion) ---
...
find_package(KF6UnitConversion REQUIRED CONFIG)

message(STATUS "find_package(KF6UnitConversion) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6UnitConversion CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6UnitConversion CONFIG)"; then
                rlPass "find_package(KF6UnitConversion CONFIG) verification passed"
            else
                rlFail "find_package(KF6UnitConversion CONFIG) verification failed"
            fi
