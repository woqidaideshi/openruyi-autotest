#!/bin/bash
# Functional test: kf6-kiconthemes-devel - cmake export integrity verification
...
find_package(KF6IconThemes REQUIRED CONFIG)

message(STATUS "find_package(KF6IconThemes) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6IconThemes CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6IconThemes CONFIG)"; then
                rlPass "find_package(KF6IconThemes CONFIG) verification passed"
            else
                rlFail "find_package(KF6IconThemes CONFIG) verification failed"
            fi
