#!/bin/bash
# Functional test: kf6-kcompletion-devel - cmake export integrity verification
...
find_package(KF6Completion REQUIRED CONFIG)

message(STATUS "find_package(KF6Completion) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Completion CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Completion CONFIG)"; then
                rlPass "find_package(KF6Completion CONFIG) verification passed"
            else
                rlFail "find_package(KF6Completion CONFIG) verification failed"
            fi
