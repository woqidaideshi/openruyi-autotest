#!/bin/bash
# Functional test: libwebsockets-devel - cmake export integrity verification
...
message(STATUS "find_package(libwebsockets) succeeded")
EOF

            rlLogInfo "Verifying find_package(libwebsockets CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libwebsockets CONFIG)"; then
                rlPass "find_package(libwebsockets CONFIG) verification passed"
            else
                rlFail "find_package(libwebsockets CONFIG) verification failed"
            fi
