#!/bin/bash
# Functional test: simdjson-devel - cmake export integrity verification
...
message(STATUS "find_package(simdjson) succeeded")
EOF

            rlLogInfo "Verifying find_package(simdjson CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package simdjson CONFIG)"; then
                rlPass "find_package(simdjson CONFIG) verification passed"
            else
                rlFail "find_package(simdjson CONFIG) verification failed"
            fi
