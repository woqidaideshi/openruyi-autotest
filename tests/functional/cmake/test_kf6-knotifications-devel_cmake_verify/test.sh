#!/bin/bash
# Functional test: kf6-knotifications-devel - cmake export integrity verification
...
find_package(KF6Notifications REQUIRED CONFIG)

message(STATUS "find_package(KF6Notifications) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Notifications CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Notifications CONFIG)"; then
                rlPass "find_package(KF6Notifications CONFIG) verification passed"
            else
                rlFail "find_package(KF6Notifications CONFIG) verification failed"
            fi
