#!/bin/bash
# Functional test: xapian-devel - cmake export integrity verification
...
message(STATUS "find_package(xapian) succeeded")
EOF

            rlLogInfo "Verifying find_package(xapian CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package xapian CONFIG)"; then
                rlPass "find_package(xapian CONFIG) verification passed"
            else
                rlFail "find_package(xapian CONFIG) verification failed"
            fi
