#!/bin/bash
# Functional test: rocm-comgr-devel - cmake export integrity verification
...
message(STATUS "find_package(amd_comgr) succeeded")
EOF

            rlLogInfo "Verifying find_package(amd_comgr CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package amd_comgr CONFIG)"; then
                rlPass "find_package(amd_comgr CONFIG) verification passed"
            else
                rlFail "find_package(amd_comgr CONFIG) verification failed"
            fi
