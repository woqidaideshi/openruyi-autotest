#!/bin/bash
# Functional test: libxslt-devel - cmake export integrity verification
...
message(STATUS "find_package(libxslt) succeeded")
EOF

            rlLogInfo "Verifying find_package(libxslt CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libxslt CONFIG)"; then
                rlPass "find_package(libxslt CONFIG) verification passed"
            else
                rlFail "find_package(libxslt CONFIG) verification failed"
            fi
