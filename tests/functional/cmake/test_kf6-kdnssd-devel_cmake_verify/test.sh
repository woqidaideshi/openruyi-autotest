#!/bin/bash
# Functional test: kf6-kdnssd-devel - cmake export integrity verification
...
find_package(KF6DNSSD REQUIRED CONFIG)

message(STATUS "find_package(KF6DNSSD) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6DNSSD CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6DNSSD CONFIG)"; then
                rlPass "find_package(KF6DNSSD CONFIG) verification passed"
            else
                rlFail "find_package(KF6DNSSD CONFIG) verification failed"
            fi
