#!/bin/bash
# Functional test: kf6-kcodecs-devel - cmake export integrity verification
...
find_package(KF6Codecs REQUIRED CONFIG)

message(STATUS "find_package(KF6Codecs) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Codecs CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Codecs CONFIG)"; then
                rlPass "find_package(KF6Codecs CONFIG) verification passed"
            else
                rlFail "find_package(KF6Codecs CONFIG) verification failed"
            fi
