#!/bin/bash
# Functional test: kf6-kparts-devel - cmake export integrity verification
...
find_package(KF6Parts REQUIRED CONFIG)

message(STATUS "find_package(KF6Parts) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Parts CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Parts CONFIG)"; then
                rlPass "find_package(KF6Parts CONFIG) verification passed"
            else
                rlFail "find_package(KF6Parts CONFIG) verification failed"
            fi
