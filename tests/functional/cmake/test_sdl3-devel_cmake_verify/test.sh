#!/bin/bash
# Functional test: SDL3-devel - cmake export integrity verification
...
message(STATUS "find_package(SDL3) succeeded")
EOF

            rlLogInfo "Verifying find_package(SDL3 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package SDL3 CONFIG)"; then
                rlPass "find_package(SDL3 CONFIG) verification passed"
            else
                rlFail "find_package(SDL3 CONFIG) verification failed"
            fi
