#!/bin/bash
# Functional test: kdsoap-devel - cmake export integrity verification
...
find_package(KDSoap-qt6 REQUIRED CONFIG)

message(STATUS "find_package(KDSoap-qt6) succeeded")
EOF

            rlLogInfo "Verifying find_package(KDSoap-qt6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KDSoap-qt6 CONFIG)"; then
                rlPass "find_package(KDSoap-qt6 CONFIG) verification passed"
            else
                rlFail "find_package(KDSoap-qt6 CONFIG) verification failed"
            fi
