#!/bin/bash
# Functional test: zziplib-devel - cmake export integrity verification
...
find_package(zziplib REQUIRED CONFIG)

message(STATUS "find_package(zziplib) succeeded")
EOF

            rlLogInfo "Verifying find_package(zziplib CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package zziplib CONFIG)"; then
                rlPass "find_package(zziplib CONFIG) verification passed"
            else
                rlFail "find_package(zziplib CONFIG) verification failed"
            fi
