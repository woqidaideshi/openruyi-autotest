#!/bin/bash
# Functional test: kf6-kpackage-devel - cmake export integrity verification
...
find_package(KF6Package REQUIRED CONFIG)

message(STATUS "find_package(KF6Package) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Package CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Package CONFIG)"; then
                rlPass "find_package(KF6Package CONFIG) verification passed"
            else
                rlFail "find_package(KF6Package CONFIG) verification failed"
            fi
