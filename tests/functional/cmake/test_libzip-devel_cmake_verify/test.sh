#!/bin/bash
# Functional test: libzip-devel - cmake export integrity verification
...
message(STATUS "find_package(libzip) succeeded")
EOF

            rlLogInfo "Verifying find_package(libzip CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libzip CONFIG)"; then
                rlPass "find_package(libzip CONFIG) verification passed"
            else
                rlFail "find_package(libzip CONFIG) verification failed"
            fi
