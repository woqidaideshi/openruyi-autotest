#!/bin/bash
# Functional test: wavpack-devel - cmake export integrity verification
...
message(STATUS "find_package(WavPack) succeeded")
EOF

            rlLogInfo "Verifying find_package(WavPack CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package WavPack CONFIG)"; then
                rlPass "find_package(WavPack CONFIG) verification passed"
            else
                rlFail "find_package(WavPack CONFIG) verification failed"
            fi
