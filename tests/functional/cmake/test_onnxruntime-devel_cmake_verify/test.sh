#!/bin/bash
# Functional test: onnxruntime-devel - cmake export integrity verification
...
message(STATUS "find_package(onnxruntime) succeeded")
EOF

            rlLogInfo "Verifying find_package(onnxruntime CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package onnxruntime CONFIG)"; then
                rlPass "find_package(onnxruntime CONFIG) verification passed"
            else
                rlFail "find_package(onnxruntime CONFIG) verification failed"
            fi
