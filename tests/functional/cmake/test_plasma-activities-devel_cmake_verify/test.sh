#!/bin/bash
# Functional test: plasma-activities-devel - cmake export integrity verification
...
find_package(PlasmaActivities REQUIRED CONFIG)

message(STATUS "find_package(PlasmaActivities) succeeded")
EOF

            rlLogInfo "Verifying find_package(PlasmaActivities CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PlasmaActivities CONFIG)"; then
                rlPass "find_package(PlasmaActivities CONFIG) verification passed"
            else
                rlFail "find_package(PlasmaActivities CONFIG) verification failed"
            fi
