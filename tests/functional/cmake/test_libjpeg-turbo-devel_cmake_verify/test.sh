#!/bin/bash
# Functional test: libjpeg-turbo-devel - cmake export integrity verification
...
message(STATUS "find_package(libjpeg-turbo) succeeded")
EOF

            rlLogInfo "Verifying find_package(libjpeg-turbo CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libjpeg-turbo CONFIG)"; then
                rlPass "find_package(libjpeg-turbo CONFIG) verification passed"
            else
                rlFail "find_package(libjpeg-turbo CONFIG) verification failed"
            fi
