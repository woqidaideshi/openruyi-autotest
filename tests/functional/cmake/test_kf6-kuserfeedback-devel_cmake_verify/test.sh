#!/bin/bash
# Functional test: kf6-kuserfeedback-devel - cmake export integrity verification
...
find_package(KF6UserFeedback REQUIRED CONFIG)

message(STATUS "find_package(KF6UserFeedback) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6UserFeedback CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6UserFeedback CONFIG)"; then
                rlPass "find_package(KF6UserFeedback CONFIG) verification passed"
            else
                rlFail "find_package(KF6UserFeedback CONFIG) verification failed"
            fi
