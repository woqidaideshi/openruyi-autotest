#!/bin/bash
# Functional test: kf6-sonnet-devel - cmake export integrity verification
...
find_package(KF6Sonnet REQUIRED CONFIG)

message(STATUS "find_package(KF6Sonnet) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Sonnet CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Sonnet CONFIG)"; then
                rlPass "find_package(KF6Sonnet CONFIG) verification passed"
            else
                rlFail "find_package(KF6Sonnet CONFIG) verification failed"
            fi
