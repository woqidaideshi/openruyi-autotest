#!/bin/bash
# Functional test: plog-devel - cmake export integrity verification
...
message(STATUS "find_package(plog) succeeded")
EOF

            rlLogInfo "Verifying find_package(plog CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package plog CONFIG)"; then
                rlPass "find_package(plog CONFIG) verification passed"
            else
                rlFail "find_package(plog CONFIG) verification failed"
            fi
