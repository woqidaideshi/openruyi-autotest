#!/bin/bash
# Functional test: kf6-kdeclarative-devel - cmake export integrity verification
...
find_package(KF6Declarative REQUIRED CONFIG)

message(STATUS "find_package(KF6Declarative) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Declarative CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Declarative CONFIG)"; then
                rlPass "find_package(KF6Declarative CONFIG) verification passed"
            else
                rlFail "find_package(KF6Declarative CONFIG) verification failed"
            fi
