#!/bin/bash
# Functional test: kaccounts-integration-devel - cmake export integrity verification
...
PKG="kaccounts-integration-devel"
...
find_package(KAccounts6 REQUIRED CONFIG)

message(STATUS "find_package(KAccounts6) succeeded")
EOF

            rlLogInfo "Verifying find_package(KAccounts6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KAccounts6 CONFIG)"; then
                rlPass "find_package(KAccounts6 CONFIG) verification passed"
            else
                rlFail "find_package(KAccounts6 CONFIG) verification failed"
            fi
