#!/bin/bash
# Functional test: kf6-kwallet-devel - cmake export integrity verification
...
find_package(KF6Wallet REQUIRED CONFIG)

message(STATUS "find_package(KF6Wallet) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Wallet CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Wallet CONFIG)"; then
                rlPass "find_package(KF6Wallet CONFIG) verification passed"
            else
                rlFail "find_package(KF6Wallet CONFIG) verification failed"
            fi
