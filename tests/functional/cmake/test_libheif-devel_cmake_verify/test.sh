#!/bin/bash
# Functional test: libheif-devel - cmake export integrity verification
...
message(STATUS "find_package(libheif) succeeded")
EOF

            rlLogInfo "Verifying find_package(libheif CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libheif CONFIG)"; then
                rlPass "find_package(libheif CONFIG) verification passed"
            else
                rlFail "find_package(libheif CONFIG) verification failed"
            fi
