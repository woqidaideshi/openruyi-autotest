#!/bin/bash
# Functional test: rocblas-devel - cmake export integrity verification
...
message(STATUS "find_package(rocblas) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocblas CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocblas CONFIG)"; then
                rlPass "find_package(rocblas CONFIG) verification passed"
            else
                rlFail "find_package(rocblas CONFIG) verification failed"
            fi
