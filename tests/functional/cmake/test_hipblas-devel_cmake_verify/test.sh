#!/bin/bash
# Functional test: hipblas-devel - cmake export integrity verification
...
message(STATUS "find_package(hipblas) succeeded")
EOF

            rlLogInfo "Verifying find_package(hipblas CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package hipblas CONFIG)"; then
                rlPass "find_package(hipblas CONFIG) verification passed"
            else
                rlFail "find_package(hipblas CONFIG) verification failed"
            fi
