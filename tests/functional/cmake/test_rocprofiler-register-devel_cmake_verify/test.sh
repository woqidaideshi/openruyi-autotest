#!/bin/bash
# Functional test: rocprofiler-register-devel - cmake export integrity verification
...
PKG="rocprofiler-register-devel"
...
find_package(rocprofiler-register REQUIRED CONFIG)

message(STATUS "find_package(rocprofiler-register) succeeded")
EOF

            rlLogInfo "Verifying find_package(rocprofiler-register CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package rocprofiler-register CONFIG)"; then
                rlPass "find_package(rocprofiler-register CONFIG) verification passed"
            else
                rlFail "find_package(rocprofiler-register CONFIG) verification failed"
            fi
