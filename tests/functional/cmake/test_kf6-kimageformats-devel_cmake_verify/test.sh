#!/bin/bash
# Functional test: kf6-kimageformats-devel - cmake export integrity verification
...
find_package(KF6ImageFormats REQUIRED CONFIG)

message(STATUS "find_package(KF6ImageFormats) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6ImageFormats CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6ImageFormats CONFIG)"; then
                rlPass "find_package(KF6ImageFormats CONFIG) verification passed"
            else
                rlFail "find_package(KF6ImageFormats CONFIG) verification failed"
            fi
