#!/bin/bash
# Functional test: knighttime-devel - cmake export integrity verification
...
find_package(KNightTime REQUIRED CONFIG)

message(STATUS "find_package(KNightTime) succeeded")
EOF

            rlLogInfo "Verifying find_package(KNightTime CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KNightTime CONFIG)"; then
                rlPass "find_package(KNightTime CONFIG) verification passed"
            else
                rlFail "find_package(KNightTime CONFIG) verification failed"
            fi
