#!/bin/bash
# Functional test: kf6-ki18n-devel - cmake export integrity verification
...
find_package(KF6I18n REQUIRED CONFIG)

message(STATUS "find_package(KF6I18n) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6I18n CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6I18n CONFIG)"; then
                rlPass "find_package(KF6I18n CONFIG) verification passed"
            else
                rlFail "find_package(KF6I18n CONFIG) verification failed"
            fi
