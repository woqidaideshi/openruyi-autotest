#!/bin/bash
# Functional test: kf6-breeze-icons-devel - cmake export integrity verification
...
find_package(KF6BreezeIcons REQUIRED CONFIG)

message(STATUS "find_package(KF6BreezeIcons) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6BreezeIcons CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6BreezeIcons CONFIG)"; then
                rlPass "find_package(KF6BreezeIcons CONFIG) verification passed"
            else
                rlFail "find_package(KF6BreezeIcons CONFIG) verification failed"
            fi
