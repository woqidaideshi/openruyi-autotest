#!/bin/bash
# Functional test: kglobalacceld-devel - cmake export integrity verification
...
find_package(KGlobalAccelD REQUIRED CONFIG)

message(STATUS "find_package(KGlobalAccelD) succeeded")
EOF

            rlLogInfo "Verifying find_package(KGlobalAccelD CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KGlobalAccelD CONFIG)"; then
                rlPass "find_package(KGlobalAccelD CONFIG) verification passed"
            else
                rlFail "find_package(KGlobalAccelD CONFIG) verification failed"
            fi
