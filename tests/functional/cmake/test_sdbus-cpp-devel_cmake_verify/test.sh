#!/bin/bash
# Functional test: sdbus-cpp-devel - cmake export integrity verification
...
message(STATUS "find_package(sdbus-c++) succeeded")
EOF

            rlLogInfo "Verifying find_package(sdbus-c++ CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package sdbus-c++ CONFIG)"; then
                rlPass "find_package(sdbus-c++ CONFIG) verification passed"
            else
                rlFail "find_package(sdbus-c++ CONFIG) verification failed"
            fi
