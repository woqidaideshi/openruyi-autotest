#!/bin/bash
# Functional test: rust-cbindgen-0.29 - cmake export integrity verification
...
rlJournalStart
    ...
    rlPhaseStartTest "cargo check - Verify crate build integrity"
        ...
        # --- cargo check cbindgen ---
        rlLogInfo "Verifying crate: cbindgen v$CRATE_VERSION"
        test_proj="$TmpDir2/test_cbindgen"
        rm -rf "$test_proj"
        mkdir -p "$test_proj/src"

        cat > "$test_proj/Cargo.toml" << CARGOEOF
[package]
name = "cmake_verify_cbindgen"
version = "0.1.0"
edition = "2021"

[dependencies]
cbindgen = "${CRATE_VERSION}"
CARGOEOF
        echo 'fn main() { println!("cmake verify ok"); }' > "$test_proj/src/main.rs"

        cd "$test_proj"
        if rlRun "cargo check 2>&1" 0 \
            "cargo check cbindgen v$CRATE_VERSION"; then
            rlPass "cargo check cbindgen passed"
        else
            rlLogWarning "cargo check cbindgen failed (may need network to download dependencies)"
        fi
