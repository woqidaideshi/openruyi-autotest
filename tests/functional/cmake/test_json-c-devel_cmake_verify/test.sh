#!/bin/bash
# Functional test: json-c-devel - cmake export integrity verification
...
message(STATUS "find_package(json-c) succeeded")
EOF

            rlLogInfo "Verifying find_package(json-c CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package json-c CONFIG)"; then
                rlPass "find_package(json-c CONFIG) verification passed"
            else
                rlFail "find_package(json-c CONFIG) verification failed"
            fi
