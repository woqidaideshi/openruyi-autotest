#!/bin/bash
# Functional test: openzl-devel - cmake export integrity verification
...
find_package(openzl REQUIRED CONFIG)

message(STATUS "find_package(openzl) succeeded")
EOF

            rlLogInfo "Verifying find_package(openzl CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package openzl CONFIG)"; then
                rlPass "find_package(openzl CONFIG) verification passed"
            else
                rlFail "find_package(openzl CONFIG) verification failed"
            fi
