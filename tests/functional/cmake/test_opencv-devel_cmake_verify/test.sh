#!/bin/bash
# Functional test: opencv-devel - cmake export integrity verification
...
message(STATUS "find_package(OpenCV) succeeded")
EOF

            rlLogInfo "Verifying find_package(OpenCV CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package OpenCV CONFIG)"; then
                rlPass "find_package(OpenCV CONFIG) verification passed"
            else
                rlFail "find_package(OpenCV CONFIG) verification failed"
            fi
