#!/bin/bash
# Functional test: pulseaudio-devel - cmake export integrity verification
...
message(STATUS "find_package(PulseAudio) succeeded")
EOF

            rlLogInfo "Verifying find_package(PulseAudio CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package PulseAudio CONFIG)"; then
                rlPass "find_package(PulseAudio CONFIG) verification passed"
            else
                rlFail "find_package(PulseAudio CONFIG) verification failed"
            fi
