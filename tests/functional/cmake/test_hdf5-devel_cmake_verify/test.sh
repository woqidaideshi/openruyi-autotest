#!/bin/bash
# Functional test: hdf5-devel - cmake export integrity verification
...
message(STATUS "find_package(hdf5) succeeded")
EOF

            rlLogInfo "Verifying find_package(hdf5 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package hdf5 CONFIG)"; then
                rlPass "find_package(hdf5 CONFIG) verification passed"
            else
                rlFail "find_package(hdf5 CONFIG) verification failed"
            fi
