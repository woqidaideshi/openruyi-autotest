#!/bin/bash
# Functional test: imath-devel - cmake export integrity verification
...
message(STATUS "find_package(Imath) succeeded")
EOF

            rlLogInfo "Verifying find_package(Imath CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Imath CONFIG)"; then
                rlPass "find_package(Imath CONFIG) verification passed"
            else
                rlFail "find_package(Imath CONFIG) verification failed"
            fi
