#!/bin/bash
# Functional test: simdutf-devel - cmake export integrity verification
...
message(STATUS "find_package(simdutf) succeeded")
EOF

            rlLogInfo "Verifying find_package(simdutf CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package simdutf CONFIG)"; then
                rlPass "find_package(simdutf CONFIG) verification passed"
            else
                rlFail "find_package(simdutf CONFIG) verification failed"
            fi
