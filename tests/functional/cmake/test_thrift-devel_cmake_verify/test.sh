#!/bin/bash
# Functional test: thrift-devel - cmake export integrity verification
...
message(STATUS "find_package(Thrift) succeeded")
EOF

            rlLogInfo "Verifying find_package(Thrift CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Thrift CONFIG)"; then
                rlPass "find_package(Thrift CONFIG) verification passed"
            else
                rlFail "find_package(Thrift CONFIG) verification failed"
            fi
