#!/bin/bash
# Functional test: zlib-ng-compat-devel - cmake export integrity verification
...
message(STATUS "find_package(ZLIB) succeeded")
EOF

            rlLogInfo "Verifying find_package(ZLIB CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package ZLIB CONFIG)"; then
                rlPass "find_package(ZLIB CONFIG) verification passed"
            else
                rlFail "find_package(ZLIB CONFIG) verification failed"
            fi
