#!/bin/bash
# Functional test: kf6-kconfig-devel - cmake export integrity verification
...
find_package(KF6Config REQUIRED CONFIG)

message(STATUS "find_package(KF6Config) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Config CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Config CONFIG)"; then
                rlPass "find_package(KF6Config CONFIG) verification passed"
            else
                rlFail "find_package(KF6Config CONFIG) verification failed"
            fi
