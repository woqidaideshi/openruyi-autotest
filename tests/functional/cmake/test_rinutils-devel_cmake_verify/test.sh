#!/bin/bash
# Functional test: rinutils-devel - cmake export integrity verification
...
find_package(Rinutils REQUIRED CONFIG)

message(STATUS "find_package(Rinutils) succeeded")
EOF

            rlLogInfo "Verifying find_package(Rinutils CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Rinutils CONFIG)"; then
                rlPass "find_package(Rinutils CONFIG) verification passed"
            else
                rlFail "find_package(Rinutils CONFIG) verification failed"
            fi
