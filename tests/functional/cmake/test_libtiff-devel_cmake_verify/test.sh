#!/bin/bash
# Functional test: libtiff-devel - cmake export integrity verification
...
message(STATUS "find_package(Tiff) succeeded")
EOF

            rlLogInfo "Verifying find_package(Tiff CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Tiff CONFIG)"; then
                rlPass "find_package(Tiff CONFIG) verification passed"
            else
                rlFail "find_package(Tiff CONFIG) verification failed"
            fi
