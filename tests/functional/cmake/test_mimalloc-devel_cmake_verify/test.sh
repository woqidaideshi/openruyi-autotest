#!/bin/bash
# Functional test: mimalloc-devel - cmake export integrity verification
...
message(STATUS "find_package(mimalloc) succeeded")
EOF

            rlLogInfo "Verifying find_package(mimalloc CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package mimalloc CONFIG)"; then
                rlPass "find_package(mimalloc CONFIG) verification passed"
            else
                rlFail "find_package(mimalloc CONFIG) verification failed"
            fi
