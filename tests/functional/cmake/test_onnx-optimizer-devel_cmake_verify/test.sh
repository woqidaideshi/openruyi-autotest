#!/bin/bash
# Functional test: onnx-optimizer-devel - cmake export integrity verification
...
find_package(ONNXOptimizer REQUIRED CONFIG)

message(STATUS "find_package(ONNXOptimizer) succeeded")
EOF

            rlLogInfo "Verifying find_package(ONNXOptimizer CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package ONNXOptimizer CONFIG)"; then
                rlPass "find_package(ONNXOptimizer CONFIG) verification passed"
            else
                rlFail "find_package(ONNXOptimizer CONFIG) verification failed"
            fi
