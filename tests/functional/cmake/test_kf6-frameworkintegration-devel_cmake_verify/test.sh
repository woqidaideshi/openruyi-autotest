#!/bin/bash
# Functional test: kf6-frameworkintegration-devel - cmake export integrity verification
...
PKG="kf6-frameworkintegration-devel"
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(KF6FrameworkIntegration) ---
...
find_package(KF6FrameworkIntegration REQUIRED CONFIG)

message(STATUS "find_package(KF6FrameworkIntegration) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6FrameworkIntegration CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6FrameworkIntegration CONFIG)"; then
                rlPass "find_package(KF6FrameworkIntegration CONFIG) verification passed"
            else
                rlFail "find_package(KF6FrameworkIntegration CONFIG) verification failed"
            fi
