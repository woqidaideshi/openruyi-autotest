#!/bin/bash
# Functional test: kwin-devel - cmake export integrity verification
...
find_package(KWin REQUIRED CONFIG)

message(STATUS "find_package(KWin) succeeded")
EOF

            rlLogInfo "Verifying find_package(KWin CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KWin CONFIG)"; then
                rlPass "find_package(KWin CONFIG) verification passed"
            else
                rlFail "find_package(KWin CONFIG) verification failed"
            fi
...
find_package(KWinDBusInterface REQUIRED CONFIG)

message(STATUS "find_package(KWinDBusInterface) succeeded")
EOF

            rlLogInfo "Verifying find_package(KWinDBusInterface CONFIG) ..."
            rm -rf "$TmpDir/build_1"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_1" 0 \
                "cmake configuration (find_package KWinDBusInterface CONFIG)"; then
                rlPass "find_package(KWinDBusInterface CONFIG) verification passed"
            else
                rlFail "find_package(KWinDBusInterface CONFIG) verification failed"
            fi
