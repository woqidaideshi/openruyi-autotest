#!/bin/bash
# Functional test: kf6-prison-devel - cmake export integrity verification
...
find_package(KF6Prison REQUIRED CONFIG)

message(STATUS "find_package(KF6Prison) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Prison CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Prison CONFIG)"; then
                rlPass "find_package(KF6Prison CONFIG) verification passed"
            else
                rlFail "find_package(KF6Prison CONFIG) verification failed"
            fi
