#!/bin/bash
# Functional test: kio-extras-devel - cmake export integrity verification
...
find_package(KioArchive6 REQUIRED CONFIG)

message(STATUS "find_package(KioArchive6) succeeded")
EOF

            rlLogInfo "Verifying find_package(KioArchive6 CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KioArchive6 CONFIG)"; then
                rlPass "find_package(KioArchive6 CONFIG) verification passed"
            else
                rlFail "find_package(KioArchive6 CONFIG) verification failed"
            fi
