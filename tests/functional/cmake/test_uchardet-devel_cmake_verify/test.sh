#!/bin/bash
# Functional test: uchardet-devel - cmake export integrity verification
...
message(STATUS "find_package(uchardet) succeeded")
EOF

            rlLogInfo "Verifying find_package(uchardet CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package uchardet CONFIG)"; then
                rlPass "find_package(uchardet CONFIG) verification passed"
            else
                rlFail "find_package(uchardet CONFIG) verification failed"
            fi
