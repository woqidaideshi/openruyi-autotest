#!/bin/bash
# Functional test: xsimd - cmake export integrity verification
...
message(STATUS "find_package(xsimd) succeeded")
EOF

            rlLogInfo "Verifying find_package(xsimd CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package xsimd CONFIG)"; then
                rlPass "find_package(xsimd CONFIG) verification passed"
            else
                rlFail "find_package(xsimd CONFIG) verification failed"
            fi
