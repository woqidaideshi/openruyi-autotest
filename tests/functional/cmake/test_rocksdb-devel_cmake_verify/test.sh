#!/bin/bash
# Functional test: rocksdb-devel - cmake export integrity verification
...
message(STATUS "find_package(RocksDB) succeeded")
EOF

            rlLogInfo "Verifying find_package(RocksDB CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package RocksDB CONFIG)"; then
                rlPass "find_package(RocksDB CONFIG) verification passed"
            else
                rlFail "find_package(RocksDB CONFIG) verification failed"
            fi
