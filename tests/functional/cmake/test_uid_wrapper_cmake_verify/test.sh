#!/bin/bash
# Functional test: uid_wrapper - cmake export integrity verification
...
find_package(uid_wrapper REQUIRED CONFIG)

message(STATUS "find_package(uid_wrapper) succeeded")
EOF

            rlLogInfo "Verifying find_package(uid_wrapper CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package uid_wrapper CONFIG)"; then
                rlPass "find_package(uid_wrapper CONFIG) verification passed"
            else
                rlFail "find_package(uid_wrapper CONFIG) verification failed"
            fi
