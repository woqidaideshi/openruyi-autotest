#!/bin/bash
# Functional test: kdsoap-ws-discovery-client-devel - cmake export integrity verification
...
PKG="kdsoap-ws-discovery-client-devel"
...
rlJournalStart
    ...
    rlPhaseStartTest "find_package(CONFIG) - Verify cmake export integrity"
        if [ "$CMAKE_COUNT" -eq 0 ]; then
            ...
            # --- find_package(KDSoapWSDiscoveryClient) ---
...
find_package(KDSoapWSDiscoveryClient REQUIRED CONFIG)

message(STATUS "find_package(KDSoapWSDiscoveryClient) succeeded")
EOF

            rlLogInfo "Verifying find_package(KDSoapWSDiscoveryClient CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KDSoapWSDiscoveryClient CONFIG)"; then
                rlPass "find_package(KDSoapWSDiscoveryClient CONFIG) verification passed"
            else
                rlFail "find_package(KDSoapWSDiscoveryClient CONFIG) verification failed"
            fi
