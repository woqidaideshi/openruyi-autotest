#!/bin/bash
# Functional test: plasma5support-devel - cmake export integrity verification
...
find_package(Plasma5Support REQUIRED CONFIG)

message(STATUS "find_package(Plasma5Support) succeeded")
EOF

            rlLogInfo "Verifying find_package(Plasma5Support CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package Plasma5Support CONFIG)"; then
                rlPass "find_package(Plasma5Support CONFIG) verification passed"
            else
                rlFail "find_package(Plasma5Support CONFIG) verification failed"
            fi
