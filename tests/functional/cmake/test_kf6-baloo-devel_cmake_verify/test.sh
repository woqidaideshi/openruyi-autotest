#!/bin/bash
# Functional test: kf6-baloo-devel - cmake export integrity verification
...
find_package(KF6Baloo REQUIRED CONFIG)

message(STATUS "find_package(KF6Baloo) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Baloo CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Baloo CONFIG)"; then
                rlPass "find_package(KF6Baloo CONFIG) verification passed"
            else
                rlFail "find_package(KF6Baloo CONFIG) verification failed"
            fi
