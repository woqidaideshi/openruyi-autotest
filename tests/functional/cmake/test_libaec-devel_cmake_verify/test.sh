#!/bin/bash
# Functional test: libaec-devel - cmake export integrity verification
...
message(STATUS "find_package(libaec) succeeded")
EOF

            rlLogInfo "Verifying find_package(libaec CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package libaec CONFIG)"; then
                rlPass "find_package(libaec CONFIG) verification passed"
            else
                rlFail "find_package(libaec CONFIG) verification failed"
            fi
