#!/bin/bash
# Functional test: kf6-syndication-devel - cmake export integrity verification
...
find_package(KF6Syndication REQUIRED CONFIG)

message(STATUS "find_package(KF6Syndication) succeeded")
EOF

            rlLogInfo "Verifying find_package(KF6Syndication CONFIG) ..."
            rm -rf "$TmpDir/build_0"
            if rlRun "cmake -S $TmpDir -B $TmpDir/build_0" 0 \
                "cmake configuration (find_package KF6Syndication CONFIG)"; then
                rlPass "find_package(KF6Syndication CONFIG) verification passed"
            else
                rlFail "find_package(KF6Syndication CONFIG) verification failed"
            fi
