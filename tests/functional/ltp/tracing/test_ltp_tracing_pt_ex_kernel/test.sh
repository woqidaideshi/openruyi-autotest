#!/bin/bash
# Functional test: ltp - tracing - pt_ex_kernel
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP tracing - pt_ex_kernel"
        rlRun "kirk -f tracing -p pt_ex_kernel 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pt_ex_kernel"
