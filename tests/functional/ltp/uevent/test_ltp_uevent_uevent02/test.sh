#!/bin/bash
# Functional test: ltp - uevent - uevent02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP uevent - uevent02"
        rlRun "kirk -f uevent -p uevent02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP uevent02"
