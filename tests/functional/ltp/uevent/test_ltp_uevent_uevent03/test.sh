#!/bin/bash
# Functional test: ltp - uevent - uevent03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP uevent - uevent03"
        rlRun "kirk -f uevent -p uevent03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP uevent03"
