#!/bin/bash
# Functional test: ltp - commands - mkfs01_btrfs_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - mkfs01_btrfs_sh"
        rlRun "kirk -f commands -p mkfs01_btrfs_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkfs01_btrfs_sh"
