#!/bin/bash
# Functional test: ltp - commands - mkfs01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - mkfs01_sh"
        rlRun "kirk -f commands -p mkfs01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkfs01_sh"
