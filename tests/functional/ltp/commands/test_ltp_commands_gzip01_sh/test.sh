#!/bin/bash
# Functional test: ltp - commands - gzip01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - gzip01_sh"
        rlRun "kirk -f commands -p gzip01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP gzip01_sh"
