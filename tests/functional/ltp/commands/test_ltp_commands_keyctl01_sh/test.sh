#!/bin/bash
# Functional test: ltp - commands - keyctl01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - keyctl01_sh"
        rlRun "kirk -f commands -p keyctl01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl01_sh"
