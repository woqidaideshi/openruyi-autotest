#!/bin/bash
# Functional test: ltp - commands - mv01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - mv01_sh"
        rlRun "kirk -f commands -p mv01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mv01_sh"
