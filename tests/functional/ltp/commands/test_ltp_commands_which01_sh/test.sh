#!/bin/bash
# Functional test: ltp - commands - which01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - which01_sh"
        rlRun "kirk -f commands -p which01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP which01_sh"
