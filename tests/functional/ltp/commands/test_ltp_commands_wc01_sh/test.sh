#!/bin/bash
# Functional test: ltp - commands - wc01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - wc01_sh"
        rlRun "kirk -f commands -p wc01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP wc01_sh"
