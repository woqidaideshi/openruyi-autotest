#!/bin/bash
# Functional test: ltp - commands - unzip01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - unzip01_sh"
        rlRun "kirk -f commands -p unzip01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unzip01_sh"
