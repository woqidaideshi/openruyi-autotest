#!/bin/bash
# Functional test: ltp - commands - cp01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - cp01_sh"
        rlRun "kirk -f commands -p cp01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cp01_sh"
