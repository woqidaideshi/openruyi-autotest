#!/bin/bash
# Functional test: ltp - commands - mkdir01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - mkdir01_sh"
        rlRun "kirk -f commands -p mkdir01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdir01_sh"
