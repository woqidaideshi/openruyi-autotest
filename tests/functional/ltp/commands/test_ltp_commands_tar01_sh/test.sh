#!/bin/bash
# Functional test: ltp - commands - tar01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - tar01_sh"
        rlRun "kirk -f commands -p tar01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tar01_sh"
