#!/bin/bash
# Functional test: ltp - commands - cpio01_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - cpio01_sh"
        rlRun "kirk -f commands -p cpio01_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cpio01_sh"
