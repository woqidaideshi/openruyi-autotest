#!/bin/bash
# Functional test: ltp - commands - shell_test01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP commands - shell_test01"
        rlRun "kirk -f commands -p shell_test01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shell_test01"
