#!/bin/bash
# Functional test: ltp - containers - pidns05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns05"
        rlRun "kirk -f containers -p pidns05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns05"
