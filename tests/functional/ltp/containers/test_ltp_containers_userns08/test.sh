#!/bin/bash
# Functional test: ltp - containers - userns08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns08"
        rlRun "kirk -f containers -p userns08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns08"
