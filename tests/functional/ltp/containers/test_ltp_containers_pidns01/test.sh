#!/bin/bash
# Functional test: ltp - containers - pidns01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns01"
        rlRun "kirk -f containers -p pidns01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns01"
