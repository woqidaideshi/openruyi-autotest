#!/bin/bash
# Functional test: ltp - containers - userns06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns06"
        rlRun "kirk -f containers -p userns06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns06"
