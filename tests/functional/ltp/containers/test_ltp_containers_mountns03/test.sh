#!/bin/bash
# Functional test: ltp - containers - mountns03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - mountns03"
        rlRun "kirk -f containers -p mountns03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mountns03"
