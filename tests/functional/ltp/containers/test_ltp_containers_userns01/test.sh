#!/bin/bash
# Functional test: ltp - containers - userns01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns01"
        rlRun "kirk -f containers -p userns01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns01"
