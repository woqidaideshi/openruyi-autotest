#!/bin/bash
# Functional test: ltp - containers - pidns16
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns16"
        rlRun "kirk -f containers -p pidns16 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns16"
