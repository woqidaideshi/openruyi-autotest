#!/bin/bash
# Functional test: ltp - containers - mountns04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - mountns04"
        rlRun "kirk -f containers -p mountns04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mountns04"
