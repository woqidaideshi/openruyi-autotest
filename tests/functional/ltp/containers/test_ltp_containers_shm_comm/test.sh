#!/bin/bash
# Functional test: ltp - containers - shm_comm
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - shm_comm"
        rlRun "kirk -f containers -p shm_comm 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shm_comm"
