#!/bin/bash
# Functional test: ltp - containers - userns03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns03"
        rlRun "kirk -f containers -p userns03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns03"
