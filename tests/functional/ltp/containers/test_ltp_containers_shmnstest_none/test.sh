#!/bin/bash
# Functional test: ltp - containers - shmnstest_none
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - shmnstest_none"
        rlRun "kirk -f containers -p shmnstest_none 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmnstest_none"
