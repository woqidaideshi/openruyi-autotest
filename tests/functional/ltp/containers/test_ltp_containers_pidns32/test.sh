#!/bin/bash
# Functional test: ltp - containers - pidns32
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns32"
        rlRun "kirk -f containers -p pidns32 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns32"
