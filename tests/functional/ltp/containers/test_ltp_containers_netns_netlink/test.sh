#!/bin/bash
# Functional test: ltp - containers - netns_netlink
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - netns_netlink"
        rlRun "kirk -f containers -p netns_netlink 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP netns_netlink"
