#!/bin/bash
# Functional test: ltp - containers - mountns02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - mountns02"
        rlRun "kirk -f containers -p mountns02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mountns02"
