#!/bin/bash
# Functional test: ltp - containers - mqns_01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - mqns_01"
        rlRun "kirk -f containers -p mqns_01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mqns_01"
