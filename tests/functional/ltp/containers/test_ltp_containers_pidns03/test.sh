#!/bin/bash
# Functional test: ltp - containers - pidns03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns03"
        rlRun "kirk -f containers -p pidns03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns03"
