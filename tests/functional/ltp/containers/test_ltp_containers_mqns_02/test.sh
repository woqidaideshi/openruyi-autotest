#!/bin/bash
# Functional test: ltp - containers - mqns_02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - mqns_02"
        rlRun "kirk -f containers -p mqns_02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mqns_02"
