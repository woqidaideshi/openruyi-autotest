#!/bin/bash
# Functional test: ltp - containers - netns_sysfs
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - netns_sysfs"
        rlRun "kirk -f containers -p netns_sysfs 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP netns_sysfs"
