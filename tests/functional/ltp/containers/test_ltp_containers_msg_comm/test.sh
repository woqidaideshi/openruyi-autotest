#!/bin/bash
# Functional test: ltp - containers - msg_comm
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - msg_comm"
        rlRun "kirk -f containers -p msg_comm 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msg_comm"
