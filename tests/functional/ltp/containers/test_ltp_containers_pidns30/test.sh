#!/bin/bash
# Functional test: ltp - containers - pidns30
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns30"
        rlRun "kirk -f containers -p pidns30 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns30"
