#!/bin/bash
# Functional test: ltp - containers - timens01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - timens01"
        rlRun "kirk -f containers -p timens01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timens01"
