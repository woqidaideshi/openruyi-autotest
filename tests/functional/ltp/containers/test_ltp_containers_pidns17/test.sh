#!/bin/bash
# Functional test: ltp - containers - pidns17
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns17"
        rlRun "kirk -f containers -p pidns17 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns17"
