#!/bin/bash
# Functional test: ltp - containers - userns02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns02"
        rlRun "kirk -f containers -p userns02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns02"
