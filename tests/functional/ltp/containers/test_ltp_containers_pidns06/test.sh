#!/bin/bash
# Functional test: ltp - containers - pidns06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns06"
        rlRun "kirk -f containers -p pidns06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns06"
