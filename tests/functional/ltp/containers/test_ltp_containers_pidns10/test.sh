#!/bin/bash
# Functional test: ltp - containers - pidns10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns10"
        rlRun "kirk -f containers -p pidns10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns10"
