#!/bin/bash
# Functional test: ltp - containers - pidns04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns04"
        rlRun "kirk -f containers -p pidns04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns04"
