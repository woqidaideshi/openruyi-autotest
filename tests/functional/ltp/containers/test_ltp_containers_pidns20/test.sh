#!/bin/bash
# Functional test: ltp - containers - pidns20
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns20"
        rlRun "kirk -f containers -p pidns20 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns20"
