#!/bin/bash
# Functional test: ltp - containers - pidns12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns12"
        rlRun "kirk -f containers -p pidns12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns12"
