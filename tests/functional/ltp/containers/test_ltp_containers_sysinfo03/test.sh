#!/bin/bash
# Functional test: ltp - containers - sysinfo03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - sysinfo03"
        rlRun "kirk -f containers -p sysinfo03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysinfo03"
