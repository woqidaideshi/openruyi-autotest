#!/bin/bash
# Functional test: ltp - containers - userns05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns05"
        rlRun "kirk -f containers -p userns05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns05"
