#!/bin/bash
# Functional test: ltp - containers - userns04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - userns04"
        rlRun "kirk -f containers -p userns04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userns04"
