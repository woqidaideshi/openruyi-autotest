#!/bin/bash
# Functional test: ltp - containers - pidns02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - pidns02"
        rlRun "kirk -f containers -p pidns02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidns02"
