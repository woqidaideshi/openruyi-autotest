#!/bin/bash
# Functional test: ltp - containers - sem_comm
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP containers - sem_comm"
        rlRun "kirk -f containers -p sem_comm 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sem_comm"
