#!/bin/bash
# Functional test: ltp - math - float_power
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP math - float_power"
        rlRun "kirk -f math -p float_power 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP float_power"
