#!/bin/bash
# Functional test: ltp - math - float_bessel
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP math - float_bessel"
        rlRun "kirk -f math -p float_bessel 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP float_bessel"
