#!/bin/bash
# Functional test: ltp - net-features - mpls01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-features - mpls01"
        rlRun "kirk -f net-features -p mpls01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mpls01"
