#!/bin/bash
# Functional test: ltp - net-features - tcp_fastopen
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-features - tcp_fastopen"
        rlRun "kirk -f net-features -p tcp_fastopen 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tcp_fastopen"
