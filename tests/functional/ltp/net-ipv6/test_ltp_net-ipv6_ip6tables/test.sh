#!/bin/bash
# Functional test: ltp - net-ipv6 - ip6tables
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-ipv6 - ip6tables"
        rlRun "kirk -f net-ipv6 -p ip6tables 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ip6tables"
