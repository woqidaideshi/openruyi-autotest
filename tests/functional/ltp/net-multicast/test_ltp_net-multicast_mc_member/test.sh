#!/bin/bash
# Functional test: ltp - net-multicast - mc_member
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-multicast - mc_member"
        rlRun "kirk -f net-multicast -p mc_member 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mc_member"
