#!/bin/bash
# Functional test: ltp - numa - migrate_pages03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP numa - migrate_pages03"
        rlRun "kirk -f numa -p migrate_pages03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP migrate_pages03"
