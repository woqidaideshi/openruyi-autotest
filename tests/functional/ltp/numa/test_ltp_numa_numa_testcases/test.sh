#!/bin/bash
# Functional test: ltp - numa - numa_testcases
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP numa - numa_testcases"
        rlRun "kirk -f numa -p numa_testcases 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP numa_testcases"
