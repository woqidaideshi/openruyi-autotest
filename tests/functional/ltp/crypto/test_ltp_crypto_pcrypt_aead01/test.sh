#!/bin/bash
# Functional test: ltp - crypto - pcrypt_aead01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP crypto - pcrypt_aead01"
        rlRun "kirk -f crypto -p pcrypt_aead01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pcrypt_aead01"
