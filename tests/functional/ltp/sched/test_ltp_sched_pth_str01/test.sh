#!/bin/bash
# Functional test: ltp - sched - pth_str01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - pth_str01"
        rlRun "kirk -f sched -p pth_str01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pth_str01"
