#!/bin/bash
# Functional test: ltp - sched - trace_sched01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - trace_sched01"
        rlRun "kirk -f sched -p trace_sched01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP trace_sched01"
