#!/bin/bash
# Functional test: ltp - sched - sched_cli_serv
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - sched_cli_serv"
        rlRun "kirk -f sched -p sched_cli_serv 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_cli_serv"
