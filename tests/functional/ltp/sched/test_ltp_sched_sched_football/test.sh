#!/bin/bash
# Functional test: ltp - sched - sched_football
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - sched_football"
        rlRun "kirk -f sched -p sched_football 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_football"
