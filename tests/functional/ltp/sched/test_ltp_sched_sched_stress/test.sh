#!/bin/bash
# Functional test: ltp - sched - sched_stress
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - sched_stress"
        rlRun "kirk -f sched -p sched_stress 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_stress"
