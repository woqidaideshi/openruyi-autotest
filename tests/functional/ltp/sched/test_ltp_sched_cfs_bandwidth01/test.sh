#!/bin/bash
# Functional test: ltp - sched - cfs_bandwidth01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP sched - cfs_bandwidth01"
        rlRun "kirk -f sched -p cfs_bandwidth01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cfs_bandwidth01"
