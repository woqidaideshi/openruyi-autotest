#!/bin/bash
# Functional test: ltp - net-tirpc-tests - tirpc_clnt_dg_create
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tirpc-tests - tirpc_clnt_dg_create"
        rlRun "kirk -f net-tirpc-tests -p tirpc_clnt_dg_create 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tirpc_clnt_dg_create"
