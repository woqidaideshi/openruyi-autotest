#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmctl03"
        rlRun "kirk -f hugetlb -p hugeshmctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmctl03"
