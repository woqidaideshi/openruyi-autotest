#!/bin/bash
# Functional test: ltp - hugetlb - hugemmap11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugemmap11"
        rlRun "kirk -f hugetlb -p hugemmap11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugemmap11"
