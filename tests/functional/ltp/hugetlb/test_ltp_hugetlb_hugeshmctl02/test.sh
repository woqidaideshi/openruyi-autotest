#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmctl02"
        rlRun "kirk -f hugetlb -p hugeshmctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmctl02"
