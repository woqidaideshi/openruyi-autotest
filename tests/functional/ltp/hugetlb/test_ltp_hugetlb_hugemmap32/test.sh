#!/bin/bash
# Functional test: ltp - hugetlb - hugemmap32
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugemmap32"
        rlRun "kirk -f hugetlb -p hugemmap32 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugemmap32"
