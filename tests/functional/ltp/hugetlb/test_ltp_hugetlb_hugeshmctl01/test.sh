#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmctl01"
        rlRun "kirk -f hugetlb -p hugeshmctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmctl01"
