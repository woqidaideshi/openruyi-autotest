#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmget02"
        rlRun "kirk -f hugetlb -p hugeshmget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmget02"
