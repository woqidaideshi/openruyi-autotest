#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmget01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmget01"
        rlRun "kirk -f hugetlb -p hugeshmget01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmget01"
