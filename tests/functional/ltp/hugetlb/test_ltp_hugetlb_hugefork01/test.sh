#!/bin/bash
# Functional test: ltp - hugetlb - hugefork01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugefork01"
        rlRun "kirk -f hugetlb -p hugefork01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugefork01"
