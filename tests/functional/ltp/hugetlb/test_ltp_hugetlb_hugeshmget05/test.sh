#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmget05"
        rlRun "kirk -f hugetlb -p hugeshmget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmget05"
