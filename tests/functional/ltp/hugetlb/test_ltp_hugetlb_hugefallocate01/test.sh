#!/bin/bash
# Functional test: ltp - hugetlb - hugefallocate01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugefallocate01"
        rlRun "kirk -f hugetlb -p hugefallocate01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugefallocate01"
