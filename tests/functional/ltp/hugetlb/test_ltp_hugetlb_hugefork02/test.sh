#!/bin/bash
# Functional test: ltp - hugetlb - hugefork02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugefork02"
        rlRun "kirk -f hugetlb -p hugefork02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugefork02"
