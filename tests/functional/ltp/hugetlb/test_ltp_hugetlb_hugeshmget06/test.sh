#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmget06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmget06"
        rlRun "kirk -f hugetlb -p hugeshmget06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmget06"
