#!/bin/bash
# Functional test: ltp - hugetlb - hugeshmat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP hugetlb - hugeshmat04"
        rlRun "kirk -f hugetlb -p hugeshmat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP hugeshmat04"
