#!/bin/bash
# Functional test: ltp - fs - read_all_dev
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP fs - read_all_dev"
        rlRun "kirk -f fs -p read_all_dev 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP read_all_dev"
