#!/bin/bash
# Functional test: ltp - fs - writetest01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP fs - writetest01"
        rlRun "kirk -f fs -p writetest01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP writetest01"
