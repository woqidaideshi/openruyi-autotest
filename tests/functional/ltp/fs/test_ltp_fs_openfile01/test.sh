#!/bin/bash
# Functional test: ltp - fs - openfile01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP fs - openfile01"
        rlRun "kirk -f fs -p openfile01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openfile01"
