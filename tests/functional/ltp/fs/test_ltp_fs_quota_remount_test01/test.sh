#!/bin/bash
# Functional test: ltp - fs - quota_remount_test01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP fs - quota_remount_test01"
        rlRun "kirk -f fs -p quota_remount_test01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quota_remount_test01"
