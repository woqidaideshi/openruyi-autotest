#!/bin/bash
# Functional test: ltp - fs - binfmt_misc01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP fs - binfmt_misc01"
        rlRun "kirk -f fs -p binfmt_misc01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP binfmt_misc01"
