#!/bin/bash
# Functional test: ltp - mm - mmapstress08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress08"
        rlRun "kirk -f mm -p mmapstress08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress08"
