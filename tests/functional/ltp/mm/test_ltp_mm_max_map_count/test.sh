#!/bin/bash
# Functional test: ltp - mm - max_map_count
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - max_map_count"
        rlRun "kirk -f mm -p max_map_count 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP max_map_count"
