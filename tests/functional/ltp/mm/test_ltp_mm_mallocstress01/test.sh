#!/bin/bash
# Functional test: ltp - mm - mallocstress01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mallocstress01"
        rlRun "kirk -f mm -p mallocstress01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mallocstress01"
