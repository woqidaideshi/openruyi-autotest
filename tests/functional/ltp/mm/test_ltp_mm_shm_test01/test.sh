#!/bin/bash
# Functional test: ltp - mm - shm_test01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - shm_test01"
        rlRun "kirk -f mm -p shm_test01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shm_test01"
