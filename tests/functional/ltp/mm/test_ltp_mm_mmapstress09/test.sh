#!/bin/bash
# Functional test: ltp - mm - mmapstress09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress09"
        rlRun "kirk -f mm -p mmapstress09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress09"
