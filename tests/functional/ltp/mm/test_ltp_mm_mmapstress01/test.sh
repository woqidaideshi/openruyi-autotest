#!/bin/bash
# Functional test: ltp - mm - mmapstress01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress01"
        rlRun "kirk -f mm -p mmapstress01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress01"
