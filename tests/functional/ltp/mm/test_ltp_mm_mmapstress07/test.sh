#!/bin/bash
# Functional test: ltp - mm - mmapstress07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress07"
        rlRun "kirk -f mm -p mmapstress07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress07"
