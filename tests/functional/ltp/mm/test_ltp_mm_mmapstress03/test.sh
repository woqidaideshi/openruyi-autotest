#!/bin/bash
# Functional test: ltp - mm - mmapstress03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress03"
        rlRun "kirk -f mm -p mmapstress03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress03"
