#!/bin/bash
# Functional test: ltp - mm - stack_space
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - stack_space"
        rlRun "kirk -f mm -p stack_space 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stack_space"
