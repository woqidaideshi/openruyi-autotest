#!/bin/bash
# Functional test: ltp - mm - overcommit_memory05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - overcommit_memory05"
        rlRun "kirk -f mm -p overcommit_memory05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP overcommit_memory05"
