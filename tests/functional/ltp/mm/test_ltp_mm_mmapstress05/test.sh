#!/bin/bash
# Functional test: ltp - mm - mmapstress05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress05"
        rlRun "kirk -f mm -p mmapstress05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress05"
