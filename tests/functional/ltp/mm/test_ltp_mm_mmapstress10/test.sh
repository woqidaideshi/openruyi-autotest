#!/bin/bash
# Functional test: ltp - mm - mmapstress10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress10"
        rlRun "kirk -f mm -p mmapstress10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress10"
