#!/bin/bash
# Functional test: ltp - mm - mmapstress02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress02"
        rlRun "kirk -f mm -p mmapstress02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress02"
