#!/bin/bash
# Functional test: ltp - mm - mmapstress06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP mm - mmapstress06"
        rlRun "kirk -f mm -p mmapstress06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmapstress06"
