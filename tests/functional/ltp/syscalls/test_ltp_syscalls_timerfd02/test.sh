#!/bin/bash
# Functional test: ltp - syscalls - timerfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timerfd02"
        rlRun "kirk -f syscalls -p timerfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timerfd02"
