#!/bin/bash
# Functional test: ltp - syscalls - rename12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename12"
        rlRun "kirk -f syscalls -p rename12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename12"
