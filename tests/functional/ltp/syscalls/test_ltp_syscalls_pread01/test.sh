#!/bin/bash
# Functional test: ltp - syscalls - pread01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pread01"
        rlRun "kirk -f syscalls -p pread01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pread01"
