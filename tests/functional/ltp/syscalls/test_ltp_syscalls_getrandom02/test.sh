#!/bin/bash
# Functional test: ltp - syscalls - getrandom02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrandom02"
        rlRun "kirk -f syscalls -p getrandom02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrandom02"
