#!/bin/bash
# Functional test: ltp - syscalls - kill11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill11"
        rlRun "kirk -f syscalls -p kill11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill11"
