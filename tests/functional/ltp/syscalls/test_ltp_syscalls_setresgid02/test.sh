#!/bin/bash
# Functional test: ltp - syscalls - setresgid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setresgid02"
        rlRun "kirk -f syscalls -p setresgid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setresgid02"
