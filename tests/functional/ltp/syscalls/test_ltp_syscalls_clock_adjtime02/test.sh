#!/bin/bash
# Functional test: ltp - syscalls - clock_adjtime02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clock_adjtime02"
        rlRun "kirk -f syscalls -p clock_adjtime02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clock_adjtime02"
