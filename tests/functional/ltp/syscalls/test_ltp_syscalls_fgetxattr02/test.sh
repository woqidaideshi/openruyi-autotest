#!/bin/bash
# Functional test: ltp - syscalls - fgetxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fgetxattr02"
        rlRun "kirk -f syscalls -p fgetxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fgetxattr02"
