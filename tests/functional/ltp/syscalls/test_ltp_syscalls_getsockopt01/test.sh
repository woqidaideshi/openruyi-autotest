#!/bin/bash
# Functional test: ltp - syscalls - getsockopt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getsockopt01"
        rlRun "kirk -f syscalls -p getsockopt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getsockopt01"
