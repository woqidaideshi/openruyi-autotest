#!/bin/bash
# Functional test: ltp - syscalls - pidfd_open02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_open02"
        rlRun "kirk -f syscalls -p pidfd_open02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_open02"
