#!/bin/bash
# Functional test: ltp - syscalls - getxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getxattr02"
        rlRun "kirk -f syscalls -p getxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getxattr02"
