#!/bin/bash
# Functional test: ltp - syscalls - msync02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msync02"
        rlRun "kirk -f syscalls -p msync02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msync02"
