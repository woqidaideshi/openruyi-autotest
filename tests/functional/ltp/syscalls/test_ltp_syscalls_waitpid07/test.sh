#!/bin/bash
# Functional test: ltp - syscalls - waitpid07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid07"
        rlRun "kirk -f syscalls -p waitpid07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid07"
