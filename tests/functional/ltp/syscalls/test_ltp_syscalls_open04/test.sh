#!/bin/bash
# Functional test: ltp - syscalls - open04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open04"
        rlRun "kirk -f syscalls -p open04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open04"
