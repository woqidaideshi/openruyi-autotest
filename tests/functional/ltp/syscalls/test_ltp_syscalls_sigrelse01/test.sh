#!/bin/bash
# Functional test: ltp - syscalls - sigrelse01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigrelse01"
        rlRun "kirk -f syscalls -p sigrelse01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigrelse01"
