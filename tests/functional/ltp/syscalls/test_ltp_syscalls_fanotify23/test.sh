#!/bin/bash
# Functional test: ltp - syscalls - fanotify23
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify23"
        rlRun "kirk -f syscalls -p fanotify23 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify23"
