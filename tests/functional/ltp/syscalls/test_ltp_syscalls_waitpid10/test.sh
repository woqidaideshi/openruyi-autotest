#!/bin/bash
# Functional test: ltp - syscalls - waitpid10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid10"
        rlRun "kirk -f syscalls -p waitpid10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid10"
