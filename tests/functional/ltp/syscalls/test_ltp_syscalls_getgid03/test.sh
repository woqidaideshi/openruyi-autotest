#!/bin/bash
# Functional test: ltp - syscalls - getgid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getgid03"
        rlRun "kirk -f syscalls -p getgid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getgid03"
