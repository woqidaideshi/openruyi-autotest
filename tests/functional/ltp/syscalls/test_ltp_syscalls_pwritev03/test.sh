#!/bin/bash
# Functional test: ltp - syscalls - pwritev03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwritev03"
        rlRun "kirk -f syscalls -p pwritev03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwritev03"
