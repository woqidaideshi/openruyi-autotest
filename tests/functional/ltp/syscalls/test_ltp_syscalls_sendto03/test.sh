#!/bin/bash
# Functional test: ltp - syscalls - sendto03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendto03"
        rlRun "kirk -f syscalls -p sendto03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendto03"
