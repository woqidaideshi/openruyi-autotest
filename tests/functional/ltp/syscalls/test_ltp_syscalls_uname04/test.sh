#!/bin/bash
# Functional test: ltp - syscalls - uname04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - uname04"
        rlRun "kirk -f syscalls -p uname04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP uname04"
