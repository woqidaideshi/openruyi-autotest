#!/bin/bash
# Functional test: ltp - syscalls - clone07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone07"
        rlRun "kirk -f syscalls -p clone07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone07"
