#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd05"
        rlRun "kirk -f syscalls -p userfaultfd05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd05"
