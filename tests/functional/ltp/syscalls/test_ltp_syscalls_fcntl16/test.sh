#!/bin/bash
# Functional test: ltp - syscalls - fcntl16
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl16"
        rlRun "kirk -f syscalls -p fcntl16 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl16"
