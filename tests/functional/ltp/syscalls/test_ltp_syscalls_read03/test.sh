#!/bin/bash
# Functional test: ltp - syscalls - read03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - read03"
        rlRun "kirk -f syscalls -p read03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP read03"
