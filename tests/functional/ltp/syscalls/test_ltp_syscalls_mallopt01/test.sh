#!/bin/bash
# Functional test: ltp - syscalls - mallopt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mallopt01"
        rlRun "kirk -f syscalls -p mallopt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mallopt01"
