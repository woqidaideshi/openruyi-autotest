#!/bin/bash
# Functional test: ltp - syscalls - umount03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umount03"
        rlRun "kirk -f syscalls -p umount03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umount03"
