#!/bin/bash
# Functional test: ltp - syscalls - epoll_wait04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll_wait04"
        rlRun "kirk -f syscalls -p epoll_wait04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll_wait04"
