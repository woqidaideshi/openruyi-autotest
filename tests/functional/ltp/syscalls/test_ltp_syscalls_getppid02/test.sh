#!/bin/bash
# Functional test: ltp - syscalls - getppid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getppid02"
        rlRun "kirk -f syscalls -p getppid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getppid02"
