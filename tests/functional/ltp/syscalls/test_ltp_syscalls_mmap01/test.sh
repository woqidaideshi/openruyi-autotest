#!/bin/bash
# Functional test: ltp - syscalls - mmap01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap01"
        rlRun "kirk -f syscalls -p mmap01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap01"
