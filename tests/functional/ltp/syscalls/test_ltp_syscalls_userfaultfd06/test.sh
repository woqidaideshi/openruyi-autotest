#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd06"
        rlRun "kirk -f syscalls -p userfaultfd06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd06"
