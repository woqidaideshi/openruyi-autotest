#!/bin/bash
# Functional test: ltp - syscalls - listxattr03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listxattr03"
        rlRun "kirk -f syscalls -p listxattr03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listxattr03"
