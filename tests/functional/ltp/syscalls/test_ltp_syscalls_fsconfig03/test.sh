#!/bin/bash
# Functional test: ltp - syscalls - fsconfig03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsconfig03"
        rlRun "kirk -f syscalls -p fsconfig03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsconfig03"
