#!/bin/bash
# Functional test: ltp - syscalls - rename05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename05"
        rlRun "kirk -f syscalls -p rename05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename05"
