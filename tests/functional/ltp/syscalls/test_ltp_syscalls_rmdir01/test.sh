#!/bin/bash
# Functional test: ltp - syscalls - rmdir01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rmdir01"
        rlRun "kirk -f syscalls -p rmdir01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rmdir01"
