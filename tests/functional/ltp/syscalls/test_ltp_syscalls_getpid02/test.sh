#!/bin/bash
# Functional test: ltp - syscalls - getpid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpid02"
        rlRun "kirk -f syscalls -p getpid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpid02"
