#!/bin/bash
# Functional test: ltp - syscalls - waitid06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid06"
        rlRun "kirk -f syscalls -p waitid06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid06"
