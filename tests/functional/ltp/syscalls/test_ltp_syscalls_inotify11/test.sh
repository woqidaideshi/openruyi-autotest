#!/bin/bash
# Functional test: ltp - syscalls - inotify11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify11"
        rlRun "kirk -f syscalls -p inotify11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify11"
