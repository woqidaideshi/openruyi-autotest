#!/bin/bash
# Functional test: ltp - syscalls - request_key02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - request_key02"
        rlRun "kirk -f syscalls -p request_key02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP request_key02"
