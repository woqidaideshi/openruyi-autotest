#!/bin/bash
# Functional test: ltp - syscalls - splice09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice09"
        rlRun "kirk -f syscalls -p splice09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice09"
