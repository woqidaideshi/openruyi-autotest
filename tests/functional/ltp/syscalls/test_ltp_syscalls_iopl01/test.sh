#!/bin/bash
# Functional test: ltp - syscalls - iopl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - iopl01"
        rlRun "kirk -f syscalls -p iopl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP iopl01"
