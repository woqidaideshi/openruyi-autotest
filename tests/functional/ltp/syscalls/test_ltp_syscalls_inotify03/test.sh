#!/bin/bash
# Functional test: ltp - syscalls - inotify03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify03"
        rlRun "kirk -f syscalls -p inotify03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify03"
