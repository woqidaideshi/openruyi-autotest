#!/bin/bash
# Functional test: ltp - syscalls - select01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - select01"
        rlRun "kirk -f syscalls -p select01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP select01"
