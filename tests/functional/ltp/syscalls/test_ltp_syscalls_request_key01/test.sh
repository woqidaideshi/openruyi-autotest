#!/bin/bash
# Functional test: ltp - syscalls - request_key01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - request_key01"
        rlRun "kirk -f syscalls -p request_key01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP request_key01"
