#!/bin/bash
# Functional test: ltp - syscalls - sendfile03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile03"
        rlRun "kirk -f syscalls -p sendfile03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile03"
