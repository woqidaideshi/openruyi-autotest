#!/bin/bash
# Functional test: ltp - syscalls - setpgrp02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setpgrp02"
        rlRun "kirk -f syscalls -p setpgrp02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setpgrp02"
