#!/bin/bash
# Functional test: ltp - syscalls - landlock10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock10"
        rlRun "kirk -f syscalls -p landlock10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock10"
