#!/bin/bash
# Functional test: ltp - syscalls - mknod07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod07"
        rlRun "kirk -f syscalls -p mknod07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod07"
