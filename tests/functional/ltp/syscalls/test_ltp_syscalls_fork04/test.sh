#!/bin/bash
# Functional test: ltp - syscalls - fork04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork04"
        rlRun "kirk -f syscalls -p fork04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork04"
