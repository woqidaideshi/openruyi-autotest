#!/bin/bash
# Functional test: ltp - syscalls - lgetxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lgetxattr02"
        rlRun "kirk -f syscalls -p lgetxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lgetxattr02"
