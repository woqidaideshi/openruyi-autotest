#!/bin/bash
# Functional test: ltp - syscalls - llseek01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - llseek01"
        rlRun "kirk -f syscalls -p llseek01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP llseek01"
