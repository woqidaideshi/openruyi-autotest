#!/bin/bash
# Functional test: ltp - syscalls - pidfd_open03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_open03"
        rlRun "kirk -f syscalls -p pidfd_open03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_open03"
