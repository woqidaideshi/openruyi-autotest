#!/bin/bash
# Functional test: ltp - syscalls - setreuid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid03"
        rlRun "kirk -f syscalls -p setreuid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid03"
