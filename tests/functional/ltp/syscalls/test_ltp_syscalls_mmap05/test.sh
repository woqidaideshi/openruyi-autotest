#!/bin/bash
# Functional test: ltp - syscalls - mmap05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap05"
        rlRun "kirk -f syscalls -p mmap05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap05"
