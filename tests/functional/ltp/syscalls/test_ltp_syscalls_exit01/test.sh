#!/bin/bash
# Functional test: ltp - syscalls - exit01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - exit01"
        rlRun "kirk -f syscalls -p exit01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP exit01"
