#!/bin/bash
# Functional test: ltp - syscalls - poll01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - poll01"
        rlRun "kirk -f syscalls -p poll01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP poll01"
