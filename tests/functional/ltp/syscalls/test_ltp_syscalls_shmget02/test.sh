#!/bin/bash
# Functional test: ltp - syscalls - shmget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmget02"
        rlRun "kirk -f syscalls -p shmget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget02"
