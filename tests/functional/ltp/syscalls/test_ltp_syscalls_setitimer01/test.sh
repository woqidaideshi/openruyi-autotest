#!/bin/bash
# Functional test: ltp - syscalls - setitimer01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setitimer01"
        rlRun "kirk -f syscalls -p setitimer01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setitimer01"
