#!/bin/bash
# Functional test: ltp - syscalls - finit_module02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - finit_module02"
        rlRun "kirk -f syscalls -p finit_module02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP finit_module02"
