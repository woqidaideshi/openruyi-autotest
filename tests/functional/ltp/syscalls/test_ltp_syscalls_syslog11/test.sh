#!/bin/bash
# Functional test: ltp - syscalls - syslog11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - syslog11"
        rlRun "kirk -f syscalls -p syslog11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP syslog11"
