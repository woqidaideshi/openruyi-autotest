#!/bin/bash
# Functional test: ltp - syscalls - nftw01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nftw01"
        rlRun "kirk -f syscalls -p nftw01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nftw01"
