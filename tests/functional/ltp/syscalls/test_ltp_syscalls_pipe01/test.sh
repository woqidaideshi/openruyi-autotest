#!/bin/bash
# Functional test: ltp - syscalls - pipe01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe01"
        rlRun "kirk -f syscalls -p pipe01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe01"
