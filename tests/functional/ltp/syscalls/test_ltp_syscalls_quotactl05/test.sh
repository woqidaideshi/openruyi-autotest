#!/bin/bash
# Functional test: ltp - syscalls - quotactl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl05"
        rlRun "kirk -f syscalls -p quotactl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl05"
