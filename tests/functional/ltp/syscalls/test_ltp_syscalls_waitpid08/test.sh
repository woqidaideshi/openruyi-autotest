#!/bin/bash
# Functional test: ltp - syscalls - waitpid08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid08"
        rlRun "kirk -f syscalls -p waitpid08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid08"
