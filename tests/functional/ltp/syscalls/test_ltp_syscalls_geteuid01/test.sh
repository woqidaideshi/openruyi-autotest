#!/bin/bash
# Functional test: ltp - syscalls - geteuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - geteuid01"
        rlRun "kirk -f syscalls -p geteuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP geteuid01"
