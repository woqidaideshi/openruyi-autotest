#!/bin/bash
# Functional test: ltp - syscalls - pipe10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe10"
        rlRun "kirk -f syscalls -p pipe10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe10"
