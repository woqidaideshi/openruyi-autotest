#!/bin/bash
# Functional test: ltp - syscalls - sysfs04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysfs04"
        rlRun "kirk -f syscalls -p sysfs04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysfs04"
