#!/bin/bash
# Functional test: ltp - syscalls - timer_settime02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timer_settime02"
        rlRun "kirk -f syscalls -p timer_settime02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timer_settime02"
