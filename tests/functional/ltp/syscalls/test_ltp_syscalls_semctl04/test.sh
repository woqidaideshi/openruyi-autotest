#!/bin/bash
# Functional test: ltp - syscalls - semctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl04"
        rlRun "kirk -f syscalls -p semctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl04"
