#!/bin/bash
# Functional test: ltp - syscalls - timerfd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timerfd01"
        rlRun "kirk -f syscalls -p timerfd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timerfd01"
