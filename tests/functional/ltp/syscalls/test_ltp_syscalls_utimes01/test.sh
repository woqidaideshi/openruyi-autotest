#!/bin/bash
# Functional test: ltp - syscalls - utimes01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utimes01"
        rlRun "kirk -f syscalls -p utimes01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utimes01"
