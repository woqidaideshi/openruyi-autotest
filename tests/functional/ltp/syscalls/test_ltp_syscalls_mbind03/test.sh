#!/bin/bash
# Functional test: ltp - syscalls - mbind03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mbind03"
        rlRun "kirk -f syscalls -p mbind03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mbind03"
