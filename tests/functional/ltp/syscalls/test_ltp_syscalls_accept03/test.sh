#!/bin/bash
# Functional test: ltp - syscalls - accept03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - accept03"
        rlRun "kirk -f syscalls -p accept03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP accept03"
