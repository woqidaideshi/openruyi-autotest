#!/bin/bash
# Functional test: ltp - syscalls - utime05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime05"
        rlRun "kirk -f syscalls -p utime05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime05"
