#!/bin/bash
# Functional test: ltp - syscalls - fgetxattr03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fgetxattr03"
        rlRun "kirk -f syscalls -p fgetxattr03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fgetxattr03"
