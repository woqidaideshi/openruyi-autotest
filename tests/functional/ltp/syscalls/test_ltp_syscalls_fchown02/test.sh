#!/bin/bash
# Functional test: ltp - syscalls - fchown02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchown02"
        rlRun "kirk -f syscalls -p fchown02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchown02"
