#!/bin/bash
# Functional test: ltp - syscalls - pipe2_02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe2_02"
        rlRun "kirk -f syscalls -p pipe2_02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe2_02"
