#!/bin/bash
# Functional test: ltp - syscalls - epoll_pwait06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll_pwait06"
        rlRun "kirk -f syscalls -p epoll_pwait06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll_pwait06"
