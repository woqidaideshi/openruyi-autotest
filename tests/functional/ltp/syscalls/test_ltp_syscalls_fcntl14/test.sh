#!/bin/bash
# Functional test: ltp - syscalls - fcntl14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl14"
        rlRun "kirk -f syscalls -p fcntl14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl14"
