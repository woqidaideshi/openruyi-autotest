#!/bin/bash
# Functional test: ltp - syscalls - waitpid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid01"
        rlRun "kirk -f syscalls -p waitpid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid01"
