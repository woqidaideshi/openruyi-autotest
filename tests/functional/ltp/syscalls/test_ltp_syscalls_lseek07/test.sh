#!/bin/bash
# Functional test: ltp - syscalls - lseek07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lseek07"
        rlRun "kirk -f syscalls -p lseek07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lseek07"
