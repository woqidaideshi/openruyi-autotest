#!/bin/bash
# Functional test: ltp - syscalls - quotactl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl02"
        rlRun "kirk -f syscalls -p quotactl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl02"
