#!/bin/bash
# Functional test: ltp - syscalls - file_attr04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - file_attr04"
        rlRun "kirk -f syscalls -p file_attr04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP file_attr04"
