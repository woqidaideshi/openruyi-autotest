#!/bin/bash
# Functional test: ltp - syscalls - setdomainname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setdomainname01"
        rlRun "kirk -f syscalls -p setdomainname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setdomainname01"
