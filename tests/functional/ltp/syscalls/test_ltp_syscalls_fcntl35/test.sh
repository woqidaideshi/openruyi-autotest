#!/bin/bash
# Functional test: ltp - syscalls - fcntl35
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl35"
        rlRun "kirk -f syscalls -p fcntl35 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl35"
