#!/bin/bash
# Functional test: ltp - syscalls - getpgrp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpgrp01"
        rlRun "kirk -f syscalls -p getpgrp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpgrp01"
