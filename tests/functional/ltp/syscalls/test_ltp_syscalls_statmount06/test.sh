#!/bin/bash
# Functional test: ltp - syscalls - statmount06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount06"
        rlRun "kirk -f syscalls -p statmount06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount06"
