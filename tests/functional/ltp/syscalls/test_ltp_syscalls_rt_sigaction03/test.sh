#!/bin/bash
# Functional test: ltp - syscalls - rt_sigaction03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rt_sigaction03"
        rlRun "kirk -f syscalls -p rt_sigaction03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rt_sigaction03"
