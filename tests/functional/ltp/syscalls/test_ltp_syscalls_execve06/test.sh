#!/bin/bash
# Functional test: ltp - syscalls - execve06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve06"
        rlRun "kirk -f syscalls -p execve06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve06"
