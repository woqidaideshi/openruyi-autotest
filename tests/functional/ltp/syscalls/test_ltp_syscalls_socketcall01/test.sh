#!/bin/bash
# Functional test: ltp - syscalls - socketcall01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socketcall01"
        rlRun "kirk -f syscalls -p socketcall01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socketcall01"
