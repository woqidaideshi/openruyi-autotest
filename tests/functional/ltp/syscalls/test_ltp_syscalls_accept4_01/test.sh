#!/bin/bash
# Functional test: ltp - syscalls - accept4_01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - accept4_01"
        rlRun "kirk -f syscalls -p accept4_01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP accept4_01"
