#!/bin/bash
# Functional test: ltp - syscalls - mq_notify02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mq_notify02"
        rlRun "kirk -f syscalls -p mq_notify02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mq_notify02"
