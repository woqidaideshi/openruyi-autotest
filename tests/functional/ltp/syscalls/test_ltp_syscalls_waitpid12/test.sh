#!/bin/bash
# Functional test: ltp - syscalls - waitpid12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid12"
        rlRun "kirk -f syscalls -p waitpid12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid12"
