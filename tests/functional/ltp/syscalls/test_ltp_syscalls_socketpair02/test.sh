#!/bin/bash
# Functional test: ltp - syscalls - socketpair02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socketpair02"
        rlRun "kirk -f syscalls -p socketpair02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socketpair02"
