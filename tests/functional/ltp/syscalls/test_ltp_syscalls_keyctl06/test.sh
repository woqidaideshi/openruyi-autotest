#!/bin/bash
# Functional test: ltp - syscalls - keyctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl06"
        rlRun "kirk -f syscalls -p keyctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl06"
