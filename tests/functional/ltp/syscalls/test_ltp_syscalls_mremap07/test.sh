#!/bin/bash
# Functional test: ltp - syscalls - mremap07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap07"
        rlRun "kirk -f syscalls -p mremap07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap07"
