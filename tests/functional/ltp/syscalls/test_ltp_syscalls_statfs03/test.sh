#!/bin/bash
# Functional test: ltp - syscalls - statfs03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statfs03"
        rlRun "kirk -f syscalls -p statfs03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statfs03"
