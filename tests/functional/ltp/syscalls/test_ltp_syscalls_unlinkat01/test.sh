#!/bin/bash
# Functional test: ltp - syscalls - unlinkat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlinkat01"
        rlRun "kirk -f syscalls -p unlinkat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlinkat01"
