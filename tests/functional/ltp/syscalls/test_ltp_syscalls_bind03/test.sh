#!/bin/bash
# Functional test: ltp - syscalls - bind03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind03"
        rlRun "kirk -f syscalls -p bind03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind03"
