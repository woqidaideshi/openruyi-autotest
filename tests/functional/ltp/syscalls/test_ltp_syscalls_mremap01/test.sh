#!/bin/bash
# Functional test: ltp - syscalls - mremap01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap01"
        rlRun "kirk -f syscalls -p mremap01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap01"
