#!/bin/bash
# Functional test: ltp - syscalls - shmat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmat03"
        rlRun "kirk -f syscalls -p shmat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat03"
