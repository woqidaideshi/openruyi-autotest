#!/bin/bash
# Functional test: ltp - syscalls - ssetmask01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ssetmask01"
        rlRun "kirk -f syscalls -p ssetmask01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ssetmask01"
