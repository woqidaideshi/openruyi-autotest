#!/bin/bash
# Functional test: ltp - syscalls - fanotify15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify15"
        rlRun "kirk -f syscalls -p fanotify15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify15"
