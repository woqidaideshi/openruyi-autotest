#!/bin/bash
# Functional test: ltp - syscalls - prctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl06"
        rlRun "kirk -f syscalls -p prctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl06"
