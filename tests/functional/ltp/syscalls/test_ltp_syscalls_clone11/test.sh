#!/bin/bash
# Functional test: ltp - syscalls - clone11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone11"
        rlRun "kirk -f syscalls -p clone11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone11"
