#!/bin/bash
# Functional test: ltp - syscalls - mmap06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap06"
        rlRun "kirk -f syscalls -p mmap06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap06"
