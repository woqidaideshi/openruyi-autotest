#!/bin/bash
# Functional test: ltp - syscalls - splice02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice02"
        rlRun "kirk -f syscalls -p splice02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice02"
