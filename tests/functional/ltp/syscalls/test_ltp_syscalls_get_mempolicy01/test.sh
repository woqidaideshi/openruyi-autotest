#!/bin/bash
# Functional test: ltp - syscalls - get_mempolicy01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - get_mempolicy01"
        rlRun "kirk -f syscalls -p get_mempolicy01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP get_mempolicy01"
