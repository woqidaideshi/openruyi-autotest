#!/bin/bash
# Functional test: ltp - syscalls - pipe04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe04"
        rlRun "kirk -f syscalls -p pipe04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe04"
