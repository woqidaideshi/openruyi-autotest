#!/bin/bash
# Functional test: ltp - syscalls - ptrace03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace03"
        rlRun "kirk -f syscalls -p ptrace03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace03"
