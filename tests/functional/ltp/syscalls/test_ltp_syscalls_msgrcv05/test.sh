#!/bin/bash
# Functional test: ltp - syscalls - msgrcv05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv05"
        rlRun "kirk -f syscalls -p msgrcv05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv05"
