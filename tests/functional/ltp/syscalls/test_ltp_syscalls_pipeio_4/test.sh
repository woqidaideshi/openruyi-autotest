#!/bin/bash
# Functional test: ltp - syscalls - pipeio_4
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipeio_4"
        rlRun "kirk -f syscalls -p pipeio_4 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipeio_4"
