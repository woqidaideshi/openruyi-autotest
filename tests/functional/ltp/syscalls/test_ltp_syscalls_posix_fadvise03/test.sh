#!/bin/bash
# Functional test: ltp - syscalls - posix_fadvise03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - posix_fadvise03"
        rlRun "kirk -f syscalls -p posix_fadvise03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP posix_fadvise03"
