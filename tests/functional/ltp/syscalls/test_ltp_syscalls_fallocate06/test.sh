#!/bin/bash
# Functional test: ltp - syscalls - fallocate06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate06"
        rlRun "kirk -f syscalls -p fallocate06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate06"
