#!/bin/bash
# Functional test: ltp - syscalls - setgid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setgid03"
        rlRun "kirk -f syscalls -p setgid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setgid03"
