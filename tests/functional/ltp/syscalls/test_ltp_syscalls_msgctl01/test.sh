#!/bin/bash
# Functional test: ltp - syscalls - msgctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgctl01"
        rlRun "kirk -f syscalls -p msgctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl01"
