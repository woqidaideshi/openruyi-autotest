#!/bin/bash
# Functional test: ltp - syscalls - semctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl03"
        rlRun "kirk -f syscalls -p semctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl03"
