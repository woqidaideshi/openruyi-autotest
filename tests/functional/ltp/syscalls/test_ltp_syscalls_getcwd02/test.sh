#!/bin/bash
# Functional test: ltp - syscalls - getcwd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcwd02"
        rlRun "kirk -f syscalls -p getcwd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcwd02"
