#!/bin/bash
# Functional test: ltp - syscalls - cachestat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - cachestat02"
        rlRun "kirk -f syscalls -p cachestat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cachestat02"
