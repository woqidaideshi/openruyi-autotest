#!/bin/bash
# Functional test: ltp - syscalls - execveat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execveat02"
        rlRun "kirk -f syscalls -p execveat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execveat02"
