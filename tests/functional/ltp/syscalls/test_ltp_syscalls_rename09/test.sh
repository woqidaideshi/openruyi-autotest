#!/bin/bash
# Functional test: ltp - syscalls - rename09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename09"
        rlRun "kirk -f syscalls -p rename09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename09"
