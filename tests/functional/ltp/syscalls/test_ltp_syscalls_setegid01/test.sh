#!/bin/bash
# Functional test: ltp - syscalls - setegid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setegid01"
        rlRun "kirk -f syscalls -p setegid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setegid01"
