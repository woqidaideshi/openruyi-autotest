#!/bin/bash
# Functional test: ltp - syscalls - sched_setparam01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_setparam01"
        rlRun "kirk -f syscalls -p sched_setparam01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_setparam01"
