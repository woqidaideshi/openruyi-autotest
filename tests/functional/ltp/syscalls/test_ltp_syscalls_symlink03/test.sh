#!/bin/bash
# Functional test: ltp - syscalls - symlink03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - symlink03"
        rlRun "kirk -f syscalls -p symlink03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP symlink03"
