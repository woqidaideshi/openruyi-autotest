#!/bin/bash
# Functional test: ltp - syscalls - mmap22
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap22"
        rlRun "kirk -f syscalls -p mmap22 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap22"
