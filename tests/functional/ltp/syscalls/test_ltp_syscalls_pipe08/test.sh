#!/bin/bash
# Functional test: ltp - syscalls - pipe08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe08"
        rlRun "kirk -f syscalls -p pipe08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe08"
