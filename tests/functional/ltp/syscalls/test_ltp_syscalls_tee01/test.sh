#!/bin/bash
# Functional test: ltp - syscalls - tee01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tee01"
        rlRun "kirk -f syscalls -p tee01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tee01"
