#!/bin/bash
# Functional test: ltp - syscalls - lchown01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lchown01"
        rlRun "kirk -f syscalls -p lchown01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lchown01"
