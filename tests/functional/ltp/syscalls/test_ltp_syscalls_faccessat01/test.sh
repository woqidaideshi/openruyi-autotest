#!/bin/bash
# Functional test: ltp - syscalls - faccessat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - faccessat01"
        rlRun "kirk -f syscalls -p faccessat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP faccessat01"
