#!/bin/bash
# Functional test: ltp - syscalls - fanotify19
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify19"
        rlRun "kirk -f syscalls -p fanotify19 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify19"
