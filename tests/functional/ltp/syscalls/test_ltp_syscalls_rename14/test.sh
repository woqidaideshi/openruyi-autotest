#!/bin/bash
# Functional test: ltp - syscalls - rename14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename14"
        rlRun "kirk -f syscalls -p rename14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename14"
