#!/bin/bash
# Functional test: ltp - syscalls - exit_group01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - exit_group01"
        rlRun "kirk -f syscalls -p exit_group01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP exit_group01"
