#!/bin/bash
# Functional test: ltp - syscalls - nanosleep02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nanosleep02"
        rlRun "kirk -f syscalls -p nanosleep02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nanosleep02"
