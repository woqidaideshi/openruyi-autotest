#!/bin/bash
# Functional test: ltp - syscalls - semctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl06"
        rlRun "kirk -f syscalls -p semctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl06"
