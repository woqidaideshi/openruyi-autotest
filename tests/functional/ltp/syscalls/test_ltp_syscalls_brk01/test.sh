#!/bin/bash
# Functional test: ltp - syscalls - brk01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - brk01"
        rlRun "kirk -f syscalls -p brk01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP brk01"
