#!/bin/bash
# Functional test: ltp - syscalls - memfd_create02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - memfd_create02"
        rlRun "kirk -f syscalls -p memfd_create02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP memfd_create02"
