#!/bin/bash
# Functional test: ltp - syscalls - pipe13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe13"
        rlRun "kirk -f syscalls -p pipe13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe13"
