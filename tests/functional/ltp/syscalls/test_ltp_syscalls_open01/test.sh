#!/bin/bash
# Functional test: ltp - syscalls - open01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open01"
        rlRun "kirk -f syscalls -p open01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open01"
