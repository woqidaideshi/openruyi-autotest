#!/bin/bash
# Functional test: ltp - syscalls - mmap03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap03"
        rlRun "kirk -f syscalls -p mmap03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap03"
