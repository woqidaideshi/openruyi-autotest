#!/bin/bash
# Functional test: ltp - syscalls - cachestat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - cachestat01"
        rlRun "kirk -f syscalls -p cachestat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cachestat01"
