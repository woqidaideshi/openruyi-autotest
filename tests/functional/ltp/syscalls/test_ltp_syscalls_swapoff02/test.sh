#!/bin/bash
# Functional test: ltp - syscalls - swapoff02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - swapoff02"
        rlRun "kirk -f syscalls -p swapoff02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP swapoff02"
