#!/bin/bash
# Functional test: ltp - syscalls - setxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setxattr02"
        rlRun "kirk -f syscalls -p setxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setxattr02"
