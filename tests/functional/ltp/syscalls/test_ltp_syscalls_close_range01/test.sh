#!/bin/bash
# Functional test: ltp - syscalls - close_range01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - close_range01"
        rlRun "kirk -f syscalls -p close_range01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP close_range01"
