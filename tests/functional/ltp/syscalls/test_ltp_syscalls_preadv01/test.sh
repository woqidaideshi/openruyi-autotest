#!/bin/bash
# Functional test: ltp - syscalls - preadv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - preadv01"
        rlRun "kirk -f syscalls -p preadv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP preadv01"
