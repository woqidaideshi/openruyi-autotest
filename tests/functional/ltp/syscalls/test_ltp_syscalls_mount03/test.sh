#!/bin/bash
# Functional test: ltp - syscalls - mount03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount03"
        rlRun "kirk -f syscalls -p mount03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount03"
