#!/bin/bash
# Functional test: ltp - syscalls - timerfd04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timerfd04"
        rlRun "kirk -f syscalls -p timerfd04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timerfd04"
