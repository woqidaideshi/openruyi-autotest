#!/bin/bash
# Functional test: ltp - syscalls - getcwd04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcwd04"
        rlRun "kirk -f syscalls -p getcwd04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcwd04"
