#!/bin/bash
# Functional test: ltp - syscalls - sysfs03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysfs03"
        rlRun "kirk -f syscalls -p sysfs03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysfs03"
