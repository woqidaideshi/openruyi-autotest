#!/bin/bash
# Functional test: ltp - syscalls - lseek11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lseek11"
        rlRun "kirk -f syscalls -p lseek11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lseek11"
