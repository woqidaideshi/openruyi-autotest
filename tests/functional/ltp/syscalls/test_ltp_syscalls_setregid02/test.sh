#!/bin/bash
# Functional test: ltp - syscalls - setregid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setregid02"
        rlRun "kirk -f syscalls -p setregid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setregid02"
