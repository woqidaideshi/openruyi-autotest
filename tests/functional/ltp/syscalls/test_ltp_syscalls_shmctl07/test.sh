#!/bin/bash
# Functional test: ltp - syscalls - shmctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl07"
        rlRun "kirk -f syscalls -p shmctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl07"
