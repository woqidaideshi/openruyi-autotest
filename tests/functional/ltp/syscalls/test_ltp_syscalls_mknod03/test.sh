#!/bin/bash
# Functional test: ltp - syscalls - mknod03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod03"
        rlRun "kirk -f syscalls -p mknod03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod03"
