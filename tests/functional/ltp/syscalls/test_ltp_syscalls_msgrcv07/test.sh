#!/bin/bash
# Functional test: ltp - syscalls - msgrcv07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv07"
        rlRun "kirk -f syscalls -p msgrcv07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv07"
