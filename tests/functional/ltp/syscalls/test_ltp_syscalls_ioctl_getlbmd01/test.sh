#!/bin/bash
# Functional test: ltp - syscalls - ioctl_getlbmd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_getlbmd01"
        rlRun "kirk -f syscalls -p ioctl_getlbmd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_getlbmd01"
