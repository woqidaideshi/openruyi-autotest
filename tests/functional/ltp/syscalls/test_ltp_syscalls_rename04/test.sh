#!/bin/bash
# Functional test: ltp - syscalls - rename04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename04"
        rlRun "kirk -f syscalls -p rename04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename04"
