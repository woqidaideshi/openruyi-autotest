#!/bin/bash
# Functional test: ltp - syscalls - ulimit01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ulimit01"
        rlRun "kirk -f syscalls -p ulimit01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ulimit01"
