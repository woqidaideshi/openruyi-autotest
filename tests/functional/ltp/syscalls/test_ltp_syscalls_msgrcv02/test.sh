#!/bin/bash
# Functional test: ltp - syscalls - msgrcv02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv02"
        rlRun "kirk -f syscalls -p msgrcv02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv02"
