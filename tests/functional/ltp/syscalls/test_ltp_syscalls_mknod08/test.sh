#!/bin/bash
# Functional test: ltp - syscalls - mknod08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod08"
        rlRun "kirk -f syscalls -p mknod08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod08"
