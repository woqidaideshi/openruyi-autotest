#!/bin/bash
# Functional test: ltp - syscalls - fstatfs02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fstatfs02"
        rlRun "kirk -f syscalls -p fstatfs02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fstatfs02"
