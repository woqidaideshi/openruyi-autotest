#!/bin/bash
# Functional test: ltp - syscalls - ptrace08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace08"
        rlRun "kirk -f syscalls -p ptrace08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace08"
