#!/bin/bash
# Functional test: ltp - syscalls - ptrace01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace01"
        rlRun "kirk -f syscalls -p ptrace01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace01"
