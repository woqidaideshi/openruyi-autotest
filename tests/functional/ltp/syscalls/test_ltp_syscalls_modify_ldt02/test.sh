#!/bin/bash
# Functional test: ltp - syscalls - modify_ldt02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - modify_ldt02"
        rlRun "kirk -f syscalls -p modify_ldt02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP modify_ldt02"
