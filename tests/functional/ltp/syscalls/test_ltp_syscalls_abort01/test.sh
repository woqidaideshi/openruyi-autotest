#!/bin/bash
# Functional test: ltp - syscalls - abort01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - abort01"
        rlRun "kirk -f syscalls -p abort01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP abort01"
