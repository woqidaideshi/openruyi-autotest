#!/bin/bash
# Functional test: ltp - syscalls - fsetxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsetxattr01"
        rlRun "kirk -f syscalls -p fsetxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsetxattr01"
