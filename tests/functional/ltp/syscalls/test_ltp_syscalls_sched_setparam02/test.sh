#!/bin/bash
# Functional test: ltp - syscalls - sched_setparam02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_setparam02"
        rlRun "kirk -f syscalls -p sched_setparam02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_setparam02"
