#!/bin/bash
# Functional test: ltp - syscalls - openat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat01"
        rlRun "kirk -f syscalls -p openat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat01"
