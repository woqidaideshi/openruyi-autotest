#!/bin/bash
# Functional test: ltp - syscalls - getrusage03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrusage03"
        rlRun "kirk -f syscalls -p getrusage03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrusage03"
