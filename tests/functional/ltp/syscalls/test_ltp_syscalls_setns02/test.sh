#!/bin/bash
# Functional test: ltp - syscalls - setns02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setns02"
        rlRun "kirk -f syscalls -p setns02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setns02"
