#!/bin/bash
# Functional test: ltp - syscalls - ptrace11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace11"
        rlRun "kirk -f syscalls -p ptrace11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace11"
