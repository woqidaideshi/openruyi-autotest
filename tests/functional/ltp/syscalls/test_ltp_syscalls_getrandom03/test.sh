#!/bin/bash
# Functional test: ltp - syscalls - getrandom03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrandom03"
        rlRun "kirk -f syscalls -p getrandom03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrandom03"
