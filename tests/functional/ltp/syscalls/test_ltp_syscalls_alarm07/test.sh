#!/bin/bash
# Functional test: ltp - syscalls - alarm07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - alarm07"
        rlRun "kirk -f syscalls -p alarm07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP alarm07"
