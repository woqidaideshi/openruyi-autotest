#!/bin/bash
# Functional test: ltp - syscalls - waitpid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid03"
        rlRun "kirk -f syscalls -p waitpid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid03"
