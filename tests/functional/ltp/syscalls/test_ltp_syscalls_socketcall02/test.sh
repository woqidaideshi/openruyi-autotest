#!/bin/bash
# Functional test: ltp - syscalls - socketcall02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socketcall02"
        rlRun "kirk -f syscalls -p socketcall02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socketcall02"
