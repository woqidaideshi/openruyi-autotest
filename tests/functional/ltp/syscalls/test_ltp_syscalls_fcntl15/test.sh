#!/bin/bash
# Functional test: ltp - syscalls - fcntl15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl15"
        rlRun "kirk -f syscalls -p fcntl15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl15"
