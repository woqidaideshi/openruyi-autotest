#!/bin/bash
# Functional test: ltp - syscalls - mmap09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap09"
        rlRun "kirk -f syscalls -p mmap09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap09"
