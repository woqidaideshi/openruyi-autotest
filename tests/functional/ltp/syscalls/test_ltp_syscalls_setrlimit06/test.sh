#!/bin/bash
# Functional test: ltp - syscalls - setrlimit06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit06"
        rlRun "kirk -f syscalls -p setrlimit06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit06"
