#!/bin/bash
# Functional test: ltp - syscalls - fallocate01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate01"
        rlRun "kirk -f syscalls -p fallocate01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate01"
