#!/bin/bash
# Functional test: ltp - syscalls - msgsnd06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgsnd06"
        rlRun "kirk -f syscalls -p msgsnd06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgsnd06"
