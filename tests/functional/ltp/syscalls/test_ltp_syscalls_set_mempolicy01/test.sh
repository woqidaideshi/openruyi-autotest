#!/bin/bash
# Functional test: ltp - syscalls - set_mempolicy01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - set_mempolicy01"
        rlRun "kirk -f syscalls -p set_mempolicy01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP set_mempolicy01"
