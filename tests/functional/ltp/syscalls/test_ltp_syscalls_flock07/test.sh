#!/bin/bash
# Functional test: ltp - syscalls - flock07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock07"
        rlRun "kirk -f syscalls -p flock07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock07"
