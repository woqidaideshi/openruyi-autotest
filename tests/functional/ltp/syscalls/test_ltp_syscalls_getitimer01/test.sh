#!/bin/bash
# Functional test: ltp - syscalls - getitimer01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getitimer01"
        rlRun "kirk -f syscalls -p getitimer01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getitimer01"
