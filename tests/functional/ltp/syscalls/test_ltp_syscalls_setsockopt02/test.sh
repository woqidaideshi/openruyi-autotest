#!/bin/bash
# Functional test: ltp - syscalls - setsockopt02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt02"
        rlRun "kirk -f syscalls -p setsockopt02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt02"
