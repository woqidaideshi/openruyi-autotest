#!/bin/bash
# Functional test: ltp - syscalls - request_key06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - request_key06"
        rlRun "kirk -f syscalls -p request_key06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP request_key06"
