#!/bin/bash
# Functional test: ltp - syscalls - shmat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmat02"
        rlRun "kirk -f syscalls -p shmat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat02"
