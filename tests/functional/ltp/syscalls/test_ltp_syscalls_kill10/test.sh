#!/bin/bash
# Functional test: ltp - syscalls - kill10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill10"
        rlRun "kirk -f syscalls -p kill10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill10"
