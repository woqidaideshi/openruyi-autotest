#!/bin/bash
# Functional test: ltp - syscalls - dup07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup07"
        rlRun "kirk -f syscalls -p dup07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup07"
