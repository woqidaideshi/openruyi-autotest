#!/bin/bash
# Functional test: ltp - syscalls - mlock01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock01"
        rlRun "kirk -f syscalls -p mlock01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock01"
