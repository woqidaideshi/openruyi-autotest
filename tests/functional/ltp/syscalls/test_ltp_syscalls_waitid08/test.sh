#!/bin/bash
# Functional test: ltp - syscalls - waitid08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid08"
        rlRun "kirk -f syscalls -p waitid08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid08"
