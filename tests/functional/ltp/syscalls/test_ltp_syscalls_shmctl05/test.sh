#!/bin/bash
# Functional test: ltp - syscalls - shmctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl05"
        rlRun "kirk -f syscalls -p shmctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl05"
