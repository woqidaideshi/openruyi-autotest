#!/bin/bash
# Functional test: ltp - syscalls - unlink05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlink05"
        rlRun "kirk -f syscalls -p unlink05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlink05"
