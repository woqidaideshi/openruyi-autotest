#!/bin/bash
# Functional test: ltp - syscalls - fsopen02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsopen02"
        rlRun "kirk -f syscalls -p fsopen02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsopen02"
