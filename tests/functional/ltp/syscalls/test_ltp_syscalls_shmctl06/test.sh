#!/bin/bash
# Functional test: ltp - syscalls - shmctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl06"
        rlRun "kirk -f syscalls -p shmctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl06"
