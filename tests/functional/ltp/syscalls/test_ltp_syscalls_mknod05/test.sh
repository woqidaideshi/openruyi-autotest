#!/bin/bash
# Functional test: ltp - syscalls - mknod05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod05"
        rlRun "kirk -f syscalls -p mknod05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod05"
