#!/bin/bash
# Functional test: ltp - syscalls - clone301
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone301"
        rlRun "kirk -f syscalls -p clone301 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone301"
