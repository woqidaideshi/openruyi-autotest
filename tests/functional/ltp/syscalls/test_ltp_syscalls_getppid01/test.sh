#!/bin/bash
# Functional test: ltp - syscalls - getppid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getppid01"
        rlRun "kirk -f syscalls -p getppid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getppid01"
