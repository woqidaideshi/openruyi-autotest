#!/bin/bash
# Functional test: ltp - syscalls - fanotify14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify14"
        rlRun "kirk -f syscalls -p fanotify14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify14"
