#!/bin/bash
# Functional test: ltp - syscalls - readdir01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readdir01"
        rlRun "kirk -f syscalls -p readdir01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readdir01"
