#!/bin/bash
# Functional test: ltp - syscalls - creat05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat05"
        rlRun "kirk -f syscalls -p creat05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat05"
