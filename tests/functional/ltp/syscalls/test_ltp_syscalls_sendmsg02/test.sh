#!/bin/bash
# Functional test: ltp - syscalls - sendmsg02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendmsg02"
        rlRun "kirk -f syscalls -p sendmsg02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendmsg02"
