#!/bin/bash
# Functional test: ltp - syscalls - fchown03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchown03"
        rlRun "kirk -f syscalls -p fchown03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchown03"
