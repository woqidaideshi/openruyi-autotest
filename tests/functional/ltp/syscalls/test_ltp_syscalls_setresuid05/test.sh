#!/bin/bash
# Functional test: ltp - syscalls - setresuid05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setresuid05"
        rlRun "kirk -f syscalls -p setresuid05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setresuid05"
