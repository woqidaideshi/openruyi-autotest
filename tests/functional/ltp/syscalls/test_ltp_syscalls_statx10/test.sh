#!/bin/bash
# Functional test: ltp - syscalls - statx10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx10"
        rlRun "kirk -f syscalls -p statx10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx10"
