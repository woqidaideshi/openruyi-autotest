#!/bin/bash
# Functional test: ltp - syscalls - sigwait01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigwait01"
        rlRun "kirk -f syscalls -p sigwait01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigwait01"
