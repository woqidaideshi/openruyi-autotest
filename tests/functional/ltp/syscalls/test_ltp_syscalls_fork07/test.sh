#!/bin/bash
# Functional test: ltp - syscalls - fork07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork07"
        rlRun "kirk -f syscalls -p fork07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork07"
