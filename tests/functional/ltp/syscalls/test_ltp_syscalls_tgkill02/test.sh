#!/bin/bash
# Functional test: ltp - syscalls - tgkill02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tgkill02"
        rlRun "kirk -f syscalls -p tgkill02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tgkill02"
