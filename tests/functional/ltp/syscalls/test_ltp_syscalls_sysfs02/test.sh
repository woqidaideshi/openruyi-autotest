#!/bin/bash
# Functional test: ltp - syscalls - sysfs02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysfs02"
        rlRun "kirk -f syscalls -p sysfs02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysfs02"
