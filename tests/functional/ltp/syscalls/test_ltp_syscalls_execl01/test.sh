#!/bin/bash
# Functional test: ltp - syscalls - execl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execl01"
        rlRun "kirk -f syscalls -p execl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execl01"
