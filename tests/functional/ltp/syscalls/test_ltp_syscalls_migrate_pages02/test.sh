#!/bin/bash
# Functional test: ltp - syscalls - migrate_pages02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - migrate_pages02"
        rlRun "kirk -f syscalls -p migrate_pages02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP migrate_pages02"
