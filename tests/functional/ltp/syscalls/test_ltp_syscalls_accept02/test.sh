#!/bin/bash
# Functional test: ltp - syscalls - accept02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - accept02"
        rlRun "kirk -f syscalls -p accept02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP accept02"
