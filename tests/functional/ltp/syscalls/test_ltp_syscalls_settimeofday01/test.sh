#!/bin/bash
# Functional test: ltp - syscalls - settimeofday01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - settimeofday01"
        rlRun "kirk -f syscalls -p settimeofday01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP settimeofday01"
