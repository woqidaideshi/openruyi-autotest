#!/bin/bash
# Functional test: ltp - syscalls - preadv02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - preadv02"
        rlRun "kirk -f syscalls -p preadv02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP preadv02"
