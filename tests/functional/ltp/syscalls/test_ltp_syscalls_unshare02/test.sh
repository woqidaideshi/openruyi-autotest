#!/bin/bash
# Functional test: ltp - syscalls - unshare02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unshare02"
        rlRun "kirk -f syscalls -p unshare02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unshare02"
