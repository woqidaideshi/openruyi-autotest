#!/bin/bash
# Functional test: ltp - syscalls - readdir21
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readdir21"
        rlRun "kirk -f syscalls -p readdir21 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readdir21"
