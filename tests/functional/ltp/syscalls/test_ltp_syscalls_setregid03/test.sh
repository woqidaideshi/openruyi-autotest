#!/bin/bash
# Functional test: ltp - syscalls - setregid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setregid03"
        rlRun "kirk -f syscalls -p setregid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setregid03"
