#!/bin/bash
# Functional test: ltp - syscalls - listxattr04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listxattr04"
        rlRun "kirk -f syscalls -p listxattr04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listxattr04"
