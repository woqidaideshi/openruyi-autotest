#!/bin/bash
# Functional test: ltp - syscalls - open08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open08"
        rlRun "kirk -f syscalls -p open08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open08"
