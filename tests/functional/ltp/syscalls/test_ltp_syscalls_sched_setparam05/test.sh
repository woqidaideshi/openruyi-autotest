#!/bin/bash
# Functional test: ltp - syscalls - sched_setparam05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_setparam05"
        rlRun "kirk -f syscalls -p sched_setparam05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_setparam05"
