#!/bin/bash
# Functional test: ltp - syscalls - ioprio_set03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioprio_set03"
        rlRun "kirk -f syscalls -p ioprio_set03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioprio_set03"
