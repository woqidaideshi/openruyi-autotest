#!/bin/bash
# Functional test: ltp - syscalls - statmount02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount02"
        rlRun "kirk -f syscalls -p statmount02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount02"
