#!/bin/bash
# Functional test: ltp - syscalls - pipe2_04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe2_04"
        rlRun "kirk -f syscalls -p pipe2_04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe2_04"
