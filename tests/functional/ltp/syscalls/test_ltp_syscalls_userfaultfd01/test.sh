#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd01"
        rlRun "kirk -f syscalls -p userfaultfd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd01"
