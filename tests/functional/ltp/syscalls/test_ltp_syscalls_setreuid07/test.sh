#!/bin/bash
# Functional test: ltp - syscalls - setreuid07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid07"
        rlRun "kirk -f syscalls -p setreuid07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid07"
