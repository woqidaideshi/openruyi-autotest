#!/bin/bash
# Functional test: ltp - syscalls - pwrite03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwrite03"
        rlRun "kirk -f syscalls -p pwrite03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwrite03"
