#!/bin/bash
# Functional test: ltp - syscalls - sigtimedwait01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigtimedwait01"
        rlRun "kirk -f syscalls -p sigtimedwait01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigtimedwait01"
