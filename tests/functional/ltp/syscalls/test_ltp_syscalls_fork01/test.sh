#!/bin/bash
# Functional test: ltp - syscalls - fork01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork01"
        rlRun "kirk -f syscalls -p fork01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork01"
