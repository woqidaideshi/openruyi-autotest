#!/bin/bash
# Functional test: ltp - syscalls - setsockopt08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt08"
        rlRun "kirk -f syscalls -p setsockopt08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt08"
