#!/bin/bash
# Functional test: ltp - syscalls - getrusage01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrusage01"
        rlRun "kirk -f syscalls -p getrusage01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrusage01"
