#!/bin/bash
# Functional test: ltp - syscalls - cachestat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - cachestat04"
        rlRun "kirk -f syscalls -p cachestat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cachestat04"
