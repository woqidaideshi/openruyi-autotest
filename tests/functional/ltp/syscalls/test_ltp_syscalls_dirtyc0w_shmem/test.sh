#!/bin/bash
# Functional test: ltp - syscalls - dirtyc0w_shmem
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dirtyc0w_shmem"
        rlRun "kirk -f syscalls -p dirtyc0w_shmem 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dirtyc0w_shmem"
