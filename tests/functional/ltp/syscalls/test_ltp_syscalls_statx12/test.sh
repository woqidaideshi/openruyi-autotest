#!/bin/bash
# Functional test: ltp - syscalls - statx12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx12"
        rlRun "kirk -f syscalls -p statx12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx12"
