#!/bin/bash
# Functional test: ltp - syscalls - lseek02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lseek02"
        rlRun "kirk -f syscalls -p lseek02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lseek02"
