#!/bin/bash
# Functional test: ltp - syscalls - writev01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - writev01"
        rlRun "kirk -f syscalls -p writev01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP writev01"
