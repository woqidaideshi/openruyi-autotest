#!/bin/bash
# Functional test: ltp - syscalls - linkat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - linkat02"
        rlRun "kirk -f syscalls -p linkat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP linkat02"
