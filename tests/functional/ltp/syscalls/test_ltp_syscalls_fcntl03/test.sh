#!/bin/bash
# Functional test: ltp - syscalls - fcntl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl03"
        rlRun "kirk -f syscalls -p fcntl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl03"
