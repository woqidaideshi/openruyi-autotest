#!/bin/bash
# Functional test: ltp - syscalls - mount02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount02"
        rlRun "kirk -f syscalls -p mount02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount02"
