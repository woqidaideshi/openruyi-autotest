#!/bin/bash
# Functional test: ltp - syscalls - alarm06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - alarm06"
        rlRun "kirk -f syscalls -p alarm06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP alarm06"
