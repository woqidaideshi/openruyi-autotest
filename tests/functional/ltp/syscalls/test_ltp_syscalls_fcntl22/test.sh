#!/bin/bash
# Functional test: ltp - syscalls - fcntl22
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl22"
        rlRun "kirk -f syscalls -p fcntl22 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl22"
