#!/bin/bash
# Functional test: ltp - syscalls - eventfd05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd05"
        rlRun "kirk -f syscalls -p eventfd05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd05"
