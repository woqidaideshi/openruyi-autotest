#!/bin/bash
# Functional test: ltp - syscalls - ustat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ustat02"
        rlRun "kirk -f syscalls -p ustat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ustat02"
