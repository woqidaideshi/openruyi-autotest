#!/bin/bash
# Functional test: ltp - syscalls - setreuid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid02"
        rlRun "kirk -f syscalls -p setreuid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid02"
