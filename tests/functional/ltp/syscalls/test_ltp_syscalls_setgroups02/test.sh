#!/bin/bash
# Functional test: ltp - syscalls - setgroups02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setgroups02"
        rlRun "kirk -f syscalls -p setgroups02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setgroups02"
