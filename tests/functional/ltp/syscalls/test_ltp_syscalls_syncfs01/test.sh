#!/bin/bash
# Functional test: ltp - syscalls - syncfs01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - syncfs01"
        rlRun "kirk -f syscalls -p syncfs01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP syncfs01"
