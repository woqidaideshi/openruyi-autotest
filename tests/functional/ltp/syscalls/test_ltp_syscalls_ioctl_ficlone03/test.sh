#!/bin/bash
# Functional test: ltp - syscalls - ioctl_ficlone03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_ficlone03"
        rlRun "kirk -f syscalls -p ioctl_ficlone03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_ficlone03"
