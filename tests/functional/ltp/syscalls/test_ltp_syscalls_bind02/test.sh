#!/bin/bash
# Functional test: ltp - syscalls - bind02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind02"
        rlRun "kirk -f syscalls -p bind02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind02"
