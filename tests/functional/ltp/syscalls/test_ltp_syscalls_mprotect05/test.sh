#!/bin/bash
# Functional test: ltp - syscalls - mprotect05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mprotect05"
        rlRun "kirk -f syscalls -p mprotect05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mprotect05"
