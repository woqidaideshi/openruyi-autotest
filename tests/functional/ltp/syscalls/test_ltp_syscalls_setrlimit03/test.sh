#!/bin/bash
# Functional test: ltp - syscalls - setrlimit03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit03"
        rlRun "kirk -f syscalls -p setrlimit03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit03"
