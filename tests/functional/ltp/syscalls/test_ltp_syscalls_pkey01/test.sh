#!/bin/bash
# Functional test: ltp - syscalls - pkey01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pkey01"
        rlRun "kirk -f syscalls -p pkey01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pkey01"
