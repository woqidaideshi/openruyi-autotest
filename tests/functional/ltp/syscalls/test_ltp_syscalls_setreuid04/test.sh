#!/bin/bash
# Functional test: ltp - syscalls - setreuid04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid04"
        rlRun "kirk -f syscalls -p setreuid04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid04"
