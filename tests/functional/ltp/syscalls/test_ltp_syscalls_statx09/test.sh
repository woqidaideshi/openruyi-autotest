#!/bin/bash
# Functional test: ltp - syscalls - statx09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx09"
        rlRun "kirk -f syscalls -p statx09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx09"
