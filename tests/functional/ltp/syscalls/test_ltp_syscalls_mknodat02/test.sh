#!/bin/bash
# Functional test: ltp - syscalls - mknodat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknodat02"
        rlRun "kirk -f syscalls -p mknodat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknodat02"
