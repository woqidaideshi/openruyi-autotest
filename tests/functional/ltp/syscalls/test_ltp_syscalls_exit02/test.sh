#!/bin/bash
# Functional test: ltp - syscalls - exit02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - exit02"
        rlRun "kirk -f syscalls -p exit02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP exit02"
