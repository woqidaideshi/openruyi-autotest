#!/bin/bash
# Functional test: ltp - syscalls - io_uring04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_uring04"
        rlRun "kirk -f syscalls -p io_uring04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_uring04"
