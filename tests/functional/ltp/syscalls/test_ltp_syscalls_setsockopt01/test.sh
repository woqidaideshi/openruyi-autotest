#!/bin/bash
# Functional test: ltp - syscalls - setsockopt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt01"
        rlRun "kirk -f syscalls -p setsockopt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt01"
