#!/bin/bash
# Functional test: ltp - syscalls - stime02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stime02"
        rlRun "kirk -f syscalls -p stime02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stime02"
