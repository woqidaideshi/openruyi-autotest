#!/bin/bash
# Functional test: ltp - syscalls - tkill01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tkill01"
        rlRun "kirk -f syscalls -p tkill01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tkill01"
