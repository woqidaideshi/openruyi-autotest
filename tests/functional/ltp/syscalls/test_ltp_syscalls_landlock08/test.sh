#!/bin/bash
# Functional test: ltp - syscalls - landlock08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock08"
        rlRun "kirk -f syscalls -p landlock08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock08"
