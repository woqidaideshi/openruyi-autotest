#!/bin/bash
# Functional test: ltp - syscalls - sigsuspend02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigsuspend02"
        rlRun "kirk -f syscalls -p sigsuspend02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigsuspend02"
