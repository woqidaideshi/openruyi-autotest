#!/bin/bash
# Functional test: ltp - syscalls - fchdir03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchdir03"
        rlRun "kirk -f syscalls -p fchdir03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchdir03"
