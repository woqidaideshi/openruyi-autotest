#!/bin/bash
# Functional test: ltp - syscalls - kill02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill02"
        rlRun "kirk -f syscalls -p kill02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill02"
