#!/bin/bash
# Functional test: ltp - syscalls - kill06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill06"
        rlRun "kirk -f syscalls -p kill06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill06"
