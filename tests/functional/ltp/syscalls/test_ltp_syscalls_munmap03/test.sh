#!/bin/bash
# Functional test: ltp - syscalls - munmap03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munmap03"
        rlRun "kirk -f syscalls -p munmap03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munmap03"
