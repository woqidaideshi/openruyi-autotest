#!/bin/bash
# Functional test: ltp - syscalls - move_mount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - move_mount01"
        rlRun "kirk -f syscalls -p move_mount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP move_mount01"
