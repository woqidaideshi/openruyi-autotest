#!/bin/bash
# Functional test: ltp - syscalls - fsmount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsmount01"
        rlRun "kirk -f syscalls -p fsmount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsmount01"
