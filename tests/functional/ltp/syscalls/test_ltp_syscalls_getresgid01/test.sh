#!/bin/bash
# Functional test: ltp - syscalls - getresgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getresgid01"
        rlRun "kirk -f syscalls -p getresgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getresgid01"
