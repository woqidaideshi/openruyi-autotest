#!/bin/bash
# Functional test: ltp - syscalls - ftruncate01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ftruncate01"
        rlRun "kirk -f syscalls -p ftruncate01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ftruncate01"
