#!/bin/bash
# Functional test: ltp - syscalls - madvise06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise06"
        rlRun "kirk -f syscalls -p madvise06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise06"
