#!/bin/bash
# Functional test: ltp - syscalls - access03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - access03"
        rlRun "kirk -f syscalls -p access03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP access03"
