#!/bin/bash
# Functional test: ltp - syscalls - setsid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsid01"
        rlRun "kirk -f syscalls -p setsid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsid01"
