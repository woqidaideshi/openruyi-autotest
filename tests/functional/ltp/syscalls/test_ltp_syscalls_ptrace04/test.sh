#!/bin/bash
# Functional test: ltp - syscalls - ptrace04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace04"
        rlRun "kirk -f syscalls -p ptrace04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace04"
