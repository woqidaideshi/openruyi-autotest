#!/bin/bash
# Functional test: ltp - syscalls - fcntl13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl13"
        rlRun "kirk -f syscalls -p fcntl13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl13"
