#!/bin/bash
# Functional test: ltp - syscalls - ptrace05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace05"
        rlRun "kirk -f syscalls -p ptrace05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace05"
