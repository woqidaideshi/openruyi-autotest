#!/bin/bash
# Functional test: ltp - syscalls - quotactl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl01"
        rlRun "kirk -f syscalls -p quotactl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl01"
