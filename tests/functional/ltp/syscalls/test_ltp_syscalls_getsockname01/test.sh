#!/bin/bash
# Functional test: ltp - syscalls - getsockname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getsockname01"
        rlRun "kirk -f syscalls -p getsockname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getsockname01"
