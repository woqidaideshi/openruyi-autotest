#!/bin/bash
# Functional test: ltp - syscalls - init_module01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - init_module01"
        rlRun "kirk -f syscalls -p init_module01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP init_module01"
