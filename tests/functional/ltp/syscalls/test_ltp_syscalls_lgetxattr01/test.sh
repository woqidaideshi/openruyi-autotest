#!/bin/bash
# Functional test: ltp - syscalls - lgetxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lgetxattr01"
        rlRun "kirk -f syscalls -p lgetxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lgetxattr01"
