#!/bin/bash
# Functional test: ltp - syscalls - fcntl23
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl23"
        rlRun "kirk -f syscalls -p fcntl23 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl23"
