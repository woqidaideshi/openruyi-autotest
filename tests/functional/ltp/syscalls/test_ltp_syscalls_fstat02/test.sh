#!/bin/bash
# Functional test: ltp - syscalls - fstat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fstat02"
        rlRun "kirk -f syscalls -p fstat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fstat02"
