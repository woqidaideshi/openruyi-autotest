#!/bin/bash
# Functional test: ltp - syscalls - read04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - read04"
        rlRun "kirk -f syscalls -p read04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP read04"
