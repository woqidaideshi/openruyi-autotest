#!/bin/bash
# Functional test: ltp - syscalls - socketcall03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socketcall03"
        rlRun "kirk -f syscalls -p socketcall03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socketcall03"
