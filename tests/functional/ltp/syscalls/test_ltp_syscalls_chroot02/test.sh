#!/bin/bash
# Functional test: ltp - syscalls - chroot02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chroot02"
        rlRun "kirk -f syscalls -p chroot02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chroot02"
