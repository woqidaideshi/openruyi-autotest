#!/bin/bash
# Functional test: ltp - syscalls - fcntl19
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl19"
        rlRun "kirk -f syscalls -p fcntl19 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl19"
