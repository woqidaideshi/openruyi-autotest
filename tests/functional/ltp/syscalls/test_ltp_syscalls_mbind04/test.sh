#!/bin/bash
# Functional test: ltp - syscalls - mbind04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mbind04"
        rlRun "kirk -f syscalls -p mbind04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mbind04"
