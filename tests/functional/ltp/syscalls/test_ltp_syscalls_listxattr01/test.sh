#!/bin/bash
# Functional test: ltp - syscalls - listxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listxattr01"
        rlRun "kirk -f syscalls -p listxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listxattr01"
