#!/bin/bash
# Functional test: ltp - syscalls - splice07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice07"
        rlRun "kirk -f syscalls -p splice07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice07"
