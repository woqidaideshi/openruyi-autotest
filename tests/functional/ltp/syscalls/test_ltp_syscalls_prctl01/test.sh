#!/bin/bash
# Functional test: ltp - syscalls - prctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl01"
        rlRun "kirk -f syscalls -p prctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl01"
