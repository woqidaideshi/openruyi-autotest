#!/bin/bash
# Functional test: ltp - syscalls - lremovexattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lremovexattr01"
        rlRun "kirk -f syscalls -p lremovexattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lremovexattr01"
