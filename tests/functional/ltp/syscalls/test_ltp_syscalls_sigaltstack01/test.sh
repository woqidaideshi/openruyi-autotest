#!/bin/bash
# Functional test: ltp - syscalls - sigaltstack01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigaltstack01"
        rlRun "kirk -f syscalls -p sigaltstack01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigaltstack01"
