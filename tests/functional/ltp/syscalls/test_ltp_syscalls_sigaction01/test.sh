#!/bin/bash
# Functional test: ltp - syscalls - sigaction01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigaction01"
        rlRun "kirk -f syscalls -p sigaction01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigaction01"
