#!/bin/bash
# Functional test: ltp - syscalls - msgstress01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgstress01"
        rlRun "kirk -f syscalls -p msgstress01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgstress01"
