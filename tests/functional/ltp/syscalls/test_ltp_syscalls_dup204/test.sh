#!/bin/bash
# Functional test: ltp - syscalls - dup204
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup204"
        rlRun "kirk -f syscalls -p dup204 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup204"
