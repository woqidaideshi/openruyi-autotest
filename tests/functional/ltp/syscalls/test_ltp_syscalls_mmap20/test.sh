#!/bin/bash
# Functional test: ltp - syscalls - mmap20
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap20"
        rlRun "kirk -f syscalls -p mmap20 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap20"
