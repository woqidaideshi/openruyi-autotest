#!/bin/bash
# Functional test: ltp - syscalls - socketpair01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socketpair01"
        rlRun "kirk -f syscalls -p socketpair01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socketpair01"
