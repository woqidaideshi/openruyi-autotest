#!/bin/bash
# Functional test: ltp - syscalls - ptrace06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace06"
        rlRun "kirk -f syscalls -p ptrace06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace06"
