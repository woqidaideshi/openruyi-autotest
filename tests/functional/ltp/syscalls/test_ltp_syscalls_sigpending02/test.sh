#!/bin/bash
# Functional test: ltp - syscalls - sigpending02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigpending02"
        rlRun "kirk -f syscalls -p sigpending02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigpending02"
