#!/bin/bash
# Functional test: ltp - syscalls - execve05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve05"
        rlRun "kirk -f syscalls -p execve05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve05"
