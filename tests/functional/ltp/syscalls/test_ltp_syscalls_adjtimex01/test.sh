#!/bin/bash
# Functional test: ltp - syscalls - adjtimex01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - adjtimex01"
        rlRun "kirk -f syscalls -p adjtimex01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP adjtimex01"
