#!/bin/bash
# Functional test: ltp - syscalls - mmap13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap13"
        rlRun "kirk -f syscalls -p mmap13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap13"
