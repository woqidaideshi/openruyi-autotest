#!/bin/bash
# Functional test: ltp - syscalls - pwrite01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwrite01"
        rlRun "kirk -f syscalls -p pwrite01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwrite01"
