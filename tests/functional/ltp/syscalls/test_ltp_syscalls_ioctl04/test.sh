#!/bin/bash
# Functional test: ltp - syscalls - ioctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl04"
        rlRun "kirk -f syscalls -p ioctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl04"
