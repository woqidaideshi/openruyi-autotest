#!/bin/bash
# Functional test: ltp - syscalls - kill12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill12"
        rlRun "kirk -f syscalls -p kill12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill12"
