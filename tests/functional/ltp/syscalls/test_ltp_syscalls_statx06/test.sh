#!/bin/bash
# Functional test: ltp - syscalls - statx06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx06"
        rlRun "kirk -f syscalls -p statx06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx06"
