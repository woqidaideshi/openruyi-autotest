#!/bin/bash
# Functional test: ltp - syscalls - pause01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pause01"
        rlRun "kirk -f syscalls -p pause01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pause01"
