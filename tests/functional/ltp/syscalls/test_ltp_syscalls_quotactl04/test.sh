#!/bin/bash
# Functional test: ltp - syscalls - quotactl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl04"
        rlRun "kirk -f syscalls -p quotactl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl04"
