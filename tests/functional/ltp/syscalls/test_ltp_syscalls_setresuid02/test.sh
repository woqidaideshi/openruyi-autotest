#!/bin/bash
# Functional test: ltp - syscalls - setresuid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setresuid02"
        rlRun "kirk -f syscalls -p setresuid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setresuid02"
