#!/bin/bash
# Functional test: ltp - syscalls - listen01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listen01"
        rlRun "kirk -f syscalls -p listen01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listen01"
