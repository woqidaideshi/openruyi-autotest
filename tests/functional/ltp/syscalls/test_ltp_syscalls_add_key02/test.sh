#!/bin/bash
# Functional test: ltp - syscalls - add_key02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - add_key02"
        rlRun "kirk -f syscalls -p add_key02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP add_key02"
