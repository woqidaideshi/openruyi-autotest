#!/bin/bash
# Functional test: ltp - syscalls - close_range02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - close_range02"
        rlRun "kirk -f syscalls -p close_range02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP close_range02"
