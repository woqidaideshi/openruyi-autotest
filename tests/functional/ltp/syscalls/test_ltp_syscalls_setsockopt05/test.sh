#!/bin/bash
# Functional test: ltp - syscalls - setsockopt05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt05"
        rlRun "kirk -f syscalls -p setsockopt05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt05"
