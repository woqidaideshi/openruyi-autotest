#!/bin/bash
# Functional test: ltp - syscalls - msgsnd05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgsnd05"
        rlRun "kirk -f syscalls -p msgsnd05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgsnd05"
