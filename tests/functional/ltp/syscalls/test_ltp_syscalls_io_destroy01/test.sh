#!/bin/bash
# Functional test: ltp - syscalls - io_destroy01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_destroy01"
        rlRun "kirk -f syscalls -p io_destroy01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_destroy01"
