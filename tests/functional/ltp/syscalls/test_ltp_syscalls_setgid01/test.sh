#!/bin/bash
# Functional test: ltp - syscalls - setgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setgid01"
        rlRun "kirk -f syscalls -p setgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setgid01"
