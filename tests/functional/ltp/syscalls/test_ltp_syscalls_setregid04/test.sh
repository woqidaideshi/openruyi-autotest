#!/bin/bash
# Functional test: ltp - syscalls - setregid04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setregid04"
        rlRun "kirk -f syscalls -p setregid04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setregid04"
