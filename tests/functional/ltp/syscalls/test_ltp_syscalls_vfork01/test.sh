#!/bin/bash
# Functional test: ltp - syscalls - vfork01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vfork01"
        rlRun "kirk -f syscalls -p vfork01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vfork01"
