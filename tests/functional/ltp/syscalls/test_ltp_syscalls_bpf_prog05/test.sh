#!/bin/bash
# Functional test: ltp - syscalls - bpf_prog05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bpf_prog05"
        rlRun "kirk -f syscalls -p bpf_prog05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bpf_prog05"
