#!/bin/bash
# Functional test: ltp - syscalls - read01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - read01"
        rlRun "kirk -f syscalls -p read01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP read01"
