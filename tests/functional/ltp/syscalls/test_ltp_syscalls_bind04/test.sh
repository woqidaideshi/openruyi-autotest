#!/bin/bash
# Functional test: ltp - syscalls - bind04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind04"
        rlRun "kirk -f syscalls -p bind04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind04"
