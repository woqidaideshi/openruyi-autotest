#!/bin/bash
# Functional test: ltp - syscalls - sigprocmask01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigprocmask01"
        rlRun "kirk -f syscalls -p sigprocmask01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigprocmask01"
