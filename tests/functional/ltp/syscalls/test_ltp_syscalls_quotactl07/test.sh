#!/bin/bash
# Functional test: ltp - syscalls - quotactl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl07"
        rlRun "kirk -f syscalls -p quotactl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl07"
