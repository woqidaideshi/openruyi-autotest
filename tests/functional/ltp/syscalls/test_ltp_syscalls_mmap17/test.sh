#!/bin/bash
# Functional test: ltp - syscalls - mmap17
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap17"
        rlRun "kirk -f syscalls -p mmap17 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap17"
