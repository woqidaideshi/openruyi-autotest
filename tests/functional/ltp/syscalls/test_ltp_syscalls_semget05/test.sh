#!/bin/bash
# Functional test: ltp - syscalls - semget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semget05"
        rlRun "kirk -f syscalls -p semget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semget05"
