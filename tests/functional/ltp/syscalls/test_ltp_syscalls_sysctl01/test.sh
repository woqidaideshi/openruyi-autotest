#!/bin/bash
# Functional test: ltp - syscalls - sysctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysctl01"
        rlRun "kirk -f syscalls -p sysctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysctl01"
