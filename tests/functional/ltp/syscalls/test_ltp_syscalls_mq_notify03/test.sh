#!/bin/bash
# Functional test: ltp - syscalls - mq_notify03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mq_notify03"
        rlRun "kirk -f syscalls -p mq_notify03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mq_notify03"
