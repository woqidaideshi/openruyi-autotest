#!/bin/bash
# Functional test: ltp - syscalls - pwritev02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwritev02"
        rlRun "kirk -f syscalls -p pwritev02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwritev02"
