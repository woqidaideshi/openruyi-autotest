#!/bin/bash
# Functional test: ltp - syscalls - quotactl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl03"
        rlRun "kirk -f syscalls -p quotactl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl03"
