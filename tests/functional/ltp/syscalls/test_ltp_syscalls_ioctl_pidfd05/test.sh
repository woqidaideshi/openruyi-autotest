#!/bin/bash
# Functional test: ltp - syscalls - ioctl_pidfd05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_pidfd05"
        rlRun "kirk -f syscalls -p ioctl_pidfd05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_pidfd05"
