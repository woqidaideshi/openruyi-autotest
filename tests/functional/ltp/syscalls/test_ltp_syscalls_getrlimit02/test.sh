#!/bin/bash
# Functional test: ltp - syscalls - getrlimit02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrlimit02"
        rlRun "kirk -f syscalls -p getrlimit02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrlimit02"
