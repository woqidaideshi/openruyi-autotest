#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd03"
        rlRun "kirk -f syscalls -p userfaultfd03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd03"
