#!/bin/bash
# Functional test: ltp - syscalls - setsockopt03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt03"
        rlRun "kirk -f syscalls -p setsockopt03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt03"
