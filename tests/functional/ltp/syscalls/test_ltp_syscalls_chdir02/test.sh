#!/bin/bash
# Functional test: ltp - syscalls - chdir02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chdir02"
        rlRun "kirk -f syscalls -p chdir02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chdir02"
