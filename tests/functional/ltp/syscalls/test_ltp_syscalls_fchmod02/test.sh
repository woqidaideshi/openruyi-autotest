#!/bin/bash
# Functional test: ltp - syscalls - fchmod02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmod02"
        rlRun "kirk -f syscalls -p fchmod02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmod02"
