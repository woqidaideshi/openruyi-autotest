#!/bin/bash
# Functional test: ltp - syscalls - readahead01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readahead01"
        rlRun "kirk -f syscalls -p readahead01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readahead01"
