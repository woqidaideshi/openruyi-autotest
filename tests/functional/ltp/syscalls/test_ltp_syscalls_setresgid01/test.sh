#!/bin/bash
# Functional test: ltp - syscalls - setresgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setresgid01"
        rlRun "kirk -f syscalls -p setresgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setresgid01"
