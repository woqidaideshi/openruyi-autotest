#!/bin/bash
# Functional test: ltp - syscalls - delete_module02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - delete_module02"
        rlRun "kirk -f syscalls -p delete_module02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP delete_module02"
