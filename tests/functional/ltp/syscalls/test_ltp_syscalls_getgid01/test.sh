#!/bin/bash
# Functional test: ltp - syscalls - getgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getgid01"
        rlRun "kirk -f syscalls -p getgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getgid01"
