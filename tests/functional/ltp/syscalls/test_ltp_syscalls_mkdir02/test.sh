#!/bin/bash
# Functional test: ltp - syscalls - mkdir02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mkdir02"
        rlRun "kirk -f syscalls -p mkdir02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdir02"
