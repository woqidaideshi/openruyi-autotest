#!/bin/bash
# Functional test: ltp - syscalls - stat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stat03"
        rlRun "kirk -f syscalls -p stat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stat03"
