#!/bin/bash
# Functional test: ltp - syscalls - mount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount01"
        rlRun "kirk -f syscalls -p mount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount01"
