#!/bin/bash
# Functional test: ltp - syscalls - ioctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl07"
        rlRun "kirk -f syscalls -p ioctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl07"
