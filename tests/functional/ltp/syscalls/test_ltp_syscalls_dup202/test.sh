#!/bin/bash
# Functional test: ltp - syscalls - dup202
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup202"
        rlRun "kirk -f syscalls -p dup202 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup202"
