#!/bin/bash
# Functional test: ltp - syscalls - sysfs05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysfs05"
        rlRun "kirk -f syscalls -p sysfs05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysfs05"
