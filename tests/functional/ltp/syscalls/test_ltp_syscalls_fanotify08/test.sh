#!/bin/bash
# Functional test: ltp - syscalls - fanotify08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify08"
        rlRun "kirk -f syscalls -p fanotify08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify08"
