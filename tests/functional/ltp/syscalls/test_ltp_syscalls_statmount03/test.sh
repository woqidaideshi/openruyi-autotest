#!/bin/bash
# Functional test: ltp - syscalls - statmount03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount03"
        rlRun "kirk -f syscalls -p statmount03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount03"
