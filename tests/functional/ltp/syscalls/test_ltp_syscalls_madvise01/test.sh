#!/bin/bash
# Functional test: ltp - syscalls - madvise01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise01"
        rlRun "kirk -f syscalls -p madvise01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise01"
