#!/bin/bash
# Functional test: ltp - syscalls - fchmod06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmod06"
        rlRun "kirk -f syscalls -p fchmod06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmod06"
