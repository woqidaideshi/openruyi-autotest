#!/bin/bash
# Functional test: ltp - syscalls - waitid05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid05"
        rlRun "kirk -f syscalls -p waitid05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid05"
