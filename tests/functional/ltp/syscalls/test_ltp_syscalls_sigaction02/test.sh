#!/bin/bash
# Functional test: ltp - syscalls - sigaction02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigaction02"
        rlRun "kirk -f syscalls -p sigaction02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigaction02"
