#!/bin/bash
# Functional test: ltp - syscalls - utime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime01"
        rlRun "kirk -f syscalls -p utime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime01"
