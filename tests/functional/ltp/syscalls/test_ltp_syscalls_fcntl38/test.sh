#!/bin/bash
# Functional test: ltp - syscalls - fcntl38
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl38"
        rlRun "kirk -f syscalls -p fcntl38 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl38"
