#!/bin/bash
# Functional test: ltp - syscalls - open07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open07"
        rlRun "kirk -f syscalls -p open07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open07"
