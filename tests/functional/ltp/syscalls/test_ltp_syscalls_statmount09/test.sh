#!/bin/bash
# Functional test: ltp - syscalls - statmount09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount09"
        rlRun "kirk -f syscalls -p statmount09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount09"
