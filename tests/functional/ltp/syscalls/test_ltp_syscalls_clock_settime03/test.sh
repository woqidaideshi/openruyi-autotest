#!/bin/bash
# Functional test: ltp - syscalls - clock_settime03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clock_settime03"
        rlRun "kirk -f syscalls -p clock_settime03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clock_settime03"
