#!/bin/bash
# Functional test: ltp - syscalls - landlock06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock06"
        rlRun "kirk -f syscalls -p landlock06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock06"
