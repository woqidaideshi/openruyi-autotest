#!/bin/bash
# Functional test: ltp - syscalls - connect02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - connect02"
        rlRun "kirk -f syscalls -p connect02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP connect02"
