#!/bin/bash
# Functional test: ltp - syscalls - kill13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill13"
        rlRun "kirk -f syscalls -p kill13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill13"
