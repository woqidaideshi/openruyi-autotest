#!/bin/bash
# Functional test: ltp - syscalls - bind01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind01"
        rlRun "kirk -f syscalls -p bind01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind01"
