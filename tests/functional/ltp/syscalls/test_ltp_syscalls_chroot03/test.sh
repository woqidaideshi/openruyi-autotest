#!/bin/bash
# Functional test: ltp - syscalls - chroot03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chroot03"
        rlRun "kirk -f syscalls -p chroot03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chroot03"
