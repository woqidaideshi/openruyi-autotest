#!/bin/bash
# Functional test: ltp - syscalls - recvmmsg01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - recvmmsg01"
        rlRun "kirk -f syscalls -p recvmmsg01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP recvmmsg01"
