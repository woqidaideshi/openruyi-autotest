#!/bin/bash
# Functional test: ltp - syscalls - symlink04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - symlink04"
        rlRun "kirk -f syscalls -p symlink04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP symlink04"
