#!/bin/bash
# Functional test: ltp - syscalls - fcntl12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl12"
        rlRun "kirk -f syscalls -p fcntl12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl12"
