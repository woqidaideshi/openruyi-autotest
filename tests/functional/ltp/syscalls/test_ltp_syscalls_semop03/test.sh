#!/bin/bash
# Functional test: ltp - syscalls - semop03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semop03"
        rlRun "kirk -f syscalls -p semop03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop03"
