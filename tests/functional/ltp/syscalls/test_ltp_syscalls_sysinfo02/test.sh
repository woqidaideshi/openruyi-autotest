#!/bin/bash
# Functional test: ltp - syscalls - sysinfo02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysinfo02"
        rlRun "kirk -f syscalls -p sysinfo02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysinfo02"
