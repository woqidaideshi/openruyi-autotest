#!/bin/bash
# Functional test: ltp - syscalls - fstatat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fstatat01"
        rlRun "kirk -f syscalls -p fstatat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fstatat01"
