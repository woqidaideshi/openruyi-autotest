#!/bin/bash
# Functional test: ltp - syscalls - shmctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl04"
        rlRun "kirk -f syscalls -p shmctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl04"
