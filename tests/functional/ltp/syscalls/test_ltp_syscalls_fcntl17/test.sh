#!/bin/bash
# Functional test: ltp - syscalls - fcntl17
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl17"
        rlRun "kirk -f syscalls -p fcntl17 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl17"
