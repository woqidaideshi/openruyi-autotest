#!/bin/bash
# Functional test: ltp - syscalls - getdents01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getdents01"
        rlRun "kirk -f syscalls -p getdents01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getdents01"
