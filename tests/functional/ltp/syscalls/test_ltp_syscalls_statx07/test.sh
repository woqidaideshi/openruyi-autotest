#!/bin/bash
# Functional test: ltp - syscalls - statx07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx07"
        rlRun "kirk -f syscalls -p statx07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx07"
