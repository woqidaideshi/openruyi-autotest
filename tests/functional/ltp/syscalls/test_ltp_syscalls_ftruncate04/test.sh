#!/bin/bash
# Functional test: ltp - syscalls - ftruncate04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ftruncate04"
        rlRun "kirk -f syscalls -p ftruncate04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ftruncate04"
