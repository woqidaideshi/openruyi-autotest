#!/bin/bash
# Functional test: ltp - syscalls - sigwaitinfo01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigwaitinfo01"
        rlRun "kirk -f syscalls -p sigwaitinfo01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigwaitinfo01"
