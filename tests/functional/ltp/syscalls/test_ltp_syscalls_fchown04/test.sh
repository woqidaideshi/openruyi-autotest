#!/bin/bash
# Functional test: ltp - syscalls - fchown04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchown04"
        rlRun "kirk -f syscalls -p fchown04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchown04"
