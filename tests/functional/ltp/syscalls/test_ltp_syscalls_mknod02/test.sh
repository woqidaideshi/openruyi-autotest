#!/bin/bash
# Functional test: ltp - syscalls - mknod02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod02"
        rlRun "kirk -f syscalls -p mknod02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod02"
