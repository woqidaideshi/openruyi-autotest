#!/bin/bash
# Functional test: ltp - syscalls - gettimeofday01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - gettimeofday01"
        rlRun "kirk -f syscalls -p gettimeofday01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP gettimeofday01"
