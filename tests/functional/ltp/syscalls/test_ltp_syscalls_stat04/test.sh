#!/bin/bash
# Functional test: ltp - syscalls - stat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stat04"
        rlRun "kirk -f syscalls -p stat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stat04"
