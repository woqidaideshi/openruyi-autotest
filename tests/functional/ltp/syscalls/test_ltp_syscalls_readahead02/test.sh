#!/bin/bash
# Functional test: ltp - syscalls - readahead02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readahead02"
        rlRun "kirk -f syscalls -p readahead02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readahead02"
