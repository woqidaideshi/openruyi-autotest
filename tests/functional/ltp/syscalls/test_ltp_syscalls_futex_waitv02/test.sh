#!/bin/bash
# Functional test: ltp - syscalls - futex_waitv02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - futex_waitv02"
        rlRun "kirk -f syscalls -p futex_waitv02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP futex_waitv02"
