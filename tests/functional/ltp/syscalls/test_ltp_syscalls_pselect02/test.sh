#!/bin/bash
# Functional test: ltp - syscalls - pselect02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pselect02"
        rlRun "kirk -f syscalls -p pselect02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pselect02"
