#!/bin/bash
# Functional test: ltp - syscalls - readv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readv01"
        rlRun "kirk -f syscalls -p readv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readv01"
