#!/bin/bash
# Functional test: ltp - syscalls - sendto01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendto01"
        rlRun "kirk -f syscalls -p sendto01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendto01"
