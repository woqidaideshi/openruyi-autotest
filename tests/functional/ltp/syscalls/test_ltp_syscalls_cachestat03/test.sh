#!/bin/bash
# Functional test: ltp - syscalls - cachestat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - cachestat03"
        rlRun "kirk -f syscalls -p cachestat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cachestat03"
