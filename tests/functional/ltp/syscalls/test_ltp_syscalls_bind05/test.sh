#!/bin/bash
# Functional test: ltp - syscalls - bind05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind05"
        rlRun "kirk -f syscalls -p bind05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind05"
