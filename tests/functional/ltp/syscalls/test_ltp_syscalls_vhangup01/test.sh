#!/bin/bash
# Functional test: ltp - syscalls - vhangup01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vhangup01"
        rlRun "kirk -f syscalls -p vhangup01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vhangup01"
