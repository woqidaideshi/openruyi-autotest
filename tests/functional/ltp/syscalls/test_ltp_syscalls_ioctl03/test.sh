#!/bin/bash
# Functional test: ltp - syscalls - ioctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl03"
        rlRun "kirk -f syscalls -p ioctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl03"
