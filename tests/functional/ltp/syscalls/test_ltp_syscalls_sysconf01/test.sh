#!/bin/bash
# Functional test: ltp - syscalls - sysconf01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysconf01"
        rlRun "kirk -f syscalls -p sysconf01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysconf01"
