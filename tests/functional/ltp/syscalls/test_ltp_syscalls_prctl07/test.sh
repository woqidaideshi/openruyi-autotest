#!/bin/bash
# Functional test: ltp - syscalls - prctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl07"
        rlRun "kirk -f syscalls -p prctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl07"
