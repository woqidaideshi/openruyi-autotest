#!/bin/bash
# Functional test: ltp - syscalls - memcmp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - memcmp01"
        rlRun "kirk -f syscalls -p memcmp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP memcmp01"
