#!/bin/bash
# Functional test: ltp - syscalls - clone01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone01"
        rlRun "kirk -f syscalls -p clone01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone01"
