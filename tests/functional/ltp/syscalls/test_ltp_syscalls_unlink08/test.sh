#!/bin/bash
# Functional test: ltp - syscalls - unlink08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlink08"
        rlRun "kirk -f syscalls -p unlink08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlink08"
