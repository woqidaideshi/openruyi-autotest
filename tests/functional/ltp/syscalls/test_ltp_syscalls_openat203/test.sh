#!/bin/bash
# Functional test: ltp - syscalls - openat203
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat203"
        rlRun "kirk -f syscalls -p openat203 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat203"
