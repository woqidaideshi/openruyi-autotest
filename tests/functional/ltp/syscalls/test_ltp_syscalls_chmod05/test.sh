#!/bin/bash
# Functional test: ltp - syscalls - chmod05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod05"
        rlRun "kirk -f syscalls -p chmod05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod05"
