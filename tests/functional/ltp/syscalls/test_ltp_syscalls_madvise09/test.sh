#!/bin/bash
# Functional test: ltp - syscalls - madvise09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise09"
        rlRun "kirk -f syscalls -p madvise09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise09"
