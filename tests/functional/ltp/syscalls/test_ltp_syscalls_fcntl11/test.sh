#!/bin/bash
# Functional test: ltp - syscalls - fcntl11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl11"
        rlRun "kirk -f syscalls -p fcntl11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl11"
