#!/bin/bash
# Functional test: ltp - syscalls - setpriority02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setpriority02"
        rlRun "kirk -f syscalls -p setpriority02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setpriority02"
