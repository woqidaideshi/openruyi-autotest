#!/bin/bash
# Functional test: ltp - syscalls - readv02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readv02"
        rlRun "kirk -f syscalls -p readv02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readv02"
