#!/bin/bash
# Functional test: ltp - syscalls - times01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - times01"
        rlRun "kirk -f syscalls -p times01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP times01"
