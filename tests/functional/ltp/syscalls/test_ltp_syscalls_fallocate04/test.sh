#!/bin/bash
# Functional test: ltp - syscalls - fallocate04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate04"
        rlRun "kirk -f syscalls -p fallocate04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate04"
