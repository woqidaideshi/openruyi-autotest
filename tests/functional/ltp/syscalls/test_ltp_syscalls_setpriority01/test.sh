#!/bin/bash
# Functional test: ltp - syscalls - setpriority01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setpriority01"
        rlRun "kirk -f syscalls -p setpriority01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setpriority01"
