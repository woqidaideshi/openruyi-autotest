#!/bin/bash
# Functional test: ltp - syscalls - statmount04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount04"
        rlRun "kirk -f syscalls -p statmount04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount04"
