#!/bin/bash
# Functional test: ltp - syscalls - sendfile04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile04"
        rlRun "kirk -f syscalls -p sendfile04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile04"
