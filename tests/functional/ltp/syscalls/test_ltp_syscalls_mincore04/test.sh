#!/bin/bash
# Functional test: ltp - syscalls - mincore04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mincore04"
        rlRun "kirk -f syscalls -p mincore04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mincore04"
