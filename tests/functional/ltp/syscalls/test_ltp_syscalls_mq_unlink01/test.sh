#!/bin/bash
# Functional test: ltp - syscalls - mq_unlink01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mq_unlink01"
        rlRun "kirk -f syscalls -p mq_unlink01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mq_unlink01"
