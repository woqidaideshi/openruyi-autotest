#!/bin/bash
# Functional test: ltp - syscalls - ioperm01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioperm01"
        rlRun "kirk -f syscalls -p ioperm01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioperm01"
