#!/bin/bash
# Functional test: ltp - syscalls - swapon02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - swapon02"
        rlRun "kirk -f syscalls -p swapon02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP swapon02"
