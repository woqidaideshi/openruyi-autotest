#!/bin/bash
# Functional test: ltp - syscalls - splice03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice03"
        rlRun "kirk -f syscalls -p splice03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice03"
