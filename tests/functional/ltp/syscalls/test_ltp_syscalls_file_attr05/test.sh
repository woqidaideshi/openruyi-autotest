#!/bin/bash
# Functional test: ltp - syscalls - file_attr05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - file_attr05"
        rlRun "kirk -f syscalls -p file_attr05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP file_attr05"
