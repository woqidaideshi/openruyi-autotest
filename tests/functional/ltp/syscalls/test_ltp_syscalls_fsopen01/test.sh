#!/bin/bash
# Functional test: ltp - syscalls - fsopen01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsopen01"
        rlRun "kirk -f syscalls -p fsopen01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsopen01"
