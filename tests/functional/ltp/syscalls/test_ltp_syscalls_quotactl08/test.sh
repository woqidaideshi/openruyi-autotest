#!/bin/bash
# Functional test: ltp - syscalls - quotactl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl08"
        rlRun "kirk -f syscalls -p quotactl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl08"
