#!/bin/bash
# Functional test: ltp - syscalls - time01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - time01"
        rlRun "kirk -f syscalls -p time01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP time01"
