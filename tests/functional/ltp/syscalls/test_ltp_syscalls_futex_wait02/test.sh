#!/bin/bash
# Functional test: ltp - syscalls - futex_wait02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - futex_wait02"
        rlRun "kirk -f syscalls -p futex_wait02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP futex_wait02"
