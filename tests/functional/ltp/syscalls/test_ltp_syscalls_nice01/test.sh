#!/bin/bash
# Functional test: ltp - syscalls - nice01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nice01"
        rlRun "kirk -f syscalls -p nice01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nice01"
