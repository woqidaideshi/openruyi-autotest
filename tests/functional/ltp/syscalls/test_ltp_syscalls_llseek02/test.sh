#!/bin/bash
# Functional test: ltp - syscalls - llseek02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - llseek02"
        rlRun "kirk -f syscalls -p llseek02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP llseek02"
