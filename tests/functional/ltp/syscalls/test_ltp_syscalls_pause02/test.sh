#!/bin/bash
# Functional test: ltp - syscalls - pause02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pause02"
        rlRun "kirk -f syscalls -p pause02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pause02"
