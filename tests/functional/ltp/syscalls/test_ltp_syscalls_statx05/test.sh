#!/bin/bash
# Functional test: ltp - syscalls - statx05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx05"
        rlRun "kirk -f syscalls -p statx05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx05"
