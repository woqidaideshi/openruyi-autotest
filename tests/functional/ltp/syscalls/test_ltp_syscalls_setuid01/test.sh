#!/bin/bash
# Functional test: ltp - syscalls - setuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setuid01"
        rlRun "kirk -f syscalls -p setuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setuid01"
