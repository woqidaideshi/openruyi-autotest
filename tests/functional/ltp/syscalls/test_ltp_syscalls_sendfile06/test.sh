#!/bin/bash
# Functional test: ltp - syscalls - sendfile06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile06"
        rlRun "kirk -f syscalls -p sendfile06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile06"
