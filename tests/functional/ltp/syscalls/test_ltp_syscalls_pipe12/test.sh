#!/bin/bash
# Functional test: ltp - syscalls - pipe12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe12"
        rlRun "kirk -f syscalls -p pipe12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe12"
