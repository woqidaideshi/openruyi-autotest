#!/bin/bash
# Functional test: ltp - syscalls - execve01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve01"
        rlRun "kirk -f syscalls -p execve01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve01"
