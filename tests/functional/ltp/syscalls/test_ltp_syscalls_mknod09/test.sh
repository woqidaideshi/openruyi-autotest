#!/bin/bash
# Functional test: ltp - syscalls - mknod09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod09"
        rlRun "kirk -f syscalls -p mknod09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod09"
