#!/bin/bash
# Functional test: ltp - syscalls - string01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - string01"
        rlRun "kirk -f syscalls -p string01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP string01"
