#!/bin/bash
# Functional test: ltp - syscalls - fdatasync02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fdatasync02"
        rlRun "kirk -f syscalls -p fdatasync02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fdatasync02"
