#!/bin/bash
# Functional test: ltp - syscalls - mmap15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap15"
        rlRun "kirk -f syscalls -p mmap15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap15"
