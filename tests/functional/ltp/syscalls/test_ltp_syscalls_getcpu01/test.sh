#!/bin/bash
# Functional test: ltp - syscalls - getcpu01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcpu01"
        rlRun "kirk -f syscalls -p getcpu01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcpu01"
