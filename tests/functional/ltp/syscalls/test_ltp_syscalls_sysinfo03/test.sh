#!/bin/bash
# Functional test: ltp - syscalls - sysinfo03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysinfo03"
        rlRun "kirk -f syscalls -p sysinfo03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysinfo03"
