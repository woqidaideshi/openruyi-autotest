#!/bin/bash
# Functional test: ltp - syscalls - close01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - close01"
        rlRun "kirk -f syscalls -p close01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP close01"
