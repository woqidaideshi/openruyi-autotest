#!/bin/bash
# Functional test: ltp - syscalls - ioctl_fiemap01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_fiemap01"
        rlRun "kirk -f syscalls -p ioctl_fiemap01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_fiemap01"
