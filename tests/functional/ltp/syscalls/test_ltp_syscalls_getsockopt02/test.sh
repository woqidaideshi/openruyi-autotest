#!/bin/bash
# Functional test: ltp - syscalls - getsockopt02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getsockopt02"
        rlRun "kirk -f syscalls -p getsockopt02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getsockopt02"
