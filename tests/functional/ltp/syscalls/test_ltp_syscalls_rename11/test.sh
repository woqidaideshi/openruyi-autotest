#!/bin/bash
# Functional test: ltp - syscalls - rename11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename11"
        rlRun "kirk -f syscalls -p rename11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename11"
