#!/bin/bash
# Functional test: ltp - syscalls - pselect01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pselect01"
        rlRun "kirk -f syscalls -p pselect01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pselect01"
