#!/bin/bash
# Functional test: ltp - syscalls - rename07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename07"
        rlRun "kirk -f syscalls -p rename07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename07"
