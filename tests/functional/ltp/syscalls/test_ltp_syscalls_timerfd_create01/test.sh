#!/bin/bash
# Functional test: ltp - syscalls - timerfd_create01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timerfd_create01"
        rlRun "kirk -f syscalls -p timerfd_create01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timerfd_create01"
