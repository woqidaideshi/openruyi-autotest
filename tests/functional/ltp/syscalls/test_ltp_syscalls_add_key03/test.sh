#!/bin/bash
# Functional test: ltp - syscalls - add_key03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - add_key03"
        rlRun "kirk -f syscalls -p add_key03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP add_key03"
