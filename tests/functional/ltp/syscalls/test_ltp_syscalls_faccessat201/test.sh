#!/bin/bash
# Functional test: ltp - syscalls - faccessat201
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - faccessat201"
        rlRun "kirk -f syscalls -p faccessat201 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP faccessat201"
