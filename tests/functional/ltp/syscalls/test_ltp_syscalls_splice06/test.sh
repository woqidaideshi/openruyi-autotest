#!/bin/bash
# Functional test: ltp - syscalls - splice06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice06"
        rlRun "kirk -f syscalls -p splice06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice06"
