#!/bin/bash
# Functional test: ltp - syscalls - symlinkat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - symlinkat01"
        rlRun "kirk -f syscalls -p symlinkat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP symlinkat01"
