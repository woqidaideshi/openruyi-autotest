#!/bin/bash
# Functional test: ltp - syscalls - msgrcv03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv03"
        rlRun "kirk -f syscalls -p msgrcv03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv03"
