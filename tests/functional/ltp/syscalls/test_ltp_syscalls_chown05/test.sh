#!/bin/bash
# Functional test: ltp - syscalls - chown05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chown05"
        rlRun "kirk -f syscalls -p chown05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chown05"
