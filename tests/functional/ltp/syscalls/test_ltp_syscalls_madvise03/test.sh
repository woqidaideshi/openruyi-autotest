#!/bin/bash
# Functional test: ltp - syscalls - madvise03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise03"
        rlRun "kirk -f syscalls -p madvise03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise03"
