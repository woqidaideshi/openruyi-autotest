#!/bin/bash
# Functional test: ltp - syscalls - dup02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup02"
        rlRun "kirk -f syscalls -p dup02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup02"
