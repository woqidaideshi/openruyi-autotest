#!/bin/bash
# Functional test: ltp - syscalls - faccessat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - faccessat02"
        rlRun "kirk -f syscalls -p faccessat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP faccessat02"
