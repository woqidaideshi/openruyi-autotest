#!/bin/bash
# Functional test: ltp - syscalls - inotify12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify12"
        rlRun "kirk -f syscalls -p inotify12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify12"
