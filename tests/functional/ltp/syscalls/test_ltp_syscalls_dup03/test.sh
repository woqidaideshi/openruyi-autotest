#!/bin/bash
# Functional test: ltp - syscalls - dup03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup03"
        rlRun "kirk -f syscalls -p dup03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup03"
