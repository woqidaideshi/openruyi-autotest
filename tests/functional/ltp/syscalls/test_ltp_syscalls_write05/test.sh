#!/bin/bash
# Functional test: ltp - syscalls - write05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - write05"
        rlRun "kirk -f syscalls -p write05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP write05"
