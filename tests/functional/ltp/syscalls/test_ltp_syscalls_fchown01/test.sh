#!/bin/bash
# Functional test: ltp - syscalls - fchown01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchown01"
        rlRun "kirk -f syscalls -p fchown01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchown01"
