#!/bin/bash
# Functional test: ltp - syscalls - open15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open15"
        rlRun "kirk -f syscalls -p open15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open15"
