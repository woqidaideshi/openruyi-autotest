#!/bin/bash
# Functional test: ltp - syscalls - clone06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone06"
        rlRun "kirk -f syscalls -p clone06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone06"
