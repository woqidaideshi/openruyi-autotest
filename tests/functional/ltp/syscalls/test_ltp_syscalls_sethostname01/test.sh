#!/bin/bash
# Functional test: ltp - syscalls - sethostname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sethostname01"
        rlRun "kirk -f syscalls -p sethostname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sethostname01"
