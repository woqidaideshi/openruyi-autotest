#!/bin/bash
# Functional test: ltp - syscalls - setuid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setuid03"
        rlRun "kirk -f syscalls -p setuid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setuid03"
