#!/bin/bash
# Functional test: ltp - syscalls - fork09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork09"
        rlRun "kirk -f syscalls -p fork09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork09"
