#!/bin/bash
# Functional test: ltp - syscalls - unlink10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlink10"
        rlRun "kirk -f syscalls -p unlink10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlink10"
