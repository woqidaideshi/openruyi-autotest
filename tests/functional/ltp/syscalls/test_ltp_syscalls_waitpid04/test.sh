#!/bin/bash
# Functional test: ltp - syscalls - waitpid04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid04"
        rlRun "kirk -f syscalls -p waitpid04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid04"
