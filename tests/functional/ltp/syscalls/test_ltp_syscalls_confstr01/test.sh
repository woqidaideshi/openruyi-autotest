#!/bin/bash
# Functional test: ltp - syscalls - confstr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - confstr01"
        rlRun "kirk -f syscalls -p confstr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP confstr01"
