#!/bin/bash
# Functional test: ltp - syscalls - statx04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx04"
        rlRun "kirk -f syscalls -p statx04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx04"
