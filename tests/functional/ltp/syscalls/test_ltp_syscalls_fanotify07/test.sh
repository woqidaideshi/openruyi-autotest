#!/bin/bash
# Functional test: ltp - syscalls - fanotify07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify07"
        rlRun "kirk -f syscalls -p fanotify07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify07"
