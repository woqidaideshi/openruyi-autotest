#!/bin/bash
# Functional test: ltp - syscalls - select03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - select03"
        rlRun "kirk -f syscalls -p select03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP select03"
