#!/bin/bash
# Functional test: ltp - syscalls - pipe11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe11"
        rlRun "kirk -f syscalls -p pipe11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe11"
