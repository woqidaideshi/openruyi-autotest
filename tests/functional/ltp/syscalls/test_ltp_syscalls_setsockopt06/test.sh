#!/bin/bash
# Functional test: ltp - syscalls - setsockopt06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt06"
        rlRun "kirk -f syscalls -p setsockopt06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt06"
