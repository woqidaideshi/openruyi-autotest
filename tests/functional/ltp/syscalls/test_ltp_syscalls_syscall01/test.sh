#!/bin/bash
# Functional test: ltp - syscalls - syscall01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - syscall01"
        rlRun "kirk -f syscalls -p syscall01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP syscall01"
