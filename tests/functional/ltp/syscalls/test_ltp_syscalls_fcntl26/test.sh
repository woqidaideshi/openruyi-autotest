#!/bin/bash
# Functional test: ltp - syscalls - fcntl26
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl26"
        rlRun "kirk -f syscalls -p fcntl26 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl26"
