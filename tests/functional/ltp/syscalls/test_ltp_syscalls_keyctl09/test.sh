#!/bin/bash
# Functional test: ltp - syscalls - keyctl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl09"
        rlRun "kirk -f syscalls -p keyctl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl09"
