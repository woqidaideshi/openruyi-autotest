#!/bin/bash
# Functional test: ltp - syscalls - mmap18
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap18"
        rlRun "kirk -f syscalls -p mmap18 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap18"
