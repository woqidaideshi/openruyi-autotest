#!/bin/bash
# Functional test: ltp - syscalls - rename08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename08"
        rlRun "kirk -f syscalls -p rename08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename08"
