#!/bin/bash
# Functional test: ltp - syscalls - fcntl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl08"
        rlRun "kirk -f syscalls -p fcntl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl08"
