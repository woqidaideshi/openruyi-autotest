#!/bin/bash
# Functional test: ltp - syscalls - fcntl31
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl31"
        rlRun "kirk -f syscalls -p fcntl31 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl31"
