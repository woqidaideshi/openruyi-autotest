#!/bin/bash
# Functional test: ltp - syscalls - move_pages06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - move_pages06"
        rlRun "kirk -f syscalls -p move_pages06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP move_pages06"
