#!/bin/bash
# Functional test: ltp - syscalls - memcpy01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - memcpy01"
        rlRun "kirk -f syscalls -p memcpy01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP memcpy01"
