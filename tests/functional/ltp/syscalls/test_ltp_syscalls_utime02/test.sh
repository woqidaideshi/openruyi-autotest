#!/bin/bash
# Functional test: ltp - syscalls - utime02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime02"
        rlRun "kirk -f syscalls -p utime02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime02"
