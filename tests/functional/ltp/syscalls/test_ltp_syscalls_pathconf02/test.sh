#!/bin/bash
# Functional test: ltp - syscalls - pathconf02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pathconf02"
        rlRun "kirk -f syscalls -p pathconf02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pathconf02"
