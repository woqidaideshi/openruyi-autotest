#!/bin/bash
# Functional test: ltp - syscalls - waitid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid03"
        rlRun "kirk -f syscalls -p waitid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid03"
