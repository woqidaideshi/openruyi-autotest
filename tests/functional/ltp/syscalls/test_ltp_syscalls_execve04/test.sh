#!/bin/bash
# Functional test: ltp - syscalls - execve04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve04"
        rlRun "kirk -f syscalls -p execve04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve04"
