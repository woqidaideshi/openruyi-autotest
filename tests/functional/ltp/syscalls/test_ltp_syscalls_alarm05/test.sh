#!/bin/bash
# Functional test: ltp - syscalls - alarm05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - alarm05"
        rlRun "kirk -f syscalls -p alarm05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP alarm05"
