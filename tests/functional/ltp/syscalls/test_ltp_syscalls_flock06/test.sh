#!/bin/bash
# Functional test: ltp - syscalls - flock06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock06"
        rlRun "kirk -f syscalls -p flock06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock06"
