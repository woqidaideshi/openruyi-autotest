#!/bin/bash
# Functional test: ltp - syscalls - setrlimit05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit05"
        rlRun "kirk -f syscalls -p setrlimit05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit05"
