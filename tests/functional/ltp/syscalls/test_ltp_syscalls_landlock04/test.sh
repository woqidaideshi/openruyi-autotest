#!/bin/bash
# Functional test: ltp - syscalls - landlock04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock04"
        rlRun "kirk -f syscalls -p landlock04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock04"
