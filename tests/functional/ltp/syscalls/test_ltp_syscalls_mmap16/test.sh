#!/bin/bash
# Functional test: ltp - syscalls - mmap16
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap16"
        rlRun "kirk -f syscalls -p mmap16 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap16"
