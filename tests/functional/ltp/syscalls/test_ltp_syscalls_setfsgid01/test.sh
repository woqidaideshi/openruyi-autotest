#!/bin/bash
# Functional test: ltp - syscalls - setfsgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setfsgid01"
        rlRun "kirk -f syscalls -p setfsgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setfsgid01"
