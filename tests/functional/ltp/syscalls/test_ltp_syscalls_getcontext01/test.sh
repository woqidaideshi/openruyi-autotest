#!/bin/bash
# Functional test: ltp - syscalls - getcontext01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcontext01"
        rlRun "kirk -f syscalls -p getcontext01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcontext01"
