#!/bin/bash
# Functional test: ltp - syscalls - msgget03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgget03"
        rlRun "kirk -f syscalls -p msgget03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget03"
