#!/bin/bash
# Functional test: ltp - syscalls - semget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semget02"
        rlRun "kirk -f syscalls -p semget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semget02"
