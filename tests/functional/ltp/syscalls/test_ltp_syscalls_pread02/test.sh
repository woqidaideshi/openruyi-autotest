#!/bin/bash
# Functional test: ltp - syscalls - pread02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pread02"
        rlRun "kirk -f syscalls -p pread02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pread02"
