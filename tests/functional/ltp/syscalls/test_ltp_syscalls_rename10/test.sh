#!/bin/bash
# Functional test: ltp - syscalls - rename10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename10"
        rlRun "kirk -f syscalls -p rename10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename10"
