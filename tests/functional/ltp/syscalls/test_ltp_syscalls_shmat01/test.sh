#!/bin/bash
# Functional test: ltp - syscalls - shmat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmat01"
        rlRun "kirk -f syscalls -p shmat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat01"
