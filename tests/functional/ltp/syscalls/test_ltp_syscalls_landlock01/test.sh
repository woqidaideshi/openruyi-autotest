#!/bin/bash
# Functional test: ltp - syscalls - landlock01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock01"
        rlRun "kirk -f syscalls -p landlock01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock01"
