#!/bin/bash
# Functional test: ltp - syscalls - clock_gettime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clock_gettime01"
        rlRun "kirk -f syscalls -p clock_gettime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clock_gettime01"
