#!/bin/bash
# Functional test: ltp - syscalls - eventfd04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd04"
        rlRun "kirk -f syscalls -p eventfd04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd04"
