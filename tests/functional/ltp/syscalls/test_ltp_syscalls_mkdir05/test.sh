#!/bin/bash
# Functional test: ltp - syscalls - mkdir05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mkdir05"
        rlRun "kirk -f syscalls -p mkdir05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdir05"
