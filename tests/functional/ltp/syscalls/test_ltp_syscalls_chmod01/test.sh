#!/bin/bash
# Functional test: ltp - syscalls - chmod01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod01"
        rlRun "kirk -f syscalls -p chmod01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod01"
