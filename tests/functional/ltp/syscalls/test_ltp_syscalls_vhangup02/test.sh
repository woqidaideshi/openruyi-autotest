#!/bin/bash
# Functional test: ltp - syscalls - vhangup02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vhangup02"
        rlRun "kirk -f syscalls -p vhangup02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vhangup02"
