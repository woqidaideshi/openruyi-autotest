#!/bin/bash
# Functional test: ltp - syscalls - fremovexattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fremovexattr02"
        rlRun "kirk -f syscalls -p fremovexattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fremovexattr02"
