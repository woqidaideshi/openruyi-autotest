#!/bin/bash
# Functional test: ltp - syscalls - gettimeofday02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - gettimeofday02"
        rlRun "kirk -f syscalls -p gettimeofday02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP gettimeofday02"
