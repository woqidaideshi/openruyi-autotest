#!/bin/bash
# Functional test: ltp - syscalls - waitpid11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid11"
        rlRun "kirk -f syscalls -p waitpid11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid11"
