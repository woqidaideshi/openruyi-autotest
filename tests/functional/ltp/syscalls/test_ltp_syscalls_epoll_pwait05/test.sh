#!/bin/bash
# Functional test: ltp - syscalls - epoll_pwait05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll_pwait05"
        rlRun "kirk -f syscalls -p epoll_pwait05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll_pwait05"
