#!/bin/bash
# Functional test: ltp - syscalls - mount04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount04"
        rlRun "kirk -f syscalls -p mount04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount04"
