#!/bin/bash
# Functional test: ltp - syscalls - migrate_pages01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - migrate_pages01"
        rlRun "kirk -f syscalls -p migrate_pages01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP migrate_pages01"
