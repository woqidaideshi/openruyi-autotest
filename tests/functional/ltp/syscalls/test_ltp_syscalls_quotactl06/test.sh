#!/bin/bash
# Functional test: ltp - syscalls - quotactl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl06"
        rlRun "kirk -f syscalls -p quotactl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl06"
