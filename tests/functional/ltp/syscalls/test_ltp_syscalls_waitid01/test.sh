#!/bin/bash
# Functional test: ltp - syscalls - waitid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid01"
        rlRun "kirk -f syscalls -p waitid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid01"
