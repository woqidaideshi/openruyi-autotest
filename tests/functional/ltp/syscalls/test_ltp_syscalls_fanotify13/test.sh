#!/bin/bash
# Functional test: ltp - syscalls - fanotify13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify13"
        rlRun "kirk -f syscalls -p fanotify13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify13"
