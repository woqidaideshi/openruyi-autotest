#!/bin/bash
# Functional test: ltp - syscalls - shmget06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmget06"
        rlRun "kirk -f syscalls -p shmget06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget06"
