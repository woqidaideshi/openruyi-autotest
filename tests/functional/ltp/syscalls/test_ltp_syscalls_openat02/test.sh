#!/bin/bash
# Functional test: ltp - syscalls - openat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat02"
        rlRun "kirk -f syscalls -p openat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat02"
