#!/bin/bash
# Functional test: ltp - syscalls - sendto02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendto02"
        rlRun "kirk -f syscalls -p sendto02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendto02"
