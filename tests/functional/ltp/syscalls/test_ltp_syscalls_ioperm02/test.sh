#!/bin/bash
# Functional test: ltp - syscalls - ioperm02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioperm02"
        rlRun "kirk -f syscalls -p ioperm02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioperm02"
