#!/bin/bash
# Functional test: ltp - syscalls - execlp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execlp01"
        rlRun "kirk -f syscalls -p execlp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execlp01"
