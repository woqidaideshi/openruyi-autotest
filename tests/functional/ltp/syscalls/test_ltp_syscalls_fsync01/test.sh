#!/bin/bash
# Functional test: ltp - syscalls - fsync01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsync01"
        rlRun "kirk -f syscalls -p fsync01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsync01"
