#!/bin/bash
# Functional test: ltp - syscalls - fanotify03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify03"
        rlRun "kirk -f syscalls -p fanotify03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify03"
