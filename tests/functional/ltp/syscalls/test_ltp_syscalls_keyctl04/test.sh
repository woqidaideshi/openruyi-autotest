#!/bin/bash
# Functional test: ltp - syscalls - keyctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl04"
        rlRun "kirk -f syscalls -p keyctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl04"
