#!/bin/bash
# Functional test: ltp - syscalls - fsmount02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsmount02"
        rlRun "kirk -f syscalls -p fsmount02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsmount02"
