#!/bin/bash
# Functional test: ltp - syscalls - io_submit04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_submit04"
        rlRun "kirk -f syscalls -p io_submit04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_submit04"
