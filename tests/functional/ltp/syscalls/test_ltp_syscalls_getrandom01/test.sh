#!/bin/bash
# Functional test: ltp - syscalls - getrandom01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrandom01"
        rlRun "kirk -f syscalls -p getrandom01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrandom01"
