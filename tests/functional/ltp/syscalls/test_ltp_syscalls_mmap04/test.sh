#!/bin/bash
# Functional test: ltp - syscalls - mmap04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap04"
        rlRun "kirk -f syscalls -p mmap04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap04"
