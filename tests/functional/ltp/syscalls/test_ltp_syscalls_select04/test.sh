#!/bin/bash
# Functional test: ltp - syscalls - select04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - select04"
        rlRun "kirk -f syscalls -p select04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP select04"
