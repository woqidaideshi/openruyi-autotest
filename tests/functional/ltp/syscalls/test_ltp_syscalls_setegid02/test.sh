#!/bin/bash
# Functional test: ltp - syscalls - setegid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setegid02"
        rlRun "kirk -f syscalls -p setegid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setegid02"
