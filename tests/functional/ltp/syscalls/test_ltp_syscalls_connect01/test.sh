#!/bin/bash
# Functional test: ltp - syscalls - connect01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - connect01"
        rlRun "kirk -f syscalls -p connect01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP connect01"
