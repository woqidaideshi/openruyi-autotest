#!/bin/bash
# Functional test: ltp - syscalls - prctl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl08"
        rlRun "kirk -f syscalls -p prctl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl08"
