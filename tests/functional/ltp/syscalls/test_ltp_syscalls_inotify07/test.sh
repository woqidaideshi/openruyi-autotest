#!/bin/bash
# Functional test: ltp - syscalls - inotify07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify07"
        rlRun "kirk -f syscalls -p inotify07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify07"
