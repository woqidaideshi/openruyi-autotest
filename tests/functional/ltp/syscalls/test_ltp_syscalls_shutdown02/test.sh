#!/bin/bash
# Functional test: ltp - syscalls - shutdown02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shutdown02"
        rlRun "kirk -f syscalls -p shutdown02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shutdown02"
