#!/bin/bash
# Functional test: ltp - syscalls - fanotify10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify10"
        rlRun "kirk -f syscalls -p fanotify10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify10"
