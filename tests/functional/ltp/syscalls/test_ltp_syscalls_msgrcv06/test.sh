#!/bin/bash
# Functional test: ltp - syscalls - msgrcv06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv06"
        rlRun "kirk -f syscalls -p msgrcv06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv06"
