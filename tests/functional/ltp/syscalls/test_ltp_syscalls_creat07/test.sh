#!/bin/bash
# Functional test: ltp - syscalls - creat07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat07"
        rlRun "kirk -f syscalls -p creat07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat07"
