#!/bin/bash
# Functional test: ltp - syscalls - waitid11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid11"
        rlRun "kirk -f syscalls -p waitid11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid11"
