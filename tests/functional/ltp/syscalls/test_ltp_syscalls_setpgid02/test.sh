#!/bin/bash
# Functional test: ltp - syscalls - setpgid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setpgid02"
        rlRun "kirk -f syscalls -p setpgid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setpgid02"
