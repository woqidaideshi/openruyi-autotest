#!/bin/bash
# Functional test: ltp - syscalls - inotify09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify09"
        rlRun "kirk -f syscalls -p inotify09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify09"
