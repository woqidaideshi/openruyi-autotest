#!/bin/bash
# Functional test: ltp - syscalls - madvise11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise11"
        rlRun "kirk -f syscalls -p madvise11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise11"
