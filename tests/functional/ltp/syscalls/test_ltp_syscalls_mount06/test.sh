#!/bin/bash
# Functional test: ltp - syscalls - mount06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount06"
        rlRun "kirk -f syscalls -p mount06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount06"
