#!/bin/bash
# Functional test: ltp - syscalls - fsetxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsetxattr02"
        rlRun "kirk -f syscalls -p fsetxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsetxattr02"
