#!/bin/bash
# Functional test: ltp - syscalls - fanotify24
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify24"
        rlRun "kirk -f syscalls -p fanotify24 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify24"
