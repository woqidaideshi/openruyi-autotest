#!/bin/bash
# Functional test: ltp - syscalls - delete_module01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - delete_module01"
        rlRun "kirk -f syscalls -p delete_module01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP delete_module01"
