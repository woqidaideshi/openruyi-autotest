#!/bin/bash
# Functional test: ltp - syscalls - shutdown01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shutdown01"
        rlRun "kirk -f syscalls -p shutdown01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shutdown01"
