#!/bin/bash
# Functional test: ltp - syscalls - shmdt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmdt01"
        rlRun "kirk -f syscalls -p shmdt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmdt01"
