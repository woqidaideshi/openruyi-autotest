#!/bin/bash
# Functional test: ltp - syscalls - munlock02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munlock02"
        rlRun "kirk -f syscalls -p munlock02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munlock02"
