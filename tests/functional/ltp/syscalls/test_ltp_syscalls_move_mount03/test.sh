#!/bin/bash
# Functional test: ltp - syscalls - move_mount03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - move_mount03"
        rlRun "kirk -f syscalls -p move_mount03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP move_mount03"
