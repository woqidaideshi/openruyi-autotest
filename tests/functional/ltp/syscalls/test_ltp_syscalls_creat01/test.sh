#!/bin/bash
# Functional test: ltp - syscalls - creat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat01"
        rlRun "kirk -f syscalls -p creat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat01"
