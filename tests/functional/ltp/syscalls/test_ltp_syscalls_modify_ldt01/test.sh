#!/bin/bash
# Functional test: ltp - syscalls - modify_ldt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - modify_ldt01"
        rlRun "kirk -f syscalls -p modify_ldt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP modify_ldt01"
