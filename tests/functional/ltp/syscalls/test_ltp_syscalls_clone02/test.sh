#!/bin/bash
# Functional test: ltp - syscalls - clone02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone02"
        rlRun "kirk -f syscalls -p clone02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone02"
