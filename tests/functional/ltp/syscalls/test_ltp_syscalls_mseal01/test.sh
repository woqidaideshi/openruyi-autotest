#!/bin/bash
# Functional test: ltp - syscalls - mseal01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mseal01"
        rlRun "kirk -f syscalls -p mseal01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mseal01"
