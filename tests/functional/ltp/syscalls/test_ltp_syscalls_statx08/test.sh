#!/bin/bash
# Functional test: ltp - syscalls - statx08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx08"
        rlRun "kirk -f syscalls -p statx08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx08"
