#!/bin/bash
# Functional test: ltp - syscalls - prctl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl09"
        rlRun "kirk -f syscalls -p prctl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl09"
