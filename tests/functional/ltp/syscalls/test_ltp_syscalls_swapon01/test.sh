#!/bin/bash
# Functional test: ltp - syscalls - swapon01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - swapon01"
        rlRun "kirk -f syscalls -p swapon01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP swapon01"
