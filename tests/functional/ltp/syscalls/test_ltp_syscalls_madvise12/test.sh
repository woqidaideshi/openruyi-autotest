#!/bin/bash
# Functional test: ltp - syscalls - madvise12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise12"
        rlRun "kirk -f syscalls -p madvise12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise12"
