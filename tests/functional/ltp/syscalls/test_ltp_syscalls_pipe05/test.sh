#!/bin/bash
# Functional test: ltp - syscalls - pipe05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe05"
        rlRun "kirk -f syscalls -p pipe05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe05"
