#!/bin/bash
# Functional test: ltp - syscalls - landlock07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock07"
        rlRun "kirk -f syscalls -p landlock07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock07"
