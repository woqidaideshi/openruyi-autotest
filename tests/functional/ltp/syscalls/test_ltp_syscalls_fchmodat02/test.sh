#!/bin/bash
# Functional test: ltp - syscalls - fchmodat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmodat02"
        rlRun "kirk -f syscalls -p fchmodat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmodat02"
