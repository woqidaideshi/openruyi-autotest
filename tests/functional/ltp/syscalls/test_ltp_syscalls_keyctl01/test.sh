#!/bin/bash
# Functional test: ltp - syscalls - keyctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl01"
        rlRun "kirk -f syscalls -p keyctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl01"
