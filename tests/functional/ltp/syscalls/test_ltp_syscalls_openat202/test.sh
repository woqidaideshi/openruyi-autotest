#!/bin/bash
# Functional test: ltp - syscalls - openat202
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat202"
        rlRun "kirk -f syscalls -p openat202 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat202"
