#!/bin/bash
# Functional test: ltp - syscalls - fcntl36
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl36"
        rlRun "kirk -f syscalls -p fcntl36 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl36"
