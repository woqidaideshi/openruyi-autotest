#!/bin/bash
# Functional test: ltp - syscalls - flistxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flistxattr02"
        rlRun "kirk -f syscalls -p flistxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flistxattr02"
