#!/bin/bash
# Functional test: ltp - syscalls - open12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open12"
        rlRun "kirk -f syscalls -p open12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open12"
