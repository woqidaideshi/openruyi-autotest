#!/bin/bash
# Functional test: ltp - syscalls - fdatasync03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fdatasync03"
        rlRun "kirk -f syscalls -p fdatasync03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fdatasync03"
