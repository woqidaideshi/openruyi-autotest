#!/bin/bash
# Functional test: ltp - syscalls - recvmsg02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - recvmsg02"
        rlRun "kirk -f syscalls -p recvmsg02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP recvmsg02"
