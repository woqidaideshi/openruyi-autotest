#!/bin/bash
# Functional test: ltp - syscalls - vmsplice03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vmsplice03"
        rlRun "kirk -f syscalls -p vmsplice03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vmsplice03"
