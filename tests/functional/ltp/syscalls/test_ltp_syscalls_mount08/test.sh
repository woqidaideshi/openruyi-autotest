#!/bin/bash
# Functional test: ltp - syscalls - mount08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount08"
        rlRun "kirk -f syscalls -p mount08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount08"
