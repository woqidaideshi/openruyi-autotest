#!/bin/bash
# Functional test: ltp - syscalls - flock02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock02"
        rlRun "kirk -f syscalls -p flock02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock02"
