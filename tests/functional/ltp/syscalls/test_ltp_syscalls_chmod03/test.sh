#!/bin/bash
# Functional test: ltp - syscalls - chmod03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod03"
        rlRun "kirk -f syscalls -p chmod03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod03"
