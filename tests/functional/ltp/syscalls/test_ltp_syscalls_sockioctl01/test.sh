#!/bin/bash
# Functional test: ltp - syscalls - sockioctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sockioctl01"
        rlRun "kirk -f syscalls -p sockioctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sockioctl01"
