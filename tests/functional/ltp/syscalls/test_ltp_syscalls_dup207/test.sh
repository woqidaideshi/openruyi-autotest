#!/bin/bash
# Functional test: ltp - syscalls - dup207
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup207"
        rlRun "kirk -f syscalls -p dup207 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup207"
