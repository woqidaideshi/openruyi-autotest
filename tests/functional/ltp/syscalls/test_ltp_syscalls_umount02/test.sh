#!/bin/bash
# Functional test: ltp - syscalls - umount02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umount02"
        rlRun "kirk -f syscalls -p umount02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umount02"
