#!/bin/bash
# Functional test: ltp - syscalls - preadv203
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - preadv203"
        rlRun "kirk -f syscalls -p preadv203 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP preadv203"
