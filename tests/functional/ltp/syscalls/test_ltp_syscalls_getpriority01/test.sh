#!/bin/bash
# Functional test: ltp - syscalls - getpriority01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpriority01"
        rlRun "kirk -f syscalls -p getpriority01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpriority01"
