#!/bin/bash
# Functional test: ltp - syscalls - geteuid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - geteuid02"
        rlRun "kirk -f syscalls -p geteuid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP geteuid02"
