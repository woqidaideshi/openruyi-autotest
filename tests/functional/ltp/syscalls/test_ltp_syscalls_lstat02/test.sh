#!/bin/bash
# Functional test: ltp - syscalls - lstat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lstat02"
        rlRun "kirk -f syscalls -p lstat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lstat02"
