#!/bin/bash
# Functional test: ltp - syscalls - prot_hsymlinks
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prot_hsymlinks"
        rlRun "kirk -f syscalls -p prot_hsymlinks 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prot_hsymlinks"
