#!/bin/bash
# Functional test: ltp - syscalls - fcntl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl02"
        rlRun "kirk -f syscalls -p fcntl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl02"
