#!/bin/bash
# Functional test: ltp - syscalls - shmdt02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmdt02"
        rlRun "kirk -f syscalls -p shmdt02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmdt02"
