#!/bin/bash
# Functional test: ltp - syscalls - getpid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpid01"
        rlRun "kirk -f syscalls -p getpid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpid01"
