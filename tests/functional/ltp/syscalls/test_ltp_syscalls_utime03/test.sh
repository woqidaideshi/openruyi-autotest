#!/bin/bash
# Functional test: ltp - syscalls - utime03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime03"
        rlRun "kirk -f syscalls -p utime03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime03"
