#!/bin/bash
# Functional test: ltp - syscalls - lstat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lstat01"
        rlRun "kirk -f syscalls -p lstat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lstat01"
