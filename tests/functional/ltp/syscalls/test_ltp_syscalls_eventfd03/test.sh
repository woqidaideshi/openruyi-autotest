#!/bin/bash
# Functional test: ltp - syscalls - eventfd03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd03"
        rlRun "kirk -f syscalls -p eventfd03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd03"
