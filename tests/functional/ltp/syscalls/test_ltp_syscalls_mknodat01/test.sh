#!/bin/bash
# Functional test: ltp - syscalls - mknodat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknodat01"
        rlRun "kirk -f syscalls -p mknodat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknodat01"
