#!/bin/bash
# Functional test: ltp - syscalls - rename01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename01"
        rlRun "kirk -f syscalls -p rename01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename01"
