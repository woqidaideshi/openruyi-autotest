#!/bin/bash
# Functional test: ltp - syscalls - getsid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getsid02"
        rlRun "kirk -f syscalls -p getsid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getsid02"
