#!/bin/bash
# Functional test: ltp - syscalls - sysctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysctl03"
        rlRun "kirk -f syscalls -p sysctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysctl03"
