#!/bin/bash
# Functional test: ltp - syscalls - statx11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx11"
        rlRun "kirk -f syscalls -p statx11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx11"
