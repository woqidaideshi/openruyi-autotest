#!/bin/bash
# Functional test: ltp - syscalls - utime07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime07"
        rlRun "kirk -f syscalls -p utime07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime07"
