#!/bin/bash
# Functional test: ltp - syscalls - unshare03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unshare03"
        rlRun "kirk -f syscalls -p unshare03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unshare03"
