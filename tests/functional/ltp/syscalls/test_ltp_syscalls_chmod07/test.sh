#!/bin/bash
# Functional test: ltp - syscalls - chmod07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod07"
        rlRun "kirk -f syscalls -p chmod07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod07"
