#!/bin/bash
# Functional test: ltp - syscalls - mmap12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap12"
        rlRun "kirk -f syscalls -p mmap12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap12"
