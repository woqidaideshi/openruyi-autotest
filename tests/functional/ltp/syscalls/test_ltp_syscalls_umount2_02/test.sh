#!/bin/bash
# Functional test: ltp - syscalls - umount2_02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umount2_02"
        rlRun "kirk -f syscalls -p umount2_02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umount2_02"
