#!/bin/bash
# Functional test: ltp - syscalls - dup206
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup206"
        rlRun "kirk -f syscalls -p dup206 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup206"
