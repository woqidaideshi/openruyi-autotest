#!/bin/bash
# Functional test: ltp - syscalls - clock_settime04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clock_settime04"
        rlRun "kirk -f syscalls -p clock_settime04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clock_settime04"
