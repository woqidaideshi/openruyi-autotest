#!/bin/bash
# Functional test: ltp - syscalls - sched_setattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_setattr01"
        rlRun "kirk -f syscalls -p sched_setattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_setattr01"
