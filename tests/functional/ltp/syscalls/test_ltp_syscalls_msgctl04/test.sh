#!/bin/bash
# Functional test: ltp - syscalls - msgctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgctl04"
        rlRun "kirk -f syscalls -p msgctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl04"
