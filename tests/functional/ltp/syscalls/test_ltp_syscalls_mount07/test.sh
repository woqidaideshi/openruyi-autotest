#!/bin/bash
# Functional test: ltp - syscalls - mount07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount07"
        rlRun "kirk -f syscalls -p mount07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount07"
