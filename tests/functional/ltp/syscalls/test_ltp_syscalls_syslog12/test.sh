#!/bin/bash
# Functional test: ltp - syscalls - syslog12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - syslog12"
        rlRun "kirk -f syscalls -p syslog12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP syslog12"
