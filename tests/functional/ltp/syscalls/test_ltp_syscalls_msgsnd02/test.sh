#!/bin/bash
# Functional test: ltp - syscalls - msgsnd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgsnd02"
        rlRun "kirk -f syscalls -p msgsnd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgsnd02"
