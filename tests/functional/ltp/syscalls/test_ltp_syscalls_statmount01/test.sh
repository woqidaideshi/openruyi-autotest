#!/bin/bash
# Functional test: ltp - syscalls - statmount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount01"
        rlRun "kirk -f syscalls -p statmount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount01"
