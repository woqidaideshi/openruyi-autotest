#!/bin/bash
# Functional test: ltp - syscalls - fanotify09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify09"
        rlRun "kirk -f syscalls -p fanotify09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify09"
