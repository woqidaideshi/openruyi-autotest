#!/bin/bash
# Functional test: ltp - syscalls - getdents02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getdents02"
        rlRun "kirk -f syscalls -p getdents02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getdents02"
