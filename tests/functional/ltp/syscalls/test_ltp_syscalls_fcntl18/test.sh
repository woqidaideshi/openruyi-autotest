#!/bin/bash
# Functional test: ltp - syscalls - fcntl18
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl18"
        rlRun "kirk -f syscalls -p fcntl18 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl18"
