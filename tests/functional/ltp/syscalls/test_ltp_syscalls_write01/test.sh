#!/bin/bash
# Functional test: ltp - syscalls - write01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - write01"
        rlRun "kirk -f syscalls -p write01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP write01"
