#!/bin/bash
# Functional test: ltp - syscalls - dup04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup04"
        rlRun "kirk -f syscalls -p dup04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup04"
