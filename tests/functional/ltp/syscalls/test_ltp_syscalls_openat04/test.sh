#!/bin/bash
# Functional test: ltp - syscalls - openat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat04"
        rlRun "kirk -f syscalls -p openat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat04"
