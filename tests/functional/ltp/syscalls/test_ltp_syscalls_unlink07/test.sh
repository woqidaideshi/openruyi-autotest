#!/bin/bash
# Functional test: ltp - syscalls - unlink07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlink07"
        rlRun "kirk -f syscalls -p unlink07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlink07"
