#!/bin/bash
# Functional test: ltp - syscalls - getpgid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpgid01"
        rlRun "kirk -f syscalls -p getpgid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpgid01"
