#!/bin/bash
# Functional test: ltp - syscalls - chmod08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod08"
        rlRun "kirk -f syscalls -p chmod08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod08"
