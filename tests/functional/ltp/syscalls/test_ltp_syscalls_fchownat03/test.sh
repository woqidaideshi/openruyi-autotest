#!/bin/bash
# Functional test: ltp - syscalls - fchownat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchownat03"
        rlRun "kirk -f syscalls -p fchownat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchownat03"
