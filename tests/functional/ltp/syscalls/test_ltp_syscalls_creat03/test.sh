#!/bin/bash
# Functional test: ltp - syscalls - creat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat03"
        rlRun "kirk -f syscalls -p creat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat03"
