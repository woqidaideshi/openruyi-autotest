#!/bin/bash
# Functional test: ltp - syscalls - sendmmsg02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendmmsg02"
        rlRun "kirk -f syscalls -p sendmmsg02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendmmsg02"
