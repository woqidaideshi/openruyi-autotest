#!/bin/bash
# Functional test: ltp - syscalls - nice04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nice04"
        rlRun "kirk -f syscalls -p nice04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nice04"
