#!/bin/bash
# Functional test: ltp - syscalls - chown01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chown01"
        rlRun "kirk -f syscalls -p chown01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chown01"
