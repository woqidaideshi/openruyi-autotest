#!/bin/bash
# Functional test: ltp - syscalls - fork13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork13"
        rlRun "kirk -f syscalls -p fork13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork13"
