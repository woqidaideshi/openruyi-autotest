#!/bin/bash
# Functional test: ltp - syscalls - fcntl20
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl20"
        rlRun "kirk -f syscalls -p fcntl20 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl20"
