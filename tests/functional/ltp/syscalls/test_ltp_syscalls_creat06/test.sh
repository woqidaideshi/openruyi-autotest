#!/bin/bash
# Functional test: ltp - syscalls - creat06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat06"
        rlRun "kirk -f syscalls -p creat06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat06"
