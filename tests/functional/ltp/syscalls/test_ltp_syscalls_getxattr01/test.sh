#!/bin/bash
# Functional test: ltp - syscalls - getxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getxattr01"
        rlRun "kirk -f syscalls -p getxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getxattr01"
