#!/bin/bash
# Functional test: ltp - syscalls - fcntl10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl10"
        rlRun "kirk -f syscalls -p fcntl10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl10"
