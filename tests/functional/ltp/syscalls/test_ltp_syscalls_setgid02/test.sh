#!/bin/bash
# Functional test: ltp - syscalls - setgid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setgid02"
        rlRun "kirk -f syscalls -p setgid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setgid02"
