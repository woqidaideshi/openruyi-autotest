#!/bin/bash
# Functional test: ltp - syscalls - fcntl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl04"
        rlRun "kirk -f syscalls -p fcntl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl04"
