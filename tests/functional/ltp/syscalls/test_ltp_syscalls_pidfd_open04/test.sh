#!/bin/bash
# Functional test: ltp - syscalls - pidfd_open04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_open04"
        rlRun "kirk -f syscalls -p pidfd_open04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_open04"
