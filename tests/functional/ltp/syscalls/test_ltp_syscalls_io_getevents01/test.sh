#!/bin/bash
# Functional test: ltp - syscalls - io_getevents01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_getevents01"
        rlRun "kirk -f syscalls -p io_getevents01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_getevents01"
