#!/bin/bash
# Functional test: ltp - syscalls - bpf_prog06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bpf_prog06"
        rlRun "kirk -f syscalls -p bpf_prog06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bpf_prog06"
