#!/bin/bash
# Functional test: ltp - syscalls - clone08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone08"
        rlRun "kirk -f syscalls -p clone08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone08"
