#!/bin/bash
# Functional test: ltp - syscalls - mlock203
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock203"
        rlRun "kirk -f syscalls -p mlock203 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock203"
