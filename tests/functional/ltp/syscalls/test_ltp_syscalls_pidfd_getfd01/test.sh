#!/bin/bash
# Functional test: ltp - syscalls - pidfd_getfd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_getfd01"
        rlRun "kirk -f syscalls -p pidfd_getfd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_getfd01"
