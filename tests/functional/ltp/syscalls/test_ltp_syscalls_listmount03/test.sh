#!/bin/bash
# Functional test: ltp - syscalls - listmount03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listmount03"
        rlRun "kirk -f syscalls -p listmount03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listmount03"
