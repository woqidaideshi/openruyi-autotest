#!/bin/bash
# Functional test: ltp - syscalls - openat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - openat03"
        rlRun "kirk -f syscalls -p openat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP openat03"
