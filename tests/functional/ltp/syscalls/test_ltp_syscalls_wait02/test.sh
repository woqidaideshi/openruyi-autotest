#!/bin/bash
# Functional test: ltp - syscalls - wait02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - wait02"
        rlRun "kirk -f syscalls -p wait02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP wait02"
