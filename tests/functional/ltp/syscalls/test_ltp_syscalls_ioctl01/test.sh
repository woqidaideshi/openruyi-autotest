#!/bin/bash
# Functional test: ltp - syscalls - ioctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl01"
        rlRun "kirk -f syscalls -p ioctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl01"
