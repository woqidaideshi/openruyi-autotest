#!/bin/bash
# Functional test: ltp - syscalls - acct02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - acct02"
        rlRun "kirk -f syscalls -p acct02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP acct02"
