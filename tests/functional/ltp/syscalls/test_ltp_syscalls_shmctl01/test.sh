#!/bin/bash
# Functional test: ltp - syscalls - shmctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl01"
        rlRun "kirk -f syscalls -p shmctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl01"
