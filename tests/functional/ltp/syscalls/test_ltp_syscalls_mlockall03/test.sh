#!/bin/bash
# Functional test: ltp - syscalls - mlockall03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlockall03"
        rlRun "kirk -f syscalls -p mlockall03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlockall03"
