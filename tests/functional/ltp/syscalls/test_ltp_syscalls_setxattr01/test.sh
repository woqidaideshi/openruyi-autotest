#!/bin/bash
# Functional test: ltp - syscalls - setxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setxattr01"
        rlRun "kirk -f syscalls -p setxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setxattr01"
