#!/bin/bash
# Functional test: ltp - syscalls - capset01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - capset01"
        rlRun "kirk -f syscalls -p capset01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP capset01"
