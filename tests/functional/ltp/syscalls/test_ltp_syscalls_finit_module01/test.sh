#!/bin/bash
# Functional test: ltp - syscalls - finit_module01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - finit_module01"
        rlRun "kirk -f syscalls -p finit_module01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP finit_module01"
