#!/bin/bash
# Functional test: ltp - syscalls - pipe03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe03"
        rlRun "kirk -f syscalls -p pipe03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe03"
