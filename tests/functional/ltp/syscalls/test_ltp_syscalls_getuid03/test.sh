#!/bin/bash
# Functional test: ltp - syscalls - getuid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getuid03"
        rlRun "kirk -f syscalls -p getuid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getuid03"
