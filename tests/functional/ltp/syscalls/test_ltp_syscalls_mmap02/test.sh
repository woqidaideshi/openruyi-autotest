#!/bin/bash
# Functional test: ltp - syscalls - mmap02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap02"
        rlRun "kirk -f syscalls -p mmap02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap02"
