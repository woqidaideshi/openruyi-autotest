#!/bin/bash
# Functional test: ltp - syscalls - fanotify02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify02"
        rlRun "kirk -f syscalls -p fanotify02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify02"
