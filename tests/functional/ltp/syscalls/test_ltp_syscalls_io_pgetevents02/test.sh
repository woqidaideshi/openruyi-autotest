#!/bin/bash
# Functional test: ltp - syscalls - io_pgetevents02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_pgetevents02"
        rlRun "kirk -f syscalls -p io_pgetevents02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_pgetevents02"
