#!/bin/bash
# Functional test: ltp - syscalls - umount2_01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umount2_01"
        rlRun "kirk -f syscalls -p umount2_01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umount2_01"
