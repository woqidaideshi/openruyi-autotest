#!/bin/bash
# Functional test: ltp - syscalls - semop02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semop02"
        rlRun "kirk -f syscalls -p semop02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop02"
