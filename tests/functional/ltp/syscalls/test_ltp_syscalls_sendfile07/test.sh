#!/bin/bash
# Functional test: ltp - syscalls - sendfile07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile07"
        rlRun "kirk -f syscalls -p sendfile07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile07"
