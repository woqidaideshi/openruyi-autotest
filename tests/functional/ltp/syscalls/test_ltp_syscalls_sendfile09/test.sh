#!/bin/bash
# Functional test: ltp - syscalls - sendfile09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile09"
        rlRun "kirk -f syscalls -p sendfile09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile09"
