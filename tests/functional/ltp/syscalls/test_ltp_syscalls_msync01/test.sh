#!/bin/bash
# Functional test: ltp - syscalls - msync01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msync01"
        rlRun "kirk -f syscalls -p msync01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msync01"
