#!/bin/bash
# Functional test: ltp - syscalls - waitpid13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid13"
        rlRun "kirk -f syscalls -p waitpid13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid13"
