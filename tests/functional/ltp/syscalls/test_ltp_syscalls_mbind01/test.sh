#!/bin/bash
# Functional test: ltp - syscalls - mbind01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mbind01"
        rlRun "kirk -f syscalls -p mbind01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mbind01"
