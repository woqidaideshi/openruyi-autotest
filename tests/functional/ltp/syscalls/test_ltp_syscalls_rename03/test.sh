#!/bin/bash
# Functional test: ltp - syscalls - rename03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename03"
        rlRun "kirk -f syscalls -p rename03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename03"
