#!/bin/bash
# Functional test: ltp - syscalls - poll04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - poll04"
        rlRun "kirk -f syscalls -p poll04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP poll04"
