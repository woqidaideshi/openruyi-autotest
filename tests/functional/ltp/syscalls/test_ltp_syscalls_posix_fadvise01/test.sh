#!/bin/bash
# Functional test: ltp - syscalls - posix_fadvise01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - posix_fadvise01"
        rlRun "kirk -f syscalls -p posix_fadvise01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP posix_fadvise01"
