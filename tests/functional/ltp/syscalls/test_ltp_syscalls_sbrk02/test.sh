#!/bin/bash
# Functional test: ltp - syscalls - sbrk02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sbrk02"
        rlRun "kirk -f syscalls -p sbrk02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sbrk02"
