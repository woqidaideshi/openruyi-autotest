#!/bin/bash
# Functional test: ltp - syscalls - setreuid06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid06"
        rlRun "kirk -f syscalls -p setreuid06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid06"
