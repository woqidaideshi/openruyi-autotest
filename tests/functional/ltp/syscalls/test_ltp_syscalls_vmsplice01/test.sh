#!/bin/bash
# Functional test: ltp - syscalls - vmsplice01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vmsplice01"
        rlRun "kirk -f syscalls -p vmsplice01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vmsplice01"
