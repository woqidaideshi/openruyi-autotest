#!/bin/bash
# Functional test: ltp - syscalls - eventfd06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd06"
        rlRun "kirk -f syscalls -p eventfd06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd06"
