#!/bin/bash
# Functional test: ltp - syscalls - flistxattr03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flistxattr03"
        rlRun "kirk -f syscalls -p flistxattr03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flistxattr03"
