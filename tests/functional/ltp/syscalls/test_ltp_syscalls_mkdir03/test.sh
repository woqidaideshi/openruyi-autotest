#!/bin/bash
# Functional test: ltp - syscalls - mkdir03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mkdir03"
        rlRun "kirk -f syscalls -p mkdir03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdir03"
