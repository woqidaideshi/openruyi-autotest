#!/bin/bash
# Functional test: ltp - syscalls - splice01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice01"
        rlRun "kirk -f syscalls -p splice01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice01"
