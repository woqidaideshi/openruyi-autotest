#!/bin/bash
# Functional test: ltp - syscalls - splice05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - splice05"
        rlRun "kirk -f syscalls -p splice05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP splice05"
