#!/bin/bash
# Functional test: ltp - syscalls - fcntl33
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl33"
        rlRun "kirk -f syscalls -p fcntl33 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl33"
