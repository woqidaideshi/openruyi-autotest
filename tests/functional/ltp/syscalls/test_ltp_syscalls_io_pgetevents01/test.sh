#!/bin/bash
# Functional test: ltp - syscalls - io_pgetevents01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_pgetevents01"
        rlRun "kirk -f syscalls -p io_pgetevents01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_pgetevents01"
