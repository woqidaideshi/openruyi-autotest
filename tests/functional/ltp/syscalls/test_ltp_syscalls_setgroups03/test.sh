#!/bin/bash
# Functional test: ltp - syscalls - setgroups03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setgroups03"
        rlRun "kirk -f syscalls -p setgroups03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setgroups03"
