#!/bin/bash
# Functional test: ltp - syscalls - open06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open06"
        rlRun "kirk -f syscalls -p open06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open06"
