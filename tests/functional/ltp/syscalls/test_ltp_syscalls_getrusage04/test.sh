#!/bin/bash
# Functional test: ltp - syscalls - getrusage04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrusage04"
        rlRun "kirk -f syscalls -p getrusage04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrusage04"
