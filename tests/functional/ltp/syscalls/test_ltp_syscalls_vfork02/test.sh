#!/bin/bash
# Functional test: ltp - syscalls - vfork02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vfork02"
        rlRun "kirk -f syscalls -p vfork02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vfork02"
