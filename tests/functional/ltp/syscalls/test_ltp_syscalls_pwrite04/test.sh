#!/bin/bash
# Functional test: ltp - syscalls - pwrite04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwrite04"
        rlRun "kirk -f syscalls -p pwrite04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwrite04"
