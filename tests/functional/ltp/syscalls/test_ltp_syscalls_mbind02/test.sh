#!/bin/bash
# Functional test: ltp - syscalls - mbind02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mbind02"
        rlRun "kirk -f syscalls -p mbind02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mbind02"
