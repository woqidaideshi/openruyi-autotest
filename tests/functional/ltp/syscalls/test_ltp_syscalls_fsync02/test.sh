#!/bin/bash
# Functional test: ltp - syscalls - fsync02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsync02"
        rlRun "kirk -f syscalls -p fsync02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsync02"
