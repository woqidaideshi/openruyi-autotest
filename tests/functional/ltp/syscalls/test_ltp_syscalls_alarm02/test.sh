#!/bin/bash
# Functional test: ltp - syscalls - alarm02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - alarm02"
        rlRun "kirk -f syscalls -p alarm02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP alarm02"
