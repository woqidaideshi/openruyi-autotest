#!/bin/bash
# Functional test: ltp - syscalls - keyctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl05"
        rlRun "kirk -f syscalls -p keyctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl05"
