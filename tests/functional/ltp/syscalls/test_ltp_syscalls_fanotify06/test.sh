#!/bin/bash
# Functional test: ltp - syscalls - fanotify06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify06"
        rlRun "kirk -f syscalls -p fanotify06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify06"
