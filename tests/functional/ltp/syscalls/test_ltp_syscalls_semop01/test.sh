#!/bin/bash
# Functional test: ltp - syscalls - semop01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semop01"
        rlRun "kirk -f syscalls -p semop01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop01"
