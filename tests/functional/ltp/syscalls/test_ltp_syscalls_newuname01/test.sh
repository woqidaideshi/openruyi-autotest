#!/bin/bash
# Functional test: ltp - syscalls - newuname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - newuname01"
        rlRun "kirk -f syscalls -p newuname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP newuname01"
