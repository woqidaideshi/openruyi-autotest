#!/bin/bash
# Functional test: ltp - syscalls - getegid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getegid02"
        rlRun "kirk -f syscalls -p getegid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getegid02"
