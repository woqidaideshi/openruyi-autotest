#!/bin/bash
# Functional test: ltp - syscalls - epoll_wait05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll_wait05"
        rlRun "kirk -f syscalls -p epoll_wait05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll_wait05"
