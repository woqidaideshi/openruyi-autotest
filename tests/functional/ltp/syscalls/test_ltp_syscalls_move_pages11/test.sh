#!/bin/bash
# Functional test: ltp - syscalls - move_pages11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - move_pages11"
        rlRun "kirk -f syscalls -p move_pages11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP move_pages11"
