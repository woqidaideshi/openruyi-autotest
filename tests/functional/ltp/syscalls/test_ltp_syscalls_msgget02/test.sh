#!/bin/bash
# Functional test: ltp - syscalls - msgget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgget02"
        rlRun "kirk -f syscalls -p msgget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget02"
