#!/bin/bash
# Functional test: ltp - syscalls - write04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - write04"
        rlRun "kirk -f syscalls -p write04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP write04"
