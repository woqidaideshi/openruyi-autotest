#!/bin/bash
# Functional test: ltp - syscalls - semctl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl08"
        rlRun "kirk -f syscalls -p semctl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl08"
