#!/bin/bash
# Functional test: ltp - syscalls - pathconf01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pathconf01"
        rlRun "kirk -f syscalls -p pathconf01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pathconf01"
