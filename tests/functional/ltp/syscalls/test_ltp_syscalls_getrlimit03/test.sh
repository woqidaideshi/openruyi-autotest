#!/bin/bash
# Functional test: ltp - syscalls - getrlimit03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrlimit03"
        rlRun "kirk -f syscalls -p getrlimit03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrlimit03"
