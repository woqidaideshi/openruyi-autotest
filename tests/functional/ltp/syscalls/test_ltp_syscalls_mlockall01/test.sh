#!/bin/bash
# Functional test: ltp - syscalls - mlockall01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlockall01"
        rlRun "kirk -f syscalls -p mlockall01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlockall01"
