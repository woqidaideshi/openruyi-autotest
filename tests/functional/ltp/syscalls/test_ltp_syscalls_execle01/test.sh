#!/bin/bash
# Functional test: ltp - syscalls - execle01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execle01"
        rlRun "kirk -f syscalls -p execle01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execle01"
