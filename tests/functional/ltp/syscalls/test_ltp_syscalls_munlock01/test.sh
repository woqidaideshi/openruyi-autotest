#!/bin/bash
# Functional test: ltp - syscalls - munlock01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munlock01"
        rlRun "kirk -f syscalls -p munlock01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munlock01"
