#!/bin/bash
# Functional test: ltp - syscalls - stat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stat01"
        rlRun "kirk -f syscalls -p stat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stat01"
