#!/bin/bash
# Functional test: ltp - syscalls - sbrk03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sbrk03"
        rlRun "kirk -f syscalls -p sbrk03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sbrk03"
