#!/bin/bash
# Functional test: ltp - syscalls - acct01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - acct01"
        rlRun "kirk -f syscalls -p acct01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP acct01"
