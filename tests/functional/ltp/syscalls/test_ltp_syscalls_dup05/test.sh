#!/bin/bash
# Functional test: ltp - syscalls - dup05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup05"
        rlRun "kirk -f syscalls -p dup05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup05"
