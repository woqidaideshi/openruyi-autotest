#!/bin/bash
# Functional test: ltp - syscalls - sendfile02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile02"
        rlRun "kirk -f syscalls -p sendfile02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile02"
