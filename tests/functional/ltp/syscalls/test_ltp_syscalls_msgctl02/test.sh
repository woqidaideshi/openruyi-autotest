#!/bin/bash
# Functional test: ltp - syscalls - msgctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgctl02"
        rlRun "kirk -f syscalls -p msgctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl02"
