#!/bin/bash
# Functional test: ltp - syscalls - setsockopt07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt07"
        rlRun "kirk -f syscalls -p setsockopt07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt07"
