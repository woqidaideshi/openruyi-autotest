#!/bin/bash
# Functional test: ltp - syscalls - statfs02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statfs02"
        rlRun "kirk -f syscalls -p statfs02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statfs02"
