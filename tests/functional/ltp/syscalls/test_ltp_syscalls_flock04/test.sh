#!/bin/bash
# Functional test: ltp - syscalls - flock04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock04"
        rlRun "kirk -f syscalls -p flock04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock04"
