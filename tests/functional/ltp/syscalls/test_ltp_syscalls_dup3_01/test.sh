#!/bin/bash
# Functional test: ltp - syscalls - dup3_01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup3_01"
        rlRun "kirk -f syscalls -p dup3_01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup3_01"
