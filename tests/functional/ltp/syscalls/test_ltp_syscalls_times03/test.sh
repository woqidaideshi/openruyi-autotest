#!/bin/bash
# Functional test: ltp - syscalls - times03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - times03"
        rlRun "kirk -f syscalls -p times03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP times03"
