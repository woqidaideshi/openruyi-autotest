#!/bin/bash
# Functional test: ltp - syscalls - wait402
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - wait402"
        rlRun "kirk -f syscalls -p wait402 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP wait402"
