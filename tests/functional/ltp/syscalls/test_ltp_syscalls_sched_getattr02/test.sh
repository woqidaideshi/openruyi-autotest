#!/bin/bash
# Functional test: ltp - syscalls - sched_getattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_getattr02"
        rlRun "kirk -f syscalls -p sched_getattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_getattr02"
