#!/bin/bash
# Functional test: ltp - syscalls - seccomp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - seccomp01"
        rlRun "kirk -f syscalls -p seccomp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP seccomp01"
