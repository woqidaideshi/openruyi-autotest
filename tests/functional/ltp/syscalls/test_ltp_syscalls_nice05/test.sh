#!/bin/bash
# Functional test: ltp - syscalls - nice05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nice05"
        rlRun "kirk -f syscalls -p nice05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nice05"
