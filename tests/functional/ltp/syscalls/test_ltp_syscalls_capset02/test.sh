#!/bin/bash
# Functional test: ltp - syscalls - capset02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - capset02"
        rlRun "kirk -f syscalls -p capset02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP capset02"
