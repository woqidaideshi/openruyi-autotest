#!/bin/bash
# Functional test: ltp - syscalls - ptrace07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace07"
        rlRun "kirk -f syscalls -p ptrace07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace07"
