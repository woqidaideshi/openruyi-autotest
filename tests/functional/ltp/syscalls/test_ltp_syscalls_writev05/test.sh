#!/bin/bash
# Functional test: ltp - syscalls - writev05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - writev05"
        rlRun "kirk -f syscalls -p writev05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP writev05"
