#!/bin/bash
# Functional test: ltp - syscalls - setrlimit01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit01"
        rlRun "kirk -f syscalls -p setrlimit01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit01"
