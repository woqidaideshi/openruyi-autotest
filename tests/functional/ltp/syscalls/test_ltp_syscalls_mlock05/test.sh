#!/bin/bash
# Functional test: ltp - syscalls - mlock05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock05"
        rlRun "kirk -f syscalls -p mlock05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock05"
