#!/bin/bash
# Functional test: ltp - syscalls - munmap01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munmap01"
        rlRun "kirk -f syscalls -p munmap01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munmap01"
