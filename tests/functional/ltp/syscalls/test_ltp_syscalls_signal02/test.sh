#!/bin/bash
# Functional test: ltp - syscalls - signal02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signal02"
        rlRun "kirk -f syscalls -p signal02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signal02"
