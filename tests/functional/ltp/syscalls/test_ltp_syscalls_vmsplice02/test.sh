#!/bin/bash
# Functional test: ltp - syscalls - vmsplice02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vmsplice02"
        rlRun "kirk -f syscalls -p vmsplice02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vmsplice02"
