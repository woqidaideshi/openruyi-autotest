#!/bin/bash
# Functional test: ltp - syscalls - mremap02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap02"
        rlRun "kirk -f syscalls -p mremap02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap02"
