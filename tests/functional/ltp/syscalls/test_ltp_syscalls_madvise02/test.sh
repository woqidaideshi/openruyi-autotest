#!/bin/bash
# Functional test: ltp - syscalls - madvise02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise02"
        rlRun "kirk -f syscalls -p madvise02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise02"
