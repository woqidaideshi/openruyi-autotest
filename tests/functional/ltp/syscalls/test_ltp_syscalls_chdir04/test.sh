#!/bin/bash
# Functional test: ltp - syscalls - chdir04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chdir04"
        rlRun "kirk -f syscalls -p chdir04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chdir04"
