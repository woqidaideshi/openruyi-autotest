#!/bin/bash
# Functional test: ltp - syscalls - fchown05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchown05"
        rlRun "kirk -f syscalls -p fchown05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchown05"
