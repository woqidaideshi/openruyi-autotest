#!/bin/bash
# Functional test: ltp - syscalls - dup201
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup201"
        rlRun "kirk -f syscalls -p dup201 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup201"
