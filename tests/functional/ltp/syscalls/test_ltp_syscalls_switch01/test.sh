#!/bin/bash
# Functional test: ltp - syscalls - switch01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - switch01"
        rlRun "kirk -f syscalls -p switch01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP switch01"
