#!/bin/bash
# Functional test: ltp - syscalls - rename13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename13"
        rlRun "kirk -f syscalls -p rename13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename13"
