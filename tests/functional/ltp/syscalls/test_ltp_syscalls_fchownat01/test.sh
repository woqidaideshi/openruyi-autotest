#!/bin/bash
# Functional test: ltp - syscalls - fchownat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchownat01"
        rlRun "kirk -f syscalls -p fchownat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchownat01"
