#!/bin/bash
# Functional test: ltp - syscalls - chdir01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chdir01"
        rlRun "kirk -f syscalls -p chdir01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chdir01"
