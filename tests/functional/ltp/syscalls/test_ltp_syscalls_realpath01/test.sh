#!/bin/bash
# Functional test: ltp - syscalls - realpath01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - realpath01"
        rlRun "kirk -f syscalls -p realpath01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP realpath01"
