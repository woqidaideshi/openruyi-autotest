#!/bin/bash
# Functional test: ltp - syscalls - tee02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tee02"
        rlRun "kirk -f syscalls -p tee02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tee02"
