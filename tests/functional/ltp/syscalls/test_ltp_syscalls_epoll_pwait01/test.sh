#!/bin/bash
# Functional test: ltp - syscalls - epoll_pwait01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll_pwait01"
        rlRun "kirk -f syscalls -p epoll_pwait01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll_pwait01"
