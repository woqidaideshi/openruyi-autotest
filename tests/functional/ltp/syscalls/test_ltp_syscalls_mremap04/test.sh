#!/bin/bash
# Functional test: ltp - syscalls - mremap04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap04"
        rlRun "kirk -f syscalls -p mremap04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap04"
