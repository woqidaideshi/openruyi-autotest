#!/bin/bash
# Functional test: ltp - syscalls - ioctl_pidfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_pidfd02"
        rlRun "kirk -f syscalls -p ioctl_pidfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_pidfd02"
