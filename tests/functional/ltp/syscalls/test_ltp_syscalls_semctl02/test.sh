#!/bin/bash
# Functional test: ltp - syscalls - semctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl02"
        rlRun "kirk -f syscalls -p semctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl02"
