#!/bin/bash
# Functional test: ltp - syscalls - pipeio_1
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipeio_1"
        rlRun "kirk -f syscalls -p pipeio_1 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipeio_1"
