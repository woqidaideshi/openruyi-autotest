#!/bin/bash
# Functional test: ltp - syscalls - keyctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl03"
        rlRun "kirk -f syscalls -p keyctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl03"
