#!/bin/bash
# Functional test: ltp - syscalls - tgkill01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tgkill01"
        rlRun "kirk -f syscalls -p tgkill01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tgkill01"
