#!/bin/bash
# Functional test: ltp - syscalls - statmount08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount08"
        rlRun "kirk -f syscalls -p statmount08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount08"
