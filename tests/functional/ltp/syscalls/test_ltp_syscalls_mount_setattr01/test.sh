#!/bin/bash
# Functional test: ltp - syscalls - mount_setattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount_setattr01"
        rlRun "kirk -f syscalls -p mount_setattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount_setattr01"
