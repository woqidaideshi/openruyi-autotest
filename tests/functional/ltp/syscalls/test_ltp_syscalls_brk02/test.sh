#!/bin/bash
# Functional test: ltp - syscalls - brk02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - brk02"
        rlRun "kirk -f syscalls -p brk02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP brk02"
