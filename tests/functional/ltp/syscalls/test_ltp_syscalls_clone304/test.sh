#!/bin/bash
# Functional test: ltp - syscalls - clone304
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone304"
        rlRun "kirk -f syscalls -p clone304 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone304"
