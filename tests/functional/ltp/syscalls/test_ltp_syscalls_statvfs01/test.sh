#!/bin/bash
# Functional test: ltp - syscalls - statvfs01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statvfs01"
        rlRun "kirk -f syscalls -p statvfs01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statvfs01"
