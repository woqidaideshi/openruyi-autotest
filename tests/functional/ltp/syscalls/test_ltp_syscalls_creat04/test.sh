#!/bin/bash
# Functional test: ltp - syscalls - creat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat04"
        rlRun "kirk -f syscalls -p creat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat04"
