#!/bin/bash
# Functional test: ltp - syscalls - uname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - uname01"
        rlRun "kirk -f syscalls -p uname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP uname01"
