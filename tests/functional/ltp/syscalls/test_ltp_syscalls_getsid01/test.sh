#!/bin/bash
# Functional test: ltp - syscalls - getsid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getsid01"
        rlRun "kirk -f syscalls -p getsid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getsid01"
