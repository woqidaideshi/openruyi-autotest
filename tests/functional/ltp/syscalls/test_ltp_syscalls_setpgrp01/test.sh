#!/bin/bash
# Functional test: ltp - syscalls - setpgrp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setpgrp01"
        rlRun "kirk -f syscalls -p setpgrp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setpgrp01"
