#!/bin/bash
# Functional test: ltp - syscalls - renameat201
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - renameat201"
        rlRun "kirk -f syscalls -p renameat201 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP renameat201"
