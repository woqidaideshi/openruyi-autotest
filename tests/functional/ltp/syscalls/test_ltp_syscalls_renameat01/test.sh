#!/bin/bash
# Functional test: ltp - syscalls - renameat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - renameat01"
        rlRun "kirk -f syscalls -p renameat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP renameat01"
