#!/bin/bash
# Functional test: ltp - syscalls - flock01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock01"
        rlRun "kirk -f syscalls -p flock01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock01"
