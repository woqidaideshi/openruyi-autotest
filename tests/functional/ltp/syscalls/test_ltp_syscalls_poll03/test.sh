#!/bin/bash
# Functional test: ltp - syscalls - poll03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - poll03"
        rlRun "kirk -f syscalls -p poll03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP poll03"
