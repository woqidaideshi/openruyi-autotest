#!/bin/bash
# Functional test: ltp - syscalls - lchown02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lchown02"
        rlRun "kirk -f syscalls -p lchown02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lchown02"
