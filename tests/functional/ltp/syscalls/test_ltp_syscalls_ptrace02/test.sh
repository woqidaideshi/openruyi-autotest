#!/bin/bash
# Functional test: ltp - syscalls - ptrace02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace02"
        rlRun "kirk -f syscalls -p ptrace02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace02"
