#!/bin/bash
# Functional test: ltp - syscalls - execveat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execveat03"
        rlRun "kirk -f syscalls -p execveat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execveat03"
