#!/bin/bash
# Functional test: ltp - syscalls - getresgid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getresgid03"
        rlRun "kirk -f syscalls -p getresgid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getresgid03"
