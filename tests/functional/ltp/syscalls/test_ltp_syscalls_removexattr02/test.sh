#!/bin/bash
# Functional test: ltp - syscalls - removexattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - removexattr02"
        rlRun "kirk -f syscalls -p removexattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP removexattr02"
