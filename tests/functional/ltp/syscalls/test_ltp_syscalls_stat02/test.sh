#!/bin/bash
# Functional test: ltp - syscalls - stat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stat02"
        rlRun "kirk -f syscalls -p stat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stat02"
