#!/bin/bash
# Functional test: ltp - syscalls - setreuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid01"
        rlRun "kirk -f syscalls -p setreuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid01"
