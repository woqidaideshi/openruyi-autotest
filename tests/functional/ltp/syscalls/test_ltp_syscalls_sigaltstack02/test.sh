#!/bin/bash
# Functional test: ltp - syscalls - sigaltstack02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigaltstack02"
        rlRun "kirk -f syscalls -p sigaltstack02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigaltstack02"
