#!/bin/bash
# Functional test: ltp - syscalls - io_uring03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_uring03"
        rlRun "kirk -f syscalls -p io_uring03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_uring03"
