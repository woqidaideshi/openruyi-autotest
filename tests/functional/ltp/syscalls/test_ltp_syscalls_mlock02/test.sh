#!/bin/bash
# Functional test: ltp - syscalls - mlock02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock02"
        rlRun "kirk -f syscalls -p mlock02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock02"
