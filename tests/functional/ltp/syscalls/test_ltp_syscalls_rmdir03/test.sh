#!/bin/bash
# Functional test: ltp - syscalls - rmdir03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rmdir03"
        rlRun "kirk -f syscalls -p rmdir03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rmdir03"
