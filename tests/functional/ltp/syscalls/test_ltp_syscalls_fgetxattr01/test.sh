#!/bin/bash
# Functional test: ltp - syscalls - fgetxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fgetxattr01"
        rlRun "kirk -f syscalls -p fgetxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fgetxattr01"
