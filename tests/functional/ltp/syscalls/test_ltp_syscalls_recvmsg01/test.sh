#!/bin/bash
# Functional test: ltp - syscalls - recvmsg01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - recvmsg01"
        rlRun "kirk -f syscalls -p recvmsg01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP recvmsg01"
