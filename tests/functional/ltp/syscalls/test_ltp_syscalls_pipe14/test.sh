#!/bin/bash
# Functional test: ltp - syscalls - pipe14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe14"
        rlRun "kirk -f syscalls -p pipe14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe14"
