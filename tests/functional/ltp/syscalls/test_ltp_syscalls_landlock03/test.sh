#!/bin/bash
# Functional test: ltp - syscalls - landlock03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock03"
        rlRun "kirk -f syscalls -p landlock03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock03"
