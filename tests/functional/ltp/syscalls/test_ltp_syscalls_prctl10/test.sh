#!/bin/bash
# Functional test: ltp - syscalls - prctl10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl10"
        rlRun "kirk -f syscalls -p prctl10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl10"
