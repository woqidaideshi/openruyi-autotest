#!/bin/bash
# Functional test: ltp - syscalls - getpriority02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpriority02"
        rlRun "kirk -f syscalls -p getpriority02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpriority02"
