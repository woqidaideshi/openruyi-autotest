#!/bin/bash
# Functional test: ltp - syscalls - clone302
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone302"
        rlRun "kirk -f syscalls -p clone302 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone302"
