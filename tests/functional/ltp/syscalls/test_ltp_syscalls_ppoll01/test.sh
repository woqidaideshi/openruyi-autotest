#!/bin/bash
# Functional test: ltp - syscalls - ppoll01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ppoll01"
        rlRun "kirk -f syscalls -p ppoll01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ppoll01"
