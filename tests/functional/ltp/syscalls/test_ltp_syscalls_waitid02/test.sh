#!/bin/bash
# Functional test: ltp - syscalls - waitid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid02"
        rlRun "kirk -f syscalls -p waitid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid02"
