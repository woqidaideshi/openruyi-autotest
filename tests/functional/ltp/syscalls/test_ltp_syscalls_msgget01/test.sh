#!/bin/bash
# Functional test: ltp - syscalls - msgget01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgget01"
        rlRun "kirk -f syscalls -p msgget01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget01"
