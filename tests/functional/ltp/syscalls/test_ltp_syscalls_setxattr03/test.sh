#!/bin/bash
# Functional test: ltp - syscalls - setxattr03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setxattr03"
        rlRun "kirk -f syscalls -p setxattr03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setxattr03"
