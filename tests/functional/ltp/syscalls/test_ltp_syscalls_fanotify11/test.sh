#!/bin/bash
# Functional test: ltp - syscalls - fanotify11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify11"
        rlRun "kirk -f syscalls -p fanotify11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify11"
