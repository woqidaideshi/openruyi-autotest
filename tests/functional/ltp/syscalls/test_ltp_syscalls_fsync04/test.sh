#!/bin/bash
# Functional test: ltp - syscalls - fsync04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsync04"
        rlRun "kirk -f syscalls -p fsync04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsync04"
