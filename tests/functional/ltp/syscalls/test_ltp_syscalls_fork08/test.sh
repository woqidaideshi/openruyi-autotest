#!/bin/bash
# Functional test: ltp - syscalls - fork08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork08"
        rlRun "kirk -f syscalls -p fork08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork08"
