#!/bin/bash
# Functional test: ltp - syscalls - statmount05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount05"
        rlRun "kirk -f syscalls -p statmount05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount05"
