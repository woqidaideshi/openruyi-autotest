#!/bin/bash
# Functional test: ltp - syscalls - memfd_create04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - memfd_create04"
        rlRun "kirk -f syscalls -p memfd_create04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP memfd_create04"
