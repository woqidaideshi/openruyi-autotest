#!/bin/bash
# Functional test: ltp - syscalls - fcntl27
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl27"
        rlRun "kirk -f syscalls -p fcntl27 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl27"
