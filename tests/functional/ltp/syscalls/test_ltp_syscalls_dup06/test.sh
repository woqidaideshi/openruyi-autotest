#!/bin/bash
# Functional test: ltp - syscalls - dup06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup06"
        rlRun "kirk -f syscalls -p dup06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup06"
