#!/bin/bash
# Functional test: ltp - syscalls - creat09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat09"
        rlRun "kirk -f syscalls -p creat09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat09"
