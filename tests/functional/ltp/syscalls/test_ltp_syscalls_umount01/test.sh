#!/bin/bash
# Functional test: ltp - syscalls - umount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umount01"
        rlRun "kirk -f syscalls -p umount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umount01"
