#!/bin/bash
# Functional test: ltp - syscalls - listmount01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listmount01"
        rlRun "kirk -f syscalls -p listmount01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listmount01"
