#!/bin/bash
# Functional test: ltp - syscalls - access04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - access04"
        rlRun "kirk -f syscalls -p access04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP access04"
