#!/bin/bash
# Functional test: ltp - syscalls - statx02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx02"
        rlRun "kirk -f syscalls -p statx02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx02"
