#!/bin/bash
# Functional test: ltp - syscalls - statvfs02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statvfs02"
        rlRun "kirk -f syscalls -p statvfs02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statvfs02"
