#!/bin/bash
# Functional test: ltp - syscalls - ioctl_ns06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_ns06"
        rlRun "kirk -f syscalls -p ioctl_ns06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_ns06"
