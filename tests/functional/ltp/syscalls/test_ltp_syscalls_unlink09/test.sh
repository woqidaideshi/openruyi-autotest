#!/bin/bash
# Functional test: ltp - syscalls - unlink09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unlink09"
        rlRun "kirk -f syscalls -p unlink09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unlink09"
