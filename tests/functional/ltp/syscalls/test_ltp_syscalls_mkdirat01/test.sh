#!/bin/bash
# Functional test: ltp - syscalls - mkdirat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mkdirat01"
        rlRun "kirk -f syscalls -p mkdirat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdirat01"
