#!/bin/bash
# Functional test: ltp - syscalls - io_cancel01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_cancel01"
        rlRun "kirk -f syscalls -p io_cancel01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_cancel01"
