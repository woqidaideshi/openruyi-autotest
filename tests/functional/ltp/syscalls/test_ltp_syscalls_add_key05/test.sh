#!/bin/bash
# Functional test: ltp - syscalls - add_key05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - add_key05"
        rlRun "kirk -f syscalls -p add_key05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP add_key05"
