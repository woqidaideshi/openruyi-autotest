#!/bin/bash
# Functional test: ltp - syscalls - kill05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill05"
        rlRun "kirk -f syscalls -p kill05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill05"
