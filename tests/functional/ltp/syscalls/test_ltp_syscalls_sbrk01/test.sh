#!/bin/bash
# Functional test: ltp - syscalls - sbrk01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sbrk01"
        rlRun "kirk -f syscalls -p sbrk01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sbrk01"
