#!/bin/bash
# Functional test: ltp - syscalls - fstatfs01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fstatfs01"
        rlRun "kirk -f syscalls -p fstatfs01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fstatfs01"
