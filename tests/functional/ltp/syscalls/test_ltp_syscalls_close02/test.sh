#!/bin/bash
# Functional test: ltp - syscalls - close02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - close02"
        rlRun "kirk -f syscalls -p close02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP close02"
