#!/bin/bash
# Functional test: ltp - syscalls - setrlimit02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit02"
        rlRun "kirk -f syscalls -p setrlimit02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit02"
