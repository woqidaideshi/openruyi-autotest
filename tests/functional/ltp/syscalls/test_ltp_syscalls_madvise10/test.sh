#!/bin/bash
# Functional test: ltp - syscalls - madvise10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise10"
        rlRun "kirk -f syscalls -p madvise10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise10"
