#!/bin/bash
# Functional test: ltp - syscalls - mknod04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mknod04"
        rlRun "kirk -f syscalls -p mknod04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mknod04"
