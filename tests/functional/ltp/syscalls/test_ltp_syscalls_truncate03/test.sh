#!/bin/bash
# Functional test: ltp - syscalls - truncate03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - truncate03"
        rlRun "kirk -f syscalls -p truncate03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP truncate03"
