#!/bin/bash
# Functional test: ltp - syscalls - mallinfo02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mallinfo02"
        rlRun "kirk -f syscalls -p mallinfo02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mallinfo02"
