#!/bin/bash
# Functional test: ltp - syscalls - shmctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl02"
        rlRun "kirk -f syscalls -p shmctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl02"
