#!/bin/bash
# Functional test: ltp - syscalls - fsconfig02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsconfig02"
        rlRun "kirk -f syscalls -p fsconfig02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsconfig02"
