#!/bin/bash
# Functional test: ltp - syscalls - dup205
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup205"
        rlRun "kirk -f syscalls -p dup205 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup205"
