#!/bin/bash
# Functional test: ltp - syscalls - open09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open09"
        rlRun "kirk -f syscalls -p open09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open09"
