#!/bin/bash
# Functional test: ltp - syscalls - getdomainname01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getdomainname01"
        rlRun "kirk -f syscalls -p getdomainname01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getdomainname01"
