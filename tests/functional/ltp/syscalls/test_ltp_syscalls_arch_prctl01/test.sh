#!/bin/bash
# Functional test: ltp - syscalls - arch_prctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - arch_prctl01"
        rlRun "kirk -f syscalls -p arch_prctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP arch_prctl01"
