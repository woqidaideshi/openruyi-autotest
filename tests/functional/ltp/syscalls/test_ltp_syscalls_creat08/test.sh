#!/bin/bash
# Functional test: ltp - syscalls - creat08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - creat08"
        rlRun "kirk -f syscalls -p creat08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP creat08"
