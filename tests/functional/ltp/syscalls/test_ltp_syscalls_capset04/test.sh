#!/bin/bash
# Functional test: ltp - syscalls - capset04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - capset04"
        rlRun "kirk -f syscalls -p capset04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP capset04"
