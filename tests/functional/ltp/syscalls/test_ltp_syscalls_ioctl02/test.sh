#!/bin/bash
# Functional test: ltp - syscalls - ioctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl02"
        rlRun "kirk -f syscalls -p ioctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl02"
