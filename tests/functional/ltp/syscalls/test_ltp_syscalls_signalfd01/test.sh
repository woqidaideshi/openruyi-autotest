#!/bin/bash
# Functional test: ltp - syscalls - signalfd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signalfd01"
        rlRun "kirk -f syscalls -p signalfd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signalfd01"
