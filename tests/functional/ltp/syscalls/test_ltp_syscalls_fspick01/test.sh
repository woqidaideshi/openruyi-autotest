#!/bin/bash
# Functional test: ltp - syscalls - fspick01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fspick01"
        rlRun "kirk -f syscalls -p fspick01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fspick01"
