#!/bin/bash
# Functional test: ltp - syscalls - listmount02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listmount02"
        rlRun "kirk -f syscalls -p listmount02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listmount02"
