#!/bin/bash
# Functional test: ltp - syscalls - shmget04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmget04"
        rlRun "kirk -f syscalls -p shmget04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget04"
