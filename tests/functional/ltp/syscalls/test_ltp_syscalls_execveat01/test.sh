#!/bin/bash
# Functional test: ltp - syscalls - execveat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execveat01"
        rlRun "kirk -f syscalls -p execveat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execveat01"
