#!/bin/bash
# Functional test: ltp - syscalls - keyctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl07"
        rlRun "kirk -f syscalls -p keyctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl07"
