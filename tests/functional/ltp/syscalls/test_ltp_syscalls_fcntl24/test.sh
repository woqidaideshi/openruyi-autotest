#!/bin/bash
# Functional test: ltp - syscalls - fcntl24
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl24"
        rlRun "kirk -f syscalls -p fcntl24 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl24"
