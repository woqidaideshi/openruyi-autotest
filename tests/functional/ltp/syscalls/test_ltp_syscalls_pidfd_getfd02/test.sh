#!/bin/bash
# Functional test: ltp - syscalls - pidfd_getfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_getfd02"
        rlRun "kirk -f syscalls -p pidfd_getfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_getfd02"
