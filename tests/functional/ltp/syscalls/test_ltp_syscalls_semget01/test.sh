#!/bin/bash
# Functional test: ltp - syscalls - semget01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semget01"
        rlRun "kirk -f syscalls -p semget01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semget01"
