#!/bin/bash
# Functional test: ltp - syscalls - execve03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve03"
        rlRun "kirk -f syscalls -p execve03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve03"
