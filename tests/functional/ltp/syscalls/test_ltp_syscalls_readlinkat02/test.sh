#!/bin/bash
# Functional test: ltp - syscalls - readlinkat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readlinkat02"
        rlRun "kirk -f syscalls -p readlinkat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readlinkat02"
