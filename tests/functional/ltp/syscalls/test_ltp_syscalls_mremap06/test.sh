#!/bin/bash
# Functional test: ltp - syscalls - mremap06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap06"
        rlRun "kirk -f syscalls -p mremap06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap06"
