#!/bin/bash
# Functional test: ltp - syscalls - open14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open14"
        rlRun "kirk -f syscalls -p open14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open14"
