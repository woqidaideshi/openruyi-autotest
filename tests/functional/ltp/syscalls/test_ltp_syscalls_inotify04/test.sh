#!/bin/bash
# Functional test: ltp - syscalls - inotify04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify04"
        rlRun "kirk -f syscalls -p inotify04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify04"
