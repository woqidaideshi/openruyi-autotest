#!/bin/bash
# Functional test: ltp - syscalls - getpeername01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpeername01"
        rlRun "kirk -f syscalls -p getpeername01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpeername01"
