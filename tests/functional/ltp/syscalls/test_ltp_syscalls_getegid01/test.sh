#!/bin/bash
# Functional test: ltp - syscalls - getegid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getegid01"
        rlRun "kirk -f syscalls -p getegid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getegid01"
