#!/bin/bash
# Functional test: ltp - syscalls - getxattr05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getxattr05"
        rlRun "kirk -f syscalls -p getxattr05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getxattr05"
