#!/bin/bash
# Functional test: ltp - syscalls - fcntl34
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl34"
        rlRun "kirk -f syscalls -p fcntl34 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl34"
