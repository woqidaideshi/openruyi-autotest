#!/bin/bash
# Functional test: ltp - syscalls - write06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - write06"
        rlRun "kirk -f syscalls -p write06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP write06"
