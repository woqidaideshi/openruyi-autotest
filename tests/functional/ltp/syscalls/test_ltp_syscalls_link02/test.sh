#!/bin/bash
# Functional test: ltp - syscalls - link02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - link02"
        rlRun "kirk -f syscalls -p link02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP link02"
