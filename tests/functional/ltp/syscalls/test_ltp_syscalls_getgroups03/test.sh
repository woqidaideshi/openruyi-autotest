#!/bin/bash
# Functional test: ltp - syscalls - getgroups03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getgroups03"
        rlRun "kirk -f syscalls -p getgroups03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getgroups03"
