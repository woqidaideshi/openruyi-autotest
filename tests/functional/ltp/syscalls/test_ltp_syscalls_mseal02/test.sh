#!/bin/bash
# Functional test: ltp - syscalls - mseal02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mseal02"
        rlRun "kirk -f syscalls -p mseal02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mseal02"
