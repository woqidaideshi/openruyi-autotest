#!/bin/bash
# Functional test: ltp - syscalls - clock_settime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clock_settime01"
        rlRun "kirk -f syscalls -p clock_settime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clock_settime01"
