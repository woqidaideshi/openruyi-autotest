#!/bin/bash
# Functional test: ltp - syscalls - link05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - link05"
        rlRun "kirk -f syscalls -p link05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP link05"
