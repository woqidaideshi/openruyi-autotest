#!/bin/bash
# Functional test: ltp - syscalls - request_key04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - request_key04"
        rlRun "kirk -f syscalls -p request_key04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP request_key04"
