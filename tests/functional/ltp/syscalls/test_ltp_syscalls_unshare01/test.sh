#!/bin/bash
# Functional test: ltp - syscalls - unshare01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unshare01"
        rlRun "kirk -f syscalls -p unshare01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unshare01"
