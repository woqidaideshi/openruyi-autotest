#!/bin/bash
# Functional test: ltp - syscalls - ioctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl06"
        rlRun "kirk -f syscalls -p ioctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl06"
