#!/bin/bash
# Functional test: ltp - syscalls - fcntl21
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl21"
        rlRun "kirk -f syscalls -p fcntl21 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl21"
