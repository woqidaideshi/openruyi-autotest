#!/bin/bash
# Functional test: ltp - syscalls - statfs01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statfs01"
        rlRun "kirk -f syscalls -p statfs01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statfs01"
