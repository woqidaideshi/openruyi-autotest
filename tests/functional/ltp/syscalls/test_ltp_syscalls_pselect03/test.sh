#!/bin/bash
# Functional test: ltp - syscalls - pselect03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pselect03"
        rlRun "kirk -f syscalls -p pselect03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pselect03"
