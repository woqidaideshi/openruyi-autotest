#!/bin/bash
# Functional test: ltp - syscalls - init_module02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - init_module02"
        rlRun "kirk -f syscalls -p init_module02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP init_module02"
