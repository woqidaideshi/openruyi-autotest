#!/bin/bash
# Functional test: ltp - syscalls - fork05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork05"
        rlRun "kirk -f syscalls -p fork05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork05"
