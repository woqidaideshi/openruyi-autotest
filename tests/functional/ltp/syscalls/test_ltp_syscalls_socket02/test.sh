#!/bin/bash
# Functional test: ltp - syscalls - socket02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socket02"
        rlRun "kirk -f syscalls -p socket02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socket02"
