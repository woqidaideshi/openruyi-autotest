#!/bin/bash
# Functional test: ltp - syscalls - fcntl29
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl29"
        rlRun "kirk -f syscalls -p fcntl29 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl29"
