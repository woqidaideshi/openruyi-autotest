#!/bin/bash
# Functional test: ltp - syscalls - prctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl02"
        rlRun "kirk -f syscalls -p prctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl02"
