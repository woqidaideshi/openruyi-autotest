#!/bin/bash
# Functional test: ltp - syscalls - swapon03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - swapon03"
        rlRun "kirk -f syscalls -p swapon03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP swapon03"
