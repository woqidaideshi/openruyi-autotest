#!/bin/bash
# Functional test: ltp - syscalls - landlock02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock02"
        rlRun "kirk -f syscalls -p landlock02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock02"
