#!/bin/bash
# Functional test: ltp - syscalls - open11
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open11"
        rlRun "kirk -f syscalls -p open11 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open11"
