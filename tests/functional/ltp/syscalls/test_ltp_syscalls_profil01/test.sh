#!/bin/bash
# Functional test: ltp - syscalls - profil01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - profil01"
        rlRun "kirk -f syscalls -p profil01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP profil01"
