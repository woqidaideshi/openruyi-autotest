#!/bin/bash
# Functional test: ltp - syscalls - wait01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - wait01"
        rlRun "kirk -f syscalls -p wait01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP wait01"
