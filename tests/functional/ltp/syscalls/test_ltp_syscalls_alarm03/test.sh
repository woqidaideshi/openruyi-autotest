#!/bin/bash
# Functional test: ltp - syscalls - alarm03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - alarm03"
        rlRun "kirk -f syscalls -p alarm03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP alarm03"
