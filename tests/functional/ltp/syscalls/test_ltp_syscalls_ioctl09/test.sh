#!/bin/bash
# Functional test: ltp - syscalls - ioctl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl09"
        rlRun "kirk -f syscalls -p ioctl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl09"
