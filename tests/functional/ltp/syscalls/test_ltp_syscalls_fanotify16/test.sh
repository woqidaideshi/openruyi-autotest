#!/bin/bash
# Functional test: ltp - syscalls - fanotify16
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify16"
        rlRun "kirk -f syscalls -p fanotify16 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify16"
