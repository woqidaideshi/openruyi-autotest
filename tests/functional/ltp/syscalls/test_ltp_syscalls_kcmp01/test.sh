#!/bin/bash
# Functional test: ltp - syscalls - kcmp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kcmp01"
        rlRun "kirk -f syscalls -p kcmp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kcmp01"
