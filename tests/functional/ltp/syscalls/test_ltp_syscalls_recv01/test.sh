#!/bin/bash
# Functional test: ltp - syscalls - recv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - recv01"
        rlRun "kirk -f syscalls -p recv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP recv01"
