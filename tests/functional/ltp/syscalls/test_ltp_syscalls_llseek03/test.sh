#!/bin/bash
# Functional test: ltp - syscalls - llseek03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - llseek03"
        rlRun "kirk -f syscalls -p llseek03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP llseek03"
