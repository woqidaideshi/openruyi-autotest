#!/bin/bash
# Functional test: ltp - syscalls - msgctl12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgctl12"
        rlRun "kirk -f syscalls -p msgctl12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl12"
