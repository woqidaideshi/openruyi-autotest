#!/bin/bash
# Functional test: ltp - syscalls - sendmmsg01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendmmsg01"
        rlRun "kirk -f syscalls -p sendmmsg01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendmmsg01"
