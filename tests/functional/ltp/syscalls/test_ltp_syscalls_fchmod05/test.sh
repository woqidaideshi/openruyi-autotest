#!/bin/bash
# Functional test: ltp - syscalls - fchmod05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmod05"
        rlRun "kirk -f syscalls -p fchmod05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmod05"
