#!/bin/bash
# Functional test: ltp - syscalls - mlock03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock03"
        rlRun "kirk -f syscalls -p mlock03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock03"
