#!/bin/bash
# Functional test: ltp - syscalls - open_tree02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open_tree02"
        rlRun "kirk -f syscalls -p open_tree02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open_tree02"
