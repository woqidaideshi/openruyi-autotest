#!/bin/bash
# Functional test: ltp - syscalls - waitid10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid10"
        rlRun "kirk -f syscalls -p waitid10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid10"
