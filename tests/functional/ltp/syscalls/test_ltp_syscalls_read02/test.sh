#!/bin/bash
# Functional test: ltp - syscalls - read02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - read02"
        rlRun "kirk -f syscalls -p read02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP read02"
