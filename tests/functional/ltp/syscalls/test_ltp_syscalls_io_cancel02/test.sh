#!/bin/bash
# Functional test: ltp - syscalls - io_cancel02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_cancel02"
        rlRun "kirk -f syscalls -p io_cancel02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_cancel02"
