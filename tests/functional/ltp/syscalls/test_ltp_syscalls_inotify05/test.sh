#!/bin/bash
# Functional test: ltp - syscalls - inotify05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify05"
        rlRun "kirk -f syscalls -p inotify05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify05"
