#!/bin/bash
# Functional test: ltp - syscalls - sysinfo01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysinfo01"
        rlRun "kirk -f syscalls -p sysinfo01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysinfo01"
