#!/bin/bash
# Functional test: ltp - syscalls - eventfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd02"
        rlRun "kirk -f syscalls -p eventfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd02"
