#!/bin/bash
# Functional test: ltp - syscalls - setsockopt04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt04"
        rlRun "kirk -f syscalls -p setsockopt04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt04"
