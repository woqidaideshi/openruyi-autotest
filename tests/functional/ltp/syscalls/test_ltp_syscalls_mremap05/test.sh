#!/bin/bash
# Functional test: ltp - syscalls - mremap05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap05"
        rlRun "kirk -f syscalls -p mremap05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap05"
