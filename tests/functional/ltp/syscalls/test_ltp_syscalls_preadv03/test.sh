#!/bin/bash
# Functional test: ltp - syscalls - preadv03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - preadv03"
        rlRun "kirk -f syscalls -p preadv03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP preadv03"
