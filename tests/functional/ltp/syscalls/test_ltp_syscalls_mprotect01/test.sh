#!/bin/bash
# Functional test: ltp - syscalls - mprotect01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mprotect01"
        rlRun "kirk -f syscalls -p mprotect01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mprotect01"
