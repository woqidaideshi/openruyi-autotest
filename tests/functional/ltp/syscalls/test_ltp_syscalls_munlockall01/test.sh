#!/bin/bash
# Functional test: ltp - syscalls - munlockall01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munlockall01"
        rlRun "kirk -f syscalls -p munlockall01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munlockall01"
