#!/bin/bash
# Functional test: ltp - syscalls - iopl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - iopl02"
        rlRun "kirk -f syscalls -p iopl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP iopl02"
