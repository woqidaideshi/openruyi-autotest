#!/bin/bash
# Functional test: ltp - syscalls - getxattr04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getxattr04"
        rlRun "kirk -f syscalls -p getxattr04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getxattr04"
