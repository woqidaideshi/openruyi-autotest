#!/bin/bash
# Functional test: ltp - syscalls - fchdir01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchdir01"
        rlRun "kirk -f syscalls -p fchdir01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchdir01"
