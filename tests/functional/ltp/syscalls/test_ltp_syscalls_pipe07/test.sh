#!/bin/bash
# Functional test: ltp - syscalls - pipe07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe07"
        rlRun "kirk -f syscalls -p pipe07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe07"
