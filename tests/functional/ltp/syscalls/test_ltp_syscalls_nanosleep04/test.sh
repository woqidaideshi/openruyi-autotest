#!/bin/bash
# Functional test: ltp - syscalls - nanosleep04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nanosleep04"
        rlRun "kirk -f syscalls -p nanosleep04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nanosleep04"
