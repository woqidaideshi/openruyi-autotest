#!/bin/bash
# Functional test: ltp - syscalls - fcntl25
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl25"
        rlRun "kirk -f syscalls -p fcntl25 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl25"
