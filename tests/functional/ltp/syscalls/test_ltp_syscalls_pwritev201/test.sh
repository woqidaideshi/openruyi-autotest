#!/bin/bash
# Functional test: ltp - syscalls - pwritev201
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwritev201"
        rlRun "kirk -f syscalls -p pwritev201 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwritev201"
