#!/bin/bash
# Functional test: ltp - syscalls - clone05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone05"
        rlRun "kirk -f syscalls -p clone05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone05"
