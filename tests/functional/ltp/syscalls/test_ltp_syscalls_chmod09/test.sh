#!/bin/bash
# Functional test: ltp - syscalls - chmod09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod09"
        rlRun "kirk -f syscalls -p chmod09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod09"
