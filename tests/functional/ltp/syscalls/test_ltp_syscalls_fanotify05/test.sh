#!/bin/bash
# Functional test: ltp - syscalls - fanotify05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify05"
        rlRun "kirk -f syscalls -p fanotify05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify05"
