#!/bin/bash
# Functional test: ltp - syscalls - prctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl05"
        rlRun "kirk -f syscalls -p prctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl05"
