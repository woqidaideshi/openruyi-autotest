#!/bin/bash
# Functional test: ltp - syscalls - signal03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signal03"
        rlRun "kirk -f syscalls -p signal03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signal03"
