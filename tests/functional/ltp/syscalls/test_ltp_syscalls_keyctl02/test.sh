#!/bin/bash
# Functional test: ltp - syscalls - keyctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - keyctl02"
        rlRun "kirk -f syscalls -p keyctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP keyctl02"
