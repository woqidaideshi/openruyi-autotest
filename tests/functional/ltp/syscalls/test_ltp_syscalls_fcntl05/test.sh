#!/bin/bash
# Functional test: ltp - syscalls - fcntl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl05"
        rlRun "kirk -f syscalls -p fcntl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl05"
