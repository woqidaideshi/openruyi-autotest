#!/bin/bash
# Functional test: ltp - syscalls - unshare04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unshare04"
        rlRun "kirk -f syscalls -p unshare04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unshare04"
