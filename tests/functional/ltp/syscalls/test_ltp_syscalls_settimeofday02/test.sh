#!/bin/bash
# Functional test: ltp - syscalls - settimeofday02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - settimeofday02"
        rlRun "kirk -f syscalls -p settimeofday02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP settimeofday02"
