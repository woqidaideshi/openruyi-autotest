#!/bin/bash
# Functional test: ltp - syscalls - waitpid06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid06"
        rlRun "kirk -f syscalls -p waitpid06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid06"
