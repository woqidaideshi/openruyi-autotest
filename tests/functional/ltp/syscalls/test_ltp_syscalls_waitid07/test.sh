#!/bin/bash
# Functional test: ltp - syscalls - waitid07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid07"
        rlRun "kirk -f syscalls -p waitid07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid07"
