#!/bin/bash
# Functional test: ltp - syscalls - clone03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone03"
        rlRun "kirk -f syscalls -p clone03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone03"
