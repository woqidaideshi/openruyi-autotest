#!/bin/bash
# Functional test: ltp - syscalls - getcpu02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcpu02"
        rlRun "kirk -f syscalls -p getcpu02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcpu02"
