#!/bin/bash
# Functional test: ltp - syscalls - sendfile05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile05"
        rlRun "kirk -f syscalls -p sendfile05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile05"
