#!/bin/bash
# Functional test: ltp - syscalls - execve02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execve02"
        rlRun "kirk -f syscalls -p execve02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execve02"
