#!/bin/bash
# Functional test: ltp - syscalls - signal01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signal01"
        rlRun "kirk -f syscalls -p signal01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signal01"
