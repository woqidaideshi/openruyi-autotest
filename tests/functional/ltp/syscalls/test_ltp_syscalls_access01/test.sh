#!/bin/bash
# Functional test: ltp - syscalls - access01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - access01"
        rlRun "kirk -f syscalls -p access01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP access01"
