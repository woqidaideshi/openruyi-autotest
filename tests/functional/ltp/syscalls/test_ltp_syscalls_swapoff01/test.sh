#!/bin/bash
# Functional test: ltp - syscalls - swapoff01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - swapoff01"
        rlRun "kirk -f syscalls -p swapoff01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP swapoff01"
