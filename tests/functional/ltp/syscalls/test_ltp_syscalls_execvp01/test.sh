#!/bin/bash
# Functional test: ltp - syscalls - execvp01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execvp01"
        rlRun "kirk -f syscalls -p execvp01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execvp01"
