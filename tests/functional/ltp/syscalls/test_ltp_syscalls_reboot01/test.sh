#!/bin/bash
# Functional test: ltp - syscalls - reboot01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - reboot01"
        rlRun "kirk -f syscalls -p reboot01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP reboot01"
