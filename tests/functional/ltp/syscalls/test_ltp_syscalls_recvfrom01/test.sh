#!/bin/bash
# Functional test: ltp - syscalls - recvfrom01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - recvfrom01"
        rlRun "kirk -f syscalls -p recvfrom01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP recvfrom01"
