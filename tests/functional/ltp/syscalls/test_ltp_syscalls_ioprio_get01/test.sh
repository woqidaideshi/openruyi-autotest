#!/bin/bash
# Functional test: ltp - syscalls - ioprio_get01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioprio_get01"
        rlRun "kirk -f syscalls -p ioprio_get01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioprio_get01"
