#!/bin/bash
# Functional test: ltp - syscalls - timer_settime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timer_settime01"
        rlRun "kirk -f syscalls -p timer_settime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timer_settime01"
