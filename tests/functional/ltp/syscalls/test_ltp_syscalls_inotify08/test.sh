#!/bin/bash
# Functional test: ltp - syscalls - inotify08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify08"
        rlRun "kirk -f syscalls -p inotify08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify08"
