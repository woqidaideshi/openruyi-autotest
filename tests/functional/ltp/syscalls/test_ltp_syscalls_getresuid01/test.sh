#!/bin/bash
# Functional test: ltp - syscalls - getresuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getresuid01"
        rlRun "kirk -f syscalls -p getresuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getresuid01"
