#!/bin/bash
# Functional test: ltp - syscalls - sigsuspend01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sigsuspend01"
        rlRun "kirk -f syscalls -p sigsuspend01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sigsuspend01"
