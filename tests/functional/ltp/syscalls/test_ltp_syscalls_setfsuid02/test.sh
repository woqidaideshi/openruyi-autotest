#!/bin/bash
# Functional test: ltp - syscalls - setfsuid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setfsuid02"
        rlRun "kirk -f syscalls -p setfsuid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setfsuid02"
