#!/bin/bash
# Functional test: ltp - syscalls - semctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl07"
        rlRun "kirk -f syscalls -p semctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl07"
