#!/bin/bash
# Functional test: ltp - syscalls - shmctl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl08"
        rlRun "kirk -f syscalls -p shmctl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl08"
