#!/bin/bash
# Functional test: ltp - syscalls - send01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - send01"
        rlRun "kirk -f syscalls -p send01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP send01"
