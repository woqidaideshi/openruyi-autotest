#!/bin/bash
# Functional test: ltp - syscalls - semctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl01"
        rlRun "kirk -f syscalls -p semctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl01"
