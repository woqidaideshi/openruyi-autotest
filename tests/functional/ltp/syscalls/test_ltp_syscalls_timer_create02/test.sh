#!/bin/bash
# Functional test: ltp - syscalls - timer_create02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timer_create02"
        rlRun "kirk -f syscalls -p timer_create02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timer_create02"
