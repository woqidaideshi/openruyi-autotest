#!/bin/bash
# Functional test: ltp - syscalls - mincore01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mincore01"
        rlRun "kirk -f syscalls -p mincore01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mincore01"
