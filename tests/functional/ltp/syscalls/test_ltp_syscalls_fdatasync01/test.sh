#!/bin/bash
# Functional test: ltp - syscalls - fdatasync01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fdatasync01"
        rlRun "kirk -f syscalls -p fdatasync01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fdatasync01"
