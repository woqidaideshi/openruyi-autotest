#!/bin/bash
# Functional test: ltp - syscalls - pipe06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe06"
        rlRun "kirk -f syscalls -p pipe06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe06"
