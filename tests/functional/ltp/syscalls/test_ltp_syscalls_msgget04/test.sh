#!/bin/bash
# Functional test: ltp - syscalls - msgget04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgget04"
        rlRun "kirk -f syscalls -p msgget04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget04"
