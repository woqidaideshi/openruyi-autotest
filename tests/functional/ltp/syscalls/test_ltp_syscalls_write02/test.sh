#!/bin/bash
# Functional test: ltp - syscalls - write02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - write02"
        rlRun "kirk -f syscalls -p write02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP write02"
