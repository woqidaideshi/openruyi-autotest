#!/bin/bash
# Functional test: ltp - syscalls - signal04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signal04"
        rlRun "kirk -f syscalls -p signal04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signal04"
