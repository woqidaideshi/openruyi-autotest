#!/bin/bash
# Functional test: ltp - syscalls - fchmod04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmod04"
        rlRun "kirk -f syscalls -p fchmod04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmod04"
