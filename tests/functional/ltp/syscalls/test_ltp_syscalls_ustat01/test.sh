#!/bin/bash
# Functional test: ltp - syscalls - ustat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ustat01"
        rlRun "kirk -f syscalls -p ustat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ustat01"
