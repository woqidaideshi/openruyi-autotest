#!/bin/bash
# Functional test: ltp - syscalls - fanotify20
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify20"
        rlRun "kirk -f syscalls -p fanotify20 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify20"
