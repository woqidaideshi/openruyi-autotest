#!/bin/bash
# Functional test: ltp - syscalls - leapsec01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - leapsec01"
        rlRun "kirk -f syscalls -p leapsec01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP leapsec01"
