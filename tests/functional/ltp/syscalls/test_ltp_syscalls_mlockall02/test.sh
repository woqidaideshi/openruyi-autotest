#!/bin/bash
# Functional test: ltp - syscalls - mlockall02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlockall02"
        rlRun "kirk -f syscalls -p mlockall02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlockall02"
