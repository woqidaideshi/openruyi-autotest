#!/bin/bash
# Functional test: ltp - syscalls - kcmp02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kcmp02"
        rlRun "kirk -f syscalls -p kcmp02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kcmp02"
