#!/bin/bash
# Functional test: ltp - syscalls - getrlimit01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrlimit01"
        rlRun "kirk -f syscalls -p getrlimit01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrlimit01"
