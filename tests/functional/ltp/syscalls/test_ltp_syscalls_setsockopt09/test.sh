#!/bin/bash
# Functional test: ltp - syscalls - setsockopt09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt09"
        rlRun "kirk -f syscalls -p setsockopt09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt09"
