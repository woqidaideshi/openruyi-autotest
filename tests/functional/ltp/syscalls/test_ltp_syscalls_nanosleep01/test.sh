#!/bin/bash
# Functional test: ltp - syscalls - nanosleep01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - nanosleep01"
        rlRun "kirk -f syscalls -p nanosleep01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nanosleep01"
