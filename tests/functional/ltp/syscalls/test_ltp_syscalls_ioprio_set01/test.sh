#!/bin/bash
# Functional test: ltp - syscalls - ioprio_set01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioprio_set01"
        rlRun "kirk -f syscalls -p ioprio_set01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioprio_set01"
