#!/bin/bash
# Functional test: ltp - syscalls - ioctl_ficlone01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_ficlone01"
        rlRun "kirk -f syscalls -p ioctl_ficlone01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_ficlone01"
