#!/bin/bash
# Functional test: ltp - syscalls - setreuid05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setreuid05"
        rlRun "kirk -f syscalls -p setreuid05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setreuid05"
