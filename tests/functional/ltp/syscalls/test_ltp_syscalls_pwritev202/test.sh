#!/bin/bash
# Functional test: ltp - syscalls - pwritev202
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwritev202"
        rlRun "kirk -f syscalls -p pwritev202 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwritev202"
