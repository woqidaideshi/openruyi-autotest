#!/bin/bash
# Functional test: ltp - syscalls - pwrite02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwrite02"
        rlRun "kirk -f syscalls -p pwrite02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwrite02"
