#!/bin/bash
# Functional test: ltp - syscalls - umask01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - umask01"
        rlRun "kirk -f syscalls -p umask01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP umask01"
