#!/bin/bash
# Functional test: ltp - syscalls - capset03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - capset03"
        rlRun "kirk -f syscalls -p capset03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP capset03"
