#!/bin/bash
# Functional test: ltp - syscalls - ioctl10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl10"
        rlRun "kirk -f syscalls -p ioctl10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl10"
