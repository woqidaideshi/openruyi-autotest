#!/bin/bash
# Functional test: ltp - syscalls - getrandom05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getrandom05"
        rlRun "kirk -f syscalls -p getrandom05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getrandom05"
