#!/bin/bash
# Functional test: ltp - syscalls - mmap14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap14"
        rlRun "kirk -f syscalls -p mmap14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap14"
