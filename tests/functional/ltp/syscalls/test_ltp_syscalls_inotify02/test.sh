#!/bin/bash
# Functional test: ltp - syscalls - inotify02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify02"
        rlRun "kirk -f syscalls -p inotify02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify02"
