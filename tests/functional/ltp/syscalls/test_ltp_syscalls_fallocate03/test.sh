#!/bin/bash
# Functional test: ltp - syscalls - fallocate03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate03"
        rlRun "kirk -f syscalls -p fallocate03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate03"
