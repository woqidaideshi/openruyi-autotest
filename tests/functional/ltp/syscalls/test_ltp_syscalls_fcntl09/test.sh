#!/bin/bash
# Functional test: ltp - syscalls - fcntl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl09"
        rlRun "kirk -f syscalls -p fcntl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl09"
