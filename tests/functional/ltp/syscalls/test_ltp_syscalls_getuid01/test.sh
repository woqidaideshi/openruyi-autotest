#!/bin/bash
# Functional test: ltp - syscalls - getuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getuid01"
        rlRun "kirk -f syscalls -p getuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getuid01"
