#!/bin/bash
# Functional test: ltp - syscalls - waitid04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid04"
        rlRun "kirk -f syscalls -p waitid04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid04"
