#!/bin/bash
# Functional test: ltp - syscalls - fpathconf01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fpathconf01"
        rlRun "kirk -f syscalls -p fpathconf01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fpathconf01"
