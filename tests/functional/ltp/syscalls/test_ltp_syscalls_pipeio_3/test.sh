#!/bin/bash
# Functional test: ltp - syscalls - pipeio_3
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipeio_3"
        rlRun "kirk -f syscalls -p pipeio_3 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipeio_3"
