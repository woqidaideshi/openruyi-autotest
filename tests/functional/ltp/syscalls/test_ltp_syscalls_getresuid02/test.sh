#!/bin/bash
# Functional test: ltp - syscalls - getresuid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getresuid02"
        rlRun "kirk -f syscalls -p getresuid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getresuid02"
