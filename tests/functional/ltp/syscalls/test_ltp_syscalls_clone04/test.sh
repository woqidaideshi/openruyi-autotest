#!/bin/bash
# Functional test: ltp - syscalls - clone04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone04"
        rlRun "kirk -f syscalls -p clone04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone04"
