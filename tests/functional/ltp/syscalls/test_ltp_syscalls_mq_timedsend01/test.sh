#!/bin/bash
# Functional test: ltp - syscalls - mq_timedsend01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mq_timedsend01"
        rlRun "kirk -f syscalls -p mq_timedsend01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mq_timedsend01"
