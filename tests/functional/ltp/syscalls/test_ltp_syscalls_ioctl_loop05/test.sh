#!/bin/bash
# Functional test: ltp - syscalls - ioctl_loop05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_loop05"
        rlRun "kirk -f syscalls -p ioctl_loop05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_loop05"
