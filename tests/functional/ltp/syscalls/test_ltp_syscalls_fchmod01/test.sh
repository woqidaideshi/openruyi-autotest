#!/bin/bash
# Functional test: ltp - syscalls - fchmod01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchmod01"
        rlRun "kirk -f syscalls -p fchmod01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchmod01"
