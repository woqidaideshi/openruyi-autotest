#!/bin/bash
# Functional test: ltp - syscalls - setns01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setns01"
        rlRun "kirk -f syscalls -p setns01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setns01"
