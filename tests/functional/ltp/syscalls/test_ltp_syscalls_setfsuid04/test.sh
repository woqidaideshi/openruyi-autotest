#!/bin/bash
# Functional test: ltp - syscalls - setfsuid04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setfsuid04"
        rlRun "kirk -f syscalls -p setfsuid04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setfsuid04"
