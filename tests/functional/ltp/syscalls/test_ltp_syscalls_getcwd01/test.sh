#!/bin/bash
# Functional test: ltp - syscalls - getcwd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getcwd01"
        rlRun "kirk -f syscalls -p getcwd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getcwd01"
