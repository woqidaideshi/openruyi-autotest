#!/bin/bash
# Functional test: ltp - syscalls - ftruncate03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ftruncate03"
        rlRun "kirk -f syscalls -p ftruncate03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ftruncate03"
