#!/bin/bash
# Functional test: ltp - syscalls - fanotify04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify04"
        rlRun "kirk -f syscalls -p fanotify04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify04"
