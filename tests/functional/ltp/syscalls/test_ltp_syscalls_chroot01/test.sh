#!/bin/bash
# Functional test: ltp - syscalls - chroot01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chroot01"
        rlRun "kirk -f syscalls -p chroot01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chroot01"
