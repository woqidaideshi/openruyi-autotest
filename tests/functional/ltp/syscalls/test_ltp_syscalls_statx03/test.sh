#!/bin/bash
# Functional test: ltp - syscalls - statx03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx03"
        rlRun "kirk -f syscalls -p statx03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx03"
