#!/bin/bash
# Functional test: ltp - syscalls - fremovexattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fremovexattr01"
        rlRun "kirk -f syscalls -p fremovexattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fremovexattr01"
