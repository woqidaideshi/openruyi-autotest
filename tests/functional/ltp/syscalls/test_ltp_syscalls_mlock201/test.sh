#!/bin/bash
# Functional test: ltp - syscalls - mlock201
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock201"
        rlRun "kirk -f syscalls -p mlock201 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock201"
