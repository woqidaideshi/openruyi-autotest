#!/bin/bash
# Functional test: ltp - syscalls - fallocate05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate05"
        rlRun "kirk -f syscalls -p fallocate05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate05"
