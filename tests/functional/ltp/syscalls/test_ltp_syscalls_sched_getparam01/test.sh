#!/bin/bash
# Functional test: ltp - syscalls - sched_getparam01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_getparam01"
        rlRun "kirk -f syscalls -p sched_getparam01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_getparam01"
