#!/bin/bash
# Functional test: ltp - syscalls - readlink03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readlink03"
        rlRun "kirk -f syscalls -p readlink03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readlink03"
