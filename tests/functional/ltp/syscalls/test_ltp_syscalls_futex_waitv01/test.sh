#!/bin/bash
# Functional test: ltp - syscalls - futex_waitv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - futex_waitv01"
        rlRun "kirk -f syscalls -p futex_waitv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP futex_waitv01"
