#!/bin/bash
# Functional test: ltp - syscalls - fork03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork03"
        rlRun "kirk -f syscalls -p fork03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork03"
