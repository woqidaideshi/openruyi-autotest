#!/bin/bash
# Functional test: ltp - syscalls - mprotect02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mprotect02"
        rlRun "kirk -f syscalls -p mprotect02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mprotect02"
