#!/bin/bash
# Functional test: ltp - syscalls - request_key05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - request_key05"
        rlRun "kirk -f syscalls -p request_key05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP request_key05"
