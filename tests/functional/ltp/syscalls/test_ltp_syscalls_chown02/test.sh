#!/bin/bash
# Functional test: ltp - syscalls - chown02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chown02"
        rlRun "kirk -f syscalls -p chown02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chown02"
