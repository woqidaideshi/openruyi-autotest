#!/bin/bash
# Functional test: ltp - syscalls - fcntl37
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl37"
        rlRun "kirk -f syscalls -p fcntl37 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl37"
