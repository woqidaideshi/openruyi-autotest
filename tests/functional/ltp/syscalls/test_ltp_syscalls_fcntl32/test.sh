#!/bin/bash
# Functional test: ltp - syscalls - fcntl32
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl32"
        rlRun "kirk -f syscalls -p fcntl32 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl32"
