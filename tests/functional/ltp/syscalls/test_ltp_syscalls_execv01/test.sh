#!/bin/bash
# Functional test: ltp - syscalls - execv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - execv01"
        rlRun "kirk -f syscalls -p execv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP execv01"
