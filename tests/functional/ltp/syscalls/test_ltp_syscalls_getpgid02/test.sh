#!/bin/bash
# Functional test: ltp - syscalls - getpgid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpgid02"
        rlRun "kirk -f syscalls -p getpgid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpgid02"
