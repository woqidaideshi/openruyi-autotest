#!/bin/bash
# Functional test: ltp - syscalls - fcntl40
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl40"
        rlRun "kirk -f syscalls -p fcntl40 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl40"
