#!/bin/bash
# Functional test: ltp - syscalls - kill03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill03"
        rlRun "kirk -f syscalls -p kill03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill03"
