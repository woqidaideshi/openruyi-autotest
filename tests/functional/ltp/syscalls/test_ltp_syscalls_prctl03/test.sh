#!/bin/bash
# Functional test: ltp - syscalls - prctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - prctl03"
        rlRun "kirk -f syscalls -p prctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP prctl03"
