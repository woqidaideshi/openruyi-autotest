#!/bin/bash
# Functional test: ltp - syscalls - tkill02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - tkill02"
        rlRun "kirk -f syscalls -p tkill02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tkill02"
