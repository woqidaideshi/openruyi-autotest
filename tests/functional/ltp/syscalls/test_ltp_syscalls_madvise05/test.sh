#!/bin/bash
# Functional test: ltp - syscalls - madvise05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise05"
        rlRun "kirk -f syscalls -p madvise05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise05"
