#!/bin/bash
# Functional test: ltp - syscalls - mlock04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock04"
        rlRun "kirk -f syscalls -p mlock04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock04"
