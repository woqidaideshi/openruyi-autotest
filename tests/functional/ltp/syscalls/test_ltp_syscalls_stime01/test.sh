#!/bin/bash
# Functional test: ltp - syscalls - stime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - stime01"
        rlRun "kirk -f syscalls -p stime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP stime01"
