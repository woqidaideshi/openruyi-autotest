#!/bin/bash
# Functional test: ltp - syscalls - readlinkat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readlinkat01"
        rlRun "kirk -f syscalls -p readlinkat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readlinkat01"
