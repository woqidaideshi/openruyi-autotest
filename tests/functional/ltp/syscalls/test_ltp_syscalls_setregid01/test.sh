#!/bin/bash
# Functional test: ltp - syscalls - setregid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setregid01"
        rlRun "kirk -f syscalls -p setregid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setregid01"
