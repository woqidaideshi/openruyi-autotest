#!/bin/bash
# Functional test: ltp - syscalls - pwritev01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pwritev01"
        rlRun "kirk -f syscalls -p pwritev01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pwritev01"
