#!/bin/bash
# Functional test: ltp - syscalls - listxattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - listxattr02"
        rlRun "kirk -f syscalls -p listxattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP listxattr02"
