#!/bin/bash
# Functional test: ltp - syscalls - chmod06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chmod06"
        rlRun "kirk -f syscalls -p chmod06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chmod06"
