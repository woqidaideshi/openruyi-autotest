#!/bin/bash
# Functional test: ltp - syscalls - pipe02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe02"
        rlRun "kirk -f syscalls -p pipe02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe02"
