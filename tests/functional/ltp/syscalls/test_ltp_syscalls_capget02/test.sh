#!/bin/bash
# Functional test: ltp - syscalls - capget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - capget02"
        rlRun "kirk -f syscalls -p capget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP capget02"
