#!/bin/bash
# Functional test: ltp - syscalls - fspick02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fspick02"
        rlRun "kirk -f syscalls -p fspick02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fspick02"
