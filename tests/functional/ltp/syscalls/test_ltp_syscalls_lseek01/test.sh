#!/bin/bash
# Functional test: ltp - syscalls - lseek01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - lseek01"
        rlRun "kirk -f syscalls -p lseek01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lseek01"
