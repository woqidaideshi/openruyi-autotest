#!/bin/bash
# Functional test: ltp - syscalls - sendmsg03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendmsg03"
        rlRun "kirk -f syscalls -p sendmsg03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendmsg03"
