#!/bin/bash
# Functional test: ltp - syscalls - fchdir02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fchdir02"
        rlRun "kirk -f syscalls -p fchdir02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fchdir02"
