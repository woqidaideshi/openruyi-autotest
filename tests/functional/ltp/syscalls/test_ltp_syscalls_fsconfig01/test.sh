#!/bin/bash
# Functional test: ltp - syscalls - fsconfig01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fsconfig01"
        rlRun "kirk -f syscalls -p fsconfig01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fsconfig01"
