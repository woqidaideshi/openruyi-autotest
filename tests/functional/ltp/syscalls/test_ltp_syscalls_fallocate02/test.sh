#!/bin/bash
# Functional test: ltp - syscalls - fallocate02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fallocate02"
        rlRun "kirk -f syscalls -p fallocate02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fallocate02"
