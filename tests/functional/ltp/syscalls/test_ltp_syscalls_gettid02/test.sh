#!/bin/bash
# Functional test: ltp - syscalls - gettid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - gettid02"
        rlRun "kirk -f syscalls -p gettid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP gettid02"
