#!/bin/bash
# Functional test: ltp - syscalls - sendmsg01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendmsg01"
        rlRun "kirk -f syscalls -p sendmsg01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendmsg01"
