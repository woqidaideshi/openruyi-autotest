#!/bin/bash
# Functional test: ltp - syscalls - fcntl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl01"
        rlRun "kirk -f syscalls -p fcntl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl01"
