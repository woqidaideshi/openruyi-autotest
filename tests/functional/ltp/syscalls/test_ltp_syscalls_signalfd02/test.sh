#!/bin/bash
# Functional test: ltp - syscalls - signalfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signalfd02"
        rlRun "kirk -f syscalls -p signalfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signalfd02"
