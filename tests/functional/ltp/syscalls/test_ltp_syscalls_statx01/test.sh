#!/bin/bash
# Functional test: ltp - syscalls - statx01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statx01"
        rlRun "kirk -f syscalls -p statx01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statx01"
