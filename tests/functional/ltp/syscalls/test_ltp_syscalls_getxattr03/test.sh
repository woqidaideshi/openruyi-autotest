#!/bin/bash
# Functional test: ltp - syscalls - getxattr03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getxattr03"
        rlRun "kirk -f syscalls -p getxattr03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getxattr03"
