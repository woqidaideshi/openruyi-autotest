#!/bin/bash
# Functional test: ltp - syscalls - fork10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork10"
        rlRun "kirk -f syscalls -p fork10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork10"
