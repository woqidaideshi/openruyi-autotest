#!/bin/bash
# Functional test: ltp - syscalls - inotify10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify10"
        rlRun "kirk -f syscalls -p inotify10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify10"
