#!/bin/bash
# Functional test: ltp - syscalls - wait401
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - wait401"
        rlRun "kirk -f syscalls -p wait401 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP wait401"
