#!/bin/bash
# Functional test: ltp - syscalls - pipe09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe09"
        rlRun "kirk -f syscalls -p pipe09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe09"
