#!/bin/bash
# Functional test: ltp - syscalls - signal05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - signal05"
        rlRun "kirk -f syscalls -p signal05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP signal05"
