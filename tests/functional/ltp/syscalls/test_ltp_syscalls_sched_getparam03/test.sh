#!/bin/bash
# Functional test: ltp - syscalls - sched_getparam03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sched_getparam03"
        rlRun "kirk -f syscalls -p sched_getparam03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sched_getparam03"
