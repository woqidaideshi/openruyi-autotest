#!/bin/bash
# Functional test: ltp - syscalls - select02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - select02"
        rlRun "kirk -f syscalls -p select02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP select02"
