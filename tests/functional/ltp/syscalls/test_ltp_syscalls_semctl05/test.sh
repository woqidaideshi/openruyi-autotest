#!/bin/bash
# Functional test: ltp - syscalls - semctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semctl05"
        rlRun "kirk -f syscalls -p semctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl05"
