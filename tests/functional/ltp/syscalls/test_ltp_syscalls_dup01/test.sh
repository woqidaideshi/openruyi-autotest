#!/bin/bash
# Functional test: ltp - syscalls - dup01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dup01"
        rlRun "kirk -f syscalls -p dup01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dup01"
