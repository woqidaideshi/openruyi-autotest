#!/bin/bash
# Functional test: ltp - syscalls - cacheflush01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - cacheflush01"
        rlRun "kirk -f syscalls -p cacheflush01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cacheflush01"
