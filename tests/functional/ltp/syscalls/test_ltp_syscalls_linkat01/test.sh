#!/bin/bash
# Functional test: ltp - syscalls - linkat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - linkat01"
        rlRun "kirk -f syscalls -p linkat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP linkat01"
