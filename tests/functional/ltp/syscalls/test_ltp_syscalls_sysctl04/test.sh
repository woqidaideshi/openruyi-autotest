#!/bin/bash
# Functional test: ltp - syscalls - sysctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysctl04"
        rlRun "kirk -f syscalls -p sysctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysctl04"
