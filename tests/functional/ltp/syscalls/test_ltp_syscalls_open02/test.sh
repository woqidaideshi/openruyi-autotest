#!/bin/bash
# Functional test: ltp - syscalls - open02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open02"
        rlRun "kirk -f syscalls -p open02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open02"
