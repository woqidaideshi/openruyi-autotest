#!/bin/bash
# Functional test: ltp - syscalls - preadv202
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - preadv202"
        rlRun "kirk -f syscalls -p preadv202 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP preadv202"
