#!/bin/bash
# Functional test: ltp - syscalls - getpagesize01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getpagesize01"
        rlRun "kirk -f syscalls -p getpagesize01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getpagesize01"
