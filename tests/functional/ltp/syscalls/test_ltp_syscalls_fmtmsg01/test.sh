#!/bin/bash
# Functional test: ltp - syscalls - fmtmsg01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fmtmsg01"
        rlRun "kirk -f syscalls -p fmtmsg01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fmtmsg01"
