#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd04"
        rlRun "kirk -f syscalls -p userfaultfd04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd04"
