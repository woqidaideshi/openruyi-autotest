#!/bin/bash
# Functional test: ltp - syscalls - flistxattr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flistxattr01"
        rlRun "kirk -f syscalls -p flistxattr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flistxattr01"
