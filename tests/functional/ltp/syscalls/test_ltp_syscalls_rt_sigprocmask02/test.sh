#!/bin/bash
# Functional test: ltp - syscalls - rt_sigprocmask02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rt_sigprocmask02"
        rlRun "kirk -f syscalls -p rt_sigprocmask02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rt_sigprocmask02"
