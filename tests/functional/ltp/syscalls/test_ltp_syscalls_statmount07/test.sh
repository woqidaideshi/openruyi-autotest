#!/bin/bash
# Functional test: ltp - syscalls - statmount07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - statmount07"
        rlRun "kirk -f syscalls -p statmount07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP statmount07"
