#!/bin/bash
# Functional test: ltp - syscalls - link04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - link04"
        rlRun "kirk -f syscalls -p link04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP link04"
