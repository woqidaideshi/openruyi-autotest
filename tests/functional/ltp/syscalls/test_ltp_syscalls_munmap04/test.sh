#!/bin/bash
# Functional test: ltp - syscalls - munmap04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - munmap04"
        rlRun "kirk -f syscalls -p munmap04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP munmap04"
