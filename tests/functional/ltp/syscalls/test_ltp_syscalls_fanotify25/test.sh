#!/bin/bash
# Functional test: ltp - syscalls - fanotify25
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify25"
        rlRun "kirk -f syscalls -p fanotify25 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify25"
