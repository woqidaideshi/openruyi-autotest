#!/bin/bash
# Functional test: ltp - syscalls - madvise07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - madvise07"
        rlRun "kirk -f syscalls -p madvise07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP madvise07"
