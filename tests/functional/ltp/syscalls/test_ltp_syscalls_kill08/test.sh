#!/bin/bash
# Functional test: ltp - syscalls - kill08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - kill08"
        rlRun "kirk -f syscalls -p kill08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP kill08"
