#!/bin/bash
# Functional test: ltp - syscalls - msync03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msync03"
        rlRun "kirk -f syscalls -p msync03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msync03"
