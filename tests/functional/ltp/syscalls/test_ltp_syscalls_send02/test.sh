#!/bin/bash
# Functional test: ltp - syscalls - send02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - send02"
        rlRun "kirk -f syscalls -p send02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP send02"
