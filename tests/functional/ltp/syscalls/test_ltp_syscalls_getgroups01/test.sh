#!/bin/bash
# Functional test: ltp - syscalls - getgroups01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getgroups01"
        rlRun "kirk -f syscalls -p getgroups01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getgroups01"
