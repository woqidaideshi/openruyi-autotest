#!/bin/bash
# Functional test: ltp - syscalls - getresgid02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - getresgid02"
        rlRun "kirk -f syscalls -p getresgid02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP getresgid02"
