#!/bin/bash
# Functional test: ltp - syscalls - rename15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename15"
        rlRun "kirk -f syscalls -p rename15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename15"
