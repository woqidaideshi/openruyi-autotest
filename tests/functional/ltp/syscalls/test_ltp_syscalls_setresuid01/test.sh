#!/bin/bash
# Functional test: ltp - syscalls - setresuid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setresuid01"
        rlRun "kirk -f syscalls -p setresuid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setresuid01"
