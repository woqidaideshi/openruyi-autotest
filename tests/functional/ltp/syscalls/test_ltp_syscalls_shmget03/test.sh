#!/bin/bash
# Functional test: ltp - syscalls - shmget03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmget03"
        rlRun "kirk -f syscalls -p shmget03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget03"
