#!/bin/bash
# Functional test: ltp - syscalls - waitid09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitid09"
        rlRun "kirk -f syscalls -p waitid09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitid09"
