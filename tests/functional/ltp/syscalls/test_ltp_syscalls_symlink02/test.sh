#!/bin/bash
# Functional test: ltp - syscalls - symlink02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - symlink02"
        rlRun "kirk -f syscalls -p symlink02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP symlink02"
