#!/bin/bash
# Functional test: ltp - syscalls - mlock202
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mlock202"
        rlRun "kirk -f syscalls -p mlock202 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mlock202"
