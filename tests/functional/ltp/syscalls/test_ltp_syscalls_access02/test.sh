#!/bin/bash
# Functional test: ltp - syscalls - access02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - access02"
        rlRun "kirk -f syscalls -p access02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP access02"
