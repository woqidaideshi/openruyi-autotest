#!/bin/bash
# Functional test: ltp - syscalls - timer_gettime01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - timer_gettime01"
        rlRun "kirk -f syscalls -p timer_gettime01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP timer_gettime01"
