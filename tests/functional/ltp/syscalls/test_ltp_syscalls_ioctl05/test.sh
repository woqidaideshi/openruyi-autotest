#!/bin/bash
# Functional test: ltp - syscalls - ioctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl05"
        rlRun "kirk -f syscalls -p ioctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl05"
