#!/bin/bash
# Functional test: ltp - syscalls - msgget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgget05"
        rlRun "kirk -f syscalls -p msgget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget05"
