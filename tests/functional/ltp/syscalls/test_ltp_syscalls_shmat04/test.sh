#!/bin/bash
# Functional test: ltp - syscalls - shmat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmat04"
        rlRun "kirk -f syscalls -p shmat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat04"
