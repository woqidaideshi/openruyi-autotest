#!/bin/bash
# Functional test: ltp - syscalls - poll02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - poll02"
        rlRun "kirk -f syscalls -p poll02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP poll02"
