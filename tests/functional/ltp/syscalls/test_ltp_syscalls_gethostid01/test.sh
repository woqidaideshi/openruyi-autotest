#!/bin/bash
# Functional test: ltp - syscalls - gethostid01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - gethostid01"
        rlRun "kirk -f syscalls -p gethostid01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP gethostid01"
