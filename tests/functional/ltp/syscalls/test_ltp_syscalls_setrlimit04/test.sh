#!/bin/bash
# Functional test: ltp - syscalls - setrlimit04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setrlimit04"
        rlRun "kirk -f syscalls -p setrlimit04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setrlimit04"
