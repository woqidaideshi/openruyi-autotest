#!/bin/bash
# Functional test: ltp - syscalls - epoll01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - epoll01"
        rlRun "kirk -f syscalls -p epoll01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP epoll01"
