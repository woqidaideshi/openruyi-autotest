#!/bin/bash
# Functional test: ltp - syscalls - pipe15
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipe15"
        rlRun "kirk -f syscalls -p pipe15 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipe15"
