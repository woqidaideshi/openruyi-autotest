#!/bin/bash
# Functional test: ltp - syscalls - shmctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - shmctl03"
        rlRun "kirk -f syscalls -p shmctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl03"
