#!/bin/bash
# Functional test: ltp - syscalls - sighold02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sighold02"
        rlRun "kirk -f syscalls -p sighold02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sighold02"
