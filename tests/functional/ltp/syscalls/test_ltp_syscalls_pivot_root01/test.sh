#!/bin/bash
# Functional test: ltp - syscalls - pivot_root01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pivot_root01"
        rlRun "kirk -f syscalls -p pivot_root01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pivot_root01"
