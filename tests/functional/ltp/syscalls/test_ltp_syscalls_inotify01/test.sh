#!/bin/bash
# Functional test: ltp - syscalls - inotify01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - inotify01"
        rlRun "kirk -f syscalls -p inotify01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP inotify01"
