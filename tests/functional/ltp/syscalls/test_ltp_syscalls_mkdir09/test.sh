#!/bin/bash
# Functional test: ltp - syscalls - mkdir09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mkdir09"
        rlRun "kirk -f syscalls -p mkdir09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mkdir09"
