#!/bin/bash
# Functional test: ltp - syscalls - sysfs01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sysfs01"
        rlRun "kirk -f syscalls -p sysfs01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sysfs01"
