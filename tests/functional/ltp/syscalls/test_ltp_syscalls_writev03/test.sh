#!/bin/bash
# Functional test: ltp - syscalls - writev03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - writev03"
        rlRun "kirk -f syscalls -p writev03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP writev03"
