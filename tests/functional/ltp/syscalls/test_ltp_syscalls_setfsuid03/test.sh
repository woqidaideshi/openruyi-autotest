#!/bin/bash
# Functional test: ltp - syscalls - setfsuid03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setfsuid03"
        rlRun "kirk -f syscalls -p setfsuid03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setfsuid03"
