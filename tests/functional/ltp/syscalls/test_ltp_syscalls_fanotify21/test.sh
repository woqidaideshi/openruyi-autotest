#!/bin/bash
# Functional test: ltp - syscalls - fanotify21
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify21"
        rlRun "kirk -f syscalls -p fanotify21 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify21"
