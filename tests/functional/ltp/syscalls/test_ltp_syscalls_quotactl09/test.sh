#!/bin/bash
# Functional test: ltp - syscalls - quotactl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - quotactl09"
        rlRun "kirk -f syscalls -p quotactl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP quotactl09"
