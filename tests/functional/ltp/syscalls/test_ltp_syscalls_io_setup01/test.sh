#!/bin/bash
# Functional test: ltp - syscalls - io_setup01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - io_setup01"
        rlRun "kirk -f syscalls -p io_setup01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP io_setup01"
