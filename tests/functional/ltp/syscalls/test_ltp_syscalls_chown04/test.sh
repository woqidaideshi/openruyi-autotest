#!/bin/bash
# Functional test: ltp - syscalls - chown04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - chown04"
        rlRun "kirk -f syscalls -p chown04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP chown04"
