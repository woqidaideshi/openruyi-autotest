#!/bin/bash
# Functional test: ltp - syscalls - ptrace09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace09"
        rlRun "kirk -f syscalls -p ptrace09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace09"
