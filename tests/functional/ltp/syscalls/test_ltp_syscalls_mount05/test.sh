#!/bin/bash
# Functional test: ltp - syscalls - mount05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount05"
        rlRun "kirk -f syscalls -p mount05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount05"
