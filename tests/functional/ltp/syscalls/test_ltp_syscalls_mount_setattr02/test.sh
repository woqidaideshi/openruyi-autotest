#!/bin/bash
# Functional test: ltp - syscalls - mount_setattr02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mount_setattr02"
        rlRun "kirk -f syscalls -p mount_setattr02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mount_setattr02"
