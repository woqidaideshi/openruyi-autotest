#!/bin/bash
# Functional test: ltp - syscalls - clone303
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone303"
        rlRun "kirk -f syscalls -p clone303 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone303"
