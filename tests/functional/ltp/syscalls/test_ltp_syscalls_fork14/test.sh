#!/bin/bash
# Functional test: ltp - syscalls - fork14
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork14"
        rlRun "kirk -f syscalls -p fork14 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork14"
