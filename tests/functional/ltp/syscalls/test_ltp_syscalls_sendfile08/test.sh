#!/bin/bash
# Functional test: ltp - syscalls - sendfile08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sendfile08"
        rlRun "kirk -f syscalls -p sendfile08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile08"
