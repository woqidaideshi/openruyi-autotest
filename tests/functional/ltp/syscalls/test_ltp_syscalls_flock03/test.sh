#!/bin/bash
# Functional test: ltp - syscalls - flock03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - flock03"
        rlRun "kirk -f syscalls -p flock03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP flock03"
