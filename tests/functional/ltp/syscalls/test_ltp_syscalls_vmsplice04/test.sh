#!/bin/bash
# Functional test: ltp - syscalls - vmsplice04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - vmsplice04"
        rlRun "kirk -f syscalls -p vmsplice04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP vmsplice04"
