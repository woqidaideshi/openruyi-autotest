#!/bin/bash
# Functional test: ltp - syscalls - readlink01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - readlink01"
        rlRun "kirk -f syscalls -p readlink01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP readlink01"
