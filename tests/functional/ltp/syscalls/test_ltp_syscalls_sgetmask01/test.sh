#!/bin/bash
# Functional test: ltp - syscalls - sgetmask01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sgetmask01"
        rlRun "kirk -f syscalls -p sgetmask01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sgetmask01"
