#!/bin/bash
# Functional test: ltp - syscalls - ptrace10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ptrace10"
        rlRun "kirk -f syscalls -p ptrace10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ptrace10"
