#!/bin/bash
# Functional test: ltp - syscalls - fanotify22
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify22"
        rlRun "kirk -f syscalls -p fanotify22 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify22"
