#!/bin/bash
# Functional test: ltp - syscalls - open13
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open13"
        rlRun "kirk -f syscalls -p open13 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open13"
