#!/bin/bash
# Functional test: ltp - syscalls - fork06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fork06"
        rlRun "kirk -f syscalls -p fork06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fork06"
