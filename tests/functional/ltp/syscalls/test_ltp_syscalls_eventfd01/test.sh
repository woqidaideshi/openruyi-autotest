#!/bin/bash
# Functional test: ltp - syscalls - eventfd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - eventfd01"
        rlRun "kirk -f syscalls -p eventfd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP eventfd01"
