#!/bin/bash
# Functional test: ltp - syscalls - mincore02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mincore02"
        rlRun "kirk -f syscalls -p mincore02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mincore02"
