#!/bin/bash
# Functional test: ltp - syscalls - msync04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msync04"
        rlRun "kirk -f syscalls -p msync04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msync04"
