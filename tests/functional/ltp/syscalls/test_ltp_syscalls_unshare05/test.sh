#!/bin/bash
# Functional test: ltp - syscalls - unshare05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - unshare05"
        rlRun "kirk -f syscalls -p unshare05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP unshare05"
