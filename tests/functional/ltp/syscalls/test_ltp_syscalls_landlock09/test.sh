#!/bin/bash
# Functional test: ltp - syscalls - landlock09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - landlock09"
        rlRun "kirk -f syscalls -p landlock09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP landlock09"
