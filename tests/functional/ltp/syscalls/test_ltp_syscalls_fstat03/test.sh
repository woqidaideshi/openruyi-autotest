#!/bin/bash
# Functional test: ltp - syscalls - fstat03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fstat03"
        rlRun "kirk -f syscalls -p fstat03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fstat03"
