#!/bin/bash
# Functional test: ltp - syscalls - semop04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - semop04"
        rlRun "kirk -f syscalls -p semop04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop04"
