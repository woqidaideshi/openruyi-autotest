#!/bin/bash
# Functional test: ltp - syscalls - open10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - open10"
        rlRun "kirk -f syscalls -p open10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP open10"
