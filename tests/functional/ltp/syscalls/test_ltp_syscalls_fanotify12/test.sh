#!/bin/bash
# Functional test: ltp - syscalls - fanotify12
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify12"
        rlRun "kirk -f syscalls -p fanotify12 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify12"
