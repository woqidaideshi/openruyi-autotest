#!/bin/bash
# Functional test: ltp - syscalls - accept01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - accept01"
        rlRun "kirk -f syscalls -p accept01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP accept01"
