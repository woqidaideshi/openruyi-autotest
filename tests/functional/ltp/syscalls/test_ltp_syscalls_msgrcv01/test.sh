#!/bin/bash
# Functional test: ltp - syscalls - msgrcv01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgrcv01"
        rlRun "kirk -f syscalls -p msgrcv01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv01"
