#!/bin/bash
# Functional test: ltp - syscalls - userfaultfd02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - userfaultfd02"
        rlRun "kirk -f syscalls -p userfaultfd02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP userfaultfd02"
