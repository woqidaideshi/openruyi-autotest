#!/bin/bash
# Functional test: ltp - syscalls - fcntl30
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl30"
        rlRun "kirk -f syscalls -p fcntl30 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl30"
