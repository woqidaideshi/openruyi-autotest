#!/bin/bash
# Functional test: ltp - syscalls - mmap19
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mmap19"
        rlRun "kirk -f syscalls -p mmap19 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mmap19"
