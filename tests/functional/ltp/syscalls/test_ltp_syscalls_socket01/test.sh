#!/bin/bash
# Functional test: ltp - syscalls - socket01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - socket01"
        rlRun "kirk -f syscalls -p socket01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP socket01"
