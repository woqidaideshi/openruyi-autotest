#!/bin/bash
# Functional test: ltp - syscalls - dirtypipe
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - dirtypipe"
        rlRun "kirk -f syscalls -p dirtypipe 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dirtypipe"
