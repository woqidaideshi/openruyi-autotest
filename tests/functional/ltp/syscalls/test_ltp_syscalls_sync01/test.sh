#!/bin/bash
# Functional test: ltp - syscalls - sync01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - sync01"
        rlRun "kirk -f syscalls -p sync01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sync01"
