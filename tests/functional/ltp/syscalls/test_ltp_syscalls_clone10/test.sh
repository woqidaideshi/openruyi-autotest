#!/bin/bash
# Functional test: ltp - syscalls - clone10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - clone10"
        rlRun "kirk -f syscalls -p clone10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP clone10"
