#!/bin/bash
# Functional test: ltp - syscalls - fanotify01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify01"
        rlRun "kirk -f syscalls -p fanotify01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify01"
