#!/bin/bash
# Functional test: ltp - syscalls - ioctl_loop06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - ioctl_loop06"
        rlRun "kirk -f syscalls -p ioctl_loop06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ioctl_loop06"
