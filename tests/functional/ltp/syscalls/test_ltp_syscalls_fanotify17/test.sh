#!/bin/bash
# Functional test: ltp - syscalls - fanotify17
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fanotify17"
        rlRun "kirk -f syscalls -p fanotify17 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fanotify17"
