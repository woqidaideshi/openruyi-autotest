#!/bin/bash
# Functional test: ltp - syscalls - fcntl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl07"
        rlRun "kirk -f syscalls -p fcntl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl07"
