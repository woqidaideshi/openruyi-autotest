#!/bin/bash
# Functional test: ltp - syscalls - pidfd_open01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pidfd_open01"
        rlRun "kirk -f syscalls -p pidfd_open01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pidfd_open01"
