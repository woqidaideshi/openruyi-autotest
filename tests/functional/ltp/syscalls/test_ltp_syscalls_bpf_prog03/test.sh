#!/bin/bash
# Functional test: ltp - syscalls - bpf_prog03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bpf_prog03"
        rlRun "kirk -f syscalls -p bpf_prog03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bpf_prog03"
