#!/bin/bash
# Functional test: ltp - syscalls - fcntl39
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - fcntl39"
        rlRun "kirk -f syscalls -p fcntl39 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP fcntl39"
