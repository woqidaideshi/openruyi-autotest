#!/bin/bash
# Functional test: ltp - syscalls - pipeio_2
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - pipeio_2"
        rlRun "kirk -f syscalls -p pipeio_2 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP pipeio_2"
