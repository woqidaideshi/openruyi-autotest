#!/bin/bash
# Functional test: ltp - syscalls - msgsnd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - msgsnd01"
        rlRun "kirk -f syscalls -p msgsnd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgsnd01"
