#!/bin/bash
# Functional test: ltp - syscalls - utime04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - utime04"
        rlRun "kirk -f syscalls -p utime04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP utime04"
