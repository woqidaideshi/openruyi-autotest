#!/bin/bash
# Functional test: ltp - syscalls - waitpid09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - waitpid09"
        rlRun "kirk -f syscalls -p waitpid09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP waitpid09"
