#!/bin/bash
# Functional test: ltp - syscalls - setsockopt10
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - setsockopt10"
        rlRun "kirk -f syscalls -p setsockopt10 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP setsockopt10"
