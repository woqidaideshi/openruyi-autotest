#!/bin/bash
# Functional test: ltp - syscalls - mremap03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - mremap03"
        rlRun "kirk -f syscalls -p mremap03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP mremap03"
