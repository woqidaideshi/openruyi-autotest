#!/bin/bash
# Functional test: ltp - syscalls - bind06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - bind06"
        rlRun "kirk -f syscalls -p bind06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP bind06"
