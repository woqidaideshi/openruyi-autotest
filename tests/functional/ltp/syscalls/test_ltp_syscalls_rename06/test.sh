#!/bin/bash
# Functional test: ltp - syscalls - rename06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls - rename06"
        rlRun "kirk -f syscalls -p rename06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rename06"
