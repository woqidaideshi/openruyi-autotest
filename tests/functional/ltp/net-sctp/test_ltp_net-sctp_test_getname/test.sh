#!/bin/bash
# Functional test: ltp - net-sctp - test_getname
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-sctp - test_getname"
        rlRun "kirk -f net-sctp -p test_getname 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP test_getname"
