#!/bin/bash
# Functional test: ltp - net-sctp - test_basic
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-sctp - test_basic"
        rlRun "kirk -f net-sctp -p test_basic 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP test_basic"
