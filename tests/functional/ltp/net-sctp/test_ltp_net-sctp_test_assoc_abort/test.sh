#!/bin/bash
# Functional test: ltp - net-sctp - test_assoc_abort
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-sctp - test_assoc_abort"
        rlRun "kirk -f net-sctp -p test_assoc_abort 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP test_assoc_abort"
