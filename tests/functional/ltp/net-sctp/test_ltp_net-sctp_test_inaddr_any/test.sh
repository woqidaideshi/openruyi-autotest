#!/bin/bash
# Functional test: ltp - net-sctp - test_inaddr_any
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-sctp - test_inaddr_any"
        rlRun "kirk -f net-sctp -p test_inaddr_any 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP test_inaddr_any"
