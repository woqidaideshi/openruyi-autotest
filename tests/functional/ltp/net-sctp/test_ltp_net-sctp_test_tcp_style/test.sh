#!/bin/bash
# Functional test: ltp - net-sctp - test_tcp_style
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-sctp - test_tcp_style"
        rlRun "kirk -f net-sctp -p test_tcp_style 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP test_tcp_style"
