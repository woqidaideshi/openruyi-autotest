#!/bin/bash
# Functional test: ltp - smack - smack_set_doi
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP smack - smack_set_doi"
        rlRun "kirk -f smack -p smack_set_doi 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP smack_set_doi"
