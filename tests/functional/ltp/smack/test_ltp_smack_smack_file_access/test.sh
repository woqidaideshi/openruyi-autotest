#!/bin/bash
# Functional test: ltp - smack - smack_file_access
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP smack - smack_file_access"
        rlRun "kirk -f smack -p smack_file_access 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP smack_file_access"
