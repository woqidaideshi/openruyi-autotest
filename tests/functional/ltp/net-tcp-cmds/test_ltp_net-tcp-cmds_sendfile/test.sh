#!/bin/bash
# Functional test: ltp - net-tcp-cmds - sendfile
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - sendfile"
        rlRun "kirk -f net-tcp-cmds -p sendfile 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP sendfile"
