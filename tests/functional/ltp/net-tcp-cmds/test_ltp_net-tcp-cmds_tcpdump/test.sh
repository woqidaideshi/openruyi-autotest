#!/bin/bash
# Functional test: ltp - net-tcp-cmds - tcpdump
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - tcpdump"
        rlRun "kirk -f net-tcp-cmds -p tcpdump 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP tcpdump"
