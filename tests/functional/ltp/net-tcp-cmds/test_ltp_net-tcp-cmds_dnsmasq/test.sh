#!/bin/bash
# Functional test: ltp - net-tcp-cmds - dnsmasq
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - dnsmasq"
        rlRun "kirk -f net-tcp-cmds -p dnsmasq 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP dnsmasq"
