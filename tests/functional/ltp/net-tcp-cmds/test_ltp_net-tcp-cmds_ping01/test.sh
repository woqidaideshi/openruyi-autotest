#!/bin/bash
# Functional test: ltp - net-tcp-cmds - ping01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - ping01"
        rlRun "kirk -f net-tcp-cmds -p ping01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ping01"
