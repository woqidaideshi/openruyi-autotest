#!/bin/bash
# Functional test: ltp - net-tcp-cmds - ping02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - ping02"
        rlRun "kirk -f net-tcp-cmds -p ping02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ping02"
