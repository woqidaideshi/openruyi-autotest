#!/bin/bash
# Functional test: ltp - net-tcp-cmds - netstat
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - netstat"
        rlRun "kirk -f net-tcp-cmds -p netstat 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP netstat"
