#!/bin/bash
# Functional test: ltp - net-tcp-cmds - nft
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - nft"
        rlRun "kirk -f net-tcp-cmds -p nft 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nft"
