#!/bin/bash
# Functional test: ltp - net-tcp-cmds - iptables
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - iptables"
        rlRun "kirk -f net-tcp-cmds -p iptables 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP iptables"
