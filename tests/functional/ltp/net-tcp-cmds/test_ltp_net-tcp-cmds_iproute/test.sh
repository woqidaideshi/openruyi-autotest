#!/bin/bash
# Functional test: ltp - net-tcp-cmds - iproute
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-tcp-cmds - iproute"
        rlRun "kirk -f net-tcp-cmds -p iproute 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP iproute"
