#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgrcv08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgrcv08"
        rlRun "kirk -f syscalls-ipc -p msgrcv08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv08"
