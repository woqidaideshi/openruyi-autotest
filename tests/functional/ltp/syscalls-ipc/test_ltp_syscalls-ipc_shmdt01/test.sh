#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmdt01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmdt01"
        rlRun "kirk -f syscalls-ipc -p shmdt01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmdt01"
