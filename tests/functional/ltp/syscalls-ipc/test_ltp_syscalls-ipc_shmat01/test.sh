#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmat01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmat01"
        rlRun "kirk -f syscalls-ipc -p shmat01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat01"
