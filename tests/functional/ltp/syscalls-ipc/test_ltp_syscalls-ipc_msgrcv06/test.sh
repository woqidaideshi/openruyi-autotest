#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgrcv06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgrcv06"
        rlRun "kirk -f syscalls-ipc -p msgrcv06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv06"
