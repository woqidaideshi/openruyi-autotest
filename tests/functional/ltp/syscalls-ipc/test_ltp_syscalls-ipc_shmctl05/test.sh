#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl05"
        rlRun "kirk -f syscalls-ipc -p shmctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl05"
