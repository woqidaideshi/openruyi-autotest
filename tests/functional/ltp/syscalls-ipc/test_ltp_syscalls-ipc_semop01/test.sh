#!/bin/bash
# Functional test: ltp - syscalls-ipc - semop01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semop01"
        rlRun "kirk -f syscalls-ipc -p semop01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop01"
