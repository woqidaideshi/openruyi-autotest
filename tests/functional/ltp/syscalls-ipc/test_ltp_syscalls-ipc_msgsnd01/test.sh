#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgsnd01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgsnd01"
        rlRun "kirk -f syscalls-ipc -p msgsnd01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgsnd01"
