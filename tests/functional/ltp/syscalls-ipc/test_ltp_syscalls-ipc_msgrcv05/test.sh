#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgrcv05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgrcv05"
        rlRun "kirk -f syscalls-ipc -p msgrcv05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgrcv05"
