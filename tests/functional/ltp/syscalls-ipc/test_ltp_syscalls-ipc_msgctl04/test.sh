#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgctl04"
        rlRun "kirk -f syscalls-ipc -p msgctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl04"
