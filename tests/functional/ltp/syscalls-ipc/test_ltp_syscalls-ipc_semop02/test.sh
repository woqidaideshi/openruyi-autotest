#!/bin/bash
# Functional test: ltp - syscalls-ipc - semop02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semop02"
        rlRun "kirk -f syscalls-ipc -p semop02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semop02"
