#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl03"
        rlRun "kirk -f syscalls-ipc -p shmctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl03"
