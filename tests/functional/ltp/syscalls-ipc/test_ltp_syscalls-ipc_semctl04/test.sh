#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl04"
        rlRun "kirk -f syscalls-ipc -p semctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl04"
