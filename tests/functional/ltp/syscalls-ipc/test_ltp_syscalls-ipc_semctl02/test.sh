#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl02"
        rlRun "kirk -f syscalls-ipc -p semctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl02"
