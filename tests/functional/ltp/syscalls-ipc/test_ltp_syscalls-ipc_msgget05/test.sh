#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgget05"
        rlRun "kirk -f syscalls-ipc -p msgget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget05"
