#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgctl05"
        rlRun "kirk -f syscalls-ipc -p msgctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl05"
