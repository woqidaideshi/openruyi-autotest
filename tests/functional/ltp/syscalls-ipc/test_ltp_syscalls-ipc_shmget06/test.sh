#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmget06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmget06"
        rlRun "kirk -f syscalls-ipc -p shmget06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget06"
