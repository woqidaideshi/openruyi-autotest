#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl03"
        rlRun "kirk -f syscalls-ipc -p semctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl03"
