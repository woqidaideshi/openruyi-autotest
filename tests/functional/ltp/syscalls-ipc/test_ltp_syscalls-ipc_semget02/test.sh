#!/bin/bash
# Functional test: ltp - syscalls-ipc - semget02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semget02"
        rlRun "kirk -f syscalls-ipc -p semget02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semget02"
