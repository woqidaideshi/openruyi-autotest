#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl08"
        rlRun "kirk -f syscalls-ipc -p semctl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl08"
