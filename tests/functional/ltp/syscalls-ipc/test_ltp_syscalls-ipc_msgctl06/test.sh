#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgctl06"
        rlRun "kirk -f syscalls-ipc -p msgctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl06"
