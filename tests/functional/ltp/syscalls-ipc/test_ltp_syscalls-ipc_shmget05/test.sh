#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmget05"
        rlRun "kirk -f syscalls-ipc -p shmget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmget05"
