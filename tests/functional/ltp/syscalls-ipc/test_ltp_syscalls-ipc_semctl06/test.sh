#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl06
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl06"
        rlRun "kirk -f syscalls-ipc -p semctl06 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl06"
