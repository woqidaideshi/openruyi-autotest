#!/bin/bash
# Functional test: ltp - syscalls-ipc - semget05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semget05"
        rlRun "kirk -f syscalls-ipc -p semget05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semget05"
