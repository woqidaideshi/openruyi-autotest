#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl01"
        rlRun "kirk -f syscalls-ipc -p shmctl01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl01"
