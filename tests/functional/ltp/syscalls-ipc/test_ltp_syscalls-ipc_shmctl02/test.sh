#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl02"
        rlRun "kirk -f syscalls-ipc -p shmctl02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl02"
