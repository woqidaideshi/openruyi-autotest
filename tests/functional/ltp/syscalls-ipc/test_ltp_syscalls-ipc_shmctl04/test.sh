#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl04"
        rlRun "kirk -f syscalls-ipc -p shmctl04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl04"
