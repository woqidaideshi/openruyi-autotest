#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgctl03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgctl03"
        rlRun "kirk -f syscalls-ipc -p msgctl03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgctl03"
