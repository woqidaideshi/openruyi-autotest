#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl07
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl07"
        rlRun "kirk -f syscalls-ipc -p semctl07 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl07"
