#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmctl08
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmctl08"
        rlRun "kirk -f syscalls-ipc -p shmctl08 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmctl08"
