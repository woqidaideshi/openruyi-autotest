#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmat04
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmat04"
        rlRun "kirk -f syscalls-ipc -p shmat04 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat04"
