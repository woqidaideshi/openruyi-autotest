#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl09
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl09"
        rlRun "kirk -f syscalls-ipc -p semctl09 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl09"
