#!/bin/bash
# Functional test: ltp - syscalls-ipc - semctl05
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - semctl05"
        rlRun "kirk -f syscalls-ipc -p semctl05 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP semctl05"
