#!/bin/bash
# Functional test: ltp - syscalls-ipc - msgget01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - msgget01"
        rlRun "kirk -f syscalls-ipc -p msgget01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP msgget01"
