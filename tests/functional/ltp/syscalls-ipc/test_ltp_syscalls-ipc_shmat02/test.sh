#!/bin/bash
# Functional test: ltp - syscalls-ipc - shmat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP syscalls-ipc - shmat02"
        rlRun "kirk -f syscalls-ipc -p shmat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP shmat02"
