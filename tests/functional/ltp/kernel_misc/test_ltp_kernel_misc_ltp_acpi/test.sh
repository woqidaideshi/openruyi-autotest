#!/bin/bash
# Functional test: ltp - kernel_misc - ltp_acpi
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - ltp_acpi"
        rlRun "kirk -f kernel_misc -p ltp_acpi 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP ltp_acpi"
