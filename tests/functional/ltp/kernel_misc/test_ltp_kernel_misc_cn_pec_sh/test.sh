#!/bin/bash
# Functional test: ltp - kernel_misc - cn_pec_sh
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - cn_pec_sh"
        rlRun "kirk -f kernel_misc -p cn_pec_sh 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cn_pec_sh"
