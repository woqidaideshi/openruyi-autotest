#!/bin/bash
# Functional test: ltp - kernel_misc - cpufreq_boost
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - cpufreq_boost"
        rlRun "kirk -f kernel_misc -p cpufreq_boost 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP cpufreq_boost"
