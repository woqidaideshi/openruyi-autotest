#!/bin/bash
# Functional test: ltp - kernel_misc - aslr01
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - aslr01"
        rlRun "kirk -f kernel_misc -p aslr01 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP aslr01"
