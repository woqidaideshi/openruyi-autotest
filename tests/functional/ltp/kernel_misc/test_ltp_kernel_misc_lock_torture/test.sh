#!/bin/bash
# Functional test: ltp - kernel_misc - lock_torture
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - lock_torture"
        rlRun "kirk -f kernel_misc -p lock_torture 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP lock_torture"
