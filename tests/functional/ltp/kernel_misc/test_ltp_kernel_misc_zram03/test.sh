#!/bin/bash
# Functional test: ltp - kernel_misc - zram03
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - zram03"
        rlRun "kirk -f kernel_misc -p zram03 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP zram03"
