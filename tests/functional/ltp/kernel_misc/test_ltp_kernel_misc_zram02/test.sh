#!/bin/bash
# Functional test: ltp - kernel_misc - zram02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP kernel_misc - zram02"
        rlRun "kirk -f kernel_misc -p zram02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP zram02"
