#!/bin/bash
# Functional test: ltp - can - can_filter
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP can - can_filter"
        rlRun "kirk -f can -p can_filter 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP can_filter"
