#!/bin/bash
# Functional test: ltp - net-nfs - nfsstat02
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-nfs - nfsstat02"
        rlRun "kirk -f net-nfs -p nfsstat02 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP nfsstat02"
