#!/bin/bash
# Functional test: ltp - net-rpc-tests - rpc_callrpc
...
rlJournalStart
    ...
    rlPhaseStartTest "LTP net-rpc-tests - rpc_callrpc"
        rlRun "kirk -f net-rpc-tests -p rpc_callrpc 2>&1 | tee /tmp/ltp_out_$$; exit ${PIPESTATUS[0]}" 0 "执行 LTP rpc_callrpc"
