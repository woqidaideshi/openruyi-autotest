#!/bin/bash
# Performance: sysbench - 内存带宽: 顺序/随机 读/写
...
rlJournalStart
    ...
    rlPhaseStartTest "顺序读写 (大块 1M)"
        echo ""
        echo "=== 内存顺序访问 ($cores 线程) ==="
        printf "%-12s %-15s\n" "Mode" "MiB/s"
        ...
        for op in read write; do
            ...
            printf "%-12s %-15s\n" "seq/$op" "${bw:-N/A}"
            rlPass "顺序${op}: ${bw:-N/A} MiB/s"
        done
    rlPhaseEnd

    rlPhaseStartTest "随机读写 (小块 8K)"
        echo ""
        echo "=== 内存随机访问 ($cores 线程) ==="
        printf "%-12s %-15s\n" "Mode" "MiB/s"
